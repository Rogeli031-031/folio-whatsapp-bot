...
| Campo | Runtime |
|-------|---------|
| `parent_intent` | **Unknown + estado válido → inherit.** Intents inheritable: `plant_diagnosis`, `expediente_comercial`, `daily_sales_deviation`, `daily_discount_deviation`, `daily_executive_brief`, `commercial_trend`, `client_profile`, `taller_mayor`, `pre_meeting_brief`, `month_close_result`, `action_status`, `igf_reviewable_supports`. `igf_status` **no** es inheritable (el hop a apoyos no se pega a IGF). Standalone ≥ 0.55 **siempre gana** (presupuesto, Taller AT-15, Querétaro/planta nueva, venta ayer, descuento/kg ayer, IGF, recortar apoyos, acciones vencidas, acción + responsable, CASA/COMISIONISTAS, cliente nombrado, Taller Mayor, y los demás intents existentes). `financial_diagnosis` / presupuesto / Taller no se pisan ni reabren Puebla por history. Tras `commercial_trend` + `active_entity` con `cliente_key`, una pregunta de perfil/acciones de **ese** cliente **no** rehereda trend: `forceIntent client_profile` + requery. Tras `pre_meeting_brief`, «qué me preocupa» / «qué falta explicar» heredan el brief; «¿Y cómo cerramos?» **sale** a `month_close_result` (requery; no reusa el mes abierto del brief como mes de cierre); detalle de acciones / apoyos / cliente / CASA / Taller Mayor **sale** a la capability canónica. Tras `month_close_result`, follow-ups de meta / canal / descuento / clientes perdidos heredan; «Háblame del cliente X» con entidad activa → `client_profile`. |
| `planta_id` | Siempre el del request autorizado. Nunca del texto ni del history. |
| `active_entities` | Máximo 1. Única en la planta actual (palabra completa / nombre exacto sobre el pack fresco). Ambiguo o ausente → clarifica. Sin fuzzy silencioso. En hilos `action_status` puede ecoar `ar_responsable` / `ar_action` (no como cliente). En hilos `taller_mayor` puede ecoar `unit` / `folio` (`unit_token`; `folio_id` solo si único/explícito). **No** se tratan como `cliente_key`. |
...
Diferido desde trend: comments de gráfica; SEH; saludo personalizado; closed-month IGF. El perfil 3M calendario es `client_profile` (abajo), no este intent. Taller Mayor por unidad es `taller_mayor` (abajo).
...
Preserva: `commercial_trend`, `daily_executive_brief`, daily sales/discount, cross-metric B, action-person, topic return, `igf_reviewable_supports`, `taller_mayor`, persistent memory.

Diferido: fuente de ingreso mensual actual; SEH; saludo personalizado; closed-month IGF.
...
Preserva: `igf_status` (snapshot sin overlay), workflow Folios M2, daily sales, daily discount, cross-metric, topic return, action-person, persistent memory, M9, `taller_mayor` (con Folio/unidad activo, «detener» no abre este listado de planta).
...
### Taller Mayor por unidad `taller_mayor` (no es módulo M0–M20)

**Implementado** (`IMPL-DIRECTOR-IA-TALLER-MAYOR-UNIDAD-001`; source **B** — reusable Taller Mayor unit read model; routing **B** — canonical `taller_mayor` parent). Chat legado. **No** IES. **No** Reasoning Engine N5. **No** phrasebook. **No** segundo router LLM. **No** persistencia. **No** HTTP interno. **No** cancela. **No** cambia cobertura de M5 ni el 52.5%. M5 **sigue PARTIAL**.

**No** es `taller_at` (unidad nombrada + `YYYY-MM` + **todos** los tipos TALLER). **No** es `folio_status`. **No** es IGF reviewable de planta. Periodo, unidad y Folio son **slots**.

Identidad: `(planta_id, token canónico de public.folios.unidad)` vía `unidad-taller`. **No** es económico. **No** es placa. **No** hay unit master. **No** fuzzy. **No** merge cross-plant. Un folio multi-unidad se **expande** (`expandTallerRows`) y se agrupa **después**. Filas sin token no entran al ranking (missing ≠ 0).

Taller Mayor: `public.folios.subcategoria` = `REPARACIÓN MAYOR` (UI). Runtime: `matchTallerTipoCol` → `tipo = "mayor"` (`REPARACION` + `MAYOR` tras NFD). Categoría `LIKE '%TALLER%'`. **No** clasificar por importe. **No** clasificar por concepto. `CANCELADO` fuera.

Periodo: «este mes» = YYYY-MM actual `America/Mexico_City` contra `mes_cargo`. **No** 90d trailing. «Mes pasado» no se resuelve en silencio.
...
| Pieza | Runtime |
|-------|---------|
| Lista | Por unidad: token, planta, periodo, `folio_count`, `SUM(importe)`, refs de Folio, importes individuales, estatus, concepto, subcategoria, reviewability cuando aplique, provenance, limitations. |
| Ranking | Tras la lista, «el más alto» = unidad con mayor `SUM(importe)`. Empate → clarificar. |
| Multi-Folio | Si la unidad ganadora tiene N>1 Folios: `active_unit` sí; `active_folio` **no** se elige en silencio. N=1 → ambos. |
| State | `parent_intent=taller_mayor`, planta, `active_period_months`, `active_unit` (kind `unit`), `active_folio` (kind `folio`) solo si único/explícito. **Routing, no evidencia.** Requery cada turno. |
| Reviewability | `classifyCancellationEligibility` del Folio **activo**. Con `active_folio`, «¿Todavía se puede detener?» evalúa **ese** Folio. **No** salta a `igf_reviewable_supports` plant-wide. Sin Folio/unidad de este hilo, el IGF reviewable de planta se preserva. |
| Invariantes reviewable | reviewable ≠ cancelar ≠ recomendación ≠ ahorro ≠ reversión contable. Director IA **read-only**. |
| Historial | Misma planta + mismo token. Default: Taller Mayor + periodo del hilo. Amplía solo si el usuario pide histórico / todos sus folios / en total. |
| IGF hipotético | Con `active_folio`, conserva Folio+unidad. Overlay filtrado si es físicamente compatible; si no, limitation. **No** candidatos de planta. **No** mutation. **No** savings realizado. |
| Partial-data | Hechos soportados + limitations. missing ≠ 0. |
| Authz | Misma puerta Folios que M5 (`assertFolioStatusAccess` / planta). Fail-closed. Lectura ≠ permiso de cancelar. |

Runtime owns: identidad, Folio, clasificación Mayor, periodo, `SUM`, estatus, reviewability, historial, authz, provenance, ausencia. GPT owns: síntesis, qué destaca, resumir el concepto **registrado**, qué investigar.

Prohibido: diagnóstico mecánico; causalidad desde concepto; recomendar cancelar porque el importe es alto; claim de ahorro.

Preserva: `folio_status`, `taller_at`, IGF reviewable plant-wide, `client_profile`, `commercial_trend`, brief diario, daily sales/discount, topic return, persistent memory, `pre_meeting_brief`, `month_close_result` (Taller Mayor **no** entra al pack de junta ni al de cierre; el follow-up de unidades sale a este intent).

Diferido: mantenimiento predictivo; catálogo/placa/económico; mutaciones/cancelación; «mes pasado» robusto; overlay IGF de planta; extender `taller_at` a Mayor.

Archivos: `lib/director-ia-taller-mayor.js`; wiring `lib/director-ia-chat.js`, `lib/director-ia-planner.js`, `lib/director-ia-conversation-state.js`, `lib/director-ia-tools.js`, `lib/director-ia-capabilities.js` (`isTallerMayorQuery`; **sin** fila nueva de matriz). Reusa `queryTallerFolios` / `expandTallerRows` / `matchTallerTipoCol` / `unidad-taller` / `classifyCancellationEligibility`. Tool `get_taller_mayor` (read-only; dominio `folios`; no contamina `taller_at` / M5 COMPLETE). Tests: `test/director-ia-taller-mayor.test.js` (17/17). Suite Director IA citada: **964/964**.
...
**No** es `plant_diagnosis`. **No** es `daily_executive_brief`. **No** es `financial_diagnosis`. Una planta autorizada. Mes = **actual/abierto** `America/Mexico_City`. Mes cerrado nombrado → limitation (`closed_month_requested_out_of_first_slice`); el runtime **sigue** cargando el mes abierto etiquetado; **no** lo contesta en silencio como si fuera el abierto.
...
pregunta de preparación de junta / pre-cierre
  ...
  → safeLoad acciones (board + overdue)
  → safeLoad IGF reviewable (pregunta sintética; mes abierto)
  → information gaps (ausencia de evidencia contextual)
  → HILO
  → una llamada OpenAI
  → síntesis ejecutiva (GPT)
```

Pack del first slice (solo estas cinco secciones):

1. **commercial** — `daily_executive_brief` + `commercial_trend` 90d both channels + `client_profile` recortado a movers ya rankeados (cap 3).
2. **open-month IGF** — proyección / versión vigente del mes abierto. No es cierre real.
3. **actions** — acciones abiertas / vencidas / responsable-estatus registrados. No `loadActionPersonBoardForChat` como compose.
...
Information gaps = **ausencia de evidencia contextual/explicativa cargada**. No son causa.

Lenguaje seguro documentado:

- «No encuentro evidencia cargada que explique…»
- «No hay resultado registrado…»
- «Conviene obtener contexto antes de la junta.»

Prohibido inventar: causa, resultado, responsable, ahorro.

Materialidad **B**: runtime entrega señales **ya existentes** (deviations, rankings, statuses, overdue, reviewability, gaps). GPT prioriza, ordena, sintetiza y sugiere qué aclarar. **No** thresholds arbitrarios nuevos. **No** learned score. **No** checklist hardcodeado como verdad.

Question anticipation: sí «Conviene estar preparado para explicar…» / «Este punto merece revisión antes de la junta.» / «Falta evidencia para responder con seguridad a…». **No** «El Consejo te va a preguntar…» / «Sé que te preguntarán…».

Read-only absoluto: no envía mensajes, no escribe comentarios, no crea/edita acciones, no cancela Folios, no muta IGF, no persiste el brief.

State (solo routing; **no** truth):

| Slot | Runtime |
|------|---------|
| plant | `planta_id` del request autorizado |
| `meeting_period` | YYYY-MM abierto vía `active_period_months` |
| `meeting_type` | `monthly_close` o `null` |
| `parent_intent` | `pre_meeting_brief` |

**No** se guarda el full pack ni raw evidence como truth. Requery fresco cada turno.

Follow-ups:

| Turno | Destino |
|-------|---------|
| ¿Qué me preocupa más? / ¿Qué falta explicar? | inherit `pre_meeting_brief` (síntesis / gaps) |
| ¿Y cómo cerramos? / ¿Y cómo cerramos el mes? | `month_close_result` (handoff; requery fresco; no reusa el mes abierto del brief) |
| ¿Qué acciones están vencidas? | capability canónica de acciones (`overdue_actions` / `action_status`) |
| ¿Qué apoyos puedo revisar? | `igf_reviewable_supports` |
| Háblame del cliente X. | `client_profile` |
| ¿Cómo vamos en CASA? | `commercial_trend` |
| ¿Qué unidades tienen Taller Mayor? | `taller_mayor` |

Los detalles **no** quedan atrapados en `pre_meeting_brief`.

Plaud: **sin integración runtime**. Uso futuro posible: evidencia de **evaluación histórica** (comparar el brief contra lo que de hecho surgió en juntas de cierre). Meeting statement ≠ causal truth. Declaración de junta ≠ hecho verificado.
...
Prepárame para la junta de cierre.
  → ¿Qué me preocupa más?
  → ¿Qué falta explicar?
  → ¿Y cómo cerramos el mes?              → month_close_result
  → ¿Qué acciones están vencidas?          → acciones
  → ¿Qué apoyos puedo revisar?            → IGF reviewable
  → Háblame del cliente X.                → client_profile
  → ¿Cómo vamos en CASA?                  → commercial_trend
  → ¿Qué unidades tienen Taller Mayor?    → taller_mayor
```

Preserva: daily brief, daily sales, daily discount, commercial trend, client profile, actions, IGF, IGF reviewable, `taller_mayor`, `month_close_result`, topic return, persistent memory, `folio_status`, `taller_at`.
...
**No** es `pre_meeting_brief`. **No** es `igf_status`. **No** es `commercial_trend`. **No** es `plant_diagnosis`. Una planta autorizada. Un mes calendario `America/Mexico_City`. Default = último mes **COMPLETE**. Mes actual nombrado = **PARTIAL**. **No** 30/90 trailing.

Cuatro clases de verdad **permanecen separadas**:

| Clase | Fuente | Qué es | Qué no es |
|-------|--------|--------|-----------|
| **ACTUAL** | ARR mensual real (`arr.ventas_diarias_cliente` / descuentos diarios) | venta, mix CASA/COMISIONISTA, descuento/kg ponderado, movimiento de clientes | TARGET, FORECAST, DERIVED_MODEL |
...
| **FORECAST** | IGF (`igf.compromiso_lines`) | proyección | actual, target, cumplimiento real |
| **DERIVED_MODEL** | `forecast_mensual` / 14d×DOW u equivalente | modelo derivado | actual, target |

Invariantes: ACTUAL ≠ TARGET_COMMITMENT ≠ FORECAST ≠ DERIVED_MODEL. Sin relabel semántico por conveniencia de presentación.
...
**ACTUAL venta:** `SUM(kg)` del mes calendario; `ton = kg / 1000`.

**CHANNELS:** clasificación canónica existente (`LIKE '%comisionista%'` → COMISIONISTA; resto → CASA). Expone kg/ton/share % si el denominador es válido.

**DISCOUNT:** `SUM(monto)/SUM(kg)`. **No** average-of-averages.

**CLIENTS:** mes pedido vs mes calendario previo. new / lost / top positive movers / top negative movers. `cliente_key` cuando corresponde. **No** fuzzy. **No** join inseguro por nombre. **mover ≠ causa.** No sustituir ingreso forecast DICF por actual de cierre.

**TARGET:** solo exact year + exact month + `is_current=true` + mapping empresa/planta autorizada. Si falta: `TARGET_MISSING_FOR_PERIOD`. **Prohibido:** carry-forward; última meta de otro mes; forecast como meta; actual previo como meta; Plaud como fuente runtime; hardcode. Target = 0 no se usa como denominador.

Si actual + target válidos: `actual_ton`, `target_ton`, `delta_ton`, `attainment_pct`.
...
Se puede comparar TARGET vs FORECAST **solo** si queda etiquetado como proyección. **No** es cumplimiento financiero real. **No** se creó fuente de actual financiero.
...
Authz: una planta; fail closed; no cross-plant; mapping empresa no salta autorización.
...
|------|---------|
| plant | `planta_id` del request autorizado |
| month | YYYY-MM vía `active_period_months` |
| `parent_intent` | `month_close_result` |

**No** raw evidence. **No** stale target. `reuse_inherited_month` solo si el parent ya era `month_close_result`. Evidence = requery.

GPT recibe el read model estructurado. Sintetiza qué destaca, tensiones, qué necesita explicación y limitations. **No** inventa target, actual financiero ni causa. **No** oculta datos unsupported.
...
Follow-ups (ejemplos de intención, **no** phrasebook):
...
| Háblame del primero. | setea entidad; hop a `client_profile` en el turno siguiente si hay entidad activa |
| ¿Cómo vamos en CASA los últimos 90 días? | `commercial_trend` standalone |
...
Diferido: actual financiero; upload de meta; `igf_metahg` como sustituto de `venta_ton`; Plaud runtime; schema; matrix.
...
| **Observaciones verificadas** | `IMPL-DIRECTOR-IA-M5-TALLER-AT-001` (integrado, merge `848d3eb1`). Tests: focales 16/16; capabilities 56/56; planner 49/49; orchestrator 26/26; suite `test/director-ia-*.test.js` 673/673; `git diff --check` limpio. TALLER ≠ GASTOS ≠ INVERSIONES. M4 familia TALLER ≠ detalle por unidad. «cómo va Taller» / acciones AT-15 siguen AR. Excel/duplicados **siguen fuera**. **Sync transversal** `IMPL-DIRECTOR-IA-TALLER-MAYOR-UNIDAD-001`: el intent `taller_mayor` agrupa Taller Mayor por unidad (token canónico + `mes_cargo`); **no** es `taller_at`; **no** completa Excel/duplicados. M5 = **PARCIAL**. **No** COMPLETE. Scoring M0–M20 del loop (COMPLETE=1.0, PARCIAL/INDIRECTA=0.5, NO INTEGRADA=0.0): 10.0/20 = 50.0% → **10.5/20 = 52.5%**. |
...
| **Tablas o vistas relacionadas** | `igf.versions`, `igf.compromiso_lines`. Schemas `igf_meta` / `igf_metahg` (UI; no en el bloque de composición IGF). En `month_close_result`, `igf_meta.versions` + `igf_meta.meta_lines` son **TARGET_COMMITMENT**, no forecast. |
...
- **Información que no puede concluirse con esta fuente:** Excel/workbook; duplicados taller; causa; responsable; atraso; mes inventado; igualdad con M4/M6/AR; lista Taller Mayor por unidad (otro intent: `taller_mayor`)

### Fuente: Taller Mayor por unidad (transversal)

- **Dominio:** Chat legado `taller_mayor` (no es un módulo M0–M20; no puntúa). Source **B**. Routing **B**. `IMPL-DIRECTOR-IA-TALLER-MAYOR-UNIDAD-001`.
- **Cobertura actual:** PARCIAL respecto a lista/ranking/detalle/reviewability/historial de Taller Mayor por unidad. **No** cambia M5 PARCIAL ni el 52.5%. **No** COMPLETE de M5.
- **Archivo de acceso:** `lib/director-ia-taller-mayor.js`; `expandTallerRows` / `matchTallerTipoCol` en `lib/taller-at-excel.js`; `lib/unidad-taller.js`; SELECT reusado de `lib/director-ia-m5-taller-at.js`.
- **Función de acceso:** `loadTallerMayorForChat` / `assembleTallerMayorPack` (in-process) → prompt + `HILO` → **una** llamada OpenAI por turno.
- **Endpoint relacionado:** `POST /api/director-ia/chat` (in-process; SELECT `public.folios`; sin HTTP interno; sin writes)
- **Tablas consultadas:** `public.folios` ⋈ `public.plantas`. Identidad: `(planta_id, token canónico de public.folios.unidad)`. **No** económico. **No** placa. **No** unit master.
- **Filtros disponibles:** planta; `mes_cargo` (default mes actual CDMX); token de unidad; Folio solo si único/explícito.
- **Clasificación:** `matchTallerTipoCol(subcategoria)===mayor`. **No** por importe. **No** por concepto.
- **Lista / ranking:** agrupa por unidad (`folio_count`, `SUM(importe)`, Folios, estatus, concepto, reviewability, provenance, limitations). «El más alto» = unidad con mayor `SUM`. N Folios → no silent pick.
- **Reviewability:** `classifyCancellationEligibility` del Folio activo. Con `active_folio`, «detener» **no** abre IGF reviewable de planta. reviewable ≠ cancelar ≠ recomendación ≠ ahorro ≠ reversión.
- **Historial:** misma planta + mismo token. Conserva Taller Mayor + periodo salvo expansión explícita (histórico / todos sus folios / en total).
- **Cross-domain IGF:** conserva Folio/unidad. No candidatos de planta. No mutation. No savings.
...
- **Scope:** exactamente una planta autorizada; `meeting_period` = YYYY-MM abierto CDMX; `meeting_type=monthly_close`.
- **Pack:** commercial; open-month IGF (forecast/versión vigente, no actual de cierre); actions; IGF reviewable supports; information gaps. **No** Taller Mayor, Mejora Continua, Plaud, IGF mes cerrado, SEH, ingreso real de cliente.
- **Isolation:** fresh load; provenance separado; limitations/gaps separados; brief partial permitido; missing ≠ 0; unsupported ≠ 0; source error ≠ hallazgo.
- **Information gaps:** ausencia de evidencia contextual/explicativa cargada. No son causa. Lenguaje seguro: «No encuentro evidencia cargada que explique…»; «No hay resultado registrado…»; «Conviene obtener contexto antes de la junta.»
- **Materialidad:** señales determinísticas ya existentes + GPT. Runtime: deviations, rankings, statuses, overdue, reviewability, gaps. GPT: prioriza, ordena, sintetiza, sugiere qué aclarar. Sin thresholds nuevos, learned score ni checklist hardcodeado.
...
- **Endpoint relacionado:** `POST /api/director-ia/chat` (in-process; sin HTTP interno; sin writes; sin snapshot persistido)
- **Clases de verdad (no mezclar):** ACTUAL = ARR mensual real. TARGET_COMMITMENT = `igf_meta` (META/COMPROMISO gerencial firmado del mes). FORECAST = IGF. DERIVED_MODEL = `forecast_mensual` / modelos derivados (no usado como meta ni actual).
- **Target:** `igf_meta.versions` + `igf_meta.meta_lines`; exact year + exact month + `is_current=true` + mapping empresa de la planta autorizada. `venta_ton` = TARGET/COMMITMENT de venta. Si falta: `TARGET_MISSING_FOR_PERIOD`. **No** carry-forward. **No** latest available. **No** forecast como meta. **No** hardcode. **No** Plaud runtime.
- **Actual sales:** `arr.ventas_diarias_cliente`; `SUM(kg)`; mes calendario; `ton = kg / 1000`. Si hay target: actual / target / delta / attainment %.
...
- **Financial:** `financial.target` = `igf_meta` (TARGET_COMMITMENT). `financial.forecast` = IGF (FORECAST). `financial.actual` = `UNSUPPORTED_METRIC`. TARGET vs FORECAST puede etiquetarse como proyección; **no** es cumplimiento financiero real.
- **Actions:** Action Register existente.
- **Gaps:** `TARGET_MISSING_FOR_PERIOD`; `FINANCIAL_ACTUAL_UNSUPPORTED`; movimiento sin explicación; acción sin resultado; source unavailable. gap ≠ cause.
...
- **Información que no puede concluirse con esta fuente:** actual financiero; causa de un mover; meta de otro mes; cumplimiento financiero usando forecast; IES/N5; Plaud como hecho
...
- **Información que no puede concluirse con esta fuente:** buen/mal día hardcoded; «el descuento provocó la venta»; ingreso diario; tendencia 30/90 CASA/COMISIONISTA (otro intent: `commercial_trend`); perfil longitudinal 3M (otro intent: `client_profile`); Taller Mayor por unidad (otro intent: `taller_mayor`); saludo personalizado; SEH; closed-month IGF; IES/N5
...
- **Información que no puede concluirse con esta fuente:** ahorro realizado; cash; mejora real del IGF; materialización contable de lo no cancelable; que el Director IA cancele o solicite cancelación; cheque operativo; closed-month; riesgo comercial inventado; reviewability de un Folio activo de `taller_mayor` (ese hilo **no** abre este listado de planta)
...
| ¿Qué unidades de Puebla tienen apoyos de Taller Mayor este mes? / Lista Taller Mayor del mes (ejemplos de intención, no phrasebook) | Sí (`taller_mayor`; agrupa por token canónico; `mes_cargo` CDMX; no económico/placa) | PARCIAL (transversal; M5 sigue PARCIAL) | `public.folios.unidad` + `subcategoria` Mayor | `loadTallerMayorForChat` / `assembleTallerMayorPack` | Inventar unidad; clasificar por importe/concepto; mes pasado silencioso | Alto si se lee como `taller_at` de todos los tipos o como catálogo |
| ¿Cuál tiene el importe más alto? (tras lista Taller Mayor) | Sí: unidad con mayor `SUM(importe)`. Empate → clarificar | PARCIAL (continuidad efímera) | Pack fresco agrupado | `rankHighestUnits` / inherit `taller_mayor` | Elegir Folio en silencio; reusar SUM del turno anterior | Alto si N Folios se colapsan a uno |
| Háblame de esa unidad. / ¿Qué reparación le están haciendo? / ¿Qué Folio es? / ¿En qué estatus está? (tras unidad Taller Mayor) | Sí si hay `active_unit`; requery. N Folios: responde a nivel unidad; no silent pick | PARCIAL | Pack fresco + `HILO` | `forceIntent taller_mayor` / loader | Diagnosticar mecánica; causalidad desde concepto; inventar Folio | Alto si se afirma causa o se elige un Folio |
| ¿Todavía se puede detener? (con `active_folio` de Taller Mayor) | Sí: `classifyCancellationEligibility` de **ese** Folio. **No** IGF plant-wide | PARCIAL (read-only) | Folio activo fresco | same loader; `classifyCancellationEligibility` | Cancelar; recomendar; ahorro; reversión contable | Alto si se lee reviewable como mandato de cancelar |
| ¿Qué otros Folios ha tenido esa unidad? / ¿Cuánto llevamos en reparaciones de esa unidad? | Sí: misma planta + mismo token. Default = Taller Mayor + periodo del hilo. Expande solo si pide histórico / todos / en total | PARCIAL | SELECT fresco; horizonte explícito | `asksHistoryExpansion` / loader | Mezclar GASTOS/INVERSIONES; cambiar el mes en silencio | Alto si se afirma lifetime cost o predictivo |
| ¿Cómo afectaría al IGF si no entrara? (con `active_folio` Taller Mayor) | Conserva Folio+unidad. Overlay filtrado si es compatible; si no, limitation. No candidatos de planta | PARCIAL | Folio activo + overlay existente si aplica | `buildSingleFolioIgfHypothetical` | Mutation; savings realizado; listado reviewable de planta | Alto si se sustituye el Folio por candidatos generales |
...
| ¿Cómo cerramos? / ¿Cómo cerró Puebla en julio? / ¿Cómo quedamos contra la meta? / ¿Cuánto nos faltó para la meta? / ¿Qué porcentaje cumplimos? (ejemplos de intención, no phrasebook) | Sí (`month_close_result`; una planta; un mes calendario CDMX; ACTUAL vs TARGET `igf_meta`) | PARCIAL (transversal; no módulo) | ARR mensual + `igf_meta` exacto + IGF forecast | `loadMonthCloseResultForChat` / `isMonthCloseQuestion` | Carry-forward de meta; forecast como meta; actual financiero inventado | Alto si se mezclan las cuatro clases de verdad |
...
| ¿Qué clientes perdimos? / Háblame del primero. (tras cierre) | Sí: new/lost/movers del mes vs mes previo. «Háblame del primero» setea entidad; hop a `client_profile` si hay entidad activa | PARCIAL | `cliente_key`; no fuzzy | `classifyClients` / handoff | Join por nombre; mover = causa | Alto si se afirma causa del movimiento |
...
El chat legado integra **Taller Mayor por unidad** (`taller_mayor` → `loadTallerMayorForChat` / `assembleTallerMayorPack`; source **B**; routing **B**): identidad `(planta_id, token canónico de public.folios.unidad)`; **no** económico/placa/unit master; Taller Mayor = `matchTallerTipoCol===mayor` (no importe, no concepto); «este mes» = YYYY-MM actual CDMX / `mes_cargo`; lista agrupada (`folio_count`, `SUM(importe)`, Folios, estatus, concepto, reviewability); ranking = unidad con mayor SUM; N Folios → no silent pick; `classifyCancellationEligibility` del Folio activo (no hop IGF plant-wide); historial misma planta+token; requery; GPT sintetiza; read-only. No puntúa módulos. M5 **sigue PARTIAL**.
...
- M5 Taller por AT (query JSON de folios TALLER por token de `public.folios.unidad` y `YYYY-MM`; no `at_id`; no catálogo; no Excel/workbook; no duplicados; el intent transversal `taller_mayor` **no** completa M5; no COMPLETE)
...
- Capacidad transversal `month_close_result` (arquitectura B; first slice C: una planta + un mes calendario CDMX; ACTUAL ARR vs TARGET_COMMITMENT `igf_meta` vs FORECAST IGF vs DERIVED_MODEL; `financial.actual` unsupported; target exacto YYYY-MM; `TARGET_MISSING_FOR_PERIOD`; requery; read-only): **no** es un módulo M0–M20; **no** cambia ningún módulo ni el 52.5%.
...
- Ingreso diario / brief matutino programado / saludo personalizado / directorio SEH / closed-month IGF / comments de gráfica vía `cliente_key` / **ingreso mensual actual de cliente** / **actual financiero de cierre** / **Plaud runtime** (el first slice **B** de `daily_executive_brief`, el first slice **B** de `commercial_trend`, el perfil `client_profile`, `taller_mayor`, `pre_meeting_brief` y `month_close_result` ya están integrados; `commercial_trend` cubre 30/90 CASA/COMISIONISTA + OLS + top-6 **sin** comments; `client_profile` cubre 3M calendario keyed **sin** ingreso actual ni join AR; `taller_mayor` cubre lista/ranking/reviewability/historial de Taller Mayor por unidad **sin** Excel M5 ni unit master; `pre_meeting_brief` cubre comercial + IGF abierto + acciones + reviewable + gaps **sin** Taller Mayor / MC / Plaud / mes cerrado en el pack; `month_close_result` cubre ACTUAL mensual vs TARGET `igf_meta` vs FORECAST IGF **sin** actual financiero ni carry-forward de meta. Plaud queda como evidencia **futura** de evaluación histórica; meeting statement ≠ causal truth)
...
| Taller Mayor por unidad | `lib/director-ia-taller-mayor.js`, `test/director-ia-taller-mayor.test.js` |
| Preparación de junta / pre-cierre | `lib/director-ia-pre-meeting.js`, `test/director-ia-pre-meeting.test.js` |
| Resultado mensual de cierre | `lib/director-ia-month-close-result.js`, `test/director-ia-month-close-result.test.js` |
