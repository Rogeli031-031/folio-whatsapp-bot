...
| **Archivos actuales relacionados** | `lib/director-ia-igf-arr.js` (`extractIgfComposition`, `formatIgfCompositionBlock`, `loadIgfCommitSnapshot`, `loadIgfArrAnnexForChat`, `loadIgfArrSourceBlocksForChat`); `lib/director-ia-financial-diagnosis.js` (ensamblaje transversal; no cambia cobertura M7); wiring existente `lib/director-ia-tools.js` (`get_igf_snapshot`), `lib/director-ia-planner.js` (`igf_status` / `financial_diagnosis`), `lib/director-ia-capabilities.js`; `igf-handler.js`, `lib/dashboard-arr-forecast.js` (referencia de producto; no transporte del slice). |
...
| **Información exacta que sí consulta** | `loadArrProyForPlant`, top clientes (`loadTopClientesDescBrief`), `dashboard-arr-forecast` helpers; `dicf.computeDicf` lee datos ARR. En `financial_diagnosis`: bloque ARR separado vía `loadIgfArrSourceBlocksForChat` (`venta_ton` / `desc_kg`; periodo real; no se copia venta IGF). |
...
| **Observaciones verificadas** | `sources.arr = false` fijo en context GET. **Sync transversal** `IMPL-DIRECTOR-IA-FINANCIAL-DIAGNOSIS-EVIDENCE-ASSEMBLY-001` (merge `f7f90270`): ARR entra como bloque propio en `financial_diagnosis`; `arr_status` se preserva. M8 **sigue PARCIAL**. Scoring M0–M20 **sin cambio**: 10.5/20 = **52.5%**. |
...
| **Observaciones verificadas** | `IMPL-DIRECTOR-IA-M9-DELTAS-001` (integrado en main, `7b3e5a98`). Tests focales 23/23; suite `test/director-ia-*.test.js` 459/459; scripts capabilities 25/25, planner 30/30, orchestrator 24/24. COMPLETE = consulta autorizada de las tres familias de periodos reales; no forecast mutante; no M19; no causalidad. **Sync transversal** `IMPL-DIRECTOR-IA-FINANCIAL-DIAGNOSIS-EVIDENCE-ASSEMBLY-001` (merge `f7f90270`): M9 entra junto a IGF/ARR en `financial_diagnosis`; intents `delta_sales` / `delta_discount` / `delta_income` se preservan in-process. M9 **sigue COMPLETA**. Scoring M0–M20 **sin cambio**: 10.5/20 = **52.5%**. |
...
- **Evidencia de integración actual:** `shouldAttachIgfArrAnnex` + sources de prompt `igf.forecast` / `arr.forecast` en modos focused; bloque ARR separado en `financial_diagnosis` (`assembleFinancialDiagnosisEvidence.sources.arr`)
...
- **Routing preservado:** `igf_status`, `arr_status`, `delta_sales`, `delta_discount`, `delta_income`, M6, M11, M12, M18.
- **Información que no puede concluirse con esta fuente:** causa confirmada; responsable; impacto causal; runtime IES/N5; GET `sources.igf|arr`
...
| Campo | Contenido |
|-------|-----------|
| **Evidencia** | `lib/director-ia-chat.js`, `lib/director-ia-planner.js`, `lib/director-ia-igf-arr.js` (`IGF_SIGNAL_RE`, `ARR_SIGNAL_RE`, `PLANT_FINANCIAL_KPI_RE`), `isCommercialStateListQuestion`, etc. El intent `financial_diagnosis` ya no cae a un solo `delta_*` ni al annex IGF/ARR exclusivo: ensambla IGF+ARR+M9. `igf_status` / `arr_status` / `delta_*` / M6 / M11 / M12 / M18 se preservan. |
| **Impacto posible** | Preguntas legítimas pueden no activar la fuente correcta fuera de `financial_diagnosis`. El listado de folios GASTOS/INVERSIONES ya va a M6; «cómo van los gastos» / margen / rentabilidad pueden ir a `financial_diagnosis` o al annex IGF según wording. Export/xlsx sigue fuera. |
