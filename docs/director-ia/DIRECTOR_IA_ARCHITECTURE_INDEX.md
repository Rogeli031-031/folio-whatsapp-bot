...
| FINANCIAL-ACTUAL-EVIDENCE-CONTRACT | Ninguno (`is_final` físico, loader ACTUAL_FINANCIAL, P&L runtime: **no existen**) | **No** — contrato de evidencia de dominio v1.0; no implementa Constitución / EKE / Evidence Builder / IES |
...
7. `FINANCIAL-ACTUAL-EVIDENCE-CONTRACT.md` no es capa de pipeline (no N6/N7; no `07`). ACTUAL_FINANCIAL **aún no alimenta** el IES oficial. No existen aún `is_final` físico, loader ACTUAL_FINANCIAL, exposición P&L runtime ni AUTHZ resuelto. Chat/runtime legado no es implementación del pipeline constitucional.
...
| Versión | 1.9 |
| Estado | APROBADO COMO ÍNDICE (incluye `05` v1.0 congelado; `06` v1.0 propuesto, no congelado; EKS runtime mínimo sincronizado, no COMPLETE constitucional; FINANCIAL-ACTUAL-EVIDENCE-CONTRACT v1.0 indexado como evidencia de dominio, runtime/IES pendientes) |
