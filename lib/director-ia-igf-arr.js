...
/** @type {{ getPlantCodeArrFromPlantaNombre?: Function, getMargenKgPorPeriodo?: Function, assertGVPlantaNombreAccess?: Function, loadArrLastUploadDay?: Function, loadDashboardForecastParity?: Function, now?: Function }} */
let deps = {};
...
function postCloseCutoffYmd(year, month) {
  if (!Number.isFinite(Number(year)) || !Number.isFinite(Number(month))) return null;
  if (Number(month) >= 12) return `${Number(year) + 1}-01-01`;
  return `${Number(year)}-${String(Number(month) + 1).padStart(2, "0")}-01`;
}

function emptyArrMetrics(extra = {}) {
  return {
    observed_venta_ton: null,
    projected_venta_ton: null,
    observed_desc_kg: null,
    projected_desc_kg: null,
    cutoff: extra.cutoff != null ? extra.cutoff : null,
    cutoff_source: extra.cutoff_source != null ? extra.cutoff_source : null,
    month_kind: extra.month_kind || null,
    projection_status: extra.projection_status || null,
    venta_ton: null,
    desc_kg: null,
  };
}
...
/**
 * Mismo orden que resolveDirectorIaEffectiveCutoff (chat), sin require circular.
 * 1) cutoff explícito en la pregunta 2) req.body.upload_day|cutoff_date 3) last upload.
 */
async function resolveArrAnnexCutoff(input = {}) {
  const fromQuestion = parseExplicitCutoffFromQuestion(input.question);
  if (fromQuestion) {
  ...
  const lastFn = input.loadArrLastUploadDay || deps.loadArrLastUploadDay;
  if (typeof lastFn === "function") {
    try {
      const last = await lastFn(input.queryable, input.year, input.month, {
      ...
      /* last upload ausente no inventa corte */
    }
  }
...
function resolveNow(opts = {}) {
  if (opts.now instanceof Date) return opts.now;
...
/**
 * Métricas ARR observadas vs proyectadas. Reusa loadDashboardForecastParity.
 * No pide proyección intra-mes sin corte.
 */
async function loadArrMetricsForPlant(poolOrClient, year, month, plantCode, opts = {}) {
  const fechaCorte = parseCutoffYmd(opts.fechaCorte || opts.upload_day || opts.cutoff_date);
  const now = resolveNow(opts);
  const historico = isMesHistoricoArrClientes(year, month, now);
  const loadParity = opts.loadParity || deps.loadDashboardForecastParity || loadDashboardForecastParity;
  const monthKind = historico ? "CLOSED" : "OPEN";

  if (!fechaCorte && !historico) {
    return emptyArrMetrics({
      cutoff: null,
      cutoff_source: opts.cutoff_source || null,
      month_kind: monthKind,
      projection_status: "CUTOFF_UNAVAILABLE",
    });
  }

  const uploadDay = fechaCorte || postCloseCutoffYmd(year, month);
  const parity = await loadParity(poolOrClient, {
    year,
    month,
    plantCode,
    plantName: opts.plantName || null,
    upload_day: uploadDay,
    fechaCorte: uploadDay,
    cutoff_date: uploadDay,
  });

  const observed = finiteOrNull(parity && parity.actual_to_date && parity.actual_to_date.venta_ton);
  const observedDesc = finiteOrNull(parity && parity.actual_to_date && parity.actual_to_date.desc_kg);
  let projected = finiteOrNull(parity && parity.forecast && parity.forecast.venta_ton);
  let projectedDesc = finiteOrNull(parity && parity.forecast && parity.forecast.desc_kg);
  if (historico) {
    projected = null;
    projectedDesc = null;
  }

  const ventaAlias = projected != null ? projected : historico ? observed : null;
  const descAlias = projectedDesc != null ? projectedDesc : historico ? observedDesc : null;
  return {
    ...
    cutoff_source: opts.cutoff_source || null,
    month_kind: monthKind,
    projection_status: historico
      ? "NOT_APPLICABLE_CLOSED"
      : projected != null
        ? "AVAILABLE"
        : "UNAVAILABLE",
    venta_ton: ventaAlias,
    desc_kg: descAlias,
  };
}
...
function formatArrAnnexBlocks(arrCurr, arrPrev, year, month, prevYm) {
  const prevSale = finiteOrNull(arrPrev && arrPrev.observed_venta_ton);
  const projected = finiteOrNull(arrCurr && arrCurr.projected_venta_ton);
  const observed = finiteOrNull(arrCurr && arrCurr.observed_venta_ton);
  const observedDesc = finiteOrNull(arrCurr && arrCurr.observed_desc_kg);
  const projectedDesc = finiteOrNull(arrCurr && arrCurr.projected_desc_kg);
  const lines = [
    "ARR — VENTA",
    `- Venta observada al corte: ${fmtNum(observed, 2)} ton`,
  ];
  if (arrCurr && arrCurr.month_kind === "CLOSED") {
    lines.push("- Proyección de cierre ARR: (mes cerrado; no se inventa proyección futura)");
  } else if (projected != null) {
    lines.push(`- Proyección de cierre ARR: ${fmtNum(projected, 2)} ton`);
  } else {
    lines.push("- Proyección de cierre ARR: (corte no disponible; no se calcula proyección sin corte)");
  }
  lines.push("");
  lines.push("ARR — DESCUENTO");
  if (observedDesc != null) {
    lines.push(`- Descuento observado al corte: ${fmtNum(observedDesc, 2)} $/kg`);
  } else {
    lines.push("- Descuento observado al corte: (no exportado por el dashboard)");
  }
  if (arrCurr && arrCurr.month_kind === "CLOSED") {
    lines.push("- Descuento proyectado ARR: (mes cerrado; no se etiqueta Desc. PROY)");
  } else if (projectedDesc != null) {
    lines.push(`- Descuento proyectado ARR: ${fmtNum(projectedDesc, 2)} $/kg`);
  } else {
    lines.push("- Descuento proyectado ARR: (no disponible)");
  }
  lines.push("");
  lines.push("MES PREVIO");
  const prevLabel = prevYm
    ? `${MESES_ES[prevYm.month - 1]} ${prevYm.year}`
    : "mes previo";
  lines.push(`- Venta mes previo (${prevLabel}): ${fmtNum(prevSale, 2)} ton`);
  lines.push("");
  lines.push("COMPARACION");
  if (projected != null && prevSale != null) {
    lines.push(
      `- Proyección cierre actual vs venta mes previo: ${signedDelta(projected - prevSale)} ton`
    );
  } else {
    lines.push("- Proyección cierre actual vs venta mes previo: (faltan cifras)");
  }
  if (year && month) {
    lines.push(`- Mes consultado: ${MESES_ES[month - 1]} ${year}`);
  }
  return lines;
}
...
async function loadIgfArrAnnexForChat(pool, plantaId, req, question) {
  ...
  try {
    ...
    let arrCurr = emptyArrMetrics();
    let arrPrev = emptyArrMetrics();
    if (wantArr || wantIgf) {
      const cutoffRes = await resolveArrAnnexCutoff({
        question,
        req,
        plantCode,
        year,
      ...
      const metricOpts = {
        fechaCorte: cutoffRes.cutoff,
        cutoff_source: cutoffRes.source,
        now: resolveNow(),
        plantName: plantaNombre,
      };
      try {
        arrCurr = await loadArrMetricsForPlant(pool, year, month, plantCode, metricOpts);
        arrPrev = await loadArrMetricsForPlant(pool, prev.year, prev.month, plantCode, metricOpts);
      } catch (e) {
        console.warn("[director-ia-igf-arr] ARR metrics:", e.message || e);
      }

      lines.push(...formatArrAnnexBlocks(arrCurr, arrPrev, year, month, prev));
      lines.push("");
    }
    ...
    if (wantIgf || wantArr) {
      ...
      if (!igf.version_id) {
        ...
        if (!row) {
          ...
          const ventaArrProy = finiteOrNull(arrCurr.projected_venta_ton);
          const descArrProy = finiteOrNull(arrCurr.projected_desc_kg);
          lines.push(`- Compromiso venta IGF: ${fmtNum(ventaIgf, 1)} ton`);
          lines.push(`- Margen $/kg: ${fmtNum(margen, 2)}`);
          lines.push(
            `- Com. y Desc. $/kg (IGF): ${fmtNum(comDesc, 2)} | Descuento proyectado ARR: ${fmtNum(descArrProy, 2)}`
          );
          lines.push(`- HG $/kg: ${fmtNum(hgKg, 2)}`);
          if (ventaArrProy != null && margen != null && descArrProy != null && hgKg != null) {
            const ingresoApprox = Math.round(
              (Number(margen) + Number(descArrProy) - Number(hgKg)) * Number(ventaArrProy) * 1000
            );
            lines.push(
              `- Ingreso aprox. cruzado (proyección ARR × margen/desc/HG IGF; no es ingreso ARR puro ni proyección ARR): ${fmtMoney(ingresoApprox)}`
            );
          }
          if (wantIgf) {
...
/**
 * Bloques IGF y ARR separados para financial_diagnosis.
 * No fusiona venta/desc. No calcula ingreso híbrido. No carga commercial_state.
 */
async function loadIgfArrSourceBlocksForChat(pool, plantaId, req, question) {
  if (!pool) {
    return { ok: false, abort: false, status: 500, error: "Pool no configurado" };
  }
  if (!deps.getPlantCodeArrFromPlantaNombre) {
    return { ok: false, abort: false, status: 500, error: "IGF/ARR chat no configurado en servidor" };
  }
  ...
  try {
    ...
    if (!plantaNombre) {
      return {
        ...
        year,
        month,
        plant: { planta_id: Number(plantaId), planta_nombre: null, plant_code: null },
        igf: null,
        arr: null,
      };
    }
    ...
    if (deps.assertGVPlantaNombreAccess) {
      ...
      if (!gvChk.ok) {
        return {
          ok: false,
          abort: true,
          status: gvChk.status || 403,
          code: "SOURCE_RESTRICTED",
          error: gvChk.error || "Sin acceso a esta planta",
        };
      }
    }
    ...
    let igf = { version_id: null, version_number: null, row: null, composition: null, load_error: null };
    try {
      const snap = await loadIgfCommitSnapshot(client, year, month, plantCode, plantaNombre);
      const composition = snap.row
        ? extractIgfComposition(snap.row, {
            year,
            month,
        ...
        : null;
      igf = { ...snap, composition, load_error: null };
    } catch (e) {
      igf = {
        version_id: null,
        version_number: null,
        row: null,
        composition: null,
        load_error: e && e.message ? String(e.message) : "Error IGF",
      };
    }

    let arr = { ...emptyArrMetrics(), load_error: null };
    try {
      const cutoffRes = await resolveArrAnnexCutoff({
        question,
        req,
      ...
      arr = {
        ...(await loadArrMetricsForPlant(pool, year, month, plantCode, {
          fechaCorte: cutoffRes.cutoff,
          cutoff_source: cutoffRes.source,
          now: resolveNow(),
          plantName: plantaNombre,
      ...
      arr = {
        ...emptyArrMetrics(),
        load_error: e && e.message ? String(e.message) : "Error ARR",
      };
    }

    return { ok: true, abort: false, year, month, plant, igf, arr };
  } catch (e) {
    console.error("[director-ia-igf-arr] source blocks", e);
    return { ok: false, abort: false, status: 500, error: e.message || "Error al cargar IGF/ARR" };
  } finally {
    client.release();
  }
}
