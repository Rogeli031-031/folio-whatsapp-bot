...
const IGF_ARR_ANNEX_SYSTEM_ADDENDUM =
  "Cuando el mensaje incluya un ANEXO — IGF / ARR, úsalo como complemento financiero de planta (venta, margen, descuento, utilidad, delta clientes). No sustituye Bitácora IA, DICF acciones ni Action Register. Cita el mes y corte del anexo. Si preguntan por dejaron/aumentaron/disminuyeron/nuevos, prioriza el bloque ESTADO COMERCIAL del anexo o del contexto commercial_state.";

/** IGF / forecast financiero / utilidad (estricto: evita capturar «margen» genérico). */
const IGF_SIGNAL_RE =
...
function isIgfForecastQuestion(question) {
  return IGF_SIGNAL_RE.test(String(question || ""));
}
...
/**
 * ¿Adjuntar anexo IGF/ARR al final del chat?
 * No se activa con «margen» solo (evitar robar routing DICF).
 */
function shouldAttachIgfArrAnnex(question) {
  ...
  if (/\bmargen\b/i.test(q) && /\b(igf|forecast|planta|utilidad|kg)\b/i.test(q)) return true;
...
async function loadIgfArrAnnexForChat(pool, plantaId, req, question) {
  ...
  const wantIgf = isIgfForecastQuestion(question) || /\bmargen\b/i.test(question);
  const wantArr =
  ...
  const wantCommercial =
    isDeltaClientesIgfQuestion(question) ||
    isIgfForecastQuestion(question) ||
    /\b(cliente|venta|ingreso|delta)\b/i.test(question);
