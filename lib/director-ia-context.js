...
const clienteComentariosLib = require("./cliente-comentarios");
...
const EMPTY_SOURCES = Object.freeze({
  ...
  bitacora_ia: false,
  cliente_comentarios: false,
  folio_comentarios: false,
});
...
async function buildDirectorIaContextPayload(req) {
  ...
  try {
    ...
    const cliente_comentarios = await clienteComentariosLib.loadClienteComentariosForDirectorIa(
      client,
      planta_id,
      80
    );
    const folio_comentarios = await clienteComentariosLib.loadFolioComentariosForDirectorIa(
      client,
      planta_id,
      80
    );
    ...
    if (cliente_comentarios.length > 0) sources.cliente_comentarios = true;
    if (folio_comentarios.length > 0) sources.folio_comentarios = true;
    return {
      ...base,
      sources,
      bitacora,
