...
async function loadPredictiveForChat(pool, plantaId, req, opts = {}) {
  ...
  if (family === "CHANNEL_FORECAST_GUARDRAIL") {
    return {
      ok: true,
      family,
      spec,
...
function buildPredictiveAnswer(payload) {
  ...
  if (payload.risk) {
    if (!payload.risk.length) return "No tengo clientes con señal de interrupción (retraso respecto de frecuencia histórica DICF). No afirmo que vayan a dejar de comprar.";
    return [
      ...
      ...payload.risk.map((r, i) =>
        [
          ...
          `   Frecuencia histórica: ${r.historical_frequency} d`,
          `   Días transcurridos: ${r.days_since_last_purchase}`,
          `   Retraso: ${r.overdue_days} d`,
        ].join("\n")
      ),
    ].join("\n");
  }
  ...
  if (payload.ranked && payload.family === "TOP_CLIENTS_BY_CHANNEL") {
    const label = spec.customer_segment === "CASA" ? "Casa" : spec.customer_segment === "COMISIONISTA" ? "Comisionistas" : "clientes";
    const lines = [`Top ${spec.limit} clientes ${label} por compra observada — ${month}`];
    payload.ranked.forEach((r, i) => {
