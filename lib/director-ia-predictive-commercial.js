...
function isExpectedNextPurchaseQuestion(question) {
  ...
  if (/\baction\s+register\b/.test(n) || /\bforecast\b/.test(n) || /\bpronostic/.test(n)) return false;
  if (/\b(fuente|tabla|datos|informacion)\b/.test(n) && /\batrasad/.test(n)) return false;
  if (/\bquien(?:es)?\b/.test(n) && /\b(atrasad|romp)/.test(n)) return false;
  if (/\bque\s+clientes\b/.test(n) && /\b(atrasad|romp)/.test(n)) return false;
  return Boolean(
    /\bcuando\s+deberia\b/.test(n) ||
      /\bcuando\s+le\s+toca\b/.test(n) ||
      /\bcuando\s+esperamos\b/.test(n) ||
      /\bcuando\s+esperamos\s+su\s+proximo\b/.test(n) ||
      /\bcuando\s+compraria\b/.test(n) ||
      /\bcuando\s+vence\s+su\s+ciclo\b/.test(n) ||
      /\bcual\s+seria\s+su\s+siguiente\b/.test(n) ||
      /\bcuando\s+se\s+esperaba\b/.test(n) ||
      /\bcada\s+cuantos\s+dias\s+compra\b/.test(n) ||
      /\bcual\s+fue\s+(su\s+|la\s+)?ultima\s+compra\b/.test(n) ||
      /\bultima\s+compra\b/.test(n) ||
      /\bcada\s+cuantos\s+dias\s+compra\b/.test(n) ||
      /\bfrecuencia\s+de\b/.test(n) ||
      /\bcuando\s+vuelve\s+a\s+comprar\b/.test(n) ||
      ...
      /\by\s+(cual\s+fue\s+)?su\s+(frecuencia|ultima\s+compra)\b/.test(n) ||
...
function classifyPredictiveFamily(question, prior) {
  ...
  if (runtime008.isTallerExpenseQuestion(question) || runtime008.isTallerExpenseFollowUp(question, prior)) return null;
  if (runtime008.isRegulationStatusQuestion(question, prior) || runtime008.isRegulationPlantDetailQuestion(question, prior)) {
    return null;
  }
  if (prior && (prior.parent_intent === "expense_analytics" || prior.family === "TALLER_EXPENSE") && runtime008.isTallerExpenseFollowUp(question, prior)) {
    return null;
  }
  ...
  if (
    runtime009.isLastPurchaseQuestion(question) ||
    runtime009.isFrequencyQuestion(question) ||
    runtime009.isOverdueQuestion(question)
  ) {
    return "EXPECTED_NEXT_PURCHASE";
  }
...
function extractExpectedClientHint(question) {
  ...
  const cleaned = n
    .replace(
      /\b(cuando|esperamos|esperariamos|deberia|deberian|volver|vuelva|volveria|comprar|compra|compro|comprado|compraria|proxima|siguiente|pedido|toca|tocaria|regresar|normalmente|estimada|estimado|atrasado|atrasada|atrasados|atrasadas|esta|este|de|del|la|el|los|las|que|cual|fue|fui|su|sus|a|otra|vez|cliente|clientes|estan|riesgo|dejar|senal|senales|interrupcion|frecuencia|historica|historial|listado|marca|quienes|quien|ultima|ultimas|dias|cuantos|lleva|sin|haber|alrededor|aproximadamente|segun|freqdays|cada|cuanto|tiene|hay|para|fecha|dia|ciclo|vence|cae|seria|serian)\b/g,
      " "
    )
    .replace(/\s+/g, " ")
    .trim();
  ...
  try {
    ...
    return (cached.rows || [])
      .map((r) => ({
        ...
        lastPurchaseDate:
          (r.lastPurchaseDate ? String(r.lastPurchaseDate).slice(0, 10) : null) ||
          lastMap.get(String(r.cliente || "").trim()) ||
          null,
...
async function loadPredictiveForChat(pool, plantaId, req, opts = {}) {
  ...
  if (family === "CLIENT_CHURN_RISK" || family === "EXPECTED_NEXT_PURCHASE") {
    const runtime008 = require("./director-ia-seh-taller-purchase-evidence-008");
    const physical = await runtime008.loadDicfClientsPhysical(db, plantLabel, {
      dicfRows: opts.dicfRows,
      computeDicf: opts.computeDicf || (opts.dicfRows ? null : require("./dicf").computeDicf),
      getMargenKgPorPeriodo: opts.getMargenKgPorPeriodo,
      arrPlantCode: opts.arrPlantCode || (codes && codes[0]),
      plantCodes: codes,
      now,
      allowComputeWithoutDb: Boolean(opts.computeDicf),
...
function finishWithData({ spec, family, now, planta_id, opts, question }) {
  ...
  if (family === "CLIENT_CHURN_RISK" || family === "EXPECTED_NEXT_PURCHASE") {
    ...
    const runtime009 = require("./director-ia-purchase-evidence-enrichment-009");
    const active = runtime010.resolveActiveClient({
      prior: opts.priorSpec,
      conversation_state: opts.conversation_state,
    });
    const inheritedClient =
      opts.priorClient ||
      (active && active.canonical_name) ||
      (opts.priorSpec && Array.isArray(opts.priorSpec.ranked_names) && opts.priorSpec.ranked_names[0]) ||
      null;
    const followUpClient =
      runtime010.isActiveClientFollowUpQuestion(question) ||
      runtime009.isLastPurchaseQuestion(question) ||
      runtime009.isFrequencyQuestion(question) ||
      runtime009.isOverdueQuestion(question);
    let matched = runtime008.matchExpectedClients(opts.dicfRows || [], hint);
    if ((matched.status === "none" || (followUpClient && inheritedClient && !hint)) && inheritedClient) {
      matched = runtime008.matchExpectedClients(opts.dicfRows || [], inheritedClient);
    }
    if (followUpClient && inheritedClient && matched.status !== "unique") {
      matched = runtime008.matchExpectedClients(opts.dicfRows || [], inheritedClient);
    }
    if (followUpClient && !inheritedClient && !hint) {
      return {
        ok: true,
        family,
        spec,
        now,
        planta_id,
        clarification: "¿De qué cliente? No hay una entidad activa para resolver el pronombre.",
      };
    }
    if (hint && matched.status === "ambiguous") {
      return {
        ok: true,
        family,
        spec,
        now,
        planta_id,
        clarification: `«${hint}» es ambiguo. Coincide con: ${matched.rows
          .map((r) => r.cliente)
          .slice(0, 8)
          .join(", ")}. ¿A cuál te refieres?`,
      };
    }
    if (hint && matched.status === "none") {
      return {
        ok: true,
        family,
        spec,
        now,
        planta_id,
        limitation: `INSUFFICIENT_EVIDENCE: no encontré ${hint} en la evidencia física de Movimiento por categoría. No elijo otro cliente.`,
      };
    }
    let pool = matched.status === "unique" ? matched.rows : opts.dicfRows || [];
    ...
    for (const r of pool) {
      ...
      const lastValid = lastDate instanceof Date && !Number.isNaN(lastDate.getTime());
      if (!last || !lastValid || !Number.isFinite(freq) || freq <= 0 || freq >= 9999) {
        missing.push({
          cliente: r.cliente,
          missing: [
            !last || !lastValid ? "last_purchase_date" : null,
            !Number.isFinite(freq) || freq <= 0 || freq >= 9999 ? "freqDays" : null,
          ].filter(Boolean),
        });
        continue;
      }
      ...
      const days = asOfValid ? Math.round((asOfDate - lastDate) / 86400000) : NaN;
      const expected = new Date(lastDate.getTime());
      expected.setDate(expected.getDate() + freq);
      let expectedNext = null;
      if (expected instanceof Date && !Number.isNaN(expected.getTime())) {
        try {
          expectedNext = expected.toISOString().slice(0, 10);
        } catch (_e) {
          expectedNext = null;
        }
      }
      if (!expectedNext || !Number.isFinite(days)) {
        missing.push({ cliente: r.cliente, missing: ["expected_next_purchase_date"] });
        continue;
      }
      scored.push({
        cliente: r.cliente,
    ...
    if (family === "EXPECTED_NEXT_PURCHASE") {
      return {
        ...
        planta_id,
        question,
        asOf: opts.asOfDate || (now && now.toISOString && now.toISOString().slice(0, 10)),
...
function buildPredictiveAnswer(payload) {
  ...
  if (payload.expected) {
    ...
    const runtime008 = require("./director-ia-seh-taller-purchase-evidence-008");
    const runtime009 = require("./director-ia-purchase-evidence-enrichment-009");
    if (payload.expected.length === 1) {
      return runtime009.buildPurchaseEvidenceAnswer(
        payload.expected[0],
        payload.question || (payload.spec && payload.spec.question),
        payload.asOf || (payload.now && payload.now.toISOString && payload.now.toISOString().slice(0, 10))
      ) || runtime008.buildExpectedNextAnswer(payload.expected[0]);
    }
    return [
  ...
  return {
    ...
    context_meta: {
      ...
      conversation_state: {
        ...
        active_entities: [
          {
            kind: payload && payload.expected && payload.expected.length === 1 ? "CLIENT" : "predictive_commercial",
            family: payload && payload.family,
            ...
            ranked_names: (
              ((payload && payload.expected) || []).map((r) => r.cliente).concat(((payload && payload.ranked) || []).map((r) => r.cliente))
            ).filter(Boolean).slice(0, 20),
            canonical_name: payload && payload.expected && payload.expected[0] && payload.expected[0].cliente,
            display: payload && payload.expected && payload.expected[0] && payload.expected[0].cliente,
            kind: payload && payload.expected && payload.expected.length === 1 ? "CLIENT" : "predictive_commercial",
          },
        ],
...
function priorPredictiveFromState(state) {
  ...
  const hit = entities.find((e) => e && (e.kind === "predictive_commercial" || e.kind === "CLIENT" || e.kind === "client"));
  ...
  return {
    ...
    ranked_names: (hit && hit.ranked_names) || (hit && hit.canonical_name ? [hit.canonical_name] : []),
    canonical_name: (hit && hit.canonical_name) || (hit && hit.display) || null,
