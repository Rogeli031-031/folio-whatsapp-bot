...
function isSalesChannelShareQuestion(question) {
  ...
  const shareCue = /\bporcentaje\b/.test(n) || /\bparticipacion\b/.test(n) || /\bpeso\b/.test(n) || /\brepresenta\b/.test(n) || /\bparte\s+corresponde\b/.test(n) || /\bcomo\s+se\s+(divide|reparte|dividio)\b/.test(n) || /\bmix\s+porcentual\b/.test(n) || /\bproporcion\b/.test(n) || /\breparte\s+la\s+proyeccion\b/.test(n);
  const channels = /\bcasa\b/.test(n) || /\bcomisionistas?\b/.test(n) || /\bcada\s+(canal|uno)\b/.test(n) || /\bcomo\s+se\s+dividio\b/.test(n) || /\breparte\s+la\s+(venta|proyeccion)\b/.test(n);
  return shareCue && channels && !/\btop\s+\d+\b/.test(n);
}

function isSalesChannelShareChangeQuestion(question) {
  const n = normalize(question);
  if (/\ben\s+el\s+mix\b/.test(n) && !/\bshare\b/.test(n) && !/\bpuntos\b/.test(n)) return false;
  if (
...
function buildFrame(question, prior, now) {
  ...
  return {
    ...
    period: periodFrame.period || (prior && prior.period) || null,
    period_kind: periodFrame.period_kind || (prior && prior.period_kind) || null,
...
async function loadPredictiveForChat(pool, plantaId, req, opts = {}) {
  ...
  if (
    ["SALES_CHANNEL_SHARE", "SALES_CHANNEL_SHARE_CHANGE", "TOP_CLIENTS_PROJECTED_SHARE", "TOP_CLIENTS_PROJECTED_CONCENTRATION", "CLIENT_PARETO", "SALES_MIX_SHIFT", "CLIENT_CHURN_RATE", "NEW_CLIENT_RETENTION"].includes(family) &&
    !hasPeriod(spec)
  ) {
    return {
      ok: true,
  ...
  if (family === "CLIENT_CHURN_RISK" || family === "EXPECTED_NEXT_PURCHASE") {
    ...
    return { ok: true, family, spec, limitation: LIMITATION[family], now, planta_id: plantIdToUse };
  }
...
function finishWithData({ spec, family, now, planta_id, opts, question }) {
  ...
  if (family === "CLIENT_CHURN_RATE" || family === "NEW_CLIENT_RETENTION") {
    ...
    if (family === "NEW_CLIENT_RETENTION") {
      ...
      if (!news) {
        return { ok: true, family, spec, now, planta_id, no_data: "No tengo una generación NEW_IN_PERIOD comparable. No afirmo FIRST_EVER." };
      }
      return {
        ok: true,
        family,
        ...
        now,
        planta_id,
        retention: { new_in_prior: news, retained, lost: lostNew.length, rate: pct(retained, news), names: retainedNames.slice(0, 20) },
  ...
  if (family === "CLIENT_CHURN_RISK" || family === "EXPECTED_NEXT_PURCHASE") {
    const rows = (opts.dicfRows || []).filter((r) => r && Number.isFinite(Number(r.freqDays)) && r.lastPurchaseDate);
    if (!rows.length) return { ok: true, family, spec, limitation: LIMITATION[family], now, planta_id };
    const asOf = opts.asOfDate || now.toISOString().slice(0, 10);
    const scored = rows.map((r) => {
      const last = String(r.lastPurchaseDate).slice(0, 10);
    ...
    if (family === "EXPECTED_NEXT_PURCHASE") {
      return { ok: true, family, spec, now, planta_id, expected: scored.sort((a, b) => a.expected_next.localeCompare(b.expected_next)).slice(0, spec.limit) };
    }
...
function buildPredictiveAnswer(payload) {
  if (!payload) return "No pude consultar esa lectura comercial.";
  ...
  if (payload.retention) {
    return `De ${payload.retention.new_in_prior} clientes nuevos de ${formatMonthLabel(payload.compare_period)} (NEW_IN_PERIOD), ${payload.retention.retained} volvieron a comprar en ${month} (${payload.retention.rate}%). No afirmo primera compra histórica.`;
  }
  if (payload.risk) {
    if (!payload.risk.length) return "No tengo clientes con señal de atraso frente a su frecuencia DICF.";
    return [
      "Clientes con señales de riesgo de interrupción de compra (días desde última compra > frecuencia histórica). No afirmo que vayan a dejar de comprar.",
      ...payload.risk.map((r, i) => `${i + 1}. ${r.cliente} — ${r.overdue_days} días sobre su frecuencia de ${r.historical_frequency} d`),
    ].join("\n");
  }
  if (payload.expected) {
    return [
      "Próxima compra estimada según frecuencia histórica (no es un compromiso):",
      ...payload.expected.map((r) => `${r.cliente}: ${r.expected_next} (última ${r.lastPurchaseDate || ""} + ${r.historical_frequency} d)`.trim()),
    ].join("\n");
  }
  ...
  return limitationAnswer(spec.family);
}
