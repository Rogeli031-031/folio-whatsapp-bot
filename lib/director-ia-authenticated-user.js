...
const LEADING_HONORIFIC_RE =
  /^(?:ingeniero|ing\.?|licenciado|lic\.?|doctora|dra\.?|doctor|dr\.?|arquitecta|arquitecto|arq\.?)\s+/i;

function stripLeadingHonorific(name) {
  const raw = String(name || "").replace(/\s+/g, " ").trim();
  if (!raw) return "";
  return raw.replace(LEADING_HONORIFIC_RE, "").replace(/\s+/g, " ").trim();
}

/** Vocativo de saludo. No altera el valor persistido de usuarios.nombre_persona. */
function nombrePersonaForGreeting(raw) {
  const persisted = sanitizeNombrePersona(raw);
  if (!persisted) return null;
  return sanitizeNombrePersona(stripLeadingHonorific(persisted));
}
...
function buildIdentityGreeting(greetingKind, nombrePersona) {
  ...
  const name = nombrePersonaForGreeting(nombrePersona);
