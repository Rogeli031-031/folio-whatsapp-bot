...
/**
 * Búsqueda fail-closed dentro de UNA planta. Nunca cruza plantas ni inventa datos.
 */
async function searchClienteContactosByPlant(client, opts = {}) {
  const plantaId = opts.planta_id != null ? Number(opts.planta_id) : null;
  if (!plantaId || !Number.isFinite(plantaId)) return { error: "planta_id requerido", matches: [] };
  const hint = normNombre(opts.nombre_hint || opts.cliente_nombre || "");
  if (!hint || hint.length < 3) return { error: "nombre insuficiente", matches: [] };
  const r = await client.query(
    `SELECT id, planta_id, cliente_nombre, canal, subcanal,
            nombre_contacto, telefono, correo, updated_by_usuario_id, updated_at
       FROM arr.cliente_contactos
      WHERE planta_id = $1
        AND (cliente_nombre_norm = $2 OR cliente_nombre_norm LIKE $3)
      ORDER BY CASE WHEN cliente_nombre_norm = $2 THEN 0 ELSE 1 END, cliente_nombre
      LIMIT 8`,
    [plantaId, hint, `%${hint}%`]
  );
  return { matches: (r.rows || []).map(mapRow), cross_plant: false };
}
