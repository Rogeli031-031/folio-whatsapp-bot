...
const { dashboardDescSigned, findMiniRowForPlant } = require("./director-ia-dashboard-forecast-adapter");
const {
...
const FINANCIAL_PRESENTATION_STATES = Object.freeze({
  FINAL: "FINAL",
  VISIBLE_NOT_FINAL: "VISIBLE_NOT_FINAL",
  DATA_MISSING: "DATA_MISSING",
});

const NOT_FINAL_LABEL = "VISTA FINANCIERA DISPONIBLE — NO FINAL";
const NOT_FINAL_WARNING =
  "Esta vista financiera no está marcada como FINAL.\nNo debe presentarse como cierre financiero definitivo.";

const MONTH_TITLE = Object.freeze([
  "",
  "Enero",
  "Febrero",
  "Marzo",
...
function isBareComplianceQuestion(q) {
  return (
    /^estamos cumpliendo$/.test(q) ||
    /^vamos bien$/.test(q) ||
    /^estamos bien o mal$/.test(q) ||
    /^vamos mejorando$/.test(q)
  );
}

function isSalesTargetPerformanceQuestion(raw) {
  const q = normalizeQuestion(raw);
  if (!q) return false;
  if (isBareComplianceQuestion(q)) return false;
  if (/\bforecast\b/.test(q) || /\bproyecc/.test(q) || /\bpresupuesto\b/.test(q)) return false;
  if (/\b(descuento|margen)\b/.test(q)) return false;
  if (/\bmejor(ando)?\b/.test(q) && /\b(mes pasado|mes anterior)\b/.test(q)) return false;
  const namesMeta = /\bmetas?\b/.test(q);
  const namesVenta = /\bventa\b/.test(q);
  const arribaAbajo = /\barriba\b/.test(q) && /\babajo\b/.test(q);
  if (arribaAbajo && namesMeta) return true;
  if (/\bsuperamos\b/.test(q) && namesMeta) return true;
  if (/\bcumplimiento\b/.test(q) && namesVenta) return true;
  return false;
}
...
function isMonthCloseQuestion(raw) {
  ...
  if (isSalesTargetPerformanceQuestion(raw)) return true;
  return false;
}
...
function isMonthCloseFollowUp(raw, kind) {
  ...
  if (/\bcontra la meta\b/.test(q) || (/\bmeta\b/.test(q) && (/\bfalt[ao]\b/.test(q) || /\bcumpl/.test(q) || /\bporcentaje\b/.test(q)))) {
    return true;
  }
  if (/\bporcentaje\b/.test(q) && /\b(llevamos|cumpl|meta)\b/.test(q)) return true;
  if (/\bfalt[ao]\b/.test(q) && !/\bdescuento\b/.test(q)) return true;
  if (/\barriba\b/.test(q) && /\babajo\b/.test(q)) return true;
  if (/\bsuperamos\b/.test(q) && /\bmetas?\b/.test(q)) return true;
...
function presentationFinite(value) {
  if (value == null || value === "") return null;
  const n = Number(value);
  return Number.isFinite(n) ? n : null;
}

function hgDisplayPct(hgPct) {
  const n = presentationFinite(hgPct);
  if (n == null) return null;
  return Math.round(n * 10000) / 100;
}

function hgDollarMxnKg(hgKg, hgPct) {
  const kg = presentationFinite(hgKg);
  const pct = presentationFinite(hgPct);
  if (kg == null || pct == null || pct === 0) return null;
  const out = Math.abs(kg / pct);
  return Number.isFinite(out) ? out : null;
}

function emptyPresentationValues() {
  return {
    venta_ton: null,
    casa_ton: null,
    comisionista_ton: null,
    margen_mxn_kg: null,
    descuento_mxn_kg: null,
    impuestos_mxn_kg: null,
    hg_pct: null,
    hg_mxn_kg: null,
    operativos_mxn: null,
    corporativos_mxn: null,
    gasto_mxn: null,
    ingreso_mxn: null,
    rentabilidad_operativa_mxn: null,
    resultado_final_mxn: null,
  };
}

function commercialVolumeValues(sales, channels) {
  return {
    venta_ton: presentationFinite(sales && sales.actual_ton),
    casa_ton: presentationFinite(channels && channels.casa_ton),
    comisionista_ton: presentationFinite(channels && channels.comisionista_ton),
  };
}

function isFinalFinancialActual(actual) {
  return Boolean(
    actual &&
      actual.status === FINANCIAL_ACTUAL_CODES.SUPPORTED &&
      actual.truth_class === "ACTUAL_FINANCIAL" &&
      actual.financial_state === "FINAL" &&
      actual.fields &&
      typeof actual.fields === "object"
  );
}

function historicalMiniPeriodMatches(mini, period) {
  if (!mini || !period) return false;
  if (mini.period && String(mini.period) !== String(period)) return false;
  if (mini.payload_year != null && mini.payload_month != null) {
    const got = toYyyyMm(mini.payload_year, mini.payload_month);
    if (got !== String(period)) return false;
  }
  return true;
}

function hasDefendableHistoricalMini(miniRow) {
  if (!miniRow || typeof miniRow !== "object") return false;
  return (
    presentationFinite(miniRow.operativos) != null ||
    presentationFinite(miniRow.corporativos) != null ||
    presentationFinite(miniRow.gasto) != null ||
    presentationFinite(miniRow.utilOperImporte) != null ||
    presentationFinite(miniRow.resultadoFinalImporte) != null
  );
}

function composeFinancialPresentation(input = {}) {
  const period = input.period || null;
  const sales = input.sales || {};
  const channels = input.channels || {};
  const volume = commercialVolumeValues(sales, channels);
  const actual = input.actual;

  if (isFinalFinancialActual(actual)) {
    const fields = actual.fields;
    return {
      state: FINANCIAL_PRESENTATION_STATES.FINAL,
      source: "financial.actual",
      period,
      is_final: true,
      values: {
        ...emptyPresentationValues(),
        ...volume,
        margen_mxn_kg: presentationFinite(fields.margen_kg),
        descuento_mxn_kg: dashboardDescSigned(fields.com_desc_kg),
        impuestos_mxn_kg: presentationFinite(fields.impuesto_kg),
        hg_pct: hgDisplayPct(fields.hg_pct),
        hg_mxn_kg: hgDollarMxnKg(fields.hg_kg, fields.hg_pct),
        ingreso_mxn: null,
        operativos_mxn: null,
        corporativos_mxn: null,
        gasto_mxn: null,
        rentabilidad_operativa_mxn: presentationFinite(fields.util_oper_importe),
        resultado_final_mxn: presentationFinite(fields.resultado_final_importe),
      },
    };
  }

  const mini = input.historical_mini || null;
  const miniRow = mini && mini.row;
  if (historicalMiniPeriodMatches(mini, period) && hasDefendableHistoricalMini(miniRow)) {
    const forecastRow = input.forecast_row || null;
    const impuesto =
      presentationFinite(forecastRow && forecastRow.impuesto_kg) != null
        ? presentationFinite(forecastRow.impuesto_kg)
        : presentationFinite(miniRow.impuestos);
    return {
      state: FINANCIAL_PRESENTATION_STATES.VISIBLE_NOT_FINAL,
      source: "igf.latest+computeIgfForecastMiniPayload+arr",
      period,
      is_final: false,
      values: {
        ...emptyPresentationValues(),
        ...volume,
        margen_mxn_kg: presentationFinite(forecastRow && forecastRow.margen_kg),
        descuento_mxn_kg: dashboardDescSigned(forecastRow && forecastRow.com_desc_kg),
        impuestos_mxn_kg: impuesto,
        hg_pct: hgDisplayPct(forecastRow && forecastRow.hg_pct),
        hg_mxn_kg: hgDollarMxnKg(forecastRow && forecastRow.hg_kg, forecastRow && forecastRow.hg_pct),
        operativos_mxn: presentationFinite(miniRow.operativos),
        corporativos_mxn: presentationFinite(miniRow.corporativos),
        gasto_mxn: presentationFinite(miniRow.gasto),
        ingreso_mxn: presentationFinite(miniRow.ingreso),
        rentabilidad_operativa_mxn: presentationFinite(miniRow.utilOperImporte),
        resultado_final_mxn: presentationFinite(miniRow.resultadoFinalImporte),
      },
    };
  }

  return {
    state: FINANCIAL_PRESENTATION_STATES.DATA_MISSING,
    source: null,
    period,
    is_final: false,
    values: {
...
function formatPresentationNumber(value, decimals) {
  const n = presentationFinite(value);
  if (n == null) return null;
  const sign = n < 0 ? "-" : "";
  const fixed = Math.abs(n).toFixed(decimals);
  const parts = fixed.split(".");
  parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return decimals > 0 ? `${sign}${parts[0]}.${parts[1]}` : `${sign}${parts[0]}`;
}

function formatPresentationTons(value) {
  const shown = formatPresentationNumber(value, 2);
  return shown == null ? "n/d" : `${shown} t`;
}

function formatPresentationPerKg(value) {
  const shown = formatPresentationNumber(value, 2);
  return shown == null ? "n/d" : `${shown} MXN/kg`;
}

function formatPresentationMoney(value) {
  const shown = formatPresentationNumber(value, 0);
  return shown == null ? "n/d" : `$${shown}`;
}

function formatPresentationPct(value) {
  const shown = formatPresentationNumber(value, 2);
  return shown == null ? "n/d" : `${shown} %`;
}

function presentationPeriodTitle(period) {
  const m = String(period || "").match(/^(\d{4})-(\d{2})$/);
  if (!m) return period || "";
  const month = Number(m[2]);
  const name = MONTH_TITLE[month] || m[2];
  return `${name} ${m[1]}`;
}

function formatFinancialPresentationAnswer(assembled) {
  const presentation = assembled && assembled.financial && assembled.financial.presentation;
  const plantName =
    (assembled && assembled.plant && assembled.plant.planta_nombre) || "la planta";
  const period = (presentation && presentation.period) || (assembled && assembled.month) || "";
  const header = `${plantName} — ${presentationPeriodTitle(period)}`.trim();
  if (!presentation) {
    return [header, "", "Datos financieros no disponibles."].join("\n");
  }
  const values = presentation.values || emptyPresentationValues();
  const lines = [header, ""];
  if (presentation.state === FINANCIAL_PRESENTATION_STATES.FINAL) {
    lines.push("FINAL");
  } else if (presentation.state === FINANCIAL_PRESENTATION_STATES.VISIBLE_NOT_FINAL) {
    lines.push(NOT_FINAL_LABEL);
  } else {
    lines.push("Datos financieros no disponibles.");
    lines.push("");
    lines.push("Venta comercial:");
    lines.push(formatPresentationTons(values.venta_ton));
    if (assembled && assembled.sales && assembled.sales.target_status === "TARGET_MISSING_FOR_PERIOD") {
      lines.push("");
      lines.push("Meta/compromiso del periodo: no cargada.");
    }
    return lines.join("\n");
  }

  const operatingLabel =
    presentation.state === FINANCIAL_PRESENTATION_STATES.VISIBLE_NOT_FINAL
  ...
  lines.push("");
  lines.push("CASA:");
  lines.push(formatPresentationTons(values.casa_ton));
  lines.push("");
  lines.push("COMISIONISTA:");
  lines.push(formatPresentationTons(values.comisionista_ton));
  lines.push("");
  lines.push("Margen:");
  lines.push(formatPresentationPerKg(values.margen_mxn_kg));
  lines.push("");
  lines.push("Descuento:");
  lines.push(formatPresentationPerKg(values.descuento_mxn_kg));
  lines.push("");
  lines.push("Impuestos:");
  lines.push(formatPresentationPerKg(values.impuestos_mxn_kg));
  lines.push("");
  lines.push("HG:");
  lines.push(formatPresentationPct(values.hg_pct));
  lines.push("");
  lines.push("HG$:");
  lines.push(formatPresentationPerKg(values.hg_mxn_kg));
  lines.push("");
  lines.push("Gastos operativos:");
  lines.push(formatPresentationMoney(values.operativos_mxn));
  lines.push("");
  lines.push("Gastos corporativos:");
  lines.push(formatPresentationMoney(values.corporativos_mxn));
  lines.push("");
  lines.push("Gasto total:");
  lines.push(formatPresentationMoney(values.gasto_mxn));
  lines.push("");
  lines.push(operatingLabel);
  lines.push(formatPresentationMoney(values.rentabilidad_operativa_mxn));
  lines.push("");
  lines.push(finalLabel);
  lines.push(formatPresentationMoney(values.resultado_final_mxn));

  if (presentation.state === FINANCIAL_PRESENTATION_STATES.VISIBLE_NOT_FINAL) {
    lines.push("");
    lines.push(NOT_FINAL_WARNING);
  }
  if (assembled && assembled.sales && assembled.sales.target_status === "TARGET_MISSING_FOR_PERIOD") {
    lines.push("");
    lines.push("Meta/compromiso del periodo: no cargada.");
...
function assembleMonthClosePack(input) {
  ...
  let remaining_ton = null;
  let over_target_ton = null;
  let vs_target = null;
  if (actual_ton != null && target_status === "OK" && target_ton != null) {
    ...
    if (target_ton === 0) {
      ...
      remaining_ton = Math.max(target_ton - actual_ton, 0);
      over_target_ton = Math.max(actual_ton - target_ton, 0);
      if (actual_ton < target_ton) vs_target = "below";
      else if (actual_ton > target_ton) vs_target = "above";
      else vs_target = "at";
    }
  }
  ...
  const pack = {
    ...
    financial: {
      ...
      presentation: composeFinancialPresentation({
        period: yyyymm,
        sales: {
          actual_ton,
        },
        channels,
        actual: financial_actual,
        forecast_row: input.forecast && input.forecast.row ? input.forecast.row : null,
        historical_mini: input.historical_mini || null,
      }),
    },
    actions,
    first_mover,
...
function mapCodesUpper(list) {
  if (!Array.isArray(list)) return [];
  return list.map((c) => String(c).trim().toUpperCase()).filter(Boolean);
}

/** Override de tests/deps: plantCodesUpper ya es string[]. No es resolvePlantCodes. */
function codesUpperFromPlantCodesOverride(override) {
  return mapCodesUpper(override);
}

/**
 * Contrato real de resolvePlantCodes: object { not_found, uniqueCodes, plantCode, matchedMeta }.
 * No usar el object entero como lista de códigos.
 */
function codesUpperFromResolvePlantCodes(resolution) {
  if (resolution == null || typeof resolution !== "object" || Array.isArray(resolution)) {
    return [];
  }
  if (resolution.not_found === true) return [];
  return mapCodesUpper(resolution.uniqueCodes);
}
...
async function loadMonthCloseResultForChat(pool, plantaId, req, opts = {}) {
  ...
  try {
    ...
    let codesUpper = codesUpperFromPlantCodesOverride(opts.plantCodesUpper);
    if (!codesUpper.length && db && plant.planta_nombre) {
      const resolution = await resolveCodes(db, plant.planta_nombre);
      codesUpper = codesUpperFromResolvePlantCodes(resolution);
    }
    ...
    let historical_mini = opts.historicalMini || null;
    if (!historical_mini && typeof opts.loadIgfForecastMiniPayload === "function") {
      try {
        const mini = await opts.loadIgfForecastMiniPayload(pool, {
          year,
          month,
          plantName: plant.planta_nombre,
          plantCode: plant.plant_code,
        });
        const payloadYear = mini && mini.year != null ? Number(mini.year) : year;
        const payloadMonth = mini && mini.month != null ? Number(mini.month) : month;
        historical_mini = {
          row: findMiniRowForPlant(mini && mini.rows, plant.planta_nombre, plant.plant_code),
          period: toYyyyMm(payloadYear, payloadMonth),
          payload_year: payloadYear,
          payload_month: payloadMonth,
        };
      } catch (_e) {
...
function formatMonthCloseContext(assembled) {
  ...
  if (presentation && presentation.state) {
    lines.push(formatFinancialPresentationAnswer(assembled));
    lines.push("");
    lines.push("El bloque anterior es la respuesta ejecutiva. No lo sustituyas por labels ACTUAL/TARGET/FORECAST.");
    lines.push("No inventes fuentes no disponibles, acciones no disponibles ni movimientos de material no explicados.");
    lines.push("");
  }
  lines.push(
  ...
  lines.push(
    ...
    `sales.delta_ton=${assembled.sales && assembled.sales.delta_ton} attainment_pct=${assembled.sales && assembled.sales.attainment_pct} remaining_ton=${assembled.sales && assembled.sales.remaining_ton} over_target_ton=${assembled.sales && assembled.sales.over_target_ton} vs_target=${assembled.sales && assembled.sales.vs_target}`,
  ...
  return {
    ...
    context_meta: {
      ...
      month_close: {
        ...
        financial_presentation: assembled.financial && assembled.financial.presentation,
