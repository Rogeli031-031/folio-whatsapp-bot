...
const SYSTEM_ADDENDUM = [
  ...
  "Cuatro clases: ACTUAL (ARR), TARGET/COMMITMENT (igf_meta), FORECAST (IGF compromiso), DERIVED_MODEL (no usar como meta ni actual).",
  "igf_meta es el compromiso gerencial del mes. No es forecast. No es actual.",
  "IGF es proyección. financial.actual está UNSUPPORTED. No afirmes cumplimiento financiero real.",
...
function assembleMonthClosePack(input) {
  ...
  const pack = {
    ...
    financial: {
      ...
      actual: { status: "UNSUPPORTED_METRIC", truth_class: null },
    },
    actions,
    first_mover,
...
function formatMonthCloseContext(assembled) {
  const lines = [
    "BLOQUE month_close_result — ACTUAL vs TARGET/COMMITMENT vs FORECAST.",
    `planta=${assembled.plant && assembled.plant.planta_nombre} month=${assembled.month} period_status=${assembled.period_status}`,
    `sales.actual_ton=${assembled.sales && assembled.sales.actual_ton} class=ACTUAL`,
    ...
    `financial.forecast=${assembled.financial && assembled.financial.forecast ? "FORECAST" : "absent"}`,
    `gaps=${((assembled.information_gaps || []).map((g) => g.kind) || []).join(",")}`,
    `limitations=${(assembled.limitations || []).join(",")}`,
    "Mover != causa. No carry-forward de meta. No llames actual al IGF ni forecast a la meta.",
  ];
  return lines.join("\n");
}
