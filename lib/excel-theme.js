 ...
 * PROM mes por día: promedio de los **dos últimos valores no vacíos** (AGGREGATE+LARGE+INDEX; compatible con Excel clásico).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: promedio de los 2 últimos valores no vacíos (orden arriba→abajo).
     * Sin FILTER/TAKE (evita reparación de libro en Excel que no soporta bien esas fórmulas en .xlsx).
     * AGGREGATE(14,6,…) = LARGE ignorando errores; posiciones relativas con ROW/(celda<>"").
     * Si solo hay 1 dato: LOOKUP devuelve ese valor.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const rel = `(ROW(${rng})-ROW(${colLetter}${fwr})+1)/(${rng}<>"")`;
      return (
        `IFERROR(IFERROR(AVERAGE(INDEX(${rng},AGGREGATE(14,6,${rel},1)),` +
        `INDEX(${rng},AGGREGATE(14,6,${rel},2))),` +
        `LOOKUP(2,1/(${rng}<>""),${rng})),"")`
      );
    };
