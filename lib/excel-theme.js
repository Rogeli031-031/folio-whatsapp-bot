...
/**
 * Antes: sobrescribía PROM con "promedio de los 2 últimos valores" por columna y ponía fórmulas en POR COMPRAR/PROY.
 * Eso **no** coincide con el dashboard (lookback 28d + `arr.pronostico_dias_seleccion`). El export ya escribe esos
 * valores desde Node; no debe reemplazarlos. Se mantiene la firma por compatibilidad.
 * @param {number} COLK - índice 0-based de la columna K (inicio bloque descuento).
 * @param {number[]} remainingByDow - (sin uso) legado
 */
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  void COLK;
  void remainingByDow;
  // Los valores PROM / POR COMPRAR / PROY los escribe el servidor (misma lógica que el modal Pronóstico).
  // La implementación anterior los reemplazaba por "promedio de los 2 últimos valores" por columna.
}
