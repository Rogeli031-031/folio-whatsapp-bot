 ...
 * PROM mes por día: equivalente a PROMEDIO(B8:B9) sobre los dos últimos valores no vacíos y no cero (AVERAGE con dos INDICE; evita INDEX:INDEX que Excel a veces elimina al abrir el xlsx).
 * ROW(rango) no vectoriza en Excel clásico; ROW(INDIRECT("filaIni:filaFin")) alinea posiciones con el rango.
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: mismo resultado que PROMEDIO(B8:B9) vía AVERAGE(INDEX(rng,iPen),INDEX(rng,iUlt)).
     * No usar INDEX:INDEX; en varios Excel la fórmula desaparece o queda vacía al abrir el libro.
     * AGGREGATE(14,6,…) = LARGE ignorando #DIV/0! cuando (<>"")*(<>0)=0. INDIRECT fuerza vector de filas.
     * Si solo hay un valor, el 2.º AGGREGATE falla y el IFERROR interior deja AVERAGE de una celda.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const anchor = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relOverOk = `(ROW(INDIRECT("${fwr}:${lwr}"))-ROW(${anchor})+1)/${ok}`;
      const iUlt = `AGGREGATE(14,6,${relOverOk},1)`;
      const iPen = `AGGREGATE(14,6,${relOverOk},2)`;
      return (
        `IFERROR(IFERROR(AVERAGE(INDEX(${rng},${iPen}),INDEX(${rng},${iUlt})),` +
        `AVERAGE(INDEX(${rng},${iUlt}))),"")`
      );
    };
