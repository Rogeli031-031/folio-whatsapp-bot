 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero (SI.ERROR/PROMEDIO/INDICE/FILA/AGREGAR(14;6;…) en español).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: fórmula en español (SI.ERROR, PROMEDIO, INDICE, FILA, AGREGAR) con ';' como
     * separador de argumentos — en Excel español AGGREGATE en inglés a menudo no se evalúa bien.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const anchor = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relOverOk = `(FILA(${rng})-FILA(${anchor})+1)/${ok}`;
      const agg1 = `AGREGAR(14;6;${relOverOk};1)`;
      const agg2 = `AGREGAR(14;6;${relOverOk};2)`;
      return `SI.ERROR(PROMEDIO(INDICE(${rng};${agg1});INDICE(${rng};${agg2}));"")`;
    };
