 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero (IFERROR/AVERAGE/INDEX/ROW/AGGREGATE en inglés y comas: OOXML lo exige; Excel en español traduce la UI al abrir).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: guardar en inglés + comas (estándar OOXML). Nombres en español o ';' en el XML
     * hacen que Excel repare el libro y borre fórmulas (p. ej. sheet Pronostico).
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const anchor = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relOverOk = `(ROW(${rng})-ROW(${anchor})+1)/${ok}`;
      const agg1 = `AGGREGATE(14,6,${relOverOk},1)`;
      const agg2 = `AGGREGATE(14,6,${relOverOk},2)`;
      return `IFERROR(AVERAGE(INDEX(${rng},${agg1}),INDEX(${rng},${agg2})),"")`;
    };
