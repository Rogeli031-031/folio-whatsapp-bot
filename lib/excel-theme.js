...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
     ...
     * Evita "<" dentro de subexpresiones complejas (riesgo de XML .xlsx inválido) usando rel<>MAX(rel*ok) para el penúltimo índice.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const s = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const rel = `(ROW(${rng})-ROW(${s})+1)`;
      const lastRel = `MAX(${rel}*${ok})`;
      const prevRel = `MAX(${rel}*${ok}*((${rel})<>(${lastRel})))`;
      return (
        `IFERROR(IF(SUMPRODUCT(${ok}*1)>=2,(INDEX(${rng},${lastRel})+INDEX(${rng},${prevRel}))/2,` +
        `LOOKUP(2,1/${ok},${rng})),"")`
      );
    };
