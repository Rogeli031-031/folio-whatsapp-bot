 ...
 * PROM mes por día: promedio de los **dos últimos valores no vacíos** en la columna (FILTER + TAKE; requiere Excel 365).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /** PROM mes por columna: solo los 2 últimos datos no vacíos en filas de semana (Excel 365: FILTER + TAKE). */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      return `IFERROR(AVERAGE(TAKE(FILTER(${rng},${rng}<>""),-2)),"")`;
    };
