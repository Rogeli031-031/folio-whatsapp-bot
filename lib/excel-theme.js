 ...
 * PROM mes por día: promedio de los **dos últimos valores no vacíos y no cero** (AGGREGATE+LARGE+INDEX).
 * ROW(rango) no vectoriza en Excel clásico; se usa ROW(INDIRECT("filaIni:filaFin")) para alinear posiciones con el rango.
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
     ...
     * INDIRECT("fwr:lwr") fuerza vector de números de fila; sin eso ROW(rng) es escalar y el respaldo LOOKUP devuelve solo el último valor.
     */
