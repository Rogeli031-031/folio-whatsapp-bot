 ...
 * PROM mes por día: equivalente a PROMEDIO(B8:B9) sobre los dos últimos valores no vacíos y no cero (AVERAGE con rango INDEX:INDEX).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: mismo efecto que PROMEDIO(B8:B9) con AVERAGE(INDEX(rng,…):INDEX(rng,…)) (un solo rango de dos celdas).
    ...
    const promLastTwoAvg = (colLetter) => {
      ...
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relOverOk = `(ROW(INDIRECT("${fwr}:${lwr}"))-ROW(${s})+1)/${ok}`;
      const iUlt = `AGGREGATE(14,6,${relOverOk},1)`;
      const iPen = `AGGREGATE(14,6,${relOverOk},2)`;
      return (
        `IFERROR(IFERROR(AVERAGE(INDEX(${rng},MIN(${iPen},${iUlt})):INDEX(${rng},MAX(${iPen},${iUlt}))),` +
