 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero.
 * COINCIDIR(2,1/ok,1)+DESREF evita AGGREGATE sobre arrays calculados (en Excel español suelen no vectorizar y queda "").
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: promedio de las dos últimas filas con dato bajando desde la última (bloque de 2 filas con DESREF).
     * Última fila con ok: m = MATCH(2,1/ok,1) (1..n dentro de rng). Si m>=2 → PROMEDIO(DESREF(primera,m-2,0,2,1)); si no → INDICE(último).
     * Si entre el penúltimo dato y el último hay filas vacías en esa columna, el bloque de 2 puede incluir un vacío; PROMEDIO lo ignora.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const first = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const m = `MATCH(2,1/${ok},1)`;
      return `IFERROR(IF(${m}>=2,AVERAGE(OFFSET(${first},${m}-2,0,2,1)),INDEX(${rng},${m})),"")`;
    };
