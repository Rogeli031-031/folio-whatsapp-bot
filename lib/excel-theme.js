 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero (AVERAGE+INDEX+AGGREGATE; IF(ok,pos,NA()) evita #DIV/0 en el vector).
 * Posiciones relativas 1…n con ROW(OFFSET($A$1,0,0,n,1)) (misma altura que el rango de semanas).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: promedio de los 2 últimos valores no vacíos y ≠0 (equivalente a PROMEDIO(B8:B9)).
     * FILA(INDIRECTO) o FILA(rango) dentro de AGGREGATE a veces no vectoriza → usar
     * FILA(DESREF($A$1,0,0,nRows,1)) = {1…n} alineado con rng (misma altura).
     * Si AGGREGATE falla por completo, LOOKUP(2,1/ok,rng) devuelve al menos el último valor válido (mejor que celda en blanco).
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const nRows = lwr - fwr + 1;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relIfOk = `IF(${ok},ROW(OFFSET($A$1,0,0,${nRows},1)),NA())`;
      const iUlt = `AGGREGATE(14,6,${relIfOk},1)`;
      const iPen = `AGGREGATE(14,6,${relIfOk},2)`;
      return (
        `IFERROR(IFERROR(IFERROR(AVERAGE(INDEX(${rng},${iPen}),INDEX(${rng},${iUlt})),` +
        `AVERAGE(INDEX(${rng},${iUlt}))),LOOKUP(2,1/${ok},${rng})),"")`
      );
    };
