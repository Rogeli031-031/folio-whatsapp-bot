...
/**
 * Valor numérico usable para PROM mes: no vacío y distinto de 0 (misma lógica que (rng<>"")*(rng<>0) en Excel).
 * @returns {number|null}
 */
function pronosticoScalarToPromAvg(v) {
...
/**
 * Lee celda ya escrita en Pronostico (número o resultado de fórmula).
 * @returns {number|null}
 */
function getPronosticoCellForPromAvg(cell) {
  const v = cell.value;
  if (v != null && typeof v === "object" && v.formula != null) {
    return pronosticoScalarToPromAvg(v.result);
  }
  return pronosticoScalarToPromAvg(v);
}

/**
 * Promedio de los dos últimos valores “válidos” en columna, de abajo hacia arriba (equiv. AGREGAR(14,6,…)).
 * Se calcula en Node para evitar AGGREGATE/AGREGAR, localización y errores de reparación del .xlsx.
 * @param {number} excelCol1Based - columna Excel 1-based (B=2).
 * @returns {number|null}
 */
function averageLastTwoNonEmptyNonZero(worksheet, excelCol1Based, rowFirst, rowLast) {
  const vals = [];
  for (let r = rowLast; r >= rowFirst && vals.length < 2; r--) {
    const n = getPronosticoCellForPromAvg(worksheet.getCell(r, excelCol1Based));
 ...
 * PROM mes por día: promedio de los dos últimos no vacíos y ≠ 0 — **valor calculado al exportar** (misma regla que la fórmula manual con AGREGAR), no fórmula en el XML.
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    for (let j = 0; j < 7; j++) {
      const excelCol = 2 + j;
      const av = averageLastTwoNonEmptyNonZero(worksheet, excelCol, fwr, lwr);
      if (av != null) setCellNumber(worksheet, pr - 1, 1 + j, av, FMT_NUMBER);
      else worksheet.getCell(pr, excelCol).value = null;
    }
    ...
    for (let j = 0; j < 7; j++) {
      const excelCol = COLK + 2 + j;
      const av = averageLastTwoNonEmptyNonZero(worksheet, excelCol, fwr, lwr);
      if (av != null) setCellNumber(worksheet, pr - 1, COLK + 1 + j, av, FMT_CURRENCY_MXN);
      else worksheet.getCell(pr, excelCol).value = null;
    }
 ...
 * @param {number} [opt.mainHeaderExcelRow1Based] - primera fila Excel de encabezados tabla grande (default 6; Compromiso con mini arriba: 14)
 * @param {number} opt.dataStartRow0 - primera fila datos tabla superior (típ. 8; con mini: 16)
...
function applyIgfForecastSheetLayout(worksheet, opt) {
  ...
  const excelRow6 = opt.mainHeaderExcelRow1Based != null ? opt.mainHeaderExcelRow1Based : 6;
  const excelRow7 = excelRow6 + 1;
  let i = 0;
  while (i < hr6.length) {
    let j = i + 1;
  ...
  const borderTopR0 = excelRow6 - 1;
  applyBordersRangeBlack(worksheet, borderTopR0, 0, lastR, nCol - 1);
}
