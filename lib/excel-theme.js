...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
     ...
     * Usa IF(ok,pos,NA()) en lugar de pos/ok: en Excel (sobre todo UI en español) #¡DIV/0! en el vector a veces
     * hace que AGGREGATE falle por completo y SI.ERROR devuelva "" en toda la celda.
     * AGGREGATE(14,6,…) = K.ESIMO.MAYOR ignorando errores; INDIRECT fuerza vector de filas.
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const anchor = `${colLetter}${fwr}`;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const relIfOk = `IF(${ok},ROW(INDIRECT("${fwr}:${lwr}"))-ROW(${anchor})+1,NA())`;
