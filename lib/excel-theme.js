 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero (misma lógica que PROMEDIO(INDICE+AGREGAR(14,6,…)) con ROW(rng)-ROW(primera).
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: PROMEDIO(INDICE(rng,AGREGAR…,1),INDICE(rng,AGREGAR…,2)) con
     * AGREGAR(14,6,(FILA(rng)-FILA(primera)+1)/((rng<>"")*(rng<>0)),k) — patrón validado en Excel.
     */
