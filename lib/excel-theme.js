 ...
 * PROM mes por día: promedio de los dos últimos valores no vacíos y no cero (dos mayores índices de fila válidos).
 * Máscara (FILA*ok) sin división ni SI matricial; inválidos = 0 y AGGREGATE(14,6,…) toma el 1.er y 2.º mayor.
...
function applyPronosticoSummaryFormulas(worksheet, blocks, COLK, remainingByDow) {
  ...
  for (const b of blocks) {
    ...
    /**
     * PROM mes por columna: promedio de los 2 últimos valores no vacíos y ≠0 (cronológicamente = 2 mayores índices 1..n).
     * masked = ROW(OFFSET($A$1,0,0,n,1))*ok → sin datos queda 0; AGGREGATE(14,6,masked,k) = k-ésimo mayor índice.
     * Sin LOOKUP de respaldo (mostraba solo el último valor). Un solo dato: 2.º mayor es 0 → IFERROR → PROMEDIO(INDICE,último).
     */
    const promLastTwoAvg = (colLetter) => {
      const rng = `${colLetter}${fwr}:${colLetter}${lwr}`;
      const nRows = lwr - fwr + 1;
      const ok = `((${rng}<>"")*(${rng}<>0))`;
      const pos = `ROW(OFFSET($A$1,0,0,${nRows},1))`;
      const masked = `(${pos})*${ok}`;
      const iUlt = `AGGREGATE(14,6,${masked},1)`;
      const iPen = `AGGREGATE(14,6,${masked},2)`;
      return (
