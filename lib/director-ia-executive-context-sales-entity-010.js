...
function isSalesStatusQuestion(question) {
  ...
  if (
    /\b(llantas?|taller|regulacion|seh|comision|forecast\s+por\s+cliente|abre\s+la\s+venta|mezcla|mix|share|participacion|canal|comisionista|clientes?|nuevos|dejaron|disminuyeron|churn|retencion)\b/.test(
      n
    )
  ) {
    return false;
...
function rowMatchesKeywordConcept(row, concept) {
  ...
  const blob = nq([row && row.concepto, row && row.descripcion, row && row.subcategoria].filter(Boolean).join(" "));
