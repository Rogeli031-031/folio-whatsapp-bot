...
const READINESS_PATH = "/health-director-ia";
const DEFAULT_CYCLE_TIMEOUT_MS = 60000;
const DEFAULT_ARR_STATEMENT_TIMEOUT_MS = 55000;
...
function parseDirectorIaTimeoutMs(raw, fallback) {
  const fallbackMs =
    Number.isFinite(fallback) && fallback > 0 ? Math.floor(fallback) : DEFAULT_CYCLE_TIMEOUT_MS;
...
function isDirectorIaTimeoutError(err) {
  if (!err) return false;
  const code = err.code != null ? String(err.code) : "";
  if (code === "57014" || code === "ARR_TIMEOUT" || code === "CYCLE_TIMEOUT" || code === "QUERY_CANCELED") {
    return true;
  }
...
function timeoutCodeOf(err) {
  const code = err && err.code != null ? String(err.code) : "";
  if (code === "CYCLE_TIMEOUT") return "CYCLE_TIMEOUT";
  return "ARR_TIMEOUT";
}

function timeoutTransportResult(code) {
  const safeCode = code === "ARR_TIMEOUT" ? "ARR_TIMEOUT" : "CYCLE_TIMEOUT";
  return {
    httpStatus: 504,
    body: {
      ok: false,
...
function raceWithTimeout(promise, timeoutMs, code) {
  const ms = parseDirectorIaTimeoutMs(timeoutMs, DEFAULT_CYCLE_TIMEOUT_MS);
  let timer = null;
  const timeoutPromise = new Promise((_, reject) => {
    timer = setTimeout(() => {
      const err = new Error("cycle_timeout");
      err.code = code || "CYCLE_TIMEOUT";
      reject(err);
    }, ms);
  });
...
async function applyDirectorIaArrStatementTimeout(client, timeoutMs) {
  if (!client || typeof client.query !== "function") return { applied: false };
  const ms = parseDirectorIaTimeoutMs(timeoutMs, DEFAULT_ARR_STATEMENT_TIMEOUT_MS);
  await client.query(`SET statement_timeout TO ${ms}`);
  return { applied: true, timeout_ms: ms };
}

async function resetDirectorIaArrStatementTimeout(client) {
  if (!client || typeof client.query !== "function") return { reset: false };
  try {
...
async function cancelDirectorIaClientQuery(client, pool) {
  const pid = client && (client.processID || client.processId);
  if (!pid || !pool || typeof pool.connect !== "function") return { canceled: false };
  let killer;
  try {
    ...
    if (killer && typeof killer.release === "function") killer.release();
  }
}
...
function createDirectorIaDashboardCycleTransport(options) {
  ...
  const cycleTimeoutMs = parseDirectorIaTimeoutMs(opts.cycleTimeoutMs, DEFAULT_CYCLE_TIMEOUT_MS);
  ...
  async function handle(input) {
    ...
    let result;
    const runPromise = opts.realCycle.run(cycleInput);
    runPromise.catch(() => {
      /* loser of deadline race must not become unhandled */
    });
    try {
      result = await raceWithTimeout(runPromise, cycleTimeoutMs, "CYCLE_TIMEOUT");
    } catch (err) {
      const duration_ms = Date.now() - startedMs;
      if (isDirectorIaTimeoutError(err)) {
        const timeoutCode = timeoutCodeOf(err);
        emitLog(logger, "cycle_request_failed", {
          request_id: requestId,
          trace_id: null,
          planta_id: plantaId,
        ...
        return timeoutTransportResult(timeoutCode);
      }
...
function configureDirectorIaDashboardCycle(injected) {
  deps = { ...deps, ...injected };
  const cycleTimeoutMs = parseDirectorIaTimeoutMs(deps.cycleTimeoutMs, DEFAULT_CYCLE_TIMEOUT_MS);
  deps.cycleTimeoutMs = cycleTimeoutMs;
  deps.arrStatementTimeoutMs = parseDirectorIaTimeoutMs(
    deps.arrStatementTimeoutMs,
...
function handleGetDirectorIaReadiness(req, res) {
  ...
  const transportReady = Boolean(deps.transport && typeof deps.transport.handle === "function");
  const poolReady = Boolean(deps.pool && typeof deps.pool.connect === "function");
  if (!transportReady || !poolReady) {
...
async function handlePostDashboardCycle(req, res) {
  ...
  const startedMs = Date.now();
  const cycleTimeoutMs = parseDirectorIaTimeoutMs(deps.cycleTimeoutMs, DEFAULT_CYCLE_TIMEOUT_MS);
  const arrStatementTimeoutMs = parseDirectorIaTimeoutMs(
    deps.arrStatementTimeoutMs,
    DEFAULT_ARR_STATEMENT_TIMEOUT_MS
  );
  let client;
  ...
  try {
    if (typeof deps.resolvePlant === "function" || deps.pool) {
      ...
      await applyDirectorIaArrStatementTimeout(client, arrStatementTimeoutMs);
    ...
    pending = deps.transport.handle(mapped);
    pending.catch(() => {
      /* deadline loser */
    });
    const out = await raceWithTimeout(pending, cycleTimeoutMs, "CYCLE_TIMEOUT");
    return res.status(out.httpStatus).json(out.body);
  } catch (err) {
    if (isDirectorIaTimeoutError(err)) {
      await cancelDirectorIaClientQuery(client, deps.pool);
      if (pending) await pending.catch(() => {});
      emitLog(deps.logger, "cycle_request_failed", {
        planta_id: plantaId,
        trace_id: null,
        duration_ms: Date.now() - startedMs,
        http_status: 504,
      ...
      const timed = timeoutTransportResult("CYCLE_TIMEOUT");
      return res.status(timed.httpStatus).json(timed.body);
    }
    return res.status(500).json({ ok: false, enabled: true, error: "Error interno" });
  } finally {
    await resetDirectorIaArrStatementTimeout(client);
