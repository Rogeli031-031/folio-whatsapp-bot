...
function roleNorm(auth) {
  return (auth && auth.role ? String(auth.role) : "").replace(/\s/g, "").toUpperCase();
}
...
function assertPlantaAcceso(auth, plantaId) {
  ...
  const allowed = (auth.plantas_permitidas || []).map((x) => getCanonicalPlantaId(x));
  const equiv = getPlantaIdsEquivalentes(canon);
  return equiv.some((id) => allowed.includes(id));
}
...
async function notifyResponsable(accionRow, responsableUser) {
  if (!deps.sendWhatsApp || !responsableUser || !responsableUser.telefono) {
    return { ok: false, error: "Sin teléfono o Twilio" };
  }
  const base = (deps.getDashboardBaseUrl && deps.getDashboardBaseUrl()) || "";
  const msg =
    `📋 Nueva acción DICF\n` +
    ...
    (base ? `Dashboard: ${base.replace(/\/$/, "")}/?t=<tu_token>\n` : "");
...
async function createAccion(client, auth, payload) {
  ...
  const notif = await notifyResponsable(row, vResp.usuario);
  await client.query(
...
async function cerrarAccion(client, auth, accionId) {
  const a = await loadAccion(client, accionId);
  if (!a) return { error: "Acción no encontrada" };
  if (!assertPlantaAcceso(auth, a.planta_id)) return { error: "Sin acceso" };
  ...
  await client.query(
    `UPDATE arr.dicf_acciones SET estado = 'hecho', cerrado_at = now(), cerrado_por_usuario_id = $2, updated_at = now() WHERE id = $1`,
    [accionId, auth.actor_id]
  );
  await insertHistorial(client, accionId, auth.actor_id, "cerrada", {});
  return { ok: true };
}
