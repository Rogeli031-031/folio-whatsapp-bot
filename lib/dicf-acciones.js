...
function normalizarNombreRolDicf(s) {
  return (s || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/\p{M}/gu, "")
...
/**
 * Auth JWT/API alineado con dashboard (ZP/AD/CF_CDMX = todas las plantas; resto = equiv de planta_id).
 */
async function buildAuthFromActorRow(client, actor) {
  ...
  const rolNormNombre = normalizarNombreRolDicf(rolNom);
  const nombreUsuarioNorm = normalizarNombreRolDicf(actor.nombre || "");
  const esAD =
    rolClave === "AD" ||
  ...
  if (esZP || esAD || esCFCDMX) {
    const r = await client.query(`SELECT id FROM public.plantas ORDER BY id`);
    plantasPermitidas = (r.rows || []).map((p) => p.id).filter(Number.isFinite);
  } else if (actor.planta_id != null) {
    plantasPermitidas = getPlantaIdsEquivalentes(actor.planta_id);
  }
  return { actor_id: actor.id, role, plantas_permitidas: plantasPermitidas };
}

function isDicfAccionesRole(auth) {
  const r = roleNorm(auth);
  return r === "ZP" || r === "GG" || r === "GV" || r === "AD";
}

/** GG/GV: planta debe estar en plantas_permitidas (equiv). ZP/AD: todo. */
function assertPlantaAcceso(auth, plantaId) {
  const r = roleNorm(auth);
  if (r === "ZP" || r === "AD") return true;
...
async function validateResponsablePlanta(client, responsableId, plantaIdCanon) {
  ...
  if (clave === "AD") {
    if (u.planta_id == null) return { ok: true, usuario: u };
    if (!equivTarget.has(u.planta_id)) {
      return { ok: false, error: "El responsable no pertenece a esta planta" };
    }
    return { ok: true, usuario: u };
  }
  return { ok: false, error: "El responsable debe ser usuario GG, GV o AD" };
}
...
async function notifyResponsable(client, accionRow, responsableUser) {
  ...
  try {
    if (client && deps.buildDicfNotifDashboardUrls) {
      ...
      if (urls && urls.accion) {
        webBlock = `\n🔗 Contestar acción (registro DICF, válido 20 h):\n${urls.accion}\n`;
      }
    }
  } catch (e) {
    webBlock = "";
...
async function listAcciones(client, auth, query) {
  ...
  const r = await client.query(
    `SELECT a.*,
            ...
            COALESCE(NULLIF(TRIM(COALESCE(cr.nombre_persona,'')), ''), cr.nombre) AS creador_nombre,
            COALESCE(NULLIF(TRIM(COALESCE(cp.nombre_persona,'')), ''), cp.nombre) AS cerrado_por_nombre
  ...
  const r = await client.query(
    `SELECT u.id,
     ...
     WHERE (u.activo IS NULL OR u.activo = true)
       AND (
         (
           u.planta_id = ANY($1::int[])
           AND (
             UPPER(TRIM(COALESCE(r.clave,''))) IN ('GG','GV')
             OR UPPER(TRIM(COALESCE(r.nombre,''))) IN ('GG','GV','GERENTE DE VENTAS')
           )
         )
         OR (
           UPPER(TRIM(COALESCE(r.clave,''))) = 'AD'
           AND (u.planta_id IS NULL OR u.planta_id = ANY($1::int[]))
         )
       )
     ORDER BY r.clave, COALESCE(NULLIF(TRIM(COALESCE(u.nombre_persona,'')), ''), u.nombre)`,
    [equiv]
  );
...
async function getHistorial(client, auth, accionId) {
  ...
  const h = await client.query(
    `SELECT h.*, COALESCE(NULLIF(TRIM(COALESCE(u.nombre_persona,'')), ''), u.nombre) AS actor_nombre
...
/**
 * Historial DICF en lote (solo lectura; Director IA / reportes).
 * @param {import("pg").PoolClient} client
 * @param {number[]} accionIds
 * @returns {Promise<Map<number, Array<{ evento: string, creado_en: string, actor_nombre: string | null, detalle: unknown }>>>}
 */
async function loadHistorialBatch(client, accionIds) {
  const ids = [...new Set((accionIds || []).map((x) => Number(x)).filter(Number.isFinite))];
  /** @type {Map<number, Array<{ evento: string, creado_en: string, actor_nombre: string | null, detalle: unknown }>>} */
  const map = new Map();
  ...
  const h = await client.query(
    `SELECT h.accion_id, h.evento, h.detalle, h.creado_en,
     ...
     ORDER BY h.accion_id ASC, h.creado_en ASC`,
    [ids]
  );

  for (const row of h.rows || []) {
    const accionId = Number(row.accion_id);
    if (!map.has(accionId)) map.set(accionId, []);
    map.get(accionId).push({
      evento: String(row.evento || ""),
      creado_en:
        row.creado_en instanceof Date ? row.creado_en.toISOString() : String(row.creado_en || ""),
      actor_nombre: row.actor_nombre != null ? String(row.actor_nombre) : null,
      detalle: row.detalle != null ? row.detalle : null,
    });
  }
...
async function cerrarAccion(client, auth, accionId, resultadoTexto) {
  ...
  const rn = (rAct && rAct.rol_clave) ? String(rAct.rol_clave).toUpperCase() : "";
  if (!["ZP", "GG", "GV", "AD"].includes(rn)) return { error: "No autorizado" };
...
async function handleCompromisoWhatsApp(client, actor, bodyTrim) {
  ...
  const auth = await buildAuthFromActorRow(client, actor);
  if (!auth) return { handled: true, reply: "No te encuentro como usuario." };
...
async function exportExcelRows(client, auth, plantaNombre) {
  ...
  if ((plantaNombre || "").trim()) {
    ...
    return { error: "ZP puede exportar sin filtro; GG/GV/AD deben indicar planta" };
  }
  ...
  const r = await client.query(
    `SELECT a.id, a.public_code, a.planta_label, a.grupo_tipo, a.canal, a.subcanal, a.cliente_nombre, a.descripcion, a.estado,
            ...
            COALESCE(NULLIF(TRIM(COALESCE(cr.nombre_persona,'')), ''), cr.nombre) AS creador,
            COALESCE(NULLIF(TRIM(COALESCE(cp.nombre_persona,'')), ''), cp.nombre) AS cerrado_por,
            a.notify_error
     FROM arr.dicf_acciones a
     LEFT JOIN public.usuarios rp ON rp.id = a.responsable_usuario_id
     LEFT JOIN public.usuarios cr ON cr.id = a.creado_por_usuario_id
