...
function buildFallbackManagerMessage(brief, openActionsByClient) {
  ...
  (openActionsByClient || []).forEach((a) => {
    const key = String(a.cliente_norm || "").trim().toLowerCase();
    if (!key) return;
    if (!actionsByClient.has(key)) actionsByClient.set(key, []);
    actionsByClient.get(key).push(a);
  });

  let msg = `📊 Delta Ingreso ${brief.planta} (${brief.periodoA} → ${brief.periodoB})\n`;
  if (brief.totalNegativoStr) msg += `${brief.totalNegativoStr}\n`;
  msg += "\n";

  if (brief.topNoCompran && brief.topNoCompran.length) {
    msg += "• No compran:\n";
    brief.topNoCompran.forEach((c) => {
      const key = String(c.cliente || "").trim().toLowerCase();
      const actions = actionsByClient.get(key) || [];
      msg += `  - ${c.cliente}: A ${c.kgAStr} ton → B ${c.kgBStr || "0.0"} ton | Delta ingreso: ${c.ingresoAStr}\n`;
      if (actions.length) {
        const a = actions[0];
        const nota = (a.last_update_text || a.what || "").trim();
        const vencida = isOverdue(a) ? " 🔴" : "";
        msg += `    Seguimiento: ${a.action_status || "OPEN"}${vencida} · Resp: ${a.who || "N/A"} · Compromiso: ${fmtDate(a.when_date)}\n`;
        if (nota) msg += `    Historial: ${nota.slice(0, 140)}${nota.length > 140 ? "…" : ""}\n`;
      } else {
        msg += "    Seguimiento: sin action register abierto\n";
      }
    });
  }
  if (brief.topMenosIngreso && brief.topMenosIngreso.length) {
    msg += "• − Ingreso:\n";
    brief.topMenosIngreso.forEach((c) => {
      const key = String(c.cliente || "").trim().toLowerCase();
      const actions = actionsByClient.get(key) || [];
      msg += `  - ${c.cliente}: A ${c.kgAStr} ton → B ${c.kgBStr} ton | Delta ingreso: ${c.deltaIngresoStr}\n`;
      if (actions.length) {
        const a = actions[0];
        const nota = (a.last_update_text || a.what || "").trim();
        const vencida = isOverdue(a) ? " 🔴" : "";
  ...
  msg += "\nEscriban y envien por este mismo chat el plan 5W2H por cada cliente (WHAT, WHY, WHEN, WHO, HOW, HOW MUCH). Para cerrar respondan: CERRADO: <cliente> + evidencia.";
  return msg;
}
