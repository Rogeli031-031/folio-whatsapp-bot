...
/** Ordena plantas por nombre; respeta opts.plantaNombre si filtra una sola. */
function orderedPlantGroups(rows) {
  const map = new Map();
  for (const r of rows || []) {
    const key = plantaKey(r);
    if (!map.has(key)) {
      map.set(key, { key, label: plantaLabel(r), rows: [] });
    }
...
/**
 * Agrupa filas con importes cercanos (union-find).
 * @param {Array<object>} rows
 * @returns {Array<object[]>}
 */
function clusterPorImporteCercano(rows) {
  const list = Array.isArray(rows) ? rows : [];
  const n = list.length;
  if (n < 2) return n ? [list] : [];
  ...
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      if (importesCercanos(list[i].importe, list[j].importe)) unite(i, j);
    }
  }
  const map = new Map();
  for (let i = 0; i < n; i++) {
...
function criterioDuplicadoGrupo(rows) {
  ...
  if (Math.abs(max - min) < 0.005) return "importe exacto";
  return "importe cercano";
}
...
async function buildTallerAtWorkbook(rows, opts) {
  ...
  // --- Duplicados (por planta): mismo AT + concepto, importe exacto o cercano (±5% o ≤$250) ---
  const wsD = wb.addWorksheet("Duplicados", { views: [{ showGridLines: true }] });
  wsD.getCell("A1").value =
    "Posibles duplicados (mismo AT + concepto; importe igual o cercano ±5% / ≤$250) — por planta";
  ...
  wsD.getCell("A3").value =
    "Incluye montos casi iguales (ej. $11,645 vs $11,445 con el mismo concepto/AT).";
  ...
  for (const group of plantGroupsDup) {
    const byAtConcepto = new Map();
    for (const r of group.rows) {
      const key = `${String(r.unidad).toLowerCase()}|${normalizeConceptoKey(r.concepto)}`;
      if (!byAtConcepto.has(key)) byAtConcepto.set(key, []);
      byAtConcepto.get(key).push(r);
    }

    const dupGroups = [];
    for (const rows of byAtConcepto.values()) {
      if (rows.length < 2) continue;
      for (const cluster of clusterPorImporteCercano(rows)) {
  ...
  if (!anyDup) {
    wsD.getCell(dRow, 1).value =
      "No se detectaron posibles duplicados (AT + concepto + importe igual/cercano) por planta.";
