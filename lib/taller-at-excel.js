 ...
 * - Si hay detalle_lineas con importes, una fila por línea (importe de cada línea).
 * - Si no, usa f.importe (aunque la UI lo muestre en columna MAYOR/PASIVO/PREVENTIVO/OTROS).
 * El tipo (mayor/pasivo/preventivo/otros) viene de subcategoria; el monto es el mismo "importe".
 */
function expandTallerRows(rows) {
  const out = [];
  for (const r of rows || []) {
    const unidad = normalizeAt(r.unidad);
...
async function buildTallerAtWorkbook(rows, opts) {
  ...
  const list = expandTallerRows(rows).filter((r) => meses.includes(r.mes_cargo));
  ...
  wsR.getCell("A4").value =
    "Importe = MAYOR + PASIVO/RECUPERACIÓN + PREVENTIVO + OTROS (misma cantidad, distinta categoría). Incluye folios TALLER no cancelados.";
  ...
  for (const mes of meses) {
    ...
    ws.getCell("A3").value =
      "El monto va en la columna de su categoría (MAYOR / PASIVO / PREVENTIVO / OTROS); Importe = suma de esas columnas.";
    ...
    monthRows.forEach((r, i) => {
      const row = d0 + i;
      const amts = amtByTipo(r.tipo, r.importe);
      ...
      cImp.value = { formula: `SUM(C${row}:F${row})` };
      cImp.numFmt = NUM_FMT;
  ...
  const byKey = new Map();
  for (const r of list) {
    const key = `${normalizeAt(r.unidad).toLowerCase()}|${normalizeConceptoKey(r.concepto)}|${r.importe.toFixed(2)}`;
