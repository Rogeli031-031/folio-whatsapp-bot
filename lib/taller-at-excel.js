...
/** Umbral de similitud de concepto (relajado: PREVENTIVO vs OTROS no importa). */
const DUP_CONCEPTO_UMBRAL = 0.55;
/** Si el importe es casi idéntico (≤ $1), basta un solape mínimo de concepto. */
const DUP_CONCEPTO_UMBRAL_IMPORTE_EXACTO = 0.35;
...
function conceptosCompatiblesParaDup(conceptoA, conceptoB, importeDiff) {
  const ca = conceptoParaDup(conceptoA);
  const cb = conceptoParaDup(conceptoB);
  if (!ca || !cb) return true;
  if (normalizeConceptoKey(ca) === normalizeConceptoKey(cb)) return true;
  const sim = folioDuplicados.conceptoSimilarity(ca, cb);
  const umbral =
    importeDiff <= 1 ? DUP_CONCEPTO_UMBRAL_IMPORTE_EXACTO : DUP_CONCEPTO_UMBRAL;
  if (sim >= umbral) return true;
  // Solape de tokens (compra / faro / niebla …) aunque el score global baje
  const ta = folioDuplicados.tokenizeConcepto(ca);
  const tb = folioDuplicados.tokenizeConcepto(cb);
  if (ta.length >= 2 && tb.length >= 2) {
    const setA = new Set(ta);
    const hit = tb.filter((t) => setA.has(t)).length;
    const need = Math.min(3, Math.min(ta.length, tb.length));
    if (hit >= need) return true;
  }
  // Importe idéntico (≤$1): mismo AT ya es señal fuerte de posible duplicado
  if (importeDiff <= 1) return true;
  return false;
}

/**
 * Folios multi-AT con el mismo juego de pipas e importes parciales parecidos
 * (ej. F-202606-208 vs F-202607-221: AT-18/24/65 a $1,175), aunque
 * uno esté en PREVENTIVO y otro en OTROS / concepto varíe.
 */
function findMultiAtStructuralDupClusters(rows) {
  const byFolio = new Map();
  for (const r of rows || []) {
    if (r.row_kind === "grupo") continue;
    const at = String(r.unidad || "").trim();
    if (!at || at.includes(",")) continue;
    const folio = String(r.numero_folio || r.folio_id || "").trim();
    if (!folio) continue;
    ...
    const g = byFolio.get(folio);
    g.rows.push(r);
    g.byAt.set(at.toLowerCase(), (g.byAt.get(at.toLowerCase()) || 0) + (Number(r.importe) || 0));
  }

  const folios = [...byFolio.values()].filter((g) => g.byAt.size >= 2);
  const out = [];
  const seenPairAt = new Set();
  ...
  for (let i = 0; i < folios.length; i++) {
    for (let j = i + 1; j < folios.length; j++) {
      ...
      if (A.byAt.size !== B.byAt.size) continue;
      let allMatch = true;
      for (const [at, impA] of A.byAt) {
        if (!B.byAt.has(at) || !importesCercanos(impA, B.byAt.get(at))) {
          allMatch = false;
          break;
        }
      }
      ...
      // Un cluster por AT (filas de ambos folios)
      for (const at of A.byAt.keys()) {
        const pairKey = `${A.planta}|${at}|${[A.folio, B.folio].sort().join("|")}`;
        if (seenPairAt.has(pairKey)) continue;
        seenPairAt.add(pairKey);
        const cluster = [
          ...A.rows.filter((r) => String(r.unidad).trim().toLowerCase() === at),
          ...B.rows.filter((r) => String(r.unidad).trim().toLowerCase() === at),
        ];
        if (cluster.length >= 2) {
          cluster._criterioExtra = "mismo juego multi-AT";
          out.push(cluster);
        }
      }
    }
 ...
 * mismo AT + importe igual/cercano + concepto compatible,
 * o mismo juego multi-AT entre dos folios (ignora PREVENTIVO vs OTROS).
...
function findDuplicateClustersTaller(rows) {
  ...
  const pushCluster = (cluster) => {
    const folios = [...new Set(cluster.map((r) => String(r.numero_folio || "")).filter(Boolean))].sort();
    const at = String(cluster[0] && cluster[0].unidad || "").toLowerCase();
    const key = `${at}|${folios.join("|")}`;
    if (folios.length < 2 || seen.has(key)) return;
    seen.add(key);
    out.push(cluster);
  };
  ...
  for (const list of byAt.values()) {
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        const diff = Math.abs((Number(a.importe) || 0) - (Number(b.importe) || 0));
        if (!conceptosCompatiblesParaDup(a.concepto, b.concepto, diff)) continue;
        unite(i, j);
      }
    }
    ...
    for (const cluster of map.values()) pushCluster(cluster);
  }

  for (const cluster of findMultiAtStructuralDupClusters(rows)) {
    pushCluster(cluster);
  }
...
function criterioDuplicadoGrupo(rows) {
  ...
  const tipos = new Set((rows || []).map((r) => String(r.tipo || "").toLowerCase()).filter(Boolean));
  const parts = [];
  ...
  if (tipos.size > 1) parts.push("distinta categoría");
  if (rows && rows._criterioExtra) parts.push(rows._criterioExtra);
  return parts.join(" + ");
}
...
async function buildTallerAtWorkbook(rows, opts) {
  ...
  wsD.getCell("A3").value =
    "Importe ±5% o ≤$250. Concepto similar. PREVENTIVO vs OTROS no excluye. También detecta mismo juego multi-AT (↳).";
