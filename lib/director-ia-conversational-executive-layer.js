...
function buildExecutiveStatusPack(input) {
  ...
  const forecastVenta = arrTon != null ? arrTon : finiteOrNull(adapterForecast && adapterForecast.venta_ton);
  const forecastDesc = arrDesc != null ? arrDesc : finiteOrNull(adapterForecast && adapterForecast.desc_kg);
  ...
  items.push(
    evidenceItem({
      ...
      availability: forecastVenta == null && forecastDesc == null ? AVAILABILITY.UNAVAILABLE : mapSourceAvailability(sources.arr),
      provenance: "dashboard-arr-forecast.computePronosticoProyByPlant via ARR",
      payload: { venta_ton: forecastVenta, desc_kg: forecastDesc, metric: "forecast_venta_desc" },
      summary:
  ...
  const economics = (input.forecastParity && input.forecastParity.economics) || {};
  const igfUtilOper = finiteOrNull(economics.util_oper_importe);
  const igfResultado = finiteOrNull(economics.resultado_final_importe);
  items.push(
    evidenceItem({
      slot: "MAGNITUDE",
  ...
  items.push(
    evidenceItem({
      slot: "MAGNITUDE",
      source: "dashboard-igf-forecast.recalcularUtilYResultado",
      scope: plantName,
      period: igfPeriod,
      truth_semantics: "FORECAST_PROJECTION",
      availability: igfUtilOper == null ? AVAILABILITY.UNAVAILABLE : AVAILABILITY.OPTIONAL,
      provenance: economics.helper || "recalcularUtilYResultado after PROY overlay",
      payload: { util_oper_importe: igfUtilOper, metric: "util_oper_importe" },
      summary:
        igfUtilOper == null
          ? "Utilidad operativa IGF Forecast no disponible. No se usa el stored de compromiso_lines como cifra del Dashboard. Ausencia no es cero."
          : `Utilidad operativa ${igfUtilOper} (FORECAST_PROJECTION vía recalcularUtilYResultado tras overlay PROY; no es stored ni ACTUAL_FINANCIAL ni TARGET).`,
    })
  );
  items.push(
    evidenceItem({
      ...
      summary:
        igfResultado == null
          ? "Resultado final IGF Forecast no disponible. No se usa el stored de compromiso_lines como cifra del Dashboard. Ausencia no es cero. No se infiere TARGET."
          : `Resultado final ${igfResultado} (FORECAST_PROJECTION vía recalcularUtilYResultado tras overlay PROY; no es stored ni ACTUAL_FINANCIAL ni TARGET).`,
    })
  );
...
function executiveQuestionFocusLines(question) {
  ...
  if (/\bdescuento\b/.test(q)) {
    lines.push(
      "La pregunta nombra descuento: prioriza MAGNITUDE de descuento (FORECAST_PROJECTION desc_kg y/o FORECAST_STORED com_desc_kg)."
    );
  }
  if (/\b(rentabilidad|utilidad|resultado)\b/.test(q)) {
    lines.push(
      "La pregunta nombra rentabilidad/utilidad/resultado: prioriza util_oper_importe y resultado_final_importe FORECAST_STORED. No uses ACTUAL_FINANCIAL como sustituto del mes abierto."
    );
  }
