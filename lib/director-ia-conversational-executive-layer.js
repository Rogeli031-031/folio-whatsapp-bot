...
function isPersonalHowAreYouQuestion(q) {
  return /\bque tal\s+estas\b/.test(q);
}

function isNamedDayOverviewQuestion(q) {
  return /\bdia\b/.test(q) && (/\b(hoy|ayer)\b/.test(q) || /\bel dia\b/.test(q));
}

function hasExecutiveStatusCue(q) {
  const howVerb =
    /\b(va|vamos|van|anda|andamos|estamos|esta|estan|venimos|se ve|se esta|esta yendo|marcha|pinta|encontramos)\b/;
  const howStatus =
    (/\bcomo\b/.test(q) && howVerb.test(q)) ||
    (/\bque tal\b/.test(q) && /\b(vamos|andamos)\b/.test(q));
  const situation =
    /\b(que esta pasando|que pasa|situacion( general)?|diagnostico|estado general|como se ve|cual es el estado)\b/.test(
      q
    ) || /\b(resumen ejecutivo|reporte ejecutivo)\b/.test(q);
...
function isExecutiveStatusQuestion(raw) {
  const q = normalizeExecutiveText(raw);
  if (!q) return false;
  if (isPersonalHowAreYouQuestion(q)) return false;
  if (isNamedDayOverviewQuestion(q)) return false;
