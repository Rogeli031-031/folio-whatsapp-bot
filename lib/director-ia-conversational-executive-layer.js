...
function formatOneMoverLine(m) {
  ...
  const deltaBit = delta == null ? "" : `, Δ ${delta} t`;
  const prev = m.venta_ton_prev != null ? m.venta_ton_prev : "—";
  const actual = m.venta_ton_actual != null ? m.venta_ton_actual : "—";
  return `${m.cliente || "Cliente"}: ${label}${deltaBit}. Prev ${prev} / Actual ${actual}. ${formatRegisteredComments(
    m.registered_comments
  )}`;
}

function formatCommercialMoversSummary(casaMovers, comiMovers) {
  const casa = Array.isArray(casaMovers) ? casaMovers : [];
  const comi = Array.isArray(comiMovers) ? comiMovers : [];
  if (!casa.length && !comi.length) return null;
  const lines = ["Movimientos del motor commercial-trend-engine (top_movers). No es DRIVERS/DICF. Mover != causa."];
  if (casa.length) {
    lines.push("CASA:");
...
function executiveQuestionFocusLines(question) {
  ...
  if (/\b(como vamos|como andamos|como estamos|que esta pasando|estado)\b/.test(q)) {
    ...
    lines.push(
      "Si COMMERCIAL_MOVERS está AVAILABLE, incluye el bloque «Movimientos comerciales relevantes» DESPUÉS de Tendencias y ANTES de Riesgos. Separa CASA y COMISIONISTA. Comentario registrado no es causa. DRIVERS (dicf_cliente_mes) no sustituye este bloque."
    );
  }
  return lines;
}
  ...
  for (const item of pack.items || []) {
    ...
    if (item.slot === "COMMERCIAL_MOVERS" && item.payload) {
      for (const key of ["casa", "comisionista"]) {
        const list = item.payload[key] || [];
        lines.push(`  ${key === "casa" ? "CASA" : "COMISIONISTA"} movers=${list.length}`);
        for (const m of list) {
