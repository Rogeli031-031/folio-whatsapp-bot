...
const { ACTUAL_TO_DATE_KIND } = require("./director-ia-dashboard-forecast-adapter");
const { buildIdentityGreeting } = require("./director-ia-authenticated-user");
...
const NO_CLOSED_DAYS_SOLD_ZERO_RE = /\bse han vendido 0(?:[.,]0+)?\s*(?:t|toneladas?)\b/gi;
const NO_CLOSED_DAYS_ACTIVITY_INFERENCE_RE =
  /\bfalta de actividad comercial(?: significativa)?\b|\bno hubo actividad\b|\bla planta no vendi[oó]\b|\bmal desempe[nñ]o\b/gi;
const NO_CLOSED_DAYS_INDICATES_RE =
  /,?\s*lo que indica (?:una )?(?:falta de actividad(?: comercial(?: significativa)?)?|ca[ií]da|mal desempe[nñ]o)[^.]*\.?/gi;

const NEED_TYPES = Object.freeze({
  EXECUTIVE_STATUS: "EXECUTIVE_STATUS",
  DIAGNOSIS_OBSERVATION_RISK: "DIAGNOSIS_OBSERVATION_RISK",
...
const IMPLEMENTED_THIS_SLICE = Object.freeze([
  NEED_TYPES.EXECUTIVE_STATUS,
  NEED_TYPES.DIAGNOSIS_OBSERVATION_RISK,
]);
...
function isSpecializedStandaloneQuestion(raw) {
  ...
  if (typeof runtime008.isRegulationStatusQuestion === "function" && runtime008.isRegulationStatusQuestion(raw)) {
    return true;
  }
  if (typeof runtime008.isTallerExpenseQuestion === "function" && runtime008.isTallerExpenseQuestion(raw)) {
    return true;
  }
  return false;
}

function isPriorityOrAttentionQuestion(q) {
  return /\b(lo mas importante|que es lo mas importante|tengo que atender|atiendo primero|hago primero|que atiendo)\b/.test(
    q
  );
}

function isDiagnosisObservationRiskQuestion(raw) {
  const q = normalizeExecutiveText(raw);
  if (!q) return false;
  if (isCauseQuestion(q)) return false;
  if (isRecommendationQuestion(q)) return false;
  if (isGreetingUtterance(q)) return false;
  if (isPriorityOrAttentionQuestion(q)) return false;
  if (/\bplanta(s)?\b/.test(q) && /\bpreocup/.test(q)) return false;
  if (/\bpre[\s-]?cierre\b/.test(q)) return false;

  const concern = /\bpreocup/.test(q);
  const failing =
    /\bfallando\b/.test(q) ||
    /\bproblemas?\b/.test(q) ||
    /\bsaliendo mal\b/.test(q) ||
    /\bdeteriorando\b/.test(q) ||
    /\bprincipal problema\b/.test(q);
  const risks =
    /\b(cual es el riesgo|cuales son los riesgos|que riesgo|que riesgos|riesgos ves)\b/.test(q) ||
    (/\briesgos?\b/.test(q) && /\b(ves|veo|hay)\b/.test(q));
  const workingAndNot = /\bfuncionando\b/.test(q) && /\bque no\b/.test(q);
  return concern || failing || risks || workingAndNot;
}

function isRiskFocusQuestion(q) {
  return isDiagnosisObservationRiskQuestion(q);
}
...
function hasMeetingPrepCue(q) {
  return /\b(junta|steering|pre-?cierre|precierre|preocup)\b/.test(q);
}

function hasForecastProspectiveCue(q) {
  return (
    /\bforecast\b/.test(q) ||
    /\bproyecc/.test(q) ||
    /\bproyect/.test(q) ||
    /\bal corte\b/.test(q) ||
    /\bestas usando\b/.test(q)
  );
}

function hasBoundedCloseCue(q) {
  if (hasMeetingPrepCue(q)) return false;
  return /\b(cierre|cerrar|cerremos|cerramos)\b/.test(q);
}

function isForecastDiscountMagnitudeQuestion(q) {
  if (!/\bdescuento\b/.test(q)) return false;
  if (/\b(cliente|ayer)\b/.test(q)) return false;
  if (/\b(igf|arr)\b/.test(q) && !/\bdelta\b/.test(q)) return false;
  if (hasMeetingPrepCue(q)) return false;
  return hasForecastProspectiveCue(q) || hasBoundedCloseCue(q);
}

function specializedBlocksForecastMagnitude(raw, q) {
  if (/\b(igf|arr)\b/.test(q) && !/\bdelta\b/.test(q)) return true;
  if (typeof isMonthCloseQuestion === "function" && isMonthCloseQuestion(raw)) return true;
  if (typeof isPreMeetingQuestion === "function" && isPreMeetingQuestion(raw)) return true;
  if (typeof isCommercialTrendQuestion === "function" && isCommercialTrendQuestion(raw)) return true;
  ...
  if (/\bayer\b/.test(q) && /\b(venta|descuento)\b/.test(q)) return true;
  if (/\bactual\s+financ/i.test(String(raw || "")) || /\bfinancial\.actual\b/.test(q)) return true;
  if (typeof isPreCloseQuestion === "function" && isPreCloseQuestion(raw)) {
    return !isForecastDiscountMagnitudeQuestion(q);
  }
  return false;
}
...
function classifyForecastMagnitudeFollowUp(raw, opts) {
  ...
  if (specializedBlocksForecastMagnitude(raw, q)) return null;
  ...
  if (isForecastDiscountMagnitudeQuestion(q)) {
...
function isExecutiveStatusQuestion(raw) {
  ...
  if (
    isDiagnosisObservationRiskQuestion(raw) ||
    isCauseQuestion(q) ||
    isRecommendationQuestion(q) ||
    isComparisonQuestion(q)
  ) {
    return false;
  }
...
function resolveExecutiveNeed(raw, _opts) {
  ...
  if (isDiagnosisObservationRiskQuestion(raw)) {
    return {
      need_type: NEED_TYPES.DIAGNOSIS_OBSERVATION_RISK,
      implemented: true,
      reason: "diagnosis_observation_risk_cue",
    };
  }
...
function extractExplicitPlant(raw, catalog) {
  ...
  if (named && named[1]) {
    const token = named[1]
      .replace(/\b(la|el|los|las|una|un|esta|este|estos|estas|planta|hoy|ahora|este mes|actual|actualmente)\b/g, " ")
      .replace(/\s+/g, " ")
      .trim();
    if (
      token &&
      ...
      !runtime010.isNonPlantBusinessToken(token) &&
      !hasArTheme(token) &&
      !/\b(regulacion(?:es)?|regulatori|seh|permisos?|seguridad|higiene|cumplimiento|documental|obligaciones)\b/.test(
        token
      )
    ) {
...
function resolveSemanticScope(raw, opts) {
  ...
  if (explicit && explicit.nombre && !runtime010.isNonPlantBusinessToken(explicit.nombre)) {
...
function formatOneMoverLine(m) {
  ...
  const registered = comments.map((c) => formatOneRegisteredComment(c, { quoted: true })).filter(Boolean).join(" ");
...
function buildExecutiveStatusPack(input) {
  ...
  const periodStartStatus =
    (authPack && authPack.actual_to_date && authPack.actual_to_date.period_start_status) ||
    (adapterActual && adapterActual.period_start_status) ||
    null;
  const lastClosedDay =
    authPack && authPack.actual_to_date && authPack.actual_to_date.last_closed_day != null
      ? authPack.actual_to_date.last_closed_day
      : adapterActual && adapterActual.last_closed_day != null
        ? adapterActual.last_closed_day
        : null;
  const noClosedDaysYet = periodStartStatus === ACTUAL_TO_DATE_KIND.NO_CLOSED_DAYS_YET;
  ...
  items.push(
    evidenceItem({
      ...
      payload: {
        ...
        period_start_status: periodStartStatus,
      },
      summary: noClosedDaysYet
        ? formatNoClosedDaysYetSummary(cutoffDate)
...
function packHasNoClosedDaysYet(pack) {
  const actual = pack && pack.authoritative_forecast && pack.authoritative_forecast.actual_to_date;
  if (actual && actual.period_start_status === ACTUAL_TO_DATE_KIND.NO_CLOSED_DAYS_YET) return true;
  return (pack && pack.items ? pack.items : []).some(
    (i) =>
      i &&
      i.truth_semantics === "ACTUAL_TO_DATE" &&
      i.payload &&
      i.payload.period_start_status === ACTUAL_TO_DATE_KIND.NO_CLOSED_DAYS_YET
  );
}

function formatNoClosedDaysYetSummary(cutoffDate) {
  const corte = cutoffDate ? `Al corte del ${cutoffDate}, ` : "";
  return `${corte}aún no hay días cerrados del mes para reportar venta acumulada al corte. El 0 de TOTAL no demuestra ausencia de ventas, caída ni mal desempeño. (${ACTUAL_TO_DATE_KIND.NO_CLOSED_DAYS_YET})`;
}
...
function executiveQuestionFocusLines(question) {
  ...
  if (/\b(como vamos|como andamos|como estamos|que esta pasando|estado)\b/.test(q)) {
    ...
    lines.push(
      "Si COMMERCIAL_MOVERS está AVAILABLE, incluye el bloque «Movimientos comerciales relevantes» DESPUÉS de Tendencias y ANTES de Riesgos, usando solo la proyección verbal compacta del pack (no imprimas los 12 del motor). Separa CASA y COMISIONISTA. Si una línea de esa proyección incluye «Comentario registrado», cópiala completa (incluye la fecha de registro entre corchetes si viene). No la omitas. No la conviertas en causa. Prohibido: «disminuyó porque [comentario]», «dejó de comprar porque [comentario]» o poner el comentario entre paréntesis como si explicara el delta. La fecha entre corchetes es la fecha de registro del comentario; no afirma que pertenezca a la ventana comercial del delta. Si la línea dice «Sin comentario reciente», no inventes comentario ni causa; no la sustituyas con Bitácora, DICF, Action Register ni ARR. El comentario no cambia el delta. DRIVERS (dicf_cliente_mes) no sustituye este bloque."
    );
  }
  return lines;
}
...
function formatPackForPrompt(pack) {
  const lines = [
    ...
    "Tras Tendencias, si COMMERCIAL_MOVERS está AVAILABLE, verbaliza «Movimientos comerciales relevantes» antes de Riesgos. No es DRIVERS. Si la línea trae «Comentario registrado», no la omitas. Comentario registrado ≠ causa. No reconcilies un comentario contradictorio con el delta. Prohibido verbalizar «disminuyó porque [comentario]».",
  ...
  for (const item of pack.items || []) {
    ...
    if (item.payload) {
      for (const key of [
        ...
        if (item.payload[key] == null) continue;
        if (
          key === "venta_ton" &&
          item.payload.period_start_status === ACTUAL_TO_DATE_KIND.NO_CLOSED_DAYS_YET
        ) {
          continue;
        }
  ...
  if (packHasNoClosedDaysYet(pack)) {
    lines.push("");
    lines.push(
      "ACTUAL_TO_DATE period_start_status=NO_CLOSED_DAYS_YET: el periodo acaba de iniciar; todavía no existe día cerrado computable. No afirmes «se han vendido 0 t» como actividad observada. Prohibido inferir, solo por este 0: falta de actividad comercial, no hubo actividad, la planta no vendió, mal desempeño, caída. No demuestra ausencia de ventas."
    );
  }
...
function applyExecutiveLanguageGuard(answer, pack) {
  ...
  if (packHasNoClosedDaysYet(pack)) {
    text = text.replace(NO_CLOSED_DAYS_SOLD_ZERO_RE, "aún no hay días cerrados del mes para reportar venta acumulada al corte");
    text = text.replace(NO_CLOSED_DAYS_INDICATES_RE, ".");
    text = text.replace(NO_CLOSED_DAYS_ACTIVITY_INFERENCE_RE, "");
  }
...
function buildNeutralGreeting(plantLabelOrOpts, identity) {
  if (plantLabelOrOpts && typeof plantLabelOrOpts === "object") {
    return buildIdentityGreeting(plantLabelOrOpts.greeting_kind, plantLabelOrOpts.nombre_persona);
  }
  const fromIdentity = identity && typeof identity === "object" ? identity.nombre_persona : null;
  return buildIdentityGreeting("hola", fromIdentity);
}
...
function buildScopeClarificationResult(opts) {
  const pending = require("./director-ia-pending-completion").buildPendingGap({
    parent_intent: opts.parent_intent || "plant_diagnosis",
    missing_fields: ["plant"],
    frame: {
      family: opts.family || "SALES_STATUS",
      pending_intent: opts.parent_intent || "plant_diagnosis",
      pending_operation: opts.operation || null,
      pending_period: opts.period || null,
      pending_comparison: opts.comparison || null,
    },
    original_question: opts.original_question || null,
    why_blocks: "Falta planta válida en contexto.",
  });
  return {
    ok: true,
    answer: opts.clarification || require("./director-ia-executive-context-sales-entity-010").plantClarification(),
    sources: [],
    context_meta: {
...
const DIAGNOSIS_OVERRIDABLE_PLANNER_INTENTS = Object.freeze(["unknown", "plant_diagnosis"]);

function shouldHandleDiagnosisObservationRisk(need, _continuityTurn, plannerIntent) {
  if (!need || need.need_type !== NEED_TYPES.DIAGNOSIS_OBSERVATION_RISK || !need.implemented) return false;
  if (need.specialized || need.steering_frontier) return false;
  if (plannerIntent != null && !DIAGNOSIS_OVERRIDABLE_PLANNER_INTENTS.includes(plannerIntent)) return false;
  return true;
}
