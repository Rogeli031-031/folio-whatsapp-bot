...
function buildExecutiveStatusPack(input) {
  ...
  const miniForecastAvailable = hasCutoff && (miniVenta != null || miniDesc != null);
  const adapterForecastAvailable = hasCutoff && (adapterVenta != null || adapterDesc != null);
  const forecastVenta = !hasCutoff ? null : miniVenta != null ? miniVenta : adapterVenta;
  const forecastDesc = !hasCutoff ? null : miniDesc != null ? miniDesc : adapterDesc;
  ...
  items.push(
    evidenceItem({
      ...
      provenance: !hasCutoff
        ...
        : miniForecastAvailable
          ? miniAuth.source || "loadProyVentaDescByPlantForIgf → computeIgfForecastMiniPayload"
          : "misma ruta autoritativa vía adapter; no ARR",
      payload: {
        ...
        governed_by: !hasCutoff
          ? "unavailable_no_cutoff"
          : miniForecastAvailable
            ? "dashboard_authoritative_mini"
            : adapterForecastAvailable
              ? "authoritative_adapter"
              : "unavailable_no_forecast",
      },
      summary: !hasCutoff
        ...
        : forecastVenta == null && forecastDesc == null
          ...
          : `Forecast al corte del ${cutoffLabel || cutoffDate}: venta ${forecastVenta == null ? "n/d" : forecastVenta + " t"}; desc ${forecastDesc == null ? "n/d" : forecastDesc + " $/kg"} (FORECAST_PROJECTION${miniForecastAvailable ? " mini IGF" : " adapter"}; misma corrida; no es acumulado ni meta ni ARR).`,
    })
  );
