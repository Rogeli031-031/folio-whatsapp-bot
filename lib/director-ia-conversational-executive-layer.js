...
function buildExecutiveStatusPack(input) {
  ...
  const arrTon = finiteOrNull(
    sources.arr && sources.arr.payload && sources.arr.payload.venta_ton != null
      ? sources.arr.payload.venta_ton
      : sources.arr && sources.arr.payload && sources.arr.payload.proy_venta_ton
  );
  const arrDesc = finiteOrNull(sources.arr && sources.arr.payload && sources.arr.payload.desc_kg);
  const adapterForecast = input.forecastParity && input.forecastParity.forecast;
  const adapterActual = input.forecastParity && input.forecastParity.actual_to_date;
  const miniAuth = (input.forecastParity && input.forecastParity.mini) || {};
  const miniVenta = finiteOrNull(miniAuth.venta_ton);
  const miniDesc = finiteOrNull(miniAuth.desc_kg);
  const adapterVenta = finiteOrNull(adapterForecast && adapterForecast.venta_ton);
  const adapterDesc = finiteOrNull(adapterForecast && adapterForecast.desc_kg);
  const miniForecastAvailable = miniVenta != null || miniDesc != null;
  const forecastVenta = miniVenta != null ? miniVenta : adapterVenta != null ? adapterVenta : arrTon;
  const forecastDesc = miniDesc != null ? miniDesc : adapterDesc != null ? adapterDesc : arrDesc;
  const actualToDateVenta = finiteOrNull(adapterActual && adapterActual.venta_ton);
  const arrPeriod =
  ...
  items.push(
    evidenceItem({
      ...
      provenance: "Dashboard venta al corte; no es proyección ni ACTUAL_FINANCIAL de cierre",
      payload: { venta_ton: actualToDateVenta, metric: "venta_ton" },
      summary:
        actualToDateVenta == null
          ? "Venta al corte no disponible. Ausencia no es cero. No es proyección."
          : `Venta al corte ${actualToDateVenta} t (ACTUAL_TO_DATE; no es forecast ni cierre FINAL).`,
    })
  );
  ...
  items.push(
    evidenceItem({
      slot: "MAGNITUDE",
      source: miniForecastAvailable ? "igf-forecast-mini.ventaTon" : "arr.proyeccion_planta",
      scope: plantName,
      period: miniForecastAvailable ? adapterPeriod : arrPeriod,
      truth_semantics: "FORECAST_PROJECTION",
      availability:
        forecastVenta == null && forecastDesc == null
          ? AVAILABILITY.UNAVAILABLE
          : miniForecastAvailable
            ? AVAILABILITY.OPTIONAL
            : mapSourceAvailability(sources.arr),
      provenance: miniForecastAvailable
        ? miniAuth.source || "computeIgfForecastMiniPayload"
        : "dashboard-arr-forecast.computePronosticoProyByPlant via ARR",
      payload: {
        venta_ton: forecastVenta,
        desc_kg: forecastDesc,
        metric: "forecast_venta_desc",
        governed_by: miniForecastAvailable ? "dashboard_authoritative_mini" : "arr_or_adapter_fallback",
      },
      summary:
        forecastVenta == null && forecastDesc == null
          ? "No hay cifra de proyección usable. Ausencia no es cero."
          : miniForecastAvailable
            ? `Forecast autoritativo Dashboard venta ${forecastVenta == null ? "n/d" : forecastVenta + " t"}; desc ${forecastDesc == null ? "n/d" : forecastDesc + " $/kg"} (FORECAST_PROJECTION mini IGF; no es ARR crudo, no es stored, no es actual ni TARGET).`
            : `Proyección ARR venta ${forecastVenta == null ? "n/d" : forecastVenta + " t"}; desc ${forecastDesc == null ? "n/d" : forecastDesc + " $/kg"} (FORECAST_PROJECTION; no es venta actual ni TARGET).`,
    })
  );

  const igfTon = igfCompositionLine(sources, "venta_ton");
  const igfDesc = igfCompositionLine(sources, "com_desc_kg");
  const igfUtilOper = finiteOrNull(miniAuth.util_oper_importe);
  const igfResultado = finiteOrNull(miniAuth.resultado_final_importe);
  items.push(
    evidenceItem({
      slot: "MAGNITUDE",
      ...
      payload: { venta_ton: igfTon, metric: "venta_ton" },
      summary:
        igfTon == null
          ? "No hay valor IGF almacenado usable. Ausencia no es cero ni se sustituye con forecast inventado."
          : `IGF almacenado ${igfTon} t (FORECAST stored, no actual, no TARGET).`,
    })
  );
  items.push(
    evidenceItem({
      ...
      provenance: "IGF stored com_desc_kg",
      payload: { desc_kg: igfDesc, com_desc_kg: igfDesc, metric: "com_desc_kg" },
      summary:
        igfDesc == null
          ? "Descuento IGF almacenado no disponible. Ausencia no es cero. No es TARGET."
          : `IGF descuento almacenado ${igfDesc} $/kg (FORECAST_STORED com_desc_kg; no es actual ni TARGET).`,
    })
  );
  items.push(
    evidenceItem({
      ...
      provenance: miniAuth.source || "GET /api/dashboard/igf-forecast include_mini",
      payload: { util_oper_importe: igfUtilOper, metric: "util_oper_importe" },
      summary:
        igfUtilOper == null
          ? "Utilidad operativa del mini IGF Forecast no disponible. No se usa stored ni recalcularUtilYResultado. Ausencia no es cero."
          : `Utilidad operativa ${igfUtilOper} (FORECAST_PROJECTION mini IGF Forecast utilOperImporte; no es stored ni ACTUAL_FINANCIAL ni TARGET).`,
    })
  );
  items.push(
    evidenceItem({
      ...
      provenance: miniAuth.source || "GET /api/dashboard/igf-forecast include_mini",
      payload: { resultado_final_importe: igfResultado, metric: "resultado_final_importe" },
      summary:
        igfResultado == null
          ...
          : `Resultado final ${igfResultado} (FORECAST_PROJECTION mini IGF Forecast resultadoFinalImporte; no es stored ni ACTUAL_FINANCIAL ni TARGET).`,
    })
  );
...
function executiveQuestionFocusLines(question) {
  ...
  const lines = [
    ...
    "Forecast no es actual. Actual al corte no es proyección. IGF stored no es TARGET. No infieras TARGET desde IGF/Forecast.",
  ];
...
function formatPackForPrompt(pack) {
  const lines = [
    ...
    "PRECEDENCIA KPI: si un MAGNITUDE FORECAST_PROJECTION del mini IGF está AVAILABLE (governed_by=dashboard_authoritative_mini), gobierna venta/desc/util/resultado. ARR crudo, computePronosticoProyByPlant y FORECAST_STORED no lo pisan. ACTUAL_TO_DATE no es forecast.",
  ...
  for (const item of pack.items || []) {
    ...
    if (item.payload) {
      for (const key of ["venta_ton", "desc_kg", "com_desc_kg", "util_oper_importe", "resultado_final_importe"]) {
