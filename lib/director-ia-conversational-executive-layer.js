...
function buildExecutiveStatusPack(input) {
  ...
  const actualToDateDesc = finiteOrNull(adapterActual && adapterActual.desc_kg);
  const cutoffDate =
    (miniAuth && miniAuth.cutoff_date) ||
    (adapterForecast && adapterForecast.cutoff_date) ||
    (adapterActual && adapterActual.cutoff_date) ||
    (input.forecastParity && input.forecastParity.period && input.forecastParity.period.cutoff_date) ||
    null;
  const arrPeriod =
    ...
    evidenceItem({
      ...
      summary:
        forecastVenta == null && forecastDesc == null
          ...
          : miniForecastAvailable
            ? `Forecast de cierre calculado con corte ${cutoffDate || "n/d"}: venta ${forecastVenta == null ? "n/d" : forecastVenta + " t"}; desc ${forecastDesc == null ? "n/d" : forecastDesc + " $/kg"} (FORECAST_PROJECTION mini IGF; misma corrida; no es acumulado ni meta).`
...
function executiveQuestionFocusLines(question) {
  ...
  const lines = [
    ...
    "Si cutoff_date está AVAILABLE, dilo con el acumulado y con el forecast. Venta y descuento forecast son de la misma corrida/corte. Venta y descuento stored son del mismo version_id.",
  ];
