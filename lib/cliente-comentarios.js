...
async function ensureClienteComentariosTable(client) {
  ...
  try {
    ...
    // Carrera concurrente o tipo/relación ya existente: si la tabla quedó lista, continuar.
  ...
  // Confirmar que la relación usable existe (tabla o vista); si no, re-lanzar.
  const check = await client.query(
    `SELECT 1
     FROM information_schema.tables
     WHERE table_schema = 'arr' AND table_name = 'cliente_comentarios'
     LIMIT 1`
  );
  if (!check.rows[0]) {
    throw new Error('No se pudo asegurar la tabla arr.cliente_comentarios');
  }
  await client.query(`
...
/**
 * Últimos N comentarios por nombre de cliente (misma semántica que venta-serie).
 * No filtra por periodo del delta. Ligados al cliente/planta, no a la ventana.
 * @returns {Promise<Map<string, {body:string, author_name:string, created_at:string}[]>>}
 */
async function loadRecentCommentsByClienteNombres(client, opts = {}) {
  const byNombre = new Map();
  const plantaIds = (Array.isArray(opts.plantaIds) ? opts.plantaIds : [])
    .map((x) => Number(x))
    .filter((n) => Number.isFinite(n) && n > 0);
  const nombres = (Array.isArray(opts.nombres) ? opts.nombres : [])
    .map((n) => String(n || "").trim())
    .filter(Boolean);
  const limitPerCliente = Math.min(Math.max(parseInt(String(opts.limitPerCliente || 2), 10) || 2, 1), 5);
  if (!client || typeof client.query !== "function" || !plantaIds.length || !nombres.length) {
    return byNombre;
  }
  ...
  const cr = await client.query(
    ...
    [plantaIds, namesLower, limitPerCliente]
  );
  for (const row of cr.rows || []) {
    ...
    byNombre.get(k).push({
      ...
      created_at: row.created_at
        ? String(row.created_at instanceof Date ? row.created_at.toISOString() : row.created_at).slice(0, 10)
        : "",
    });
  }
  return byNombre;
...
module.exports = {
  ...
  loadRecentCommentsByClienteNombres,
