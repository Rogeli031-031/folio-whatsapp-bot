...
async function computeDeltaIngresoForecast(client, plantCode, yearA, monthA, yearB, monthB, getMargenKg, plantaNombre) {
  ...
  const descAMap = new Map((descA.rows || []).map((r) => [r.cliente_norm, Number(r.monto || 0)]));
  ...
  for (const cliente of allClientes) {
    ...
    const descA = descAMap.get(cliente) || 0;
    const descBReal = descBRealMap.get(cliente) || 0;

    const descKgA = kgA > 0 ? Math.abs(descA) / kgA : 0;
    let descKgBProj = 0;
    let kgBProj = kgBReal;
    if (state !== "Inactivo" && atten > 0) {
      const extraKg = projectKgToMonthEnd(kgSeries, startStr, maxFechaStr, endMesB);
      kgBProj = kgBReal + extraKg;
      ...
      const descProj = descBReal + descRate * extraKg;
