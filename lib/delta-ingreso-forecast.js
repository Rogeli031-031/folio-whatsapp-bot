...
async function computeDeltaIngresoForecast(client, plantCode, yearA, monthA, yearB, monthB, getMargenKg, plantaNombre) {
  ...
  for (const cliente of allClientes) {
    ...
    if (kgSeries.length > 0) {
      /** Hasta 8 semanas (56 días); la UI permite 4–8 semanas visibles. */
      const windowDays = 56;
  ...
  // Persistir resumen por cliente (ingresos reales/proyectados) para este periodo B.
  try {
    await client.query(
      `DELETE FROM arr.delta_ingreso_forecast_cliente WHERE plant_code = $1 AND year = $2 AND month = $3`,
      [plantCode, yearB, monthB]
    );
    const insertText = `
      INSERT INTO arr.delta_ingreso_forecast_cliente (
        plant_code, year, month, cliente_norm, canal, subcanal, estado,
        freq_days, days_since_last, kg_a, ingreso_a, kg_b_real, kg_b_proj, ingreso_b, delta_ingreso
      )
    ...
    // No romper el flujo si falla el cache.
    console.warn("delta_ingreso_forecast_cliente cache error:", e.message);
  }
