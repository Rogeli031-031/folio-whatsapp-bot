...
function extractKeyword(q) {
  ...
  const tokens = work
    ...
    .map((t) => t.trim())
    .filter(Boolean)
    .filter((t) => !KEYWORD_STOP.has(t) && !/^\d+$/.test(t));
...
function extractExpenseAnalyticsSpec(question, opts = {}) {
  ...
  const attributable =
    !folioMatch &&
    ((Boolean(exclusive) && (exactCue || !domain)) || (exactCue && Boolean(keyword) && !exclusive));
...
function buildExpenseAnalyticsAnswer(payload) {
  ...
  if (spec.metric === METRICS.COUNT) {
    return `${lead} fueron ${analysis.eligible_count || 0}.`;
  }
  ...
  const total = formatMxn(analysis.sum || 0);
  if (spec.domain === DOMAINS.TALLER && spec.metric === METRICS.SUM) {
    ...
    if (records.length) {
      const paid = records.filter((r) => String(r.estatus || "").toUpperCase() === "PAGADO").length;
      const pending = records.filter((r) => String(r.estatus || "").toUpperCase().includes("PENDIENTE")).length;
      if (paid || pending) extras.push(`${paid} pagados / ${pending} pendientes`);
    ...
    return [`De ${formatPeriodLabel(spec)} se gastaron ${total} en Taller.`, extras.length ? extras.join("\n") : ""]
      .filter(Boolean)
      .join("\n");
  }
  if (spec.semantic_label === SEMANTIC_LABELS.TOTAL_FOLIOS_MATCHING_KEYWORD) {
    const base = spec.domain
      ? `${lead} suman ${total}.`
      : `Los folios coincidentes suman ${total}.`;
    return `${base} ${keywordLimitation(spec.keyword)}`;
  }
