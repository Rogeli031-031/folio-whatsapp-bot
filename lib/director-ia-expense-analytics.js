...
function hasExactCostCue(q) {
  return /\bexactamente\b/.test(q) || /\bexclusiv/.test(q);
}
...
function extractExpenseAnalyticsSpec(question, opts = {}) {
  ...
  const attributable =
    Boolean(exclusive) && !folioMatch && (exactCue || !domain);

  const metric = detectMetric(q, {
    forceAttributable: attributable,
    spendContext: Boolean(domain || folioMatch || exclusive),
  });
...
function buildExpenseAnalyticsAnswer(payload) {
  ...
  if (spec.semantic_label === SEMANTIC_LABELS.TOTAL_FOLIOS_MATCHING_KEYWORD) {
    ...
    if (exclusiveKeywordOf(spec.keyword) || spec.folio_match_frame) {
...
async function loadExpenseAnalyticsForChat(pool, plantaId, req, opts = {}) {
  ...
  const spec = extractExpenseAnalyticsSpec(question, { now: opts.now });
