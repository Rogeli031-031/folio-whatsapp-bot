 ...
 * Integración de infraestructura: createEksRuntime (pool dedicado, start/stop).
 * No produce Bundles. No escribe Snapshots al arrancar. No es endpoint ni chat.
 */
"use strict";
...
function normalizeQueryContextMetadata(value) {
  if (!isPlainObject(value)) return null;
  return structuredCloneJson(value);
}
    ...
    async insertSnapshot(row) {
      return withTraceLock(row.trace_id, async () => {
        ...
        const stored = {
          ...
          query_context_metadata: normalizeQueryContextMetadata(row.query_context_metadata),
        };
  ...
  return {
      ...
      try {
        ...
        const ins = await client.query(
          `INSERT INTO eks.snapshots
            (snapshot_id, bundle_id, trace_id, version, persisted_at, bundle, integrity, query_context_metadata)
           VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7, $8::jsonb)
           RETURNING snapshot_id, bundle_id, trace_id, version, persisted_at, bundle, integrity, query_context_metadata`,
          [
            row.snapshot_id,
            row.bundle_id,
            row.trace_id,
            ...
            row.integrity,
            row.query_context_metadata == null ? null : JSON.stringify(row.query_context_metadata),
          ]
        );
    ...
    async getBySnapshotId(snapshotId) {
      const res = await pool.query(
        `SELECT snapshot_id, bundle_id, trace_id, version, persisted_at, bundle, integrity, query_context_metadata
 ...
 */
function directorIaFlagEnabled(env) {
  const raw = env && env.ENABLE_DIRECTOR_IA;
...
/**
 * Servicio interno de infraestructura EKS (D8).
 * No abre pool por operación. No append al start. No usa el pool operacional.
 * @param {{ env?: NodeJS.ProcessEnv, operationalPool?: object, createDedicatedPool?: Function, createEks?: Function }} [options]
 */
function createEksRuntime(options = {}) {
  const env = options.env || process.env;
  const makePool = options.createDedicatedPool || createDedicatedPool;
  const makeEks = options.createEks || createEks;
  const operationalPool = options.operationalPool || null;

  let pool = null;
  let eks = null;
  let started = false;
  let status = { enabled: false, reason: "not_started" };
  ...
  function getStatus() {
    return {
      ...
      eks,
      operationalPool,
      poolIsDedicated: Boolean(pool) && pool !== operationalPool,
    };
  }
  ...
  function start() {
    ...
    if (!directorIaFlagEnabled(env)) {
      status = { enabled: false, reason: "director_ia_disabled" };
      return getStatus();
    }

    const connectionString = typeof env.DATABASE_URL === "string" ? env.DATABASE_URL.trim() : "";
    if (!connectionString) {
      console.warn("[EKS] deshabilitado: falta DATABASE_URL (mismo patrón que el bot).");
      status = { enabled: false, reason: "missing_database_url" };
      return getStatus();
    }

    const maxRaw = parseInt(env.EKS_POOL_MAX || "5", 10);
    const sslFalse = env.DATABASE_SSL === "false";
    pool = makePool(connectionString, {
      databaseSsl: sslFalse ? false : undefined,
      ssl: sslFalse ? false : undefined,
      max: Number.isFinite(maxRaw) && maxRaw > 0 ? maxRaw : 5,
    });
    if (operationalPool && pool === operationalPool) {
      pool = null;
      status = { enabled: false, reason: "pool_not_dedicated" };
      return getStatus();
    }
    eks = makeEks({ pool });
    started = true;
    status = { enabled: true, reason: "started" };
    return getStatus();
  }
  ...
  async function stop() {
    ...
    eks = null;
    started = false;
    status = { enabled: false, reason: "stopped" };
  ...
  return {
    ...
    async append_snapshot(bundle, queryContextMetadata) {
      ...
      const row = await store.insertSnapshot({
        ...
        integrity,
        query_context_metadata: normalizeQueryContextMetadata(queryContextMetadata),
      });
