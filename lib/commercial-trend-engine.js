...
function normalizeClienteNorm(raw) {
...
function assemblePoints(salesRows, discountRows) {
  ...
  for (const row of salesRows || []) {
    ...
    if (!byFecha.has(f)) byFecha.set(f, { fecha: f, venta_ton: 0, descuento_mxn: 0, has_descuento: false });
  ...
  for (const row of discountRows || []) {
    ...
    rec.descuento_mxn += row.descuento_mxn != null ? Number(row.descuento_mxn) : 0;
    rec.has_descuento = true;
  }
  return [...byFecha.values()]
    .sort((a, b) => a.fecha.localeCompare(b.fecha))
    .filter((p) => p.venta_ton !== 0 || p.descuento_mxn !== 0)
    .map((p) => {
      const venta_ton = round3(p.venta_ton);
      const descuento_mxn = round2(p.descuento_mxn);
      const kg = venta_ton * 1000;
      const descuento_kg =
        p.has_descuento && kg > 0 && Number.isFinite(descuento_mxn)
          ? Math.round((Math.abs(descuento_mxn) / kg) * 1000) / 1000
          : null;
      return { fecha: p.fecha, venta_ton, descuento_mxn, descuento_kg };
    });
}
...
function selectClientMovement(cliCurRows, cliPrevRows, clienteNorm) {
  const want = normalizeClienteNorm(clienteNorm);
  const pick = (rows) => {
    if (!want) return 0;
    const hit = (rows || []).find(
      (row) => normalizeClienteNorm(row && row.cliente) && normalizeClienteNorm(row.cliente).toLowerCase() === want.toLowerCase()
    );
    return hit ? Number(hit.venta_ton) || 0 : 0;
  };
  const actual = pick(cliCurRows);
  const previo = pick(cliPrevRows);
  const delta = round3(actual - previo);
  let tipo = "sin_cambio";
  if (previo <= 0 && actual > 0) tipo = "nuevo";
  else if (previo > 0 && actual <= 0) tipo = "perdido";
  else if (delta < -0.0005) tipo = "disminucion";
  else if (delta > 0.0005) tipo = "aumento";
  return [
    {
      cliente: want,
      venta_ton_actual: round3(actual),
...
function assembleCommercialTrend(input) {
  ...
  const clientes_top = input.cliente_norm
    ? selectClientMovement(input.cliCurRows, input.cliPrevRows, input.cliente_norm)
    : selectTopMovers(input.cliCurRows, input.cliPrevRows);
  ...
  return {
    ...
    points,
    clientes_top,
    cliente_norm: input.cliente_norm || null,
    trend,
    period_total,
...
function emptyEngineResult(opts) {
  return {
    ...
    points: [],
    clientes_top: [],
    cliente_norm: (opts && opts.cliente_norm) || null,
...
function toVentaSerieHttpBody(result, empresa) {
  return {
    ...
    cliente_norm: result.cliente_norm || undefined,
...
function clienteNormSql(alias, hasCliente) {
  return hasCliente ? `AND LOWER(TRIM(${alias}.cliente_norm)) = LOWER(TRIM($4))` : "";
}

async function querySalesSeries(client, codesUpper, startStr, endStr, canalFilter, clienteNorm) {
  const canalSql = canalSqlFor(canalFilter, "sales");
  const clienteSql = clienteNormSql("v", Boolean(clienteNorm));
  const params = clienteNorm
    ? [codesUpper, startStr, endStr, clienteNorm]
...
async function queryDiscountSeries(client, codesUpper, startStr, endStr, canalFilter, clienteNorm) {
  const canalSql = canalSqlFor(canalFilter, "discount");
  const clienteSql = clienteNormSql("d", Boolean(clienteNorm));
  const params = clienteNorm
    ? [codesUpper, startStr, endStr, clienteNorm]
    : [codesUpper, startStr, endStr];
  return client.query(
...
async function queryClientTons(client, codesUpper, startStr, endStr, canalFilter, clienteNorm) {
  const canalSql = canalSqlFor(canalFilter, "sales");
  const clienteSql = clienteNormSql("v", Boolean(clienteNorm));
  const params = clienteNorm
    ? [codesUpper, startStr, endStr, clienteNorm]
...
async function loadCommercialTrend(client, opts = {}) {
  ...
  const clienteNorm = normalizeClienteNorm((opts && (opts.cliente_norm || opts.cliente)) || "");
  if (!empresa) {
  ...
  const [salesRes, discRes, cliCur, cliPrev] = await Promise.all([
    querySales(client, codesUpper, window.fecha_desde, window.fecha_hasta, canalFilter, clienteNorm),
    queryDisc(client, codesUpper, window.fecha_desde, window.fecha_hasta, canalFilter, clienteNorm),
    queryCli(client, codesUpper, window.fecha_desde, window.fecha_hasta, canalFilter, clienteNorm),
    queryCli(client, codesUpper, window.fecha_prev_desde, window.fecha_prev_hasta, canalFilter, clienteNorm),
  ]);
