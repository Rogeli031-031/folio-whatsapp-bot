...
function normalizeSalesRow(row) {
  ...
  const month = row.month || (row.fecha && String(row.fecha).slice(0, 7)) || null;
  const fecha = row.fecha || (month ? `${month}-01` : null);
  return {
    ...row,
    fecha,
    month: month || (fecha ? String(fecha).slice(0, 7) : null),
    kg: row.kg != null ? row.kg : row.venta_kg,
    canal: row.canal || row.categoria,
    subcategoria: row.subcategoria || row.subcanal,
    subcanal: row.subcanal || row.subcategoria,
  };
}

async function loadPhysicalSalesRows(opts = {}) {
  const now = opts.now instanceof Date ? opts.now : new Date();
  const resolveCodes = opts.resolvePlantCodes;
  const qSales = opts.queryMonthlySales;
  const pool = opts.db || opts.pool;
  let codes = opts.plantCodes;
  if (!codes && typeof resolveCodes === "function" && opts.plant_label) {
    const resolved = await resolveCodes(pool, opts.plant_label);
    codes = (resolved && (resolved.uniqueCodes || resolved.codes || resolved.plantCodes)) || null;
    if (!Array.isArray(codes) && resolved && resolved.plant_code) codes = [resolved.plant_code];
  }
  if (!Array.isArray(codes) || !codes.length || typeof qSales !== "function") {
    return { rows: [], source_mode: "arr.ventas_diarias_cliente", load_error: "plant_codes_unresolved" };
  }
  const start = new Date(now.getFullYear(), now.getMonth() - 13, 1);
  const end = new Date(now.getFullYear(), now.getMonth() + 1, 0);
  const result = await qSales(pool, codes, ymdLocal(start), ymdLocal(end), "ambos");
  const raw = (result && result.rows) || result || [];
  return {
    rows: (Array.isArray(raw) ? raw : []).map(normalizeSalesRow),
    source_mode: "arr.ventas_diarias_cliente",
  };
}

function normalizeActionRegisterItems(items) {
  return (Array.isArray(items) ? items : []).map((it) => ({
    ...it,
    titulo: it.titulo || it.title,
    title: it.title || it.titulo,
    estatus: it.estatus || it.status || (it.closed ? "cerrada" : "abierta"),
    responsable: it.responsable || it.responsable_nombre,
    created_at: it.created_at || it.creada_ymd || it.dicf_creada_ymd,
    creada_ymd: it.creada_ymd || resolveCreatedYmd(it),
    dicf_creada_ymd: it.dicf_creada_ymd || it.creada_ymd || null,
    tema: it.tema || it.theme,
    vencida: it.vencida || it.overdue || false,
    dias_vencido: Number(it.dias_vencido) || 0,
  }));
}
...
module.exports = {
  ...
  normalizeSalesRow,
  loadPhysicalSalesRows,
  normalizeActionRegisterItems,
