...
function createDirectorIaArrInput(options) {
  ...
  async function run(input) {
    ...
    let queryContextMetadata = null;
    if (isPlainObject(original.query_context_metadata)) {
      queryContextMetadata = cloneJson(original.query_context_metadata);
    } else if (isPlainObject(original.queryContextMetadata)) {
      queryContextMetadata = cloneJson(original.queryContextMetadata);
    }
    ...
    const snapshot = await opts.eks.append_snapshot(
      bundle,
      queryContextMetadata ? Object.assign(queryContextMetadata, { trace_id: traceId }) : null
    );
    return {
      trace_id: traceId,
      envelopes,
