...
function resolveFinancialDiagnosisNow(opts) {
  if (opts && opts.now instanceof Date && !Number.isNaN(opts.now.getTime())) return opts.now;
...
function evaluateM9TemporalSafety(assembled, now) {
  const align = (assembled && assembled.alignment) || {};
  const m9 = assembled && assembled.sources && assembled.sources.m9;
  const periodA = align.m9_period_a || (m9 && m9.period && m9.period.period_a) || null;
  const periodB = align.m9_period_b || (m9 && m9.period && m9.period.period_b) || null;
  const currentYm = yyyyMmFromDate(now);
  const currentB = Boolean(periodB && String(periodB) === currentYm);
  return {
    period_a: periodA,
    period_b: periodB,
    current_ym: currentYm,
    M9_CURRENT_PERIOD_MODE: currentB ? "OBSERVED_MTD" : "HISTORICAL",
    M9_CUTOFF_PARITY: "NO",
    M9_TEMPORAL_COMPARABILITY: currentB ? "UNSAFE" : "NOT_CURRENT_MTD",
    M9_FINANCIAL_DIAGNOSIS_USAGE: currentB ? "CONTEXT_ONLY" : "AVAILABLE",
    unsafe: currentB,
  };
}

function formatM9TemporalSafetyBlock(safety) {
  return [
    `BLOQUE M9 | temporal safety gate`,
    `M9 periodo ${safety.period_a || "—"} -> ${safety.period_b || "—"}`,
    `M9_CURRENT_PERIOD_MODE=${safety.M9_CURRENT_PERIOD_MODE}`,
    `current side = OBSERVED_MTD`,
    `M9_CUTOFF_PARITY=${safety.M9_CUTOFF_PARITY}`,
    `cutoff parity = NO`,
    `M9_TEMPORAL_COMPARABILITY=${safety.M9_TEMPORAL_COMPARABILITY}`,
    `uso financiero comparativo = UNSAFE`,
    `M9_FINANCIAL_DIAGNOSIS_USAGE=${safety.M9_FINANCIAL_DIAGNOSIS_USAGE}`,
    "M9 compara el mes previo con observación MTD del mes abierto sin paridad de corte. No se usa para explicar el resultado financiero de cierre/proyección. No se exponen totales de buckets.",
  ].join("\n");
}

function applyM9TemporalContextGate(fullContext, safety) {
  if (!safety || !safety.unsafe) return fullContext;
  const start = fullContext.indexOf("BLOQUE M9");
  if (start < 0) return fullContext;
  const end = fullContext.indexOf("Fin de bloques");
  const prefix = fullContext.slice(0, start);
  const suffix = end >= 0 ? fullContext.slice(end) : "";
  return `${prefix}${formatM9TemporalSafetyBlock(safety)}\n\n${suffix}`;
}

function buildFinancialDiagnosisPromptControl(assembled, question, opts) {
  const align = (assembled && assembled.alignment) || {};
  ...
  const safety = evaluateM9TemporalSafety(assembled, resolveFinancialDiagnosisNow(opts));
  const missing = [
  ...
  const lines = [
    ...
    `PERIOD_LABEL_ALIGNMENT=${alignmentStatus}`,
    `TEMPORAL_WINDOW_ALIGNMENT=${safety.unsafe ? "UNSAFE" : safety.M9_TEMPORAL_COMPARABILITY}`,
    "alignment.status comparable = PERIOD_LABEL_ALIGNMENT only, not temporal-window parity.",
  ...
  lines.push(
    ...
    "REQUIRED: si mencionas un total de bucket disminuyeron, dilo como reducción bruta acumulada del bucket de clientes que disminuyeron, no como caída neta de planta.",
  ...
  if (safety.unsafe) {
    lines.push(
      "FORBIDDEN: decir que todos los bloques son temporalmente comparables.",
      "FORBIDDEN: comparar magnitudes M9 con IGF/ARR.",
      "FORBIDDEN: usar M9 como tensión contra IGF/ARR.",
      "FORBIDDEN: usar M9 para explicar la caída del ingreso.",
      "REQUIRED: M9 compara el mes previo con observación MTD del mes abierto sin paridad de corte; no uses M9 para explicar el resultado financiero de cierre/proyección."
    );
  }
  if (missing.length) {
...
function buildFinancialDiagnosisPrompt(assembled, question, opts) {
  const safety = evaluateM9TemporalSafety(assembled, resolveFinancialDiagnosisNow(opts));
  const rawContext = formatFinancialDiagnosisContext(assembled);
  const context = applyM9TemporalContextGate(rawContext, safety);
  const control = buildFinancialDiagnosisPromptControl(assembled, question, opts);
  const systemPrompt = [
