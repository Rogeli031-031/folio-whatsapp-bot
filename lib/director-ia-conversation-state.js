...
function resolveConversationTurn(opts) {
  ...
  const dailyParent = parent_intent === "daily_sales_deviation";
  const dailyFollowUp = dailyParent && isDailyFollowUpKind(kind);
  const standalone = isStandaloneDetected(detected) && !dailyFollowUp;
  const topicConflict =
    kind === "period_switch" || kind === "topic_return" || kind === "plant_switch" || switchInvalidates;
  ...
  let inherit = false;
  if (validInheritContext && !standalone && (isolatedUnknown || dailyFollowUp)) {
    inherit = true;
  }
  ...
  const outOfSliceClarify =
    kind === "period_switch" ||
    kind === "topic_return" ||
    (kind === "reference_probe" && parent_intent && parent_intent !== "daily_sales_deviation");

  const active_date =
    switchInvalidates || kind === "period_switch" || kind === "topic_return"
      ? null
