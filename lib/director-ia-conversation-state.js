...
const FOLIO_SEARCH_SPEC_VERSION = 1;
const FOLIO_SEARCH_SCOPES = Object.freeze(["ALL_PUBLIC_FOLIOS", "SUPPORT_FAMILIES"]);
const FOLIO_SEARCH_PERIOD_MODES = Object.freeze(["RANGE", "SINGLE", "ANY"]);
const FOLIO_SEARCH_CONCEPT_MODES = Object.freeze(["SINGLE", "ANY"]);
const FOLIO_SEARCH_OPERATIONS = Object.freeze(["keyword_search", "concept_sequence"]);
const FOLIO_SEARCH_YM_RE = /^\d{4}-(0[1-9]|1[0-2])$/;
const FOLIO_SEARCH_CONCEPT_MAX = 80;
const FOLIO_SEARCH_ALT_MAX = 8;
...
const FORECAST_CUTOFF_ORIGINS = Object.freeze([
  "EXPLICIT_QUESTION",
  "REQUEST_UPLOAD_DAY",
  "PLANT_LAST_UPLOAD",
  "UNAVAILABLE",
...
function sanitizeYmdField(raw) {
  const s = String(raw || "").trim().slice(0, 10);
  return /^\d{4}-\d{2}-\d{2}$/.test(s) ? s : null;
}

function sanitizeForecastRun(raw) {
  if (!raw || typeof raw !== "object") return null;
  const cutoff = sanitizeYmdField(raw.effective_cutoff_date || raw.upload_day || raw.corte_day);
  const origin = FORECAST_CUTOFF_ORIGINS.includes(raw.cutoff_origin)
    ? raw.cutoff_origin
    : cutoff
      ? "REQUEST_UPLOAD_DAY"
      : "UNAVAILABLE";
  ...
  return {
    plant_code: raw.plant_code != null && String(raw.plant_code).trim() ? String(raw.plant_code).trim() : null,
    year: Number.isFinite(year) ? year : null,
    month: Number.isFinite(month) && month >= 1 && month <= 12 ? month : null,
    upload_day: sanitizeYmdField(raw.upload_day) || cutoff,
    effective_cutoff_date: cutoff,
    corte_day: sanitizeYmdField(raw.corte_day) || cutoff,
    cutoff_origin: origin,
  };
}

function forecastMagnitudeFinite(v) {
  if (v == null || v === "") return null;
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
}

function forecastRunIdentityKey(run) {
  const sane = sanitizeForecastRun(run);
  if (!sane || !sane.effective_cutoff_date) return null;
  return [
    sane.plant_code || "",
    sane.year != null ? sane.year : "",
    sane.month != null ? sane.month : "",
    sane.upload_day || sane.effective_cutoff_date,
    sane.corte_day || "",
  ].join("|");
}

function sanitizeForecastMagnitudes(raw, run) {
  const key = forecastRunIdentityKey(run);
  if (!key || !raw || typeof raw !== "object") return null;
  if (raw.run_key && String(raw.run_key) !== key) return null;
  if (raw.semantics && raw.semantics !== "FORECAST") return null;
  return {
    run_key: key,
    venta: forecastMagnitudeFinite(raw.venta),
    descuento: forecastMagnitudeFinite(raw.descuento),
    utilidad_operativa: forecastMagnitudeFinite(raw.utilidad_operativa),
    resultado_final: forecastMagnitudeFinite(raw.resultado_final),
    semantics: "FORECAST",
  };
}

function forecastMagnitudesFromAuthoritativePack(pack, run) {
  if (!pack || pack.status !== "AVAILABLE" || !pack.forecast) return null;
  const identity =
    run ||
    (pack.run_identity
      ? {
          plant_code: pack.run_identity.plant_code,
          year: pack.run_identity.year,
          month: pack.run_identity.month,
          upload_day: pack.run_identity.upload_day,
          effective_cutoff_date: pack.run_identity.upload_day,
          corte_day: pack.run_identity.corte_day,
        }
      : null);
  return sanitizeForecastMagnitudes(
    {
      venta: pack.forecast.venta,
      descuento: pack.forecast.descuento,
      utilidad_operativa: pack.forecast.utilidad_operativa,
      resultado_final: pack.forecast.resultado_final,
      semantics: "FORECAST",
    },
    identity
  );
...
function emptyConversationState(plantaId) {
  return {
    ...
    previous_frame: null,
    forecast_run: null,
    forecast_magnitudes: null,
    active_subtopic: null,
    folio_search_spec: null,
    pending_ui_action: null,
  };
}

function sanitizeFolioSearchYm(raw) {
  const s = String(raw || "").trim();
  return FOLIO_SEARCH_YM_RE.test(s) ? s : null;
}

function sanitizeFolioSearchConceptString(raw) {
  if (raw == null) return null;
  const s = String(raw).trim().slice(0, FOLIO_SEARCH_CONCEPT_MAX);
  if (!s) return null;
  if (/[;$]|--|\/\*|\*\//.test(s)) return null;
  return s;
}

function sanitizeFolioSearchSpec(raw, requestPlantaId) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
  const requestPlant = Number(requestPlantaId);
  const specPlant = Number(raw.planta_id);
  if (!Number.isFinite(specPlant) || specPlant <= 0) return null;
  if (Number.isFinite(requestPlant) && requestPlant > 0 && specPlant !== requestPlant) return null;

  const version = raw.version == null ? FOLIO_SEARCH_SPEC_VERSION : Number(raw.version);
  if (version !== FOLIO_SEARCH_SPEC_VERSION) return null;

  const scope = String(raw.scope || "").trim();
  if (!FOLIO_SEARCH_SCOPES.includes(scope)) return null;

  const period_mode = String(raw.period_mode || "").trim();
  if (!FOLIO_SEARCH_PERIOD_MODES.includes(period_mode)) return null;

  const period_field = String(raw.period_field || "mes_cargo").trim();
  if (period_field !== "mes_cargo") return null;

  const period_month = sanitizeFolioSearchYm(raw.period_month);
  const period_start = sanitizeFolioSearchYm(raw.period_start);
  const period_end = sanitizeFolioSearchYm(raw.period_end);
  if (period_mode === "RANGE") {
    if (!period_start || !period_end || period_start > period_end) return null;
  } else if (period_mode === "ANY") {
    // existencia / universo histórico: no exige mes_cargo
  } else if (!period_month) {
    return null;
  }

  const concept_mode = String(raw.concept_mode || "SINGLE").trim();
  if (!FOLIO_SEARCH_CONCEPT_MODES.includes(concept_mode)) return null;

  const operation = String(raw.operation || "").trim();
  if (!FOLIO_SEARCH_OPERATIONS.includes(operation)) return null;

  const concept_query = sanitizeFolioSearchConceptString(raw.concept_query);
  const alternatives = Array.isArray(raw.concept_alternatives)
    ? raw.concept_alternatives.map(sanitizeFolioSearchConceptString).filter(Boolean).slice(0, FOLIO_SEARCH_ALT_MAX)
    : [];
  if (concept_mode === "ANY") {
    if (!alternatives.length) return null;
  } else if (!concept_query) {
    return null;
  }

  return {
    version: FOLIO_SEARCH_SPEC_VERSION,
    planta_id: specPlant,
    scope,
    period_mode,
    ...
    concept_mode,
    concept_query: concept_mode === "ANY" ? null : concept_query,
    concept_alternatives: concept_mode === "ANY" ? alternatives : [],
    operation,
  };
}

function isValidFolioSearchSpec(raw, requestPlantaId) {
  return Boolean(sanitizeFolioSearchSpec(raw, requestPlantaId));
}

function isFolioExistenceFollowUp(question) {
  const q = normalizeText(question);
  if (!q) return false;
  if (/\b(existe|existen)\b/.test(q) && /\b(folios?|apoyos?)\b/.test(q)) return false;
  if (/\bgastamos\b/.test(q) || /\bgasto\b/.test(q)) return false;
  if (/\bahora\s+los\s+de\b/.test(q) || /\bpero\s+en\b/.test(q)) {
    return true;
  }
  return (
    /^(y\s+)?cuantos\s+hay\b/.test(q) ||
    /^cuantos\s+hay\??$/.test(q) ||
    /^cuantos\??$/.test(q) ||
    /\bcual\s+fue\s+el\s+mas\s+reciente\b/.test(q) ||
    /\bcual\s+es\s+el\s+ultimo\b/.test(q) ||
    /\bcual\s+fue\s+el\s+primero\b/.test(q) ||
    /\bcual\s+fue\s+el\s+ultimo\b/.test(q) ||
    /\bcuales\s+estan\s+pagados\b/.test(q) ||
    /\bcuanto\s+suman\b/.test(q) ||
    ...
    /\ben\s+que\s+etapa\b/.test(q) ||
    /\bque\s+estatus\b/.test(q) ||
    /\bque\s+fecha\s+tienen\b/.test(q) ||
    /\bde\s+cuando\s+son\b/.test(q) ||
    /\bsiguen\s+abiertos\b/.test(q) ||
    /\balguno\s+pendiente\b/.test(q) ||
    /\balguno\s+cancelado\b/.test(q) ||
    /\bmayor\s+importe\b/.test(q) ||
    /\bmas\s+barato\b/.test(q) ||
    /\bmenor\s+importe\b/.test(q) ||
    /\bdame\s+el\s+detalle\b/.test(q) ||
    /\bquiero\s+ver\s+esos\b/.test(q) ||
    /\bdesglosamelos\b/.test(q) ||
    /\bcuantos\s+estan\s+pagados\b/.test(q) ||
    /\bcuales\s+estan\s+pendientes\b/.test(q) ||
    /\bcuantos\s+siguen\s+pendientes\b/.test(q) ||
    /\ben\s+que\s+mes\s+fue\s+el\s+ultimo\b/.test(q) ||
    /\bultimo\s+que\s+pagamos\b/.test(q) ||
    /\bcuales\s+se\s+pagaron\b/.test(q) ||
    /\by\s+en\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre)\b/.test(q) ||
    /\bcuanto\s+suman\s+las\s+pagadas\b/.test(q) ||
    /\bcuantas\s+fueron\s+de\s+gastos\b/.test(q) ||
    /\babre\s+el\s+(primero|segundo|tercero|ultimo|penultimo)\b/.test(q) ||
    /\by\s+el\s+(segundo|tercero|primero)\b/.test(q) ||
    /\bregresa\s+al\s+primero\b/.test(q)
  );
}

function isFolioSearchAggregateFollowUp(question) {
  const q = normalizeText(question);
  if (!q) return false;
  if (/\bcuantos\s+fueron\b/.test(q)) return false;
  if (/^y\s+solo\b/.test(q)) return false;
  if (!/\bpor\s+mes\b/.test(q)) return false;
  return (
    /\bsumarlos\b/.test(q) ||
    /\bsumalos\b/.test(q) ||
    /\bsuma\s+(los|esos|el)\b/.test(q) ||
    /\bpuedes\s+sumar/.test(q) ||
    /\bdame\s+(el|un)\s+total\b/.test(q) ||
    /\bel\s+total\s+por\s+mes\b/.test(q)
  );
}

function sanitizeActiveSubtopic(raw) {
  if (raw == null || raw === "") return null;
  const s = String(raw).trim().toLowerCase();
  if (!/^[a-z][a-z0-9_]{0,31}(\.[a-z][a-z0-9_]{0,31}){0,3}$/.test(s)) return null;
  return s;
}

function sanitizeCycleMode(raw) {
  return raw === "PRE_CLOSE" ? "PRE_CLOSE" : null;
}

function sanitizePortfolioScope(raw) {
  return raw === "PORTFOLIO" || raw === "ONE_PLANT" ? raw : null;
}
...
function sanitizeActiveEntities(raw) {
  ...
  if (kind === "folio_result_set") {
    const items = Array.isArray(first.items)
      ? first.items
          .map((row) => ({
            numero_folio: String(row.numero_folio || row.display || "").trim(),
            folio_id: row.folio_id != null && Number.isFinite(Number(row.folio_id)) ? Number(row.folio_id) : null,
          }))
          .filter((row) => row.numero_folio)
          .slice(0, 40)
      : [];
    if (!items.length) return [];
    return [
      {
        kind: "folio_result_set",
        display: String(first.display || items.map((row) => row.numero_folio).join(" | ")).trim(),
        items,
        plant_id: first.plant_id != null && Number(first.plant_id) > 0 ? Number(first.plant_id) : null,
      },
    ];
  }
  if (kind === "client_movement") {
    const ranked = Array.isArray(first.ranked_names)
      ? first.ranked_names.map((name) => String(name).trim()).filter(Boolean).slice(0, 20)
      : [];
    const ids = Array.isArray(first.result_set_ids)
      ? first.result_set_ids.map((name) => String(name).trim()).filter(Boolean).slice(0, 80)
      : Array.isArray(first.result_set)
        ? first.result_set.map((name) => String(name).trim()).filter(Boolean).slice(0, 80)
        : ranked;
    const displayNames = ids.length ? ids : ranked;
    const movement = first.movement || first.result_set_family || null;
    return [
      {
        kind: "client_movement",
        display: String(first.display || displayNames.slice(0, 3).join(" | ") || movement || "client_movement").trim(),
        movement,
        metric: first.metric || null,
        limit: Number(first.limit) > 0 ? Number(first.limit) : 10,
        period: first.period && /^\d{4}-\d{2}$/.test(String(first.period)) ? String(first.period) : first.result_set_period || null,
        channel: first.channel || "ALL",
        ranked_names: ranked.length ? ranked : ids.slice(0, 20),
        result_set: ids,
        result_set_family: first.result_set_family || movement || null,
        result_set_period:
          first.result_set_period && /^\d{4}-\d{2}$/.test(String(first.result_set_period))
            ? String(first.result_set_period)
            : first.period && /^\d{4}-\d{2}$/.test(String(first.period))
              ? String(first.period)
              : null,
        result_set_ids: ids,
        result_set_operation: first.result_set_operation || first.pending_operation || null,
        want_aggregate: Boolean(first.want_aggregate),
        want_discount: Boolean(first.want_discount),
        want_count: Boolean(first.want_count),
        pending_operation: first.pending_operation || first.result_set_operation || null,
        pending_family: first.pending_family || null,
        pending_intent: first.pending_intent || "client_movement",
      },
    ];
  }
    ...
    return [
      {
        kind: "seh_entity",
        display,
        key: first.key != null && String(first.key).trim() !== "" ? String(first.key).trim() : "",
        scope: first.scope ? String(first.scope).trim() : null,
        metric: first.metric ? String(first.metric).trim() : null,
        plant_label: first.plant_label ? String(first.plant_label).trim() : null,
        plant_id: first.plant_id != null && Number(first.plant_id) > 0 ? Number(first.plant_id) : null,
        focus_status: first.focus_status ? String(first.focus_status).trim() : null,
      },
    ];
  }
  if (kind === "channel_sales" || kind === "client_comments" || kind === "action_register" || kind === "bitacora_topic") {
    return [
      {
        kind,
        display: String(first.display || first.topic || first.channel || kind).trim(),
        ...
        limit: Number(first.limit) > 0 ? Number(first.limit) : null,
        movement: first.movement || null,
        operation: first.operation || null,
        result_set: Array.isArray(first.result_set) ? first.result_set.slice(0, 80) : [],
        ranked_names: Array.isArray(first.ranked_names) ? first.ranked_names.slice(0, 20) : [],
      },
    ];
  }
  if (kind === "CLIENT" || kind === "client" || kind === "predictive_commercial") {
    const ranked = Array.isArray(first.ranked_names)
      ? first.ranked_names.map((name) => String(name).trim()).filter(Boolean).slice(0, 20)
      : [];
    const canonical = String(first.canonical_name || first.display || ranked[0] || "").trim();
    if (!canonical && kind !== "predictive_commercial") return [];
    return [
      {
        kind: kind === "predictive_commercial" ? "predictive_commercial" : "CLIENT",
        display: canonical || String(first.family || "predictive_commercial"),
        canonical_name: canonical || null,
        ranked_names: ranked.length ? ranked : canonical ? [canonical] : [],
        family: first.family || null,
      },
    ];
  }
  if (kind === "FOLIO_KEYWORD") {
    const ids = Array.isArray(first.folio_ids)
      ? first.folio_ids.map((id) => (id == null ? "" : String(id))).filter(Boolean).slice(0, 80)
      : [];
    return [
      {
        kind: "FOLIO_KEYWORD",
        display: String(first.display || first.keyword || "folios").trim(),
        keyword: first.keyword || null,
        folio_ids: ids,
        period_start: first.period_start || null,
        period_end: first.period_end || null,
        operation: first.operation || "LIST",
        result_set_family: "FOLIO_KEYWORD",
      },
    ];
  }
  if (kind === "client_ranking") {
    return [
      {
        kind: "client_ranking",
        display,
        key: first.key ? String(first.key).trim() : "",
        ranking_direction: first.ranking_direction === "BOTTOM" ? "BOTTOM" : "TOP",
        limit: Number(first.limit) > 0 ? Number(first.limit) : 5,
        customer_segment:
          first.customer_segment === "CASA" || first.customer_segment === "COMISIONISTA"
            ? first.customer_segment
            : "ALL",
        metric: first.metric === "DISCOUNT" ? "DISCOUNT" : "VENTA_TON",
        plant_label: first.plant_label ? String(first.plant_label).trim() : null,
        plant_id: first.plant_id != null && Number(first.plant_id) > 0 ? Number(first.plant_id) : null,
        period: first.period && /^\d{4}-\d{2}$/.test(String(first.period)) ? String(first.period) : null,
        ranked_names: Array.isArray(first.ranked_names)
          ? first.ranked_names.map((name) => String(name).trim()).filter(Boolean).slice(0, 20)
          : [],
      },
    ];
  ...
  if (kind === "entity_set") {
    const names = Array.isArray(first.names)
      ? first.names.map((name) => String(name).trim()).filter(Boolean).slice(0, 20)
      : [];
...
function sanitizeEchoedState(echoed, requestPlantaId) {
  ...
  return {
    ...
    cycle_mode: plantMismatch ? null : sanitizeCycleMode(echoed.cycle_mode),
    portfolio_scope: plantMismatch ? null : sanitizePortfolioScope(echoed.portfolio_scope),
    previous_frame: plantMismatch ? null : sanitizePreviousFrame(echoed.previous_frame, requestPlant),
    forecast_run: plantMismatch ? null : sanitizeForecastRun(echoed.forecast_run),
    forecast_magnitudes: plantMismatch
      ? null
      : sanitizeForecastMagnitudes(echoed.forecast_magnitudes, echoed.forecast_run),
    active_subtopic: plantMismatch || !parent ? null : sanitizeActiveSubtopic(echoed.active_subtopic),
    folio_search_spec:
      plantMismatch || parent !== "folio_search"
        ? null
        : sanitizeFolioSearchSpec(echoed.folio_search_spec, requestPlant),
    plant_mismatch: plantMismatch,
    echoed_planta_id: Number.isFinite(echoedPlant) ? echoedPlant : null,
    pending_ui_action: plantMismatch ? null : sanitizePendingUiActionLocal(echoed.pending_ui_action),
  };
}

function sanitizePendingUiActionLocal(raw) {
  try {
    return require("./director-ia-category-commission-007").sanitizePendingUiAction(raw);
  } catch (_e) {
    return null;
  }
}
...
function snapshotCurrentFrame(state) {
  ...
  return {
    ...
    active_subtopic: sanitizeActiveSubtopic(state.active_subtopic),
    folio_search_spec: sanitizeFolioSearchSpec(state.folio_search_spec, plantaId),
    pending_ui_action: sanitizePendingUiActionLocal(state.pending_ui_action),
  };
}
...
function isProfitabilitySnapshotParent(intent) {
  return intent === "profitability_deterioro_snapshot";
}

function isProfitabilityExpenseFollowUp(raw) {
...
function preserveFramesOnClarify(incoming, plantaId) {
  const incomingPlant = Number(incoming && incoming.planta_id);
  const requestPlant = Number(plantaId);
  const crossPlant =
    Number.isFinite(incomingPlant) && Number.isFinite(requestPlant) && incomingPlant !== requestPlant;
  const current = crossPlant ? null : snapshotCurrentFrame(incoming);
  const parked = crossPlant
    ? null
  ...
  return buildConversationState({
    ...
    forecast_run: sanitizeForecastRun(incoming && incoming.forecast_run),
    forecast_magnitudes: sanitizeForecastMagnitudes(
      incoming && incoming.forecast_magnitudes,
      incoming && incoming.forecast_run
    ),
    active_subtopic: current ? current.active_subtopic : null,
    folio_search_spec: current ? current.folio_search_spec : null,
  });
}
...
const LEADING_Y_REST_RE = /^\s*¿?\s*[yY]\s+(.+)$/;
const LEADING_Y_NAME_TOKEN_RE = /^[A-ZÁÉÍÓÚÑ][\wÁÉÍÓÚÑáéíóúñ.'’-]*$/;

function extractLeadingYHintCandidates(question) {
  const raw = String(question || "").trim();
  if (!raw) return null;
  const matched = raw.match(LEADING_Y_REST_RE);
  if (!matched || !matched[1]) return null;
  const nameTokens = [];
  for (const part of String(matched[1])
    .trim()
    .split(/\s+/)
    .filter(Boolean)) {
    const tok = part.replace(/[?¿!.]+$/g, "");
    if (!tok || !LEADING_Y_NAME_TOKEN_RE.test(tok)) break;
    if (isNonIdentityToken(tok) || isPronounToken(tok)) break;
    nameTokens.push(tok);
  }
  if (!nameTokens.length) return null;
  return {
    conversational: nameTokens[0],
    canonical: `Y ${nameTokens.join(" ")}`,
    name_token_count: nameTokens.length,
    requires_canonical_evidence: nameTokens.length >= 2,
  };
}

function leadingYHintList(parsed, fallbackHint) {
  const out = [];
  const seen = new Set();
  for (const hint of [parsed && parsed.canonical, parsed && parsed.conversational, fallbackHint]) {
...
function entityRecordFromCandidate(c, display, dn) {
  ...
  return {
    ...
    identity: keys[0] || `name:${dn}`,
  };
}

function resolveUniqueEntityFromHints(hints, candidates, opts) {
  const list = leadingYHintList(null, null);
  const seenHint = new Set();
  for (const hint of hints || []) {
    const t = String(hint || "").trim();
    const key = normalizeText(t);
    if (!t || !key || seenHint.has(key)) continue;
    seenHint.add(key);
    list.push(t);
  }
  if (!list.length) return { status: "none" };
  const requireCanonical = Boolean(opts && opts.requires_canonical_evidence);
  if (!requireCanonical) {
    const conversational = list[list.length - 1];
    const leadingYForm = normalizeText(`Y ${conversational}`);
    const withoutLeadingYHomonym = (candidates || []).filter((c) => {
      const display = String(
        (c && (c.display || c.cliente_nombre || c.nombre_canonico || c.cliente_norm)) || ""
      ).trim();
      return normalizeText(display) !== leadingYForm;
    });
    return resolveUniqueEntity(conversational, withoutLeadingYHomonym);
  }
  ...
  for (const hint of ordered) {
    ...
    for (const c of candidates || []) {
      const display = String((c && (c.display || c.cliente_nombre || c.nombre_canonico || c.cliente_norm)) || "").trim();
      const dn = normalizeText(display);
      if (!dn || dn !== h) continue;
      const rec = entityRecordFromCandidate(c, display, dn);
      if (seenKeys.has(rec.identity)) continue;
      seenKeys.add(rec.identity);
      hits.push(rec);
    }
    if (hits.length === 1) return { status: "unique", entity: hits[0], hint };
    if (hits.length > 1) return { status: "ambiguous", matches: hits, hint };
  }
  if (requireCanonical) return { status: "none" };
  return resolveUniqueEntity(list[list.length - 1], candidates);
}
...
function resolveConversationTurn(opts) {
  ...
  const profileFollowUp =
    profileParent &&
    !isCommercialTrendQuestion(question) &&
    (isDailyFollowUpKind(kind) || isolatedUnknownEarly || isClientProfileFollowUp(question, kind, { hasActiveClient: true }));
  ...
  const profitabilityParent = isProfitabilitySnapshotParent(parent_intent);
  const profitabilityExpenseFollowUp =
    profitabilityParent &&
    !switchInvalidates &&
    typeof isProfitabilityExpenseFollowUp === "function" &&
    isProfitabilityExpenseFollowUp(question);
  const standalone = isStandaloneDetected(detected) && !dailyFollowUp && !trendFollowUp && !profileHandoffFromTrend && !profileFollowUp && !tallerMayorFollowUp && !preMeetingFollowUp && !monthCloseHandoffFromPreMeeting && !monthCloseFollowUp && !profileHandoffFromMonthClose && !profitabilityExpenseFollowUp;
  ...
  const leadingY = extractLeadingYHintCandidates(question);
  const embedded = extractEmbeddedClientHintCandidates(question);
  const entity_hint_candidates = leadingYHintList(leadingY, currentHint);
  for (const span of (embedded && embedded.spans) || []) {
    const key = normalizeText(span);
    if (!key || entity_hint_candidates.some((h) => normalizeText(h) === key)) continue;
    entity_hint_candidates.push(span);
  }
  const leading_y_requires_canonical = Boolean(leadingY && leadingY.requires_canonical_evidence);
  const embedded_client_requires_canonical = Boolean(
    embedded &&
      embedded.requires_canonical_evidence &&
      (!currentHint ||
        normalizeText(currentHint) !== normalizeText(embedded.longest) ||
        (embedded.name_token_count || 0) >= 2)
  );
  if (embedded && embedded.longest) {
    const longKey = normalizeText(embedded.longest);
    for (let i = entity_hint_candidates.length - 1; i >= 0; i -= 1) {
      const key = normalizeText(entity_hint_candidates[i]);
      if (key && key !== longKey && (longKey.startsWith(`${key} `) || longKey.endsWith(` ${key}`))) {
        entity_hint_candidates.splice(i, 1);
      }
    }
  }
  if (
  ...
  if (embedded && embedded.longest) {
    if (!entity_hint || normalizeText(embedded.longest).length > normalizeText(entity_hint).length) {
      entity_hint = embedded.longest;
    }
  }
...
function buildConversationState(opts) {
  ...
  return {
    ...
    cycle_mode: sanitizeCycleMode(opts && opts.cycle_mode),
    portfolio_scope: sanitizePortfolioScope(opts && opts.portfolio_scope),
    previous_frame: sanitizePreviousFrame(opts && opts.previous_frame, plantaId),
    forecast_run: sanitizeForecastRun(opts && opts.forecast_run),
    forecast_magnitudes: sanitizeForecastMagnitudes(opts && opts.forecast_magnitudes, opts && opts.forecast_run),
    active_subtopic: parent_intent ? sanitizeActiveSubtopic(opts && opts.active_subtopic) : null,
    folio_search_spec:
      parent_intent === "folio_search"
        ? sanitizeFolioSearchSpec(opts && opts.folio_search_spec, plantaId)
        : null,
    pending_ui_action: sanitizePendingUiActionLocal(opts && opts.pending_ui_action),
  };
}
...
function formatConversationHiloForModel(state) {
  ...
  const lines = [
    ...
    `active_subtopic=${s.active_subtopic || "ninguno"}`,
    s.folio_search_spec
      ? `folio_search_spec.scope=${s.folio_search_spec.scope} period=${
          s.folio_search_spec.period_mode === "RANGE"
            ? `${s.folio_search_spec.period_start}..${s.folio_search_spec.period_end}`
            : s.folio_search_spec.period_month || "ninguno"
        } concept=${
          s.folio_search_spec.concept_query ||
          (s.folio_search_spec.concept_alternatives || []).join("|") ||
          "ninguno"
        } operation=${s.folio_search_spec.operation}`
      : "folio_search_spec=ninguno",
    `meeting_type=${s.meeting_type || "ninguno"}`,
    `cycle_mode=${s.cycle_mode || "ninguno"}`,
    `portfolio_scope=${s.portfolio_scope || "ninguno"}`,
    s.forecast_run && s.forecast_run.effective_cutoff_date
      ? `forecast_run.effective_cutoff_date=${s.forecast_run.effective_cutoff_date} origin=${s.forecast_run.cutoff_origin || "—"} corte_day=${s.forecast_run.corte_day || "—"}`
      : "forecast_run=ninguno",
    s.forecast_magnitudes && s.forecast_magnitudes.run_key
      ? `forecast_magnitudes.bound_to=${s.forecast_magnitudes.run_key} semantics=FORECAST (eco del pack; no son fuente)`
      : "forecast_magnitudes=ninguno",
