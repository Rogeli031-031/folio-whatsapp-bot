...
function resolveConversationTurn(opts) {
  ...
  const standalone = isStandaloneDetected(detected) && !dailyFollowUp && !trendFollowUp && !profileHandoffFromTrend && !profileFollowUp && !tallerMayorFollowUp;
  ...
  if (restorePrevious || stayOnCurrent) {
    ...
    validInheritContext &&
    !standalone &&
    (isolatedUnknown || dailyFollowUp || trendFollowUp || profileFollowUp || profileHandoffFromTrend || tallerMayorFollowUp)
  ) {
    inherit = true;
  }
