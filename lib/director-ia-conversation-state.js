...
function resolveConversationTurn(opts) {
  ...
  const trendFollowUp = trendParent && (isDailyFollowUpKind(kind) || isolatedUnknownEarly);
  const standalone = isStandaloneDetected(detected) && !dailyFollowUp && !trendFollowUp;
  ...
  if (restorePrevious || stayOnCurrent) {
    inherit = true;
  } else if (validInheritContext && !standalone && (isolatedUnknown || dailyFollowUp || trendFollowUp)) {
    inherit = true;
  }
