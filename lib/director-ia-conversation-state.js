...
const FOLIO_SEARCH_CONCEPT_MODES = Object.freeze(["SINGLE", "ANY", "NONE"]);
...
function sanitizeFolioSearchSpec(raw, requestPlantaId) {
  ...
  } else if (concept_mode === "NONE") {
    // conjunto reproducible por periodo/estatus, sin concepto
  } else if (!concept_query) {
    return null;
  }
  const status_filter = raw.status_filter === "PAGADO" ? "PAGADO" : raw.status_filter ? String(raw.status_filter).trim() : null;
  const category_filter = raw.category_filter ? String(raw.category_filter).trim().slice(0, 40) : null;
  const beneficiary_filter = raw.beneficiary_filter ? String(raw.beneficiary_filter).trim().slice(0, 80) : null;
  ...
  return {
    ...
    concept_mode,
    concept_query: concept_mode === "SINGLE" ? concept_query : null,
    concept_alternatives: concept_mode === "ANY" ? alternatives : [],
    operation,
    status_filter: status_filter || null,
    category_filter: category_filter || null,
    beneficiary_filter: beneficiary_filter || null,
    result_count: Number.isFinite(Number(raw.result_count)) ? Number(raw.result_count) : null,
  };
}
...
function isFolioResultSetFollowUp(question) {
  const q = normalizeText(question);
  if (!q) return false;
  if (/\b(existe|existen)\b/.test(q) && /\b(folios?|apoyos?)\b/.test(q) && !/\bahora\b/.test(q)) return false;
  if (/\bgastamos\b/.test(q) || /\bgasto\b/.test(q)) return false;
  return Boolean(
    /\bahora\s+los\s+de\b/.test(q) ||
      /\bpero\s+en\b/.test(q) ||
      /^(y\s+)?cuantos\s+hay\b/.test(q) ||
      /^cuantos\??$/.test(q) ||
      /\bcual\s+es\s+la\s+suma\b/.test(q) ||
      /\bsuma\s+total\b/.test(q) ||
      /\bcuanto\s+suman\b/.test(q) ||
      /\bcual\s+fue\s+el\s+de\s+mayor\s+importe\b/.test(q) ||
      /\bcual\s+fue\s+el\s+mayor\b/.test(q) ||
      /\bcual\s+fue\s+el\s+menor\b/.test(q) ||
      /\bmayor\s+importe\b/.test(q) ||
      /\bmenor\s+importe\b/.test(q) ||
      /\bcuantos\s+son\s+de\s+taller\b/.test(q) ||
      /\bquienes\s+son\s+los\s+beneficiarios\b/.test(q) ||
      /\bbeneficiarios\b/.test(q) ||
      ...
      /\bcuantos\s+son\s+de\b/.test(q) ||
      /\bfiltra\b/.test(q) ||
      /\bagrupa\s+por\b/.test(q) ||
      /\bconjunto\b/.test(q) ||
      /\bencontramos\b/.test(q) ||
      /\bmas\s+antiguo\b/.test(q)
  );
}

function isFolioExistenceFollowUp(question) {
  return isFolioResultSetFollowUp(question);
}
...
function sanitizeActiveEntities(raw) {
  ...
  if (kind === "client_ranking" || kind === "client_entity_set") {
    const names = Array.isArray(first.entity_names)
      ? first.entity_names.map((n) => String(n).trim()).filter(Boolean).slice(0, 20)
      : Array.isArray(first.ranked_names)
        ? first.ranked_names.map((n) => String(n).trim()).filter(Boolean).slice(0, 20)
        : Array.isArray(first.entities)
          ? first.entities.map((e) => String((e && e.display) || "").trim()).filter(Boolean).slice(0, 20)
          : [];
    return [
      {
        kind,
        ...
        customer_segment:
          first.customer_segment === "CASA" ||
          first.customer_segment === "COMISIONISTA" ||
          first.segment === "CASA" ||
          first.segment === "COMISIONISTA"
            ? first.customer_segment || first.segment
            : first.customer_segment === "DUAL" || first.segment === "DUAL"
              ? "DUAL"
              : "ALL",
        metric: first.metric === "DESCUENTO_POR_KG" ? "DESCUENTO_POR_KG" : first.metric || "VENTA_TON",
        plant_label: first.plant_label ? String(first.plant_label).trim() : null,
        plant_id: first.plant_id != null && Number(first.plant_id) > 0 ? Number(first.plant_id) : null,
        period: first.period && /^\d{4}-\d{2}$/.test(String(first.period)) ? String(first.period) : null,
        previous_period:
          first.previous_period && /^\d{4}-\d{2}$/.test(String(first.previous_period))
            ? String(first.previous_period)
            : null,
        movement_type: first.movement_type ? String(first.movement_type).trim() : null,
        domain: first.domain ? String(first.domain).trim() : kind,
        entity_names: names,
        ranked_names: names,
        entities: names.map((name) => ({ display: name, key: name })),
      },
    ];
  }
