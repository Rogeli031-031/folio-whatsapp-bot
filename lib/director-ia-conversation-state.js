...
function resolveConversationTurn(opts) {
  ...
  const standalone = isStandaloneDetected(detected) && !dailyFollowUp && !trendFollowUp && !profileHandoffFromTrend && !profileFollowUp && !tallerMayorFollowUp && !preMeetingFollowUp;
  ...
  if (restorePrevious || stayOnCurrent) {
    ...
    validInheritContext &&
    !standalone &&
    (isolatedUnknown || dailyFollowUp || trendFollowUp || profileFollowUp || profileHandoffFromTrend || tallerMayorFollowUp || preMeetingFollowUp)
  ) {
    inherit = true;
  }
