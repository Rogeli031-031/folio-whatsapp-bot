...
function isClientMovementQuestion(question) {
  ...
  if (/\bdescuento/.test(n) && !/\b(dejaron|dejo|disminuy|aumentaron)\b/.test(n)) return false;
...
function extractClientMovementSpec(question, prior) {
  const n = normalize(question);
  if (!isClientMovementQuestion(question) && !(prior && prior.movement)) return { ok: false };
  let movement = (prior && prior.movement) || "DISMINUYERON";
  if (
    ...
    /\bclientes?\s+perdidos?\b/.test(n) ||
    /\bvolumen\s+perdido\b/.test(n) ||
    /\bperdimos\b/.test(n) && /\bclientes?\b/.test(n)
  ) {
    ...
    movement = "INACTIVOS";
  } else if (/\bdisminuy/.test(n) || /\bbajaron\b/.test(n) || /\bmovimiento\s+a\s+la\s+baja\b/.test(n)) {
