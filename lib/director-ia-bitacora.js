...
async function createBitacoraEntry(client, req, body) {
  ...
  const empresa = body.empresa != null ? String(body.empresa).trim().slice(0, 120) : null;
...
async function listBitacoraEntries(client, req, query) {
  ...
  params.push(limit);
  sql += ` ORDER BY fecha DESC, created_at DESC LIMIT $${params.length}`;
...
async function getBitacoraEntry(client, req, id) {
  ...
  const r = await client.query(
    `SELECT * FROM arr.director_ia_bitacora WHERE id = $1 AND is_active = true`,
    [entryId]
  );
