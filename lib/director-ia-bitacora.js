...
const RESUMEN_IA_MAX_CHARS = 4000;
const CHAT_CONTENIDO_MAX_CHARS = 25000;
...
/**
 * Normaliza texto para búsqueda en bitácora (sin acentos).
 * @param {string} raw
 */
function normalizeBitacoraSearchText(raw) {
  return String(raw || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/\s+/g, " ")
...
/**
 * Resumen extractivo: intro + líneas con clientes/volumen (Plaud suele mencionar clientes después del min 3).
...
function buildExtractiveResumen(contenido) {
  ...
  for (const line of lines) {
    if (introLen + line.length > 900) break;
    intro.push(line);
    introLen += line.length + 1;
  }

  const clientSignals =
    /\b(toneladas?|\d+\s*t\b|compr[oó]|consigui[oó]|proveedor|credito|credito|ca[ií]da|baj[oó]|estaciones?|comisionista|clausur)\b/i;
  const clientLines = [];
  const seen = new Set();
  for (const line of lines) {
    if (!clientSignals.test(line)) continue;
    const key = normalizeBitacoraSearchText(line).slice(0, 100);
  ...
  if (clientLines.length > 0) {
    out += `\n\n--- Clientes y volumen (extracto) ---\n${clientLines.slice(0, 30).join("\n")}`;
  }
  if (out.length > RESUMEN_IA_MAX_CHARS) {
    return `${out.slice(0, RESUMEN_IA_MAX_CHARS).trim()}…`;
  }
...
/**
 * Extrae líneas de la transcripción donde aparece un cliente buscado.
 * @param {string} contenido
 * @param {string[]} searchTexts
 * @param {number} [maxChars]
 */
function extractBitacoraExcerptForSearch(contenido, searchTexts, maxChars = 1400) {
  const texts = (searchTexts || []).filter(Boolean);
  if (!texts.length || !contenido) return null;

  const lines = String(contenido).split(/\n/).map((l) => l.trim()).filter(Boolean);
  const matchedIdx = new Set();

  for (let i = 0; i < lines.length; i++) {
    const norm = normalizeBitacoraSearchText(lines[i]);
    const hit = texts.some((tok) => {
      const t = normalizeBitacoraSearchText(tok);
...
async function loadBitacoraForChat(client, planta_id, limit = CHAT_CONTEXT_LIMIT) {
  ...
  const r = await client.query(
    `WITH latest AS (
     ...
     SELECT b.fecha, b.tipo, b.titulo, b.resumen_ia, b.contenido, b.created_at, p.nombre AS planta_nombre
     FROM arr.director_ia_bitacora b
     JOIN public.plantas p ON p.id = b.planta_id
     CROSS JOIN latest l
     WHERE b.planta_id = $1 AND b.is_active = true
  ...
  return (r.rows || []).map((row) => {
    const contenidoRaw = row.contenido != null ? String(row.contenido) : "";
    const contenido =
      contenidoRaw.length > CHAT_CONTENIDO_MAX_CHARS
        ? `${contenidoRaw.slice(0, CHAT_CONTENIDO_MAX_CHARS)}…`
        : contenidoRaw;
    return {
      fecha: row.fecha instanceof Date ? row.fecha.toISOString().slice(0, 10) : String(row.fecha || "").slice(0, 10),
      tipo: String(row.tipo || ""),
