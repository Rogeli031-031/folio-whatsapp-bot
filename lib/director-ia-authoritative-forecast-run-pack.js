...
function emptyActual(ventaTon, runIdentity = {}, flags = {}) {
  const venta = finiteOrNull(ventaTon);
  const cutoff = (runIdentity && (runIdentity.corte_day || runIdentity.upload_day)) || flags.cutoff_date || null;
  const lastClosedDay = deriveLastClosedDay(runIdentity && runIdentity.year, runIdentity && runIdentity.month, cutoff);
  const noCutoff = Boolean(flags.no_cutoff) || !cutoff;
  const period_start_status = classifyActualToDateKind(venta, lastClosedDay, {
    source_error: Boolean(flags.source_error),
    no_cutoff: noCutoff,
  });
  return {
    venta,
...
function buildAuthoritativeForecastRunPack(input) {
  ...
  if (src.source_error) {
    return {
      ...
      actual_to_date: emptyActual(null, run_identity, { source_error: true, no_cutoff: !uploadDay }),
      forecast_stored,
      target_commitment,
      provenance: {
  ...
  if (!uploadDay) {
    return {
      ...
      actual_to_date: emptyActual(null, run_identity, { no_cutoff: true }),
      forecast_stored,
      target_commitment,
      provenance: {
        source: "computeIgfForecastMiniPayload",
  ...
  return {
    ...
    actual_to_date: emptyActual(actualFromInput.venta_ton, run_identity, {
      cutoff_date: actualFromInput.cutoff_date,
    }),
    forecast_stored,
    target_commitment,
    provenance: {
