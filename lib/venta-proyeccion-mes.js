...
async function fetchKgSumByDowWindow(client, plantCode, startStr, endStr) {
  const r = await client.query(
    `SELECT fecha::date AS fecha, SUM(kg) AS kg
...
async function fetchVentasCanalSubRows(client, plantCode, startStr, endStr) {
  const r = await client.query(
    `SELECT fecha::date AS fecha, canal, COALESCE(subcanal, '') AS subcanal, SUM(kg) AS kg
     FROM arr.ventas_diarias_cliente
     WHERE plant_code = $1 AND fecha >= $2::date AND fecha <= $3::date
     GROUP BY fecha::date, canal, COALESCE(subcanal, '')`,
    [plantCode, startStr, endStr]
  );
  return r.rows || [];
}
...
async function fetchDescuentosCanalSubRows(client, plantCode, startStr, endStr, catYear, catMonth) {
  const r = await client.query(
    `SELECT d.fecha::date AS fecha, COALESCE(c.canal, 'Casa') AS canal, COALESCE(c.subcanal, '') AS subcanal, SUM(d.monto) AS monto
     FROM arr.descuentos_diarios_cliente d
     LEFT JOIN arr.cliente_categoria_mes c ON c.plant_code = d.plant_code AND c.year = $4 AND c.month = $5 AND c.cliente_norm = d.cliente_norm
     WHERE d.plant_code = $1 AND d.fecha >= $2::date AND d.fecha <= $3::date
