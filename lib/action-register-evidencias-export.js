/**
 * Hoja(s) EVIDENCIAS del Action Register (fotos embebidas comprimidas).
 * — Una hoja por mes (export ARR): buildActionRegisterEvidenciasWorkbook
 * — Una hoja por planta y rango de fechas (IGF Forecast): buildActionRegisterEvidenciasMultiWorkbook
 */
...
function monthToInclusiveRange(year, month) {
...
function safeExcelSheetName(name, wb) {
  let base = String(name || "Planta")
    .replace(/[\\/*?:\[\]]/g, " ")
    .trim()
    .slice(0, 31);
  if (!base) base = "Planta";
...
function setupEvidenciasSheetHeaders(wsE) {
  wsE.columns = [
...
function isYmdInRange(ymd, dateFrom, dateTo) {
  if (!ymd || !/^\d{4}-\d{2}-\d{2}$/.test(ymd)) return false;
  return ymd >= dateFrom && ymd <= dateTo;
}

/**
 * Añade una hoja de evidencias al libro (mismo layout que export ARR).
 * @param {import("exceljs").Workbook} wb
 * @param {import("pg").PoolClient} client
 */
async function appendActionRegisterEvidenciasSheet(wb, client, opts) {
  const {
    planta_id,
    dateFrom,
    dateTo,
  ...
  const wsName = safeExcelSheetName(sheetName, wb);
  const wsE = wb.addWorksheet(wsName);
  setupEvidenciasSheetHeaders(wsE);
      ...
      const dAttRes = await client.query(
        `SELECT a.id, a.dicf_accion_id, a.file_name, a.content_type, a.s3_key, a.s3_url, a.data
         ...
         WHERE da.planta_id = ANY($1::int[])
           AND (da.created_at AT TIME ZONE 'America/Mexico_City')::date >= $2::date
           AND (da.created_at AT TIME ZONE 'America/Mexico_City')::date <= $3::date
  ...
  if (revisions.length > 0 && dicfAccionesRows.length > 0) {
    ...
    for (const a of dicfAccionesRows) {
      ...
      if (!isYmdInRange(revYmd, dateFrom, dateTo)) continue;
...
/**
 * Una hoja EVIDENCIAS para un mes calendario (export ARR — sin cambios de contrato).
 */
async function buildActionRegisterEvidenciasWorkbook(client, opts) {
  const { year, month, sheetName = "EVIDENCIAS", ExcelJS, ...deps } = opts;
  const { dateFrom, dateTo } = monthToInclusiveRange(year, month);
  const wb = new ExcelJS.Workbook();
  wb.creator = "Folio WhatsApp Bot";
  wb.created = new Date();
  await appendActionRegisterEvidenciasSheet(wb, client, {
...
/**
 * Un libro con una hoja por planta (Zona Provincia) en un rango de fechas inclusivo.
 */
async function buildActionRegisterEvidenciasMultiWorkbook(client, opts) {
  const { dateFrom, dateTo, plantas, ExcelJS, ...deps } = opts;
  const wb = new ExcelJS.Workbook();
  wb.creator = "Folio WhatsApp Bot";
  wb.created = new Date();
...
module.exports = {
  ...
  appendActionRegisterEvidenciasSheet,
  buildActionRegisterEvidenciasWorkbook,
  buildActionRegisterEvidenciasMultiWorkbook,
};
