...
function classifyFolioExistenceFollowUp(question) {
  const q = normalizeQuestion(question);
  if (!q) return null;
  if (/^(y\s+)?cuantos\s+hay\b/.test(q) || /^cuantos\s+hay\b/.test(q)) return "COUNT";
  if (/\bcual\s+fue\s+el\s+mas\s+reciente\b/.test(q)) return "LATEST";
  if (/\bcuales\s+estan\s+pagados\b/.test(q)) return "PAGADO";
...
async function loadFolioSearchForChat(pool, plantaId, req, opts = {}) {
  ...
  } else if (existenceFollowUp === "COUNT") {
    filters.analysis_mode = "EXISTENCE";
    filters.existence = true;
  } else if (existenceFollowUp === "LATEST" || existenceFollowUp === "PAGADO") {
    filters.analysis_mode = existenceFollowUp;
    filters.existence = true;
  } else if (filters.period_mode === "ANY" && !filters.analysis_mode) {
    filters.analysis_mode = "EXISTENCE";
    filters.existence = true;
  }
  const shaped = {
  ...
  async function run(client) {
    ...
    if (shaped.analysis_mode === "LATEST") {
      deduped = deduped
        .slice()
        .sort((a, b) => {
          const cmp = String(b.mes_cargo || "").localeCompare(String(a.mes_cargo || ""));
          if (cmp) return cmp;
...
function buildFolioSearchAnswer(payload) {
  ...
  if (filters.existence || filters.period_mode === "ANY") {
    ...
    if (filters.analysis_mode === "LATEST") {
      if (!payload.records || !payload.records.length) return `No encontré folios relacionados con ${term}.`;
      const row = payload.records[0];
      return `El más reciente relacionado con ${term} es ${row.numero_folio || row.folio_id} (${row.periodo || "mes_cargo no registrado"}).`;
    }
