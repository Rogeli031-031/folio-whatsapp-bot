...
const REQUEST_WRAPPER_RE = /^(?:dame(?:\s+los)?|muestrame|muestra|mostrar|listar|lista)\s+/;
...
function boundConceptSpan(text, start, end, periodSpan) {
  ...
  return shrinkEstanPeriodBridge(text, leftover, periodSpan);
}
...
function extractAnalyticModelFromControl(control) {
  ...
  const cumulative = /\bacumulado\s+por\s+mes\b/.test(text) || /\bcuanto\s+llevamos\s+acumulado\b/.test(text);
  return {
...
function extractConceptModel(q) {
  ...
  const bounded = stripLeadingBoundaryConnector(framed);
  if (!bounded) {
    return { concept_mode: "SINGLE", concept_query: null, concept_alternatives: [] };
  }
  const parts = bounded.split(" o ").map((p) => p.trim()).filter(Boolean);
...
function rowMatchesConcept(row, conceptQuery, alternatives) {
  const phrases =
  ...
  return phrases.some((phrase) => phraseMatchesRow(row, phrase));
}
...
async function loadFolioSearchForChat(pool, plantaId, req, opts = {}) {
  ...
  async function run(client) {
    ...
    const matched = scoped.filter((row) =>
      rowMatchesConcept(row, shaped.concept_query, shaped.concept_alternatives)
    );
