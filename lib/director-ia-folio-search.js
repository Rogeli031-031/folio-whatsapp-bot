...
function hasPaidStatusSemantics(n) {
  ...
  return Boolean(
    /\bpagad/.test(q) ||
      ...
      (/\b(liquidad[oa]s?|liquidaron|liquidamos|se\s+liquido)\b/.test(q) && !/\bliquidaciones?\b/.test(q)) ||
      /\bcubier/.test(q) ||
...
const RANGE_SPAN_RES = [
  ...
  new RegExp(`\\bacumulado\\s+${MONTH_POINT_SRC}-${MONTH_POINT_SRC}\\b`),
  new RegExp(`\\bdel\\s+1\\s+de\\s+${MONTH_POINT_SRC}\\s+al\\s+\\d{1,2}\\s+de\\s+${MONTH_POINT_SRC}\\b`),
];
...
function looksLikeFolioEnumerativeRequest(q) {
  const n = normalizeQuestion(q);
  if (!n) return false;
  return Boolean(
    /\b(lista|enlista|listado|listar)\b/.test(n) ||
      /\b(dame|muestrame|ensename|muestra)\s+(los\s+)?(folios?|apoyos?|listado)\b/.test(n) ||
      /\b(que|cuales)\s+(son\s+los\s+)?(folios?|apoyos?)\b/.test(n) ||
      /\b(que|cuales)\s+(folios?|apoyos?)\b/.test(n)
  );
}
...
function looksLikeFolioExistence(q) {
  ...
  if (looksLikeFolioEnumerativeRequest(n)) return false;
  return Boolean(
    /\bexiste/.test(n) ||
      /\bexisten\b/.test(n) ||
      /\bhay\b/.test(n) ||
...
function extractFolioSearchFilters(question, opts = {}) {
  ...
  const plantHint = extractNamedPlant(question);
  if (
    concept.concept_query &&
    plantHint &&
    normalizeQuestion(concept.concept_query) === normalizeQuestion(plantHint)
  ) {
    const plantNorm = normalizeQuestion(plantHint).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const withoutPlant = subject
      .replace(new RegExp(`\\b(?:de|en|para)\\s+${plantNorm}\\b`, "g"), " ")
      .replace(/\s+/g, " ")
      .trim();
    const retrySpan = locateConceptSpan(withoutPlant, periodSpan);
    if (retrySpan && retrySpan.raw) {
      concept = conceptModelFromProtectedSpan(retrySpan.raw);
    } else {
      concept = extractConceptModel(range ? range.remainder : withoutPlant);
    }
    if (
      concept.concept_query &&
      normalizeQuestion(concept.concept_query) === normalizeQuestion(plantHint)
  ...
  const existence =
    ...
    !countByConcept &&
    !looksLikeFolioEnumerativeRequest(q);
  return {
    scope,
...
async function loadFolioSearchForChat(pool, plantaId, req, opts = {}) {
  ...
  if (shaped.period_mode !== "ANY" && (!fetchMonths || !fetchMonths.length)) {
    ...
    return {
      ok: true,
      clarification: `¿De qué mes o rango de meses quieres los folios de ${term}?`,
      error: `¿De qué mes o rango de meses quieres los folios de ${term}?`,
      question,
  ...
  async function run(client) {
    ...
    for (const mesCargo of fetchMonths) {
      let part;
      if (typeof opts.queryPublicFolios === "function") {
        try {
          part = await opts.queryPublicFolios(client, Number(plantaId), mesCargo, {
            resolveEquivalentIds: opts.resolveEquivalentIds,
          });
        } catch (e) {
      ...
      } else if (Array.isArray(opts.folioItems)) {
        part = opts.folioItems.filter((row) => String((row && (row.mes_cargo || row.periodo)) || "") === String(mesCargo));
      } else {
        try {
...
function buildFolioSearchAnswer(payload) {
  if (payload && payload.clarification) return payload.clarification;
  ...
  return {
    ...
    context_meta: {
      ...
      conversation_state: (() => {
        ...
        if (payload && payload.pending_period) {
          return stateLib.buildConversationState({
            ...
            pending_information_gap: {
              ...
              frame: {
                concept_query: payload.filters && payload.filters.concept_query,
                concept_mode: payload.filters && payload.filters.concept_mode,
                scope: payload.filters && payload.filters.scope,
                operation: payload.filters && payload.filters.operation || "concept_sequence",
                analysis_mode: (payload.filters && payload.filters.analysis_mode) || "LIST",
                plant_id: planta_id,
              },
              original_question: payload.question || null,
              why_blocks: "Falta periodo seguro. No invento ALL TIME.",
            },
          });
        }
        ...
        return stateLib.buildConversationState({
          ...
          active_entities: records.length
            ? [
                {
                  kind: "folio_result_set",
                  display: records
                    .map((row) => row.numero_folio)
                    .filter(Boolean)
                    .slice(0, 8)
                    .join(" | "),
                  items: records.slice(0, 40),
                  plant_id: planta_id,
                },
              ]
            : [],
