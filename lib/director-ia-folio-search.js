...
function buildFolioSearchSpecFromFilters(filters, plantaId) {
  ...
  const hasConcept = Boolean(filters.concept_query || (filters.concept_alternatives && filters.concept_alternatives.length));
  const concept_mode =
    filters.concept_mode === "ANY"
      ? "ANY"
      : hasConcept
        ? filters.concept_mode || "SINGLE"
        : "NONE";
  return conversationStateLib().sanitizeFolioSearchSpec(
    {
      ...
      beneficiary_filter: filters.beneficiary_filter || null,
...
function filtersFromFolioSearchSpec(spec) {
  ...
  return {
    ...
    status_filter: spec.status_filter || null,
    category_filter: spec.category_filter || null,
    beneficiary_filter: spec.beneficiary_filter || null,
  };
}
...
function classifyFolioExistenceFollowUp(question) {
  ...
  if (/\bmayor\s+importe\b/.test(q) || /\bpago\s+mas\s+alto\b/.test(q) || /\bcual\s+fue\s+el\s+de\s+mayor\b/.test(q) || /\bcual\s+fue\s+el\s+mayor\b/.test(q)) {
    return "HIGHEST_AMOUNT";
  }
  if (/\bmas\s+barato\b/.test(q) || /\bmenor\s+importe\b/.test(q) || /\bcual\s+fue\s+el\s+menor\b/.test(q)) {
    return "LOWEST_AMOUNT";
  }
  if (/\bcuanto\s+suman\b/.test(q) || /\bcual\s+es\s+la\s+suma\b/.test(q) || /\bsuma\s+total\b/.test(q)) return "SUM";
  if (/\bcuantos\s+son\s+de\s+taller\b/.test(q) || /\bde\s+taller\b/.test(q) && /\bcuantos\b/.test(q)) return "FILTER_CATEGORY";
  if (/\bquienes\s+son\s+los\s+beneficiarios\b/.test(q) || /\bbeneficiarios\b/.test(q)) return "GROUP_BY_BENEFICIARY";
...
function extractFolioSearchFilters(question, opts = {}) {
  ...
  const existence = !period_month && hasConcept && looksLikeFolioExistence(q);
  return {
    scope,
...
async function loadFolioSearchForChat(pool, plantaId, req, opts = {}) {
  ...
  if (existenceFollowUp === "SHIFT_PERIOD") {
  ...
  } else if (existenceFollowUp === "FILTER_CATEGORY") {
    filters = { ...filters, analysis_mode: "FILTER_CATEGORY", category_filter: "TALLER" };
  } else if (existenceFollowUp === "GROUP_BY_BENEFICIARY") {
    filters = { ...filters, analysis_mode: "GROUP_BY_BENEFICIARY" };
    ...
    existenceFollowUp === "FILTER_CATEGORY" ||
    existenceFollowUp === "GROUP_BY_BENEFICIARY"
  ) {
  ...
  async function run(client) {
    ...
    if (shaped.category_filter || shaped.analysis_mode === "FILTER_CATEGORY") {
      const wanted = String(shaped.category_filter || "TALLER").toUpperCase();
      deduped = deduped.filter((row) => {
        const cat = String((row && row.categoria) || "").toUpperCase();
        return cat.includes(wanted) || supportFamilyOf(row.categoria) === wanted;
      });
    }
    if (shaped.beneficiary_filter) {
      const wanted = String(shaped.beneficiary_filter).toLowerCase();
      deduped = deduped.filter((row) => String((row && row.beneficiario) || "").toLowerCase().includes(wanted));
    }
    ...
    const plantaNombre = sanitizePlantScopeLabel((deduped[0] && (deduped[0].planta_nombre || null)) || null);
    if (shaped.analysis_mode === "GROUP_BY_BENEFICIARY") {
      const groups = new Map();
      for (const row of deduped) {
        const name = String((row && row.beneficiario) || "").trim() || "beneficiario no registrado";
        const prev = groups.get(name) || { beneficiario: name, count: 0, importe: 0 };
        prev.count += 1;
        prev.importe += Number(row.importe || 0) || 0;
      ...
      return {
        ...
        truncated: false,
        records: grouped.map((g) => ({
          numero_folio: `${g.count} folio(s)`,
          beneficiario: g.beneficiario,
          importe: g.importe,
          concepto: "beneficiario",
          categoria: null,
          estatus: null,
          periodo: shaped.period_month,
        })),
...
function buildFolioSearchAnswer(payload) {
  ...
  if (filters.existence || filters.period_mode === "ANY") {
    ...
    if (filters.analysis_mode === "FILTER_CATEGORY") {
      if (!payload.count) return `No encontré folios de ${filters.category_filter || "esa categoría"} en el conjunto actual.`;
      return `Encontré ${payload.count} folio(s) de ${filters.category_filter || "esa categoría"} en el conjunto actual.`;
    }
    if (filters.analysis_mode === "GROUP_BY_BENEFICIARY") {
      const rows = payload.records || [];
      if (!rows.length) return `No encontré beneficiarios en el conjunto actual.`;
      return `Beneficiarios del conjunto actual:\n${rows
        .map((row, i) => `${i + 1}. ${row.beneficiario} — ${row.numero_folio}; importe ${formatImporteRegistrado(row.importe)}`)
        .join("\n")}`;
    }
  ...
  const { sanitizePlantScopeLabel } = require("./director-ia-display-sanitize");
  const scope = sanitizePlantScopeLabel(payload.planta_nombre) || "la planta consultada";
  const bits = [];
  if (filters.period_mode === "RANGE" && filters.period_start && filters.period_end) {
  ...
  if (filters.analysis_mode === "FILTER_CATEGORY") {
    if (!payload.count) return `No encontré folios de ${filters.category_filter || "esa categoría"} en el conjunto actual.${filterBit}`;
    return `Encontré ${payload.count} folio(s) de ${filters.category_filter || "esa categoría"} en el conjunto actual.${filterBit}`;
  }
  if (filters.analysis_mode === "GROUP_BY_BENEFICIARY") {
    const grouped = payload.records || [];
    if (!grouped.length) return `No encontré beneficiarios en el conjunto actual.${filterBit}`;
    return `Beneficiarios del conjunto actual:${filterBit}\n${grouped
      .map((row, i) => `${i + 1}. ${row.beneficiario} — ${row.numero_folio}; importe ${formatImporteRegistrado(row.importe)}`)
      .join("\n")}`;
  }
  if (filters.analysis_mode === "HIGHEST_AMOUNT" || filters.analysis_mode === "LOWEST_AMOUNT") {
    const row = (payload.records || [])[0];
    if (!row) return `No encontré folios con esos filtros.${filterBit}`;
    const label = filters.analysis_mode === "LOWEST_AMOUNT" ? "menor importe" : "mayor importe";
    return `El folio de ${label} del conjunto actual es ${row.numero_folio || row.folio_id} (${formatImporteRegistrado(row.importe)}).${filterBit}`;
  }
  if (filters.analysis_mode === "AGGREGATE") {
    if (matchCount === 0) {
...
function buildFolioSearchChatResult(payload, opts = {}) {
  ...
  return {
    ...
    context_meta: {
      ...
      conversation_state: (() => {
        const spec = okPayload
          ? buildFolioSearchSpecFromFilters(
              { ...(payload.filters || {}), result_count: payload.count },
              planta_id
            )
          : null;
