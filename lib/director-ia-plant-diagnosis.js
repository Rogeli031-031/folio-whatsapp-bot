...
const MATERIALITY_TOP_N = 5;
const MAGNITUDE_FIELD = "kg_mes_real";
const MAGNITUDE_UNIT = "kg";
...
const PLANT_DIAGNOSIS_SYSTEM_ADDENDUM = [
  ...
  "MATERIALIDAD COMERCIAL (kg homogéneos): usa SOLO kg_mes_real del periodo declarado. kg_mes_forecast es proyección a cierre de mes, NO venta perdida. No restes forecast-real. No mezcles kg con MXN ni con días vencidos. No inventes score.",
  "Concentración: cita denominador, periodo y participación. Explica DÓNDE está la magnitud, no POR QUÉ ocurrió.",
  "Cobertura DICF: solo por cliente_key. Sin acción DICF asociada no prueba negligencia ni que nadie trabaje el caso.",
  "Responsable de una acción DICF no es responsable de la caída comercial. Puedes pedir actualización de ESA acción a la persona registrada.",
  "Si no hay explicación en la evidencia: dilo y sugiere validar el motivo. No inventes causa ni quién debe responder.",
  "Un comentario que mencione competencia es declaración almacenada, no prueba causal.",
  "No recomiendes recuperar por volumen. No autorizes descuento. No asignes responsable. No es Recommendation N5 ni MAT_*.",
].join(" ");
...
function assemblePlantDiagnosisEvidence(input) {
  ...
  const commercial_materiality =
    commercial_state.payload && commercial_state.payload.commercial_materiality
      ? commercial_state.payload.commercial_materiality
      : null;
  return {
    ok: true,
    abort: false,
...
function commercialRowIdentity(row) {
  return [
    normalizeKeyPart(row && row.cliente_norm),
    normalizeKeyPart(row && row.canal),
    normalizeKeyPart(row && row.subcanal),
  ].join("|");
}

function classifyCommercialMovement(row) {
  if (isTruthyFlag(row && row.es_recuperable)) return "dejaron";
  const key = normalizeEstadoKey(row && row.estado);
  if (key === "dejaron") return "dejaron";
  if (key === "disminuyeron") return "disminuyeron";
  return null;
}

function deriveClienteKeysForRow(plantaId, row) {
  const canon = getCanonicalPlantaId(plantaId);
  if (!Number.isFinite(canon) || canon <= 0) return [];
  const groups = new Set(DICF_GRUPO_LABELS);
  if (row && row.estado) groups.add(String(row.estado).trim());
  const keys = [];
  for (const grupo of groups) {
    keys.push(
      buildClienteKey(canon, grupo, (row && row.canal) || "", (row && row.subcanal) || "", (row && row.cliente_norm) || "")
    );
  }
  return [...new Set(keys.filter(Boolean))];
}

function observedKgForMovement(row, priorRow, category, currentPeriod, priorPeriod) {
  if (category === "dejaron") {
    if (!priorRow) {
      return {
        kg: null,
        ...
        unit: MAGNITUDE_UNIT,
        missing_reason: "prior_month_row_absent",
      };
    }
    const kg = finiteKg(priorRow.kg_mes_real);
    return {
      kg,
      period: priorPeriod,
      field: MAGNITUDE_FIELD,
      unit: MAGNITUDE_UNIT,
      missing_reason: kg == null ? "prior_month_kg_mes_real_null" : null,
    };
  }
  const kg = finiteKg(row && row.kg_mes_real);
  return {
    kg,
    period: currentPeriod,
    field: MAGNITUDE_FIELD,
    unit: MAGNITUDE_UNIT,
    missing_reason: kg == null ? "current_month_kg_mes_real_null" : null,
  };
}

function compareMaterialClients(a, b) {
  if (b.observed_magnitude_kg !== a.observed_magnitude_kg) {
    return b.observed_magnitude_kg - a.observed_magnitude_kg;
  }
  const name = String(a.cliente_display || "").localeCompare(String(b.cliente_display || ""), "es");
  if (name) return name;
  const canal = String(a.canal || "").localeCompare(String(b.canal || ""), "es");
  if (canal) return canal;
  return String(a.subcanal || "").localeCompare(String(b.subcanal || ""), "es");
}

function buildCommercialMateriality(input) {
  const currentRows = Array.isArray(input && input.currentRows) ? input.currentRows : [];
  const priorRows = Array.isArray(input && input.priorRows) ? input.priorRows : [];
  const year = input && input.year;
  const month = input && input.month;
  const topN =
    input && Number.isFinite(Number(input.topN)) && Number(input.topN) > 0
      ? Math.floor(Number(input.topN))
      : MATERIALITY_TOP_N;
  const plantaId = input && input.plantaId;
  const currentPeriod = toYyyyMm(year, month);
  const prev = previousYearMonth(year, month);
  const priorPeriod = prev ? toYyyyMm(prev.year, prev.month) : null;
  const priorById = new Map();
  for (const row of priorRows) {
    priorById.set(commercialRowIdentity(row), row);
  }

  const groups = { dejaron: [], disminuyeron: [] };
  for (const row of currentRows) {
    const category = classifyCommercialMovement(row);
    if (!category || !groups[category]) continue;
    const identity = commercialRowIdentity(row);
    const observed = observedKgForMovement(row, priorById.get(identity), category, currentPeriod, priorPeriod);
    groups[category].push({
      identity,
      category,
      cliente_display: row.cliente_norm != null ? String(row.cliente_norm) : null,
      ...
      es_recuperable: isTruthyFlag(row.es_recuperable),
      cliente_keys: deriveClienteKeysForRow(plantaId, row),
      observed_magnitude_kg: observed.kg,
      magnitude_field: observed.field,
      unit: observed.unit,
      period: observed.period,
      missing_reason: observed.missing_reason,
      kg_mes_forecast_not_used: finiteKg(row.kg_mes_forecast),
      provenance: CS_SOURCE,
    });
  }

  const categories = [];
  for (const category of ["dejaron", "disminuyeron"]) {
    const all = groups[category];
    const rankable = all.filter((c) => c.observed_magnitude_kg != null);
    const unknown = all.filter((c) => c.observed_magnitude_kg == null);
    const denominator_kg = rankable.reduce((s, c) => s + c.observed_magnitude_kg, 0);
    const sorted = rankable.slice().sort(compareMaterialClients);
    const top = sorted.slice(0, topN).map((c) => {
      const share =
        denominator_kg !== 0 ? c.observed_magnitude_kg / denominator_kg : null;
      return {
        ...c,
        share_of_observed_magnitude: share,
      };
    });
    const topNKg = top.reduce((s, c) => s + c.observed_magnitude_kg, 0);
    const period = category === "dejaron" ? priorPeriod : currentPeriod;
    categories.push({
      category,
      period,
      ...
      denominator_definition:
        category === "dejaron"
          ? "suma de kg_mes_real finitos del mes previo (misma identidad cliente_norm+canal+subcanal) para filas vigentes con es_recuperable o estado dejaron"
          : "suma de kg_mes_real finitos del mes vigente para filas con estado disminuyeron",
      client_count_in_category: all.length,
      client_count_ranked: rankable.length,
      client_count_magnitude_unknown: unknown.length,
      top_n: topN,
      top_n_share: denominator_kg !== 0 ? topNKg / denominator_kg : null,
      top_clients: top,
      magnitude_unknown_clients: unknown.map((c) => ({
        cliente_display: c.cliente_display,
        missing_reason: c.missing_reason,
        period: c.period,
      })),
      forecast_not_used_as_loss: true,
    });
  }
  ...
  return {
    ...
    plant_code: (input && input.plantCode) || null,
    provenance: CS_SOURCE,
    categories,
    note: "kg_mes_real es kg observado del mes de la fila. kg_mes_forecast no se usa como venta perdida.",
  };
}

function isDicfActionClosed(action) {
  if (action && action.cerrado_at) return true;
  return String((action && action.estado) || "").trim().toLowerCase() === "hecho";
}

function isDicfActionOverdue(action, todayYmd) {
  if (isDicfActionClosed(action)) return false;
  const ymd = ymdFromDate(action && action.fecha_compromiso);
  if (!ymd || !todayYmd) return false;
  return ymd < String(todayYmd);
}

function coverageLabel(hasAction, openCount, overdueCount) {
  if (hasAction == null) return "coverage_unknown";
  if (!hasAction) return "material_without_action";
  if (overdueCount > 0) return "material_with_overdue_action";
  if (openCount > 0) return "material_with_open_action";
  return "material_with_action";
}

function applyDicfCoverageToMateriality(materiality, actions, opts = {}) {
  if (!materiality || !materiality.categories) return materiality;
  const todayYmd = opts.todayYmd || todayYmdMexicoCity();
  const byKey = new Map();
  for (const action of actions || []) {
    const k = action && action.cliente_key != null ? String(action.cliente_key).trim() : "";
    if (!k) continue;
    if (!byKey.has(k)) byKey.set(k, []);
    byKey.get(k).push(action);
  }
  const categories = materiality.categories.map((cat) => ({
    ...cat,
    top_clients: (cat.top_clients || []).map((client) => {
      const keys = Array.isArray(client.cliente_keys) ? client.cliente_keys : [];
      if (!keys.length || opts.plantaId == null || !Number.isFinite(Number(opts.plantaId))) {
        return {
          ...client,
          coverage_status: "coverage_unknown",
          has_dicf_action: null,
          open_action_count: null,
          overdue_action_count: null,
          latest_action: null,
          review_reasons: ["observed_kg_concentration", "coverage_unknown"],
        };
      }
      const matched = [];
      const seen = new Set();
      for (const k of keys) {
        for (const action of byKey.get(k) || []) {
          const id = action.id != null ? Number(action.id) : action.public_code;
          if (seen.has(id)) continue;
          seen.add(id);
          matched.push(action);
        }
      }
      const open = matched.filter((a) => !isDicfActionClosed(a));
      const overdue = open.filter((a) => isDicfActionOverdue(a, todayYmd));
      const latest = matched.slice().sort((a, b) => {
        const ta = String(a.created_at || "");
        const tb = String(b.created_at || "");
        return tb.localeCompare(ta);
      })[0];
      const hasAction = matched.length > 0;
      const status = coverageLabel(hasAction, open.length, overdue.length);
      const reasons = ["observed_kg_concentration"];
      if (status === "material_without_action") reasons.push("no_dicf_action");
      if (status === "material_with_overdue_action") reasons.push("dicf_action_overdue");
      if (status === "coverage_unknown") reasons.push("coverage_unknown");
      return {
        ...client,
        coverage_status: status,
        has_dicf_action: hasAction,
        open_action_count: open.length,
        overdue_action_count: overdue.length,
        matched_cliente_keys: [...new Set(matched.map((a) => String(a.cliente_key)))],
        latest_action: latest
          ? {
              public_code: latest.public_code != null ? String(latest.public_code) : null,
              estado: latest.estado != null ? String(latest.estado) : null,
              fecha_compromiso: ymdFromDate(latest.fecha_compromiso),
              cerrado_at: latest.cerrado_at ? ymdFromDate(latest.cerrado_at) : null,
              responsable: latest.responsable != null && String(latest.responsable).trim() !== ""
                ? String(latest.responsable).trim()
                : null,
              resultado_cierre:
                latest.resultado_cierre != null && String(latest.resultado_cierre).trim() !== ""
                  ? String(latest.resultado_cierre).trim()
                  : null,
            }
          : null,
  ...
  return { ...materiality, categories, coverage_join: "cliente_key", coverage_name_join: false };
}

async function loadDicfActionsByKeys(client, plantaId, keys) {
  const uniq = [...new Set((keys || []).map((k) => String(k || "").trim()).filter(Boolean))];
  if (!uniq.length || !client) return [];
  const equiv = getPlantaIdsEquivalentes(plantaId);
  if (!equiv.length) return [];
  const r = await client.query(
    `SELECT a.id, a.public_code, a.cliente_key, a.cliente_nombre, a.estado,
...
async function defaultLoadCommercialStateSelect(client, plantaId) {
  ...
  if (prev) {
    const prior = await client.query(
      `SELECT plant_code, year, month, cliente_norm, canal, subcanal, kg_mes_real
         FROM arr.dicf_cliente_mes
        WHERE UPPER(TRIM(plant_code)) = UPPER(TRIM($1))
          AND year = $2 AND month = $3`,
      [plantCode, prev.year, prev.month]
    );
    priorRows = prior.rows || [];
  }
  let commercial_materiality = buildCommercialMateriality({
    currentRows: rows,
    priorRows,
    year,
    month,
    plantCode,
    plantaId,
    topN: MATERIALITY_TOP_N,
  });
  const allKeys = [];
  for (const cat of commercial_materiality.categories || []) {
    for (const c of cat.top_clients || []) {
      for (const k of c.cliente_keys || []) allKeys.push(k);
    }
  }
  try {
    const actions = await loadDicfActionsByKeys(client, plantaId, allKeys);
    commercial_materiality = applyDicfCoverageToMateriality(commercial_materiality, actions, {
      plantaId,
      todayYmd: todayYmdMexicoCity(),
    });
  } catch (e) {
    commercial_materiality = {
      ...commercial_materiality,
      coverage_error: e && e.message ? String(e.message) : "Error cobertura DICF",
    };
  }
  summarized.payload.commercial_materiality = commercial_materiality;
...
function formatCsPayload(block) {
  ...
  const mat = p.commercial_materiality;
  if (mat && mat.enabled) {
    lines.push("", formatCommercialMaterialityLines(mat).join("\n"));
  }
  return lines;
}

function formatCommercialMaterialityLines(mat) {
  const lines = [
    "MATERIALIDAD COMERCIAL (kg homogéneos; no es causa; no es score; no es N5)",
    `campo=${mat.magnitude_field} unidad=${mat.unit} periodo_vigente=${mat.current_period || "—"} periodo_previo=${mat.prior_period || "—"}`,
    "kg_mes_forecast NO se usa como venta perdida. forecast-real NO se calcula.",
    `join cobertura=cliente_key (sin join por nombre). ${mat.note || ""}`,
  ];
  for (const cat of mat.categories || []) {
    lines.push(
      `categoría=${cat.category} periodo_magnitud=${cat.period || "—"} denominador_kg=${
        cat.denominator_kg != null ? cat.denominator_kg : "—"
      } definición="${cat.denominator_definition}" ranked=${cat.client_count_ranked}/${cat.client_count_in_category} unknown_kg=${cat.client_count_magnitude_unknown} top_n_share=${
        cat.top_n_share != null ? cat.top_n_share : "—"
      }`
    );
    for (const c of cat.top_clients || []) {
      const share = c.share_of_observed_magnitude != null ? c.share_of_observed_magnitude : "—";
      const keys = (c.cliente_keys || []).join(",") || "—";
      const latest = c.latest_action;
      const resp =
        latest && latest.responsable
          ? `responsable_accion=${latest.responsable}`
          : "responsable_accion=no registrado";
      lines.push(
        `- ${c.cliente_display || "—"} | kg=${c.observed_magnitude_kg} ${c.unit} periodo=${c.period || "—"} share=${share} coverage=${c.coverage_status || "—"} has_dicf=${
          c.has_dicf_action == null ? "unknown" : c.has_dicf_action
        } open=${c.open_action_count == null ? "—" : c.open_action_count} overdue=${
          c.overdue_action_count == null ? "—" : c.overdue_action_count
        } ${resp} keys=${keys} reasons=${(c.review_reasons || []).join(",")}`
      );
      if (latest) {
        lines.push(
          `  acción ${latest.public_code || "—"} estado=${latest.estado || "—"} compromiso=${latest.fecha_compromiso || "—"} resultado_cierre=${
            latest.resultado_cierre || "null"
          }`
        );
      }
    }
    for (const u of cat.magnitude_unknown_clients || []) {
      lines.push(`- ${u.cliente_display || "—"} | kg=null (no es 0) reason=${u.missing_reason || "—"} periodo=${u.period || "—"}`);
    }
  }
  return lines;
}
...
function buildPlantDiagnosisPrompt(assembled, question) {
  ...
  const userContent = [
    ...
    String(question || ""),
    "",
    "Si hay MATERIALIDAD COMERCIAL, señala primero los clientes que concentran kg observados y su cobertura DICF (con/sin acción, vencida si aplica).",
    "Explica magnitud y cobertura como razones separadas. No inventes causa. Si falta explicación, dilo y sugiere validar el motivo.",
    "Si una acción tiene responsable registrado, puedes pedir actualización de ESA acción; no lo culpes del movimiento comercial.",
    "Resume el resto de bloques. Declara limitaciones, SOURCE_RESTRICTED y period mismatch si existen.",
  ].join("\n");
