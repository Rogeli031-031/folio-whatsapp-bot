...
function resultSetNames(state) {
  const ents = (state && state.active_entities) || [];
  const names = [];
  const seen = new Set();
  for (const ent of ents) {
    if (!ent || ent.kind === "CLIENT" || ent.kind === "client") continue;
    const bag = []
      .concat(ent.result_set || [])
      .concat(ent.result_set_ids || [])
      .concat(ent.ranked_names || []);
...
function extractExplicitClientName(question, prior, rows) {
  const n = nq(question);
  if (!n) return null;
  const catalog = []
    .concat(Array.isArray(rows) ? rows : [])
    .concat(((prior && prior.catalog_rows) || []).map((name) => ({ cliente: name })))
    .concat(resultSetNames(prior).map((name) => ({ cliente: name })));
  const catalogHits = [];
  const seen = new Set();
...
function extractClientHint(question, prior, rows) {
  const explicit = extractExplicitClientName(question, prior, rows);
...
function activeEntitiesForInactiveList(list, inheritedClient) {
  const names = [];
  const seen = new Set();
  for (const row of list || []) {
    const name = String((row && row.cliente) || "").trim();
    const key = nclient(name);
    if (!name || !key || seen.has(key)) continue;
    seen.add(key);
    names.push(name);
    ...
    return [
      {
        ...
        display: "clientes inactivos",
        movement: "INACTIVE_CLIENTS",
        result_set_family: "INACTIVE_CLIENTS",
        ranked_names: names.slice(0, 20),
        result_set: names.slice(0, 80),
        result_set_ids: names.slice(0, 80),
      },
    ];
  }
  if (inheritedClient && inheritedClient.canonical_name) {
    return [
      {
        kind: "CLIENT",
        display: inheritedClient.canonical_name,
        canonical_name: inheritedClient.canonical_name,
      },
    ];
  }
  return [];
...
function preserveResultSetEntities(state) {
  const names = resultSetNames(state);
...
function resolveOpenClientTarget(question, prior, rows) {
  const catalog = Array.isArray(rows) ? rows : [];
  const explicit = extractExplicitClientName(question, prior, catalog);
  if (explicit) {
    const resolved = resolveClientInCatalog(explicit, catalog.length ? catalog : [{ cliente: explicit }]);
    if (resolved.status === "unique") return { status: "unique", cliente: resolved.cliente, row: resolved.row };
    if (resolved.status === "ambiguous") return resolved;
    if (catalog.length) return { status: "not_found" };
    return { status: "unique", cliente: explicit, row: { cliente: explicit } };
  }
  const active = runtime010.resolveActiveClient({ prior, conversation_state: prior });
  if (active && active.canonical_name) {
    return { status: "unique", cliente: active.canonical_name, row: { cliente: active.canonical_name }, source: "active_entity" };
  }
  const set = resultSetNames(prior);
  if (set.length === 1) {
    return { status: "unique", cliente: set[0], row: { cliente: set[0] }, source: "unique_result_set" };
  }
  if (set.length > 1) {
    return { status: "ambiguous", candidates: set, reason: "multi_result_set" };
  }
  return { status: "missing" };
}

function selectUniqueClientFromPrior(question, prior, rows) {
  if (classify011Family(question, prior)) return null;
  const n = nq(question);
  if (!n) return null;
  const pool = resultSetNames(prior).concat(
    (rows || []).map((row) => String((row && (row.cliente || row.canonical_name)) || "").trim()).filter(Boolean)
  );
  const hits = [];
  const seen = new Set();
  for (const name of pool) {
    ...
    if (n === key || n === `y ${key}` || n === `el cliente ${key}` || n.includes(key)) {
      const leftover = n
        .replace(key, " ")
        .replace(/\b(y|el|la|cliente|de)\b/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      if (!leftover) {
        seen.add(key);
  ...
  const leftover = n.replace(/\b(y|el|la|cliente|de)\b/g, " ").replace(/\s+/g, " ").trim();
  if (
    leftover &&
    leftover.split(" ").length <= 6 &&
    leftover.length >= 3 &&
    !/^(cual|que|como|cuando|cuanto|dame|dime|abre)/.test(leftover) &&
    !runtime010.isNonPlantBusinessToken(leftover)
  ) {
    const resolved = resolveClientInCatalog(leftover, (rows || []).concat(resultSetNames(prior).map((name) => ({ cliente: name }))));
    if (resolved.status === "unique") return resolved.cliente;
  }
  return null;
}
...
function buildOpenClientAnswer(resolution, plantLabel) {
  if (resolution.status === "ambiguous") {
    const names = (resolution.candidates || []).slice(0, 8).join(", ");
    return names
      ? `Hay más de un cliente (${names}). ¿Cuál quieres abrir?`
      : "Hay más de un cliente en la lista. ¿Cuál quieres abrir?";
  }
  if (resolution.status === "not_found" || resolution.status === "missing") {
    return resolution.status === "missing"
      ? "¿Cuál cliente quieres abrir? No selecciono el primero de la lista."
      : "No encontré ese cliente en el catálogo de la planta. No abro otro cliente.";
  }
...
module.exports = {
  ...
  extractClientHint,
  extractExplicitClientName,
  resultSetNames,
  activeEntitiesForInactiveList,
  preserveResultSetEntities,
  resolveOpenClientTarget,
  selectUniqueClientFromPrior,
