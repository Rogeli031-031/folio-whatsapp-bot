...
function createIgfComoCambioToken(payload) {
  return jwt.sign(
    { igfExcel: true, planta: payload.planta, yearOtra: payload.yearOtra, monthOtra: payload.monthOtra, versionOtra: payload.versionOtra },
    JWT_SECRET,
    { expiresIn: IGF_EXCEL_EXPIRES }
  );
}
...
function verifyIgfComoCambioToken(token) {
  ...
  try {
    const decoded = jwt.verify(token.trim(), JWT_SECRET);
    return decoded && decoded.igfExcel === true ? decoded : null;
  } catch (e) {
    return null;
  }
}
