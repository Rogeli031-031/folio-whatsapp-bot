 ...
 * @param {Object} payload - { role, actor_id, plantas_permitidas, default_filters?, permisos?, scope?, actor_nombre?, actor_telefono? }
 *   scope: "seh" = acceso exclusivo a la página/API SEH
 * @returns {string} JWT
 */
function createDashboardToken(payload) {
  ...
  if (payload.scope) body.scope = String(payload.scope);
  if (payload.actor_nombre) body.actor_nombre = String(payload.actor_nombre).slice(0, 150);
  if (payload.actor_telefono) body.actor_telefono = String(payload.actor_telefono).slice(0, 50);
  return jwt.sign(body, JWT_SECRET, { expiresIn: JWT_EXPIRES_IN });
}

/** Token con scope seh: solo /api/seh y /api/dashboard/plantas. */
function isSehOnlyAuth(auth) {
  return !!(auth && (auth.scope === "seh" || auth.seh_only === true));
}
...
function dashboardAuthMiddleware(req, res, next) {
  ...
  if (isSehOnlyAuth(payload)) {
    const full = String(req.originalUrl || req.url || "");
    const pathOnly = full.split("?")[0];
    const allowed =
      pathOnly === "/api/seh" ||
      pathOnly.startsWith("/api/seh/") ||
      pathOnly === "/api/dashboard/plantas";
    if (!allowed) {
      return res.status(403).json({ error: "Este enlace solo permite acceso a SEH." });
    }
  }
  next();
}
