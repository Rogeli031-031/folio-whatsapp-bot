...
function createDashboardToken(payload) {
  ...
  if (payload.permisos && typeof payload.permisos === "object") {
 ...
 * encodeURIComponent no convierte "." y WhatsApp corta el hipervínculo
 * en el primer punto del JWT (header.payload.signature), dejando solo
 * la cabecera clicable → token incompleto → "Acceso no autorizado".
 * %2E se decodifica de nuevo a "." en el navegador / Express.
 * @param {string} token
 * @returns {string}
 */
