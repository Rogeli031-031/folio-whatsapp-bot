...
async function buildActionRegisterBoardPayload(client, planta_id, opts) {
  ...
  const r = await client.query(
    `SELECT e.revision_id, e.position,
            i.id, i.tema, i.parent_id, i.title, i.responsable, i.responsable_usuario_id, i.due_date, i.closed,
            i.created_at,
            to_char((i.created_at AT TIME ZONE 'America/Mexico_City')::date, 'YYYY-MM-DD') AS creada_ymd,
  ...
  for (const row of r.rows || []) {
    ...
    byRevisionTema[rid][tema].push({
      ...
      responsable_usuario_id:
        row.responsable_usuario_id != null && Number.isFinite(Number(row.responsable_usuario_id))
          ? Number(row.responsable_usuario_id)
          : null,
      due_date: row.due_date,
      closed: row.closed === true,
      created_at: row.created_at || null,
      creada_ymd: row.creada_ymd || pgCalendarDateToYmd(row.created_at) || "",
      ...
      if (equivPlantas.length > 0) {
        ...
        for (const a of dicfRows.rows || []) {
          ...
          existing.push({
            ...
            responsable_usuario_id:
              a.responsable_usuario_id != null && Number.isFinite(Number(a.responsable_usuario_id))
                ? Number(a.responsable_usuario_id)
                : null,
            ...
            creada_ymd: a.creada_ymd || "",
            dicf_creada_ymd: a.creada_ymd || "",
          });
        }
      }
    } catch (e) {
