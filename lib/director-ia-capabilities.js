...
const CAPABILITIES = [
  ...
  },
  {
    ...
    canWrite: false,
    description:
      "Margen $/kg de meses calendario: FINAL cerrado, FORECAST del mes abierto, protección de futuro.",
    limitations:
      "Read-only; closed exige unique FINAL; matcher de planta único; 0 es valor; no latest-as-FINAL; no OpenAI.",
  },
  {
    id: "historical_new_clients",
    ...
    canWrite: false,
    description:
      "Lista completa de clientes nuevos de un mes calendario cerrado: kg reales y descuento/kg real.",
    limitations:
      "Read-only; mes cerrado; sin forecast; ausencia de descuento ≠ 0; no es commercial_trend ni IGF margen.",
  },
  {
  ...
  },
  {
    ...
    canWrite: false,
    description:
      "Listado read-only de public.folios por planta, mes_cargo y concepto libre. Scope ALL_PUBLIC_FOLIOS o SUPPORT_FAMILIES.",
    limitations:
      "Reutiliza la lectura ancha existente. No es estatus individual, IGF-reviewable, M4 ni M6 GASTOS-only. Requiere mes_cargo. No SQL nuevo.",
    relatedReadable: ["folios"],
  },
  {
    id: "inversiones",
  ...
  },
  {
    ...
    canRead: true,
    canWrite: false,
    description: "Lectura del tablero SEH (public.seh_equipos / GET /api/seh) por planta.",
    limitations: "Solo lectura. No edita equipos, fotos ni fechas. No cubre Carpetas Legales ni Regulación.",
    relatedReadable: [],
  },
  {
    id: "client_ranking",
    ...
    canRead: true,
    canWrite: false,
    description: "Ranking TOP/BOTTOM por VENTA_TON o DESCUENTO_POR_KG sobre ARR (CASA / COMISIONISTA / ALL).",
    limitations: "Solo lectura. No inventa toneladas ni descuentos. Periodo y planta deben resolverse con evidencia.",
    relatedReadable: ["arr"],
  },
  {
    id: "client_movement",
    ...
    canRead: true,
    canWrite: false,
    description: "AUMENTO/DISMINUCION/DEJO_DE_COMPRAR/NUEVO/INACTIVO con valores crudos ARR, no forecast.",
    limitations: "No usa redondeo de presentación. INACTIVO exige ventana explícita. No inventa fechas diarias.",
    relatedReadable: ["arr"],
  },
  {
    id: "client_evidence",
    ...
    canRead: true,
    canWrite: false,
    description: "Comentarios, DICF, Action Register y bitácora con source_type preservado.",
    limitations: "No recategoriza fuentes. Comentarios no son causalidad.",
    relatedReadable: ["dicf", "action_register", "bitacora", "cliente_comentarios"],
  },
];
...
function isPreCloseQuery(raw) {
  const q = normalizeQuestion(raw);
  if (!q) return false;
  if (/\bcomo cerramos\b/.test(q) || /\bcomo quedamos\b/.test(q) || /\bcomo quedo\b/.test(q)) return false;
  ...
  if (/\bpeor\b/.test(q) && /\bmeta\b/.test(q)) return true;
  return false;
}
...
function isMonthCloseQuery(raw) {
  ...
  if (
    (/\butil(idad)?\s+oper/.test(q) || /\bresultado\s+final\b/.test(q) || /\bfinancieramente\b/.test(q)) &&
    (/\breal/.test(q) ||
      /\bcierre\b/.test(q) ||
      /\bmes\b/.test(q) ||
      /\b(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/.test(q))
  ) {
    return true;
  }
  if (/\butil(idad)?\b/.test(q) && /\breal/.test(q)) return true;
  if (
    (/\bforecast\b/.test(q) || /\bproyeccion\b/.test(q) || /\bigf\b/.test(q)) &&
    (/\bcierre\b/.test(q) || /\breal\b/.test(q)) &&
    (/\bdiferenc/.test(q) || /\bversus\b/.test(q) || /\bvs\b/.test(q) || /\bcompar/.test(q))
  ) {
    return true;
  }
  return false;
...
function matchesAllowedReadableIntent(q) {
  if (
    /\b(extintores?|estaciones?|pipas?|autotanques?|sistema\s+contra\s+incendio|seguridad\s+e\s+higiene)\b/.test(q)
  ) {
    return true;
  }
  if (isMonthCloseQuery(q)) return true;
  if (isPreCloseQuery(q)) return true;
