 ...
 * @param {string} question
 */
function buildBitacoraAnnex(sessions, question) {
...
function resolveDirectorIaChatRouting(question, chatContext) {
  ...
  const hasBitacoraData = bitacoraQuestion && (chatContext.bitacora || []).length > 0;
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (mejoraContinua) {
    ...
    if (bitacoraQuestion && bitacoraSessions.length > 0) {
      bitacoraAnnex = buildBitacoraAnnex(bitacoraSessions, q);
    }
  ...
  const narrative =
    mejoraContinua ? false : isNarrativeQuestion(q) && !promptOpts.dicfFocused;
