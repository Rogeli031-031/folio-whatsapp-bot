...
- Utiliza únicamente la información del contexto de BITÁCORA IA entregado en este mensaje (campo resumen_ia).
...
- Si no existe evidencia suficiente en la bitácora, responde exactamente:
  "No existe evidencia suficiente en Bitácora IA para responder esa pregunta."
- No menciones bases de datos, JSON, APIs ni estructuras técnicas.
- Responde con fecha, tipo y título de la sesión cuando ayude a ubicar la información.`;

const DIRECTOR_IA_SYSTEM_PROMPT_BITACORA_DICF = `${DIRECTOR_IA_SYSTEM_PROMPT_BITACORA}

Además incluye contexto DICF (clientes / ventas) en el mismo mensaje cuando esté presente.
Para la parte comercial usa resultado_cierre e historial DICF como evidencia validada.
Separa claramente lo observado en campo (Bitácora) de lo registrado en DICF (clientes / cierres).`;
 ...
 * @param {string} question
 */
function filterDicfDetailsByQuestion(details, question) {
 ...
 * @param {string} question
 */
function buildFocusedDicfContext(chatContext, question) {
  const all = chatContext.dicf_details || [];
  const matched = filterDicfDetailsByQuestion(all, question);
  const lines = [
 ...
 * @param {string} question
 */
function filterBitacoraByQuestion(sessions, question) {
  let pool = [...(sessions || [])];
  const qNorm = normalizeDicfSearchText(question);
  ...
  if (/\briesgo(?:s)?\b/i.test(question) && /\bvisita\b|\breuni[oó]n\b/i.test(question)) {
...
function buildFocusedBitacoraContext(sessions, question) {
  ...
  const lines = [
    ...
    "Resúmenes de visitas, reuniones y conversaciones (Plaud). Usar únicamente resumen_ia.",
    "",
    "SESIONES RELEVANTES:",
    "",
  ];
  ...
  if (matched.length === 0) {
    lines.push("(sin bitácoras coincidentes para esta planta)");
  }
...
/**
 * @param {{ text: string, meta: object }} bitacoraPart
 * @param {{ text: string, meta: object }} dicfPart
 */
function combineBitacoraDicfContext(bitacoraPart, dicfPart) {
  return {
    text: [bitacoraPart.text, "", "---", "", dicfPart.text].join("\n"),
    meta: {
      mode: "bitacora_focused",
      focus: "bitacora_ia+dicf",
      combined: true,
      bitacora: bitacoraPart.meta,
      dicf: dicfPart.meta,
    },
  };
}
...
function buildDirectorIaChatPrompt(context, question, opts = {}) {
  ...
  } else if (opts.combinedBitacoraDicf && opts.useFocused && opts.focusedText) {
    promptMode = "bitacora_focused";
    systemPrompt = DIRECTOR_IA_SYSTEM_PROMPT_BITACORA_DICF;
    userContent = [
      "Contexto combinado — Bitácora IA (campo) + DICF (clientes / ventas):",
      opts.focusedText,
      "",
      "Pregunta del ejecutivo:",
      q,
      "",
      "Responde por separado lo de campo (bitácora) y lo comercial (DICF) cuando ambos apliquen.",
    ].join("\n");
  } else if (opts.bitacoraFocused && opts.useFocused && opts.focusedText) {
    promptMode = "bitacora_focused";
    systemPrompt = DIRECTOR_IA_SYSTEM_PROMPT_BITACORA;
    userContent = [
      "Contexto Bitácora IA (conocimiento de campo — solo resumen_ia):",
      opts.focusedText,
      "",
      "Pregunta del ejecutivo:",
      q,
...
function inferSourcesFromChat(context, question, answer, opts = {}) {
  if (opts.promptMode === "bitacora_focused") {
    const sources = ["bitacora_ia.sessions", "bitacora_ia.context"];
    if (opts.combinedDicf) {
      sources.push("action_register.dicf_details", "dicf.historial");
    }
    return sources.sort();
  }

  if (opts.promptMode === "dicf_focused") {
    return ["action_register.dicf_details", "dicf.historial"];
  }
  ...
  if (isDicfContextQuestion(question)) {
    sources.add("action_register.dicf_details");
    sources.add("dicf.historial");
  }
...
function resolveDirectorIaChatRouting(question, chatContext) {
  ...
  const bitacoraQuestion = isBitacoraQuestion(q);
  const wantDicf = dicfQuestion && chatContext.dicf_details?.length > 0;
  const wantBitacora = bitacoraQuestion;

  if (wantBitacora && wantDicf) {
    return {
      promptMode: "bitacora_focused",
      bitacoraFocused: true,
      dicfFocused: true,
      combinedBitacoraDicf: true,
    };
  }
  if (wantDicf) return { promptMode: "dicf_focused", dicfFocused: true };
  if (wantBitacora) return { promptMode: "bitacora_focused", bitacoraFocused: true };
  if (isNarrativeQuestion(q)) return { promptMode: "focused", narrative: true };
  if (isAggregateQuestion(q)) return { promptMode: "full", aggregate: true };
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (mejoraContinua) {
    ...
    if (wantBitacora && wantDicf) {
      const bitacoraPart = buildFocusedBitacoraContext(bitacoraSessions, q);
      const dicfPart = buildFocusedDicfContext(chatContext, q);
      focused = combineBitacoraDicfContext(bitacoraPart, dicfPart);
      promptOpts = {
        useFocused: true,
      ...
      focused = buildFocusedDicfContext(chatContext, q);
      promptOpts = { useFocused: true, focusedText: focused.text, dicfFocused: true };
    } else if (wantBitacora) {
      focused = buildFocusedBitacoraContext(bitacoraSessions, q);
      promptOpts = { useFocused: true, focusedText: focused.text, bitacoraFocused: true };
  ...
  const narrative =
    mejoraContinua ? false : isNarrativeQuestion(q) && !promptOpts.dicfFocused && !promptOpts.bitacoraFocused;
  ...
  const sources = mejoraContinua
    ...
    : inferSourcesFromChat(chatContext, q, answer, {
        promptMode,
        combinedDicf: Boolean(promptOpts.combinedBitacoraDicf),
      });
...
module.exports = {
  ...
  DIRECTOR_IA_SYSTEM_PROMPT_BITACORA_DICF,
