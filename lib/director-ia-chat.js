...
/** Preguntas que deben usar dicf_details (historial + cierre). */
const DICF_CONTEXT_SIGNAL_RE =
  /\b(clientes?|dicf|cierre|cerrad|resultado|historial|concluy|conclusi[oó]n|aprendimos|dejaron\s+de\s+comprar|dej[oó]\s+de\s+comprar|qu[eé]\s+pas[oó]|ca[ií]da|estacional|estructural)\b/i;
...
function buildDirectorIaChatPrompt(context, question, opts = {}) {
  ...
  if (opts.mejoraContinua && opts.useFocused && opts.focusedText) {
    ...
    userContent = [
      "Contexto DICF (clientes — incluye historial y resultados de cierre):",
      opts.focusedText,
      "",
      "Pregunta del ejecutivo:",
      q,
...
function inferSourcesFromChat(context, question, answer, opts = {}) {
  ...
  if (opts.promptMode === "focused" || opts.promptMode === "dicf_focused" || isNarrativeQuestion(question)) {
    sources.add("action_register.tema_details");
  }

  if (opts.promptMode === "dicf_focused" || isDicfContextQuestion(question)) {
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (mejoraContinua) {
    ...
    if (dicfQuestion && chatContext.dicf_details?.length) {
  ...
  const narrative = mejoraContinua ? false : isNarrativeQuestion(q) && !promptOpts.dicfFocused;
