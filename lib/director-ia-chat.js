...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (
    ...
    if (directorIaPlan.intent === "open_client_delta_forecast") {
      ...
      const resolution = runtime011.resolveClientInCatalog(hint, rows.length ? rows : hint ? [{ cliente: hint }] : []);
      const uniqueFallback =
        resolution.status === "not_found" && hint && !rows.length
          ? { status: "unique", cliente: String(hint).trim(), row: { cliente: hint } }
          : resolution;
      const finalRes = rows.length ? resolution : uniqueFallback;
          ...
          conversation_state: conversationStateForIntent(
            {
              ...
              active_entities: list.slice(0, 1).map((r) => ({
                kind: "CLIENT",
                display: r.cliente,
                canonical_name: r.cliente,
              })),
            },
            echoedState,
            "client_inactivity",
