...
const { computeDeltaIngresoForecast } = require("./delta-ingreso-forecast");
const {
...
async function loadDeltaIngresoForecastNegativeTopN(pool, plantaId, req, opts = {}) {
  ...
  try {
    ...
    const compute = chatDeps.computeDeltaIngresoForecast || computeDeltaIngresoForecast;
    const getMargen = chatDeps.getMargenKgPorPeriodo || getMargenKgPorPeriodo;
    const data = await compute(client, plantCode, yearA, monthA, yearB, monthB, getMargen, plantaNombre);
    ...
    const rows = shown.map((r) => {
      ...
      const latest = pickLatestComment(commentsMap && commentsMap.get && commentsMap.get(key));
      ...
      return {
        ...
        comment: body ? { body, created_at } : null,
        comment_missing: !body,
      };
    });
...
function buildDeltaIngresoForecastAnswer(payload) {
  ...
  const lines = [
    ...
    `Periodo A ${payload.periodo_a} (real) vs Periodo B ${payload.periodo_b} (forecast). Fuente: computeDeltaIngresoForecast. No es movimiento de kg ni Delta Ingreso histórico M9.`,
...
function buildDeltaIngresoForecastChatResult(payload, opts = {}) {
  ...
  return {
    ok: true,
    answer,
    sources: ok ? ["computeDeltaIngresoForecast"] : [],
    context_meta: {
    ...
    delta_ingreso_forecast: ok
      ? {
          ...
          source: "computeDeltaIngresoForecast",
          source_helper: "computeDeltaIngresoForecast",
          unit: "MXN",
          top_n: payload.top_n,
