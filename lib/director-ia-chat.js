...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (directorIaPlan.intent === "executive_sales_context") {
    ...
    const salesRows = chatDeps.predictiveSalesRows || chatDeps.clientRankingSalesRows || chatDeps.executiveSalesRows;
  ...
  if (directorIaPlan.intent === "client_comments") {
    const comments = Array.isArray(chatDeps.clienteComentariosRows)
      ? chatDeps.clienteComentariosRows
      : [];
    const filtered = runtime012.filterComments(comments, q, echoedState, chatDeps.now);
    return {
      ok: true,
      answer: runtime012.buildCommentsAnswer(filtered.rows, filtered),
      sources: ["arr.cliente_comentarios"],
      context_meta: {
  ...
  if (directorIaPlan.intent === "action_register_recent" || (directorIaPlan.intent === "action_status" && runtime012.classify012Family(q, echoedState) === "ACTION_REGISTER_DIRECT")) {
    const items = Array.isArray(chatDeps.actionRegisterItems) ? chatDeps.actionRegisterItems : [];
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    ...
    return {
      ok: true,
      answer,
      sources: ["arr.action_register_items"],
      context_meta: {
  ...
  if (directorIaPlan.intent === "bitacora_topic_lookup") {
    ...
    let entries = Array.isArray(chatDeps.bitacoraEntries) ? chatDeps.bitacoraEntries.slice() : [];
    if (!entries.length && planta_id) {
      const loadB = chatDeps.loadBitacoraForChat || loadBitacoraForChat;
      if (typeof loadB === "function" && chatDeps.pool && typeof chatDeps.pool.query === "function") {
        entries = await loadB(chatDeps.pool, planta_id);
      }
    }
    ...
    return {
      ...
      sources: ["arr.director_ia_bitacora"],
      context_meta: {
