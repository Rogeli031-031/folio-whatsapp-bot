...
/**
 * @param {{ fecha: string, tipo: string, titulo: string | null, resumen_ia: string, planta_nombre?: string | null }} session
 */
function formatBitacoraSessionLines(session) {
  const lines = [
 ...
 * @param {{ prioritizeRecent?: boolean, anchorSessions?: Array }} [opts]
 */
function formatBitacoraMonthlyBlocks(matched, opts = {}) {
  ...
  for (const { monthKey, sessions } of grouped) {
    ...
    if (sessions.length === 0) {
      ...
      for (const s of sessions) {
        lines.push(...formatBitacoraSessionLines(s));
        lines.push("");
      }
    }
    lines.push("");
 ...
 * @param {number} [months]
 */
function filterDicfToMonthWindow(details, months = BITACORA_CHAT_MONTH_WINDOW) {
  const anchor = getLatestDicfActivityDate(details);
...
/**
 * Bitácora reciente debe ganar a DICF histórico en preguntas de situación actual.
 * @param {string} question
 * @param {ReturnType<typeof extractChatContextFromPayload> | null} chatContext
 */
function shouldPrioritizeBitacoraOverDicf(question, chatContext) {
  ...
  if (!bitacoraIsNewerThanDicf(chatContext)) return false;
  ...
  if (/\b(actual|reciente|hoy|esta\s+semana|visita|julio|junio|mes)\b/i.test(q)) return true;
  return false;
}
...
function buildFocusedDicfContext(chatContext, question, commercialResolution = null) {
  ...
  const matched = filterDicfToMonthWindow(filtered, BITACORA_CHAT_MONTH_WINDOW);
  const anchor = getLatestDicfActivityDate(filtered.length > 0 ? filtered : all);
...
function buildBitacoraAnnex(sessions, question, commercialResolution = null, opts = {}) {
  ...
  const lines = [
    ...
    "",
    ...formatBitacoraMonthlyBlocks(matched, { anchorSessions: sessions }),
  ];
...
function resolveDirectorIaChatRouting(question, chatContext) {
  ...
  const wantBitacoraPrimary = shouldPrioritizeBitacoraOverDicf(q, chatContext);
  const wantDicf =
    !wantCommercialState && !wantBitacoraPrimary && shouldUseDicfFocusedChat(q, chatContext);
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (mejoraContinua) {
    ...
    if (wantBitacoraPrimary) {
      focused = buildFocusedBitacoraContext(bitacoraSessions, q, { prioritizeRecent: true });
      promptOpts = {
        bitacoraOnlyFallback: true,
        bitacoraPrioritized: true,
      ...
      directorIaDebug("[DIRECTOR_IA] bitacora_prioritized_over_dicf:", {
