...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (priorForPlan) planOptions.prior = priorForPlan;
  if (
    !planOptions.forceIntent &&
    echoedState &&
  ...
  if (directorIaPlan.intent === "open_pronostico") {
    ...
    return {
      ok: true,
      answer: spec.plant
        ? `Puedo abrir el modal Pronóstico de ${spec.plant}. No afirmo que ya se abrió hasta que el frontend ejecute la acción.`
        : "Puedo abrir el modal Pronóstico de la planta autorizada. No afirmo que ya se abrió hasta que el frontend ejecute la acción.",
      ui_action: spec.ui_action || {
        type: "OPEN_PRONOSTICO",
        plant: spec.plant || null,
        do_not_pretend_opened: true,
      },
      sources: ["igf"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "open_pronostico" },
          echoedState,
          "open_pronostico",
          false
        ),
  ...
  if (directorIaPlan.intent === "open_daily_sales_view") {
    ...
    return {
      ok: true,
      answer: spec.plant
        ? `Acción de navegación: abrir la venta diaria de ${spec.plant}. No finjo que la pantalla ya se abrió.`
        : "Acción de navegación: abrir la venta diaria de la planta autorizada. No finjo que la pantalla ya se abrió.",
      ui_action: spec.ui_action,
      sources: ["arr"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "open_daily_sales_view" },
          echoedState,
