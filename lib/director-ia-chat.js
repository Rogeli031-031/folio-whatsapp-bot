...
function continuityNeedsUniqueEntity(turn) {
  ...
  return turn.kind === "entity_intro" || turn.kind === "pronoun" || turn.kind === "action";
}
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (memoryStore && userScopeKey && plantaAuthorized && memoryTurn.kind === "resume") {
  ...
  } else if (resumeItem && resumeItem.parent_intent) {
  ...
  if (continuityTurn.out_of_slice_clarify) {
    ...
    return buildUnknownClarificationResult({
      ...
      conversation_state: buildConversationState({
        plantaId: planta_id,
        parent_intent: continuityTurn.invalidate_entity ? null : continuityTurn.parent_intent,
      }),
    });
  }
  ...
  if (directorIaPlan.intent === "budget_status") {
    ...
    const presupuestoPayload = await loadPresupuestoSemanalForChat(chatDeps.pool, planta_id, req, {
      question: q,
    });
    return buildPresupuestoSemanalChatResult(presupuestoPayload, { planta_id });
  }
  ...
  if (directorIaPlan.intent === "taller_at") {
    ...
    const tallerPayload = await loadTallerAtForChat(chatDeps.pool, planta_id, req, {
      question: q,
    });
    return buildTallerAtChatResult(tallerPayload, { planta_id });
  }
  ...
  if (directorIaPlan.intent === "daily_sales_deviation") {
    ...
    const activeEntities =
      entityResolution.status === "unique" ? [entityResolution.entity] : [];
  ...
  if (directorIaPlan.intent === "financial_diagnosis") {
    ...
    return buildFinancialDiagnosisChatResult(assembled, {
      answer,
      planta_id,
      openai_called: true,
    });
  ...
  if (directorIaPlan.intent === "plant_diagnosis") {
    ...
    const activeEntities =
      entityResolution.status === "unique" && !continuityTurn.invalidate_entity
    ...
    if (persistEntity && userScopeKey && plantaAuthorized && !skipPersistAfterClose) {
    ...
    if (continuityTurn.kind === "gap_who") {
      return {
        ok: true,
        answer: buildGapWhoAnswer(pendingGap),
        sources: (assembled && assembled.sources && Object.keys(assembled.sources)) || [],
        context_meta: attachMemoryMeta({
    ...
    if (continuityTurn.kind === "gap_what") {
      return {
        ok: true,
        answer: buildGapWhatAnswer(pendingGap),
        sources: [],
        context_meta: attachMemoryMeta({
          mode: "plant_diagnosis",
          prompt_mode: "plant_diagnosis",
    ...
    if (continuityTurn.kind === "gap_why_need") {
      return {
        ok: true,
        answer: buildGapWhyNeedAnswer(pendingGap),
        sources: [],
        context_meta: attachMemoryMeta({
          mode: "plant_diagnosis",
          prompt_mode: "plant_diagnosis",
