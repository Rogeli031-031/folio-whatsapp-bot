...
function buildIgfStatusSnapshotAnswer(assembled, question) {
  ...
  const header = `${plant} — IGF vigente de ${periodLabel}.`;
  ...
  const operativa = ["Utilidad operativa:", utilMxn, utilKg];
  const finalBlock = ["Resultado final:", finMxn, finKg];
  const body = [];
  if (lead === "final") {
  ...
  for (const spec of IGF_STATUS_VARIABLES) {
    body.push(`- ${formatIgfStatusVariable(spec, igfCompositionValue(composition, spec.key))}`);
  }
...
function buildIgfStatusSnapshotChatResult(assembled, opts = {}) {
  ...
  const answer = missing
    ...
    : buildIgfStatusSnapshotAnswer(assembled, opts.question);
  const year = assembled && assembled.year;
  const month = assembled && assembled.month;
  const period =
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (directorIaPlan.intent === "igf_status") {
    ...
    const igfStatusResult = buildIgfStatusSnapshotChatResult(assembled, {
      planta_id,
      question: rawQuestion || q,
    });
