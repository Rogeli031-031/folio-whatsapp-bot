...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (directorIaPlan.intent === "folio_search") {
    ...
    if (
      ...
      priorFolioSet &&
      Array.isArray(priorFolioSet.items) &&
      priorFolioSet.items.length &&
      backlog.isFolioResultNavigationQuestion(q, priorFolioSet.items)
    ) {
      const hit = backlog.selectOrdinal(priorFolioSet.items, ordinalSel);
      if (hit) {
        return {
          ok: true,
          answer: `El ${
            !ordinalSel || ordinalSel.kind === "folio_id"
              ? "folio"
              : ordinalSel.index === 1
                ? "primer folio"
                : ordinalSel.index === 2
                  ...
                  : ordinalSel.index === 3
                    ...
                    : ordinalSel.index === -1
                      ...
                      : ordinalSel.index === -2
                        ? "penúltimo folio"
                        : `folio número ${ordinalSel.index}`
          } es ${hit.numero_folio}.`,
          ui_action: {
            type: "OPEN_FOLIO",
            numero_folio: hit.numero_folio,
            folio_id: hit.folio_id,
