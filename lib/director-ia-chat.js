...
const { extractBitacoraExcerptForSearch, loadBitacoraForChat } = require("./director-ia-bitacora");
const { buildComentariosAnnexText, loadRecentCommentsByClienteNombres } = require("./cliente-comentarios");
const { computeDeltaIngresoClientesPorMes } = require("./delta-ingreso-clientes-por-mes");
const {
...
} = require("./director-ia-authenticated-user");
...
} = require("./director-ia-new-clients");
const {
  loadHistoricalMarginForChat,
  buildHistoricalMarginChatResult,
} = require("./director-ia-historical-margin");
const {
...
const { handleExecutiveDiagnosisForChat } = require("./director-ia-executive-diagnosis-observations-risks");
const {
...
} = require("./director-ia-expense-analytics");
const {
  ...
  priorSpecFromState,
} = require("./director-ia-seh-operation-status");
const {
...
} = require("./director-ia-client-movement");
const {
...
const runtime004 = require("./director-ia-commercial-runtime-004");
const runtime005 = require("./director-ia-commercial-runtime-hardening-005");
...
const runtime011 = require("./director-ia-client-inactivity-direct-answers-ui-011");
const runtime012 = require("./director-ia-routing-precedence-dimensions-012");
const { queryMonthlySales } = require("./director-ia-client-profile");
const { resolvePlantCodes } = require("./commercial-trend-engine");
const { loadClienteComentariosForDirectorIa } = require("./cliente-comentarios");
const runtime009 = require("./director-ia-purchase-evidence-enrichment-009");
const {
  classifyExecutiveFamily,
  limitationForFamily,
  extractOpenPronosticoSpec,
} = require("./director-ia-executive-coverage");
const { searchClienteContactosByPlant } = require("./cliente-contacto");
const backlog = require("./director-ia-executive-backlog");
const pendingCompletion = require("./director-ia-pending-completion");
const {
...
function firstInjectedArray(...candidates) {
...
async function resolve012SalesRows(plantLabel) {
  const injected = firstInjectedArray(
    chatDeps.predictiveSalesRows,
    chatDeps.clientRankingSalesRows,
    chatDeps.executiveSalesRows
  );
  if (injected) {
    return {
      rows: injected.map(runtime012.normalizeSalesRow),
      source_mode: "injected_test_rows",
    };
  }
  return runtime012.loadPhysicalSalesRows({
    pool: chatDeps.pool,
    db: chatDeps.pool,
    plant_label: plantLabel,
    plantCodes: chatDeps.predictivePlantCodes || chatDeps.clientRankingPlantCodes,
    now: chatDeps.now,
    resolvePlantCodes: chatDeps.resolveClientRankingPlantCodes || resolvePlantCodes,
    queryMonthlySales: chatDeps.queryMonthlySales || queryMonthlySales,
  });
}

async function resolve012Comments(plantaId) {
  if (Array.isArray(chatDeps.clienteComentariosRows)) {
    return { rows: chatDeps.clienteComentariosRows, source_mode: "injected_test_rows" };
  }
  const loadFn = chatDeps.loadClienteComentariosForDirectorIa || loadClienteComentariosForDirectorIa;
  if (!plantaId || typeof loadFn !== "function") {
    return { rows: [], source_mode: "arr.cliente_comentarios", load_error: "loader_unavailable" };
  }
  const rows = await loadFn(chatDeps.pool, plantaId, 200);
  return { rows: Array.isArray(rows) ? rows : [], source_mode: "arr.cliente_comentarios" };
}

async function resolve012ActionItems(req, plantaId) {
  if (Array.isArray(chatDeps.actionRegisterItems)) {
    return {
      items: runtime012.normalizeActionRegisterItems(chatDeps.actionRegisterItems),
      source_mode: "injected_test_rows",
    };
  }
  const loadFn = chatDeps.loadActionPersonBoardForChat || loadActionPersonBoardForChat;
  if (!plantaId || typeof loadFn !== "function") {
    return { items: [], source_mode: "arr.action_register_items", load_error: "loader_unavailable" };
  }
  const loaded = await loadFn(chatDeps.pool, plantaId, req, {
    ensureActionRegisterTables: chatDeps.ensureActionRegisterTables,
  });
  const items = loaded && loaded.ok !== false ? loaded.items || loaded.rows || [] : [];
  return {
    items: runtime012.normalizeActionRegisterItems(items),
    source_mode: "arr.action_register_items",
  };
}

async function resolve012BitacoraEntries(plantaId) {
  if (Array.isArray(chatDeps.bitacoraEntries) && chatDeps.bitacoraEntries.length) {
    return { entries: chatDeps.bitacoraEntries.slice(), source_mode: "injected_test_rows" };
  }
  const loadB = chatDeps.loadBitacoraForChat || loadBitacoraForChat;
  const canCallInjected = typeof chatDeps.loadBitacoraForChat === "function";
  const canCallPool = chatDeps.pool && typeof chatDeps.pool.query === "function";
  if (!plantaId || typeof loadB !== "function" || (!canCallInjected && !canCallPool)) {
    return { entries: [], source_mode: "arr.director_ia_bitacora" };
  }
  const entries = await loadB(chatDeps.pool, plantaId);
  return { entries: Array.isArray(entries) ? entries : [], source_mode: "arr.director_ia_bitacora" };
}

const FORECAST_MONTH_NAMES = Object.freeze([
  "",
  "enero",
  "febrero",
  "marzo",
...
function extractRequestedTopN(question, fallback = 5) {
  const m = String(question || "").match(/\b(\d{1,2})\s+clientes?\b/i);
...
function pickLatestComment(list) {
  if (!Array.isArray(list) || !list.length) return null;
  return list[0];
}

function formatForecastCommentLine(comment) {
...
function titleMonth(year, month) {
  const name = FORECAST_MONTH_NAMES[Number(month)] || String(month);
  return `${name.charAt(0).toUpperCase()}${name.slice(1)} ${year}`;
}

const IGF_STATUS_VARIABLES = Object.freeze([
  { key: "venta_ton", label: "Venta", kind: "ton" },
  { key: "margen_kg", label: "Margen", kind: "kg" },
  { key: "com_desc_kg", label: "Comisiones y descuentos", kind: "kg" },
  { key: "impuesto_kg", label: "Impuestos", kind: "kg" },
  { key: "hg_kg", label: "HG", kind: "kg" },
  { key: "gtos_apoyos_corp_kg", label: "Gastos/apoyos corporativos", kind: "kg" },
]);

function normalizeIgfStatusQuestion(raw) {
  return String(raw || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
...
function igfStatusLeadKind(question) {
  const q = normalizeIgfStatusQuestion(question);
  if (/\bresultado\s+final\b/.test(q) || /\brentabilidad\s+final\b/.test(q)) return "final";
  if (/\butilidad\s+operativ/.test(q) || /\brentabilidad\s+operativ/.test(q)) return "operativa";
  return "both";
}

function igfCompositionValue(composition, key) {
  const lines = composition && Array.isArray(composition.lines) ? composition.lines : [];
  const found = lines.find((l) => l && l.line_key === key);
  if (!found || found.value == null || found.value === "") return null;
  const n = Number(found.value);
  return Number.isFinite(n) ? n : null;
}

function formatIgfStatusMxn(value) {
...
function formatIgfStatusPerKg(value) {
  if (value == null || !Number.isFinite(Number(value))) return "n.d.";
  return `${Number(value).toFixed(2)} MXN/kg`;
}

function formatIgfStatusTons(value) {
  if (value == null || !Number.isFinite(Number(value))) return "n.d.";
  return `${Number(value).toFixed(1)} t`;
}

function formatIgfStatusVariable(spec, value) {
  if (spec.kind === "ton") return `${spec.label}: ${formatIgfStatusTons(value)}`;
  return `${spec.label}: ${formatIgfStatusPerKg(value)}`;
}

const IGF_STATUS_SOURCE_MODE = Object.freeze({
  MINI_FORECAST_PROY: "MINI_FORECAST_PROY",
  IGF_COMMIT_SNAPSHOT: "IGF_COMMIT_SNAPSHOT",
  NEVER_CURRENT_MINI: "NEVER_CURRENT_MINI",
});

function yearMonthFromDateCdmx(date) {
...
function isExplicitIgfBudgetQuestion(question) {
  const q = normalizeIgfStatusQuestion(question);
  return (
    /\bpresupuesto\b/.test(q) ||
    /\bpresupuestad[oa]s?\b/.test(q) ||
    /\bcompromiso\b/.test(q) ||
    /\bcomprometid[oa]s?\b/.test(q) ||
    /\bigf\s+original\b/.test(q) ||
    /\bplan\s+original\b/.test(q)
  );
}

function isExplicitProjectionCloseQuestion(question) {
  const q = normalizeIgfStatusQuestion(question);
  if (/\bpara\s+cerrar\b/.test(q)) return true;
  if (/\bproyectamos\b/.test(q)) return true;
  return /\bproyectad[oa]\b/.test(q) && /\bcerrar\b/.test(q);
}

function selectIgfStatusSourceMode(question, opts = {}) {
  const now = opts.now instanceof Date && !Number.isNaN(opts.now.getTime()) ? opts.now : new Date();
  const current = opts.currentYearMonth || yearMonthFromDateCdmx(now);
  const resolved =
    Number.isFinite(Number(opts.year)) && Number.isFinite(Number(opts.month))
      ? { year: Number(opts.year), month: Number(opts.month) }
      : resolveYearMonthFromQuestion(question, current);
  if (isExplicitIgfBudgetQuestion(question)) {
    return {
      mode: IGF_STATUS_SOURCE_MODE.IGF_COMMIT_SNAPSHOT,
      reason: "EXPLICIT_BUDGET_COMMITMENT",
      year: resolved.year,
      month: resolved.month,
    };
  }
  const currentKey = Number(current.year) * 12 + Number(current.month);
  const resolvedKey = Number(resolved.year) * 12 + Number(resolved.month);
  if (Number.isFinite(resolvedKey) && Number.isFinite(currentKey) && resolvedKey < currentKey) {
    return {
      mode: IGF_STATUS_SOURCE_MODE.NEVER_CURRENT_MINI,
      reason: "PAST_MONTH",
      year: resolved.year,
      month: resolved.month,
    };
  ...
  return {
    mode: IGF_STATUS_SOURCE_MODE.MINI_FORECAST_PROY,
    reason: isExplicitProjectionCloseQuestion(question)
...
function formatIgfStatusProjectedTons(value) {
  if (value == null || !Number.isFinite(Number(value))) return "n.d.";
...
function formatIgfStatusCutoffDisplay(ymd) {
...
function finiteMiniField(row, camel, authKey, auth) {
  if (row && Object.prototype.hasOwnProperty.call(row, camel)) {
    const raw = row[camel];
    if (raw == null || raw === "") return null;
    return Number.isFinite(Number(raw)) ? Number(raw) : null;
  }
  if (auth && auth[authKey] != null && auth[authKey] !== "" && Number.isFinite(Number(auth[authKey]))) {
    return Number(auth[authKey]);
  }
  return null;
}

function buildIgfStatusSnapshotAnswer(assembled, question, opts = {}) {
  const plant = (assembled && assembled.plant && assembled.plant.planta_nombre) || "la planta";
  const year = assembled && assembled.year;
  const month = assembled && assembled.month;
  const periodLabel =
    Number.isFinite(Number(year)) && Number.isFinite(Number(month)) ? titleMonth(year, month) : "el periodo vigente";
  const budget = Boolean(opts.budget);
  const header = budget
    ? `${plant} — presupuesto/compromiso IGF de ${periodLabel}.`
    : `${plant} — IGF vigente de ${periodLabel}.`;
  const igf = assembled && assembled.igf;
  const composition = igf && igf.composition;
  const hasRow = Boolean(igf && igf.row && composition && composition.ok === true);
  if (!hasRow) {
    return `${header}\n\nNo hay un snapshot IGF defendible para esta planta y periodo. DATA_NOT_FOUND.`;
  }
  const lead = igfStatusLeadKind(question);
  const utilMxn = formatIgfStatusMxn(igfCompositionValue(composition, "util_oper_importe"));
  const utilKg = formatIgfStatusPerKg(igfCompositionValue(composition, "util_oper_kg"));
  const finMxn = formatIgfStatusMxn(igfCompositionValue(composition, "resultado_final_importe"));
  const finKg = formatIgfStatusPerKg(igfCompositionValue(composition, "resultado_final_kg"));
  const utilLabel = budget ? "Utilidad operativa (compromiso):" : "Utilidad operativa:";
  const finalLabel = budget ? "Resultado final (compromiso):" : "Resultado final:";
  const operativa = [utilLabel, utilMxn, utilKg];
  const finalBlock = [finalLabel, finMxn, finKg];
  const body = [];
  if (lead === "final") {
    body.push(...finalBlock, "", ...operativa);
  } else {
    body.push(...operativa, "", ...finalBlock);
  }
  body.push("", "Variables disponibles:");
  for (const spec of IGF_STATUS_VARIABLES) {
    const label = spec.key === "venta_ton" && budget ? "Venta (compromiso)" : spec.label;
    body.push(`- ${formatIgfStatusVariable({ ...spec, label }, igfCompositionValue(composition, spec.key))}`);
  }
  if (budget) {
    body.push("", "Estos importes son presupuesto/compromiso IGF stored. No son la proyección vigente del mes.");
  }
  return [header, "", ...body].join("\n");
}

function buildIgfStatusMiniForecastAnswer(input) {
  const assembled = input && input.assembled;
  const row = input && input.row;
  const auth = input && input.auth;
  const cutoff = input && input.cutoff;
  ...
  const periodLabel =
    Number.isFinite(Number(year)) && Number.isFinite(Number(month)) ? titleMonth(year, month) : "el periodo vigente";
  const cutoffDisplay = formatIgfStatusCutoffDisplay(cutoff);
  const header = cutoffDisplay
    ? `${plant} — proyección vigente de ${periodLabel} al corte ${cutoffDisplay}.`
    : `${plant} — proyección vigente de ${periodLabel}.`;
  const ingreso = finiteMiniField(row, "ingreso", "ingreso", auth);
  const operativos = finiteMiniField(row, "operativos", "operativos", auth);
  const corporativos = finiteMiniField(row, "corporativos", "corporativos", auth);
  const gasto = finiteMiniField(row, "gasto", "gasto", auth);
  const util = finiteMiniField(row, "utilOperImporte", "util_oper_importe", auth);
  const fin = finiteMiniField(row, "resultadoFinalImporte", "resultado_final_importe", auth);
  const venta = finiteMiniField(row, "ventaTon", "venta_ton", auth);
  const margen = finiteMiniField(row, "margen", "margen", auth);
  const comDesc = finiteMiniField(row, "comDesc", "desc_kg", auth);
  const impuestos = finiteMiniField(row, "impuestos", "impuestos", auth);
  const hg = finiteMiniField(row, "hgKg", "hg_kg", auth);
  const lead = igfStatusLeadKind(question);
  const operativa = [
    "Utilidad operativa proyectada:",
    formatIgfStatusMxn(util),
    `- Ingreso proyectado: ${formatIgfStatusMxn(ingreso)}`,
    `- Gastos operativos proyectados: ${formatIgfStatusMxn(operativos)}`,
  ];
  const finalBlock = [
    "Resultado final proyectado:",
    formatIgfStatusMxn(fin),
    `- Gastos corporativos proyectados: ${formatIgfStatusMxn(corporativos)}`,
  ];
  const body = [];
  if (lead === "final") {
    body.push(...finalBlock, "", ...operativa);
  ...
  body.push("", `Gasto total proyectado: ${formatIgfStatusMxn(gasto)}`);
  body.push("", "Variables:");
  body.push(`- Venta proyectada del mes: ${formatIgfStatusProjectedTons(venta)}`);
  body.push(`- Margen: ${formatIgfStatusPerKg(margen)}`);
  body.push(`- Comisiones y descuentos: ${formatIgfStatusPerKg(comDesc)}`);
  body.push(`- Impuestos: ${formatIgfStatusPerKg(impuestos)}`);
  body.push(`- HG: ${formatIgfStatusPerKg(hg)}`);
  body.push("");
  body.push(
    "En este mini, la utilidad operativa proyectada es ingreso proyectado menos gastos operativos proyectados."
  );
  body.push(
    "El resultado final proyectado es la utilidad operativa proyectada menos gastos corporativos proyectados."
  );
  body.push("Esta cifra de venta es la proyección mensual del mini. No la trate como volumen ya ocurrido ni como meta presupuestada.");
  return [header, "", ...body].join("\n");
}

function buildIgfStatusMiniUnavailableResult(assembled, opts = {}) {
  const plant = (assembled && assembled.plant && assembled.plant.planta_nombre) || "la planta";
  const year = assembled && assembled.year;
  const month = assembled && assembled.month;
  const periodLabel =
    Number.isFinite(Number(year)) && Number.isFinite(Number(month)) ? titleMonth(year, month) : "el periodo vigente";
  const igf = assembled && assembled.igf;
  const hasCommit = Boolean(igf && igf.row && igf.composition && igf.composition.ok === true);
  const lines = [
    `${plant} — proyección vigente de ${periodLabel} no disponible. DATA_NOT_FOUND.`,
  ];
  if (hasCommit) {
    const ver = igf.version_number != null ? ` (v${igf.version_number})` : "";
    lines.push(
      `Existe un compromiso IGF${ver} por separado; no es la rentabilidad actual ni la proyección vigente.`
    );
  }
  const period =
    Number.isFinite(Number(year)) && Number.isFinite(Number(month))
  ...
  return {
    ...
    context_meta: {
      mode: "igf_status",
      source_mode: IGF_STATUS_SOURCE_MODE.MINI_FORECAST_PROY,
      loader: "readIgfForecastMiniAuthoritative",
      source: "computeIgfForecastMiniPayload",
      openai_called: false,
      planta_id: opts.planta_id != null ? opts.planta_id : assembled && assembled.plant && assembled.plant.planta_id,
      period,
      tool: "get_igf_snapshot",
      cutoff: null,
      version_id: igf && igf.version_id != null ? igf.version_id : null,
      version_number: igf && igf.version_number != null ? igf.version_number : null,
      commit_label_if_exposed: hasCommit ? "compromiso" : null,
    },
    status: 200,
...
function buildIgfStatusSnapshotChatResult(assembled, opts = {}) {
  const missing = !assembled || assembled.ok === false || assembled.abort;
  const igf = assembled && assembled.igf;
  const hasRow = Boolean(igf && igf.row && igf.composition && igf.composition.ok === true);
  const answer = missing
    ? assembled && assembled.error
      ? String(assembled.error)
      : "No pude cargar el snapshot IGF. DATA_NOT_FOUND."
    : buildIgfStatusSnapshotAnswer(assembled, opts.question, { budget: Boolean(opts.budget) });
  const year = assembled && assembled.year;
  const month = assembled && assembled.month;
  const period =
    Number.isFinite(Number(year)) && Number.isFinite(Number(month))
      ? `${Number(year)}-${String(Number(month)).padStart(2, "0")}`
      : null;
  const sourceMode = opts.source_mode || IGF_STATUS_SOURCE_MODE.IGF_COMMIT_SNAPSHOT;
  return {
    ok: !missing,
    answer,
    sources: hasRow ? [IGF_COMPOSITION_SOURCE] : [],
    context_meta: {
      mode: "igf_status",
    ...
    status: missing ? assembled && assembled.status : 200,
    error: missing ? (assembled && assembled.error) || "DATA_NOT_FOUND" : undefined,
    code: missing ? assembled && assembled.code : undefined,
  };
}

async function buildIgfStatusMiniForecastChatResult(assembled, opts = {}) {
  const loadMini = chatDeps.loadIgfForecastMiniPayload;
  if (typeof loadMini !== "function") {
    return buildIgfStatusMiniUnavailableResult(assembled, opts);
  }
  const cutoffRes = resolveDirectorIaEffectiveCutoff({
    question: opts.question,
    body: opts.req && opts.req.body,
  });
  const uploadDay = cutoffRes && cutoffRes.cutoff ? cutoffRes.cutoff : null;
  let mini = null;
  try {
    mini = await loadMini(chatDeps.pool, {
      year: assembled && assembled.year,
      month: assembled && assembled.month,
      upload_day: uploadDay,
      plantName: assembled && assembled.plant && assembled.plant.planta_nombre,
    });
  } catch (_e) {
    return buildIgfStatusMiniUnavailableResult(assembled, opts);
  }
  ...
  const plantName = assembled && assembled.plant && assembled.plant.planta_nombre;
  const plantCode = assembled && assembled.plant && assembled.plant.plant_code;
  const auth = adapter.readIgfForecastMiniAuthoritative(mini, plantName, plantCode);
  const row = adapter.findMiniRowForPlant(mini.rows, plantName, plantCode);
  if (!row) {
    return buildIgfStatusMiniUnavailableResult(assembled, opts);
  }
  const cutoff = adapter.parseCutoffYmd(mini.upload_day) || (auth && auth.cutoff_date) || null;
  const igf = assembled && assembled.igf;
  const year = assembled && assembled.year;
  const month = assembled && assembled.month;
  const period =
  ...
  return {
    ok: true,
    answer: buildIgfStatusMiniForecastAnswer({
      assembled,
      row,
      auth,
      cutoff,
    ...
    sources: ["computeIgfForecastMiniPayload"],
    context_meta: {
      mode: "igf_status",
      source_mode: IGF_STATUS_SOURCE_MODE.MINI_FORECAST_PROY,
      loader: "readIgfForecastMiniAuthoritative",
...
async function resolveForecastPlantaRow(client, plantaId) {
  if (typeof chatDeps.resolveDeltaIngresoForecastPlanta === "function") {
    return chatDeps.resolveDeltaIngresoForecastPlanta(client, plantaId);
  }
...
async function loadDeltaIngresoForecastNegativeTopN(pool, plantaId, req, opts = {}) {
  const question = opts.question || "";
  const now = opts.now instanceof Date && !Number.isNaN(opts.now.getTime()) ? opts.now : chatDeps.now || new Date();
  const months = opts.months || resolveCalendarCompareMonths(question, now);
  if (!months || !months.month_a || !months.month_b) {
    return {
      ok: false,
      error: "No pude resolver el mes solicitado para Delta Ingreso Forecast. No invento el periodo.",
    };
  }
  const yearA = months.month_a.year;
  const monthA = months.month_a.month;
  const yearB = months.month_b.year;
  const monthB = months.month_b.month;
  const topN = extractRequestedTopN(question, 5);
  ...
  try {
    ...
    const planta = await resolveForecastPlantaRow(client, plantaId);
    if (!planta) {
      return { ok: false, error: "Planta no encontrada para Delta Ingreso Forecast." };
    }
    const plantaNombre = String(planta.nombre || "").trim();
    let plantCode = plantaNombre;
    const mapPlant = chatDeps.getPlantCodeArrFromPlantaNombre;
    if (typeof mapPlant === "function") {
      const mapped = await mapPlant(client, plantaNombre);
      if (mapped) plantCode = mapped;
    }
    const compute = chatDeps.computeDeltaIngresoClientesPorMes || computeDeltaIngresoClientesPorMes;
    const { parseCutoffYmd } = require("./director-ia-dashboard-forecast-adapter");
    const requestedUploadDay = parseCutoffYmd(
      req && req.body && (req.body.upload_day || req.body.cutoff_date)
    );
    const data = await compute(client, plantCode, yearA, monthA, yearB, monthB, {
      now,
      computeClientesDescuentoMes: chatDeps.computeClientesDescuentoMes,
      loadIgfPlantMetrics: chatDeps.loadIgfPlantMetrics,
      loadEffectiveIgfTarget: chatDeps.loadEffectiveIgfTarget,
      upload_day: requestedUploadDay,
    });
    const rawRows = Array.isArray(data && data.rows) ? data.rows : [];
    const mapRow = (r, comment) => {
      const cliente = String(r.cliente || r.cliente_norm || "").trim();
      const delta = Number(r.deltaIngreso != null ? r.deltaIngreso : r.delta_ingreso);
      return {
        cliente,
        delta_ingreso: delta,
        ingreso_a: r.ingresoA != null ? Number(r.ingresoA) : r.ingreso_a != null ? Number(r.ingreso_a) : null,
        ingreso_b: r.ingresoB != null ? Number(r.ingresoB) : r.ingreso_b != null ? Number(r.ingreso_b) : null,
        kg_a: r.kgA != null ? Number(r.kgA) : r.kg_a != null ? Number(r.kg_a) : null,
        kg_b: r.kgB != null ? Number(r.kgB) : r.kg_b != null ? Number(r.kg_b) : null,
        desc_kg_a: r.descKgA != null ? Number(r.descKgA) : r.desc_kg_a != null ? Number(r.desc_kg_a) : null,
        desc_kg_b: r.descKgB != null ? Number(r.descKgB) : r.desc_kg_b != null ? Number(r.desc_kg_b) : null,
        comment: comment || null,
        comment_missing: !comment,
      };
    };
    const rows_all = rawRows.map((r) => mapRow(r, null));
    const negatives = rawRows
      .filter((r) => Number(r.deltaIngreso != null ? r.deltaIngreso : r.delta_ingreso) < 0)
      .sort((a, b) => {
        const da = Number(a.deltaIngreso != null ? a.deltaIngreso : a.delta_ingreso);
        const db = Number(b.deltaIngreso != null ? b.deltaIngreso : b.delta_ingreso);
        return da - db;
      });
    const shown = negatives.slice(0, topN);
    const names = shown.map((r) => r.cliente || r.cliente_norm).filter(Boolean);
    const loadComments = chatDeps.loadRecentCommentsByClienteNombres || loadRecentCommentsByClienteNombres;
    let commentsMap = new Map();
    try {
      commentsMap = (await loadComments(client, {
        plantaIds: [Number(planta.id) || Number(plantaId)].filter((n) => Number.isFinite(n) && n > 0),
        nombres: names,
        limitPerCliente: 1,
      })) || new Map();
    } catch (_e) {
    ...
    const rows = shown.map((r) => {
      const cliente = String(r.cliente || r.cliente_norm || "").trim();
      const latest = pickLatestComment(commentsMap && commentsMap.get && commentsMap.get(cliente.toLowerCase()));
      const body = latest && String(latest.body || latest.text || "").trim();
      const created_at = latest ? commentDateOnly(latest.created_at || latest.date) : "";
      return mapRow(r, body ? { body, created_at } : null);
    });
    const impacto = rows.reduce((s, r) => s + (Number(r.delta_ingreso) || 0), 0);
    const sourceHelper = (data && data.source_helper) || "computeDeltaIngresoClientesPorMes";
    return {
      ok: true,
      ...
      yearB,
      monthB,
      periodo_a: yyyyMm(yearA, monthA),
      periodo_b: yyyyMm(yearB, monthB),
      top_n: rows.length,
      requested_n: topN,
      list_total_negative: negatives.length,
      list_truncated: negatives.length > rows.length,
      impacto_top_n: impacto,
      rows,
      rows_all,
      target_kg: data && data.targetKg != null ? Number(data.targetKg) : null,
      igf_venta_ton: data && data.igf_venta_ton != null ? Number(data.igf_venta_ton) : null,
      igf_financial_state: (data && data.igf_financial_state) || null,
      igf_version_number: data && data.igf_version_number != null ? Number(data.igf_version_number) : null,
      target_source: (data && data.target_source) || null,
      upload_day: (data && data.upload_day) || requestedUploadDay || null,
      source_helper: sourceHelper,
      physical_source: (data && data.physical_source) || "dashboard-arr-forecast.computeClientesDescuentoMes",
...
function buildDeltaIngresoForecastAnswer(payload) {
  if (!payload || payload.ok !== true) {
    return (
      (payload && payload.error) ||
      "No pude completar Delta Ingreso Forecast. No invento clientes, importes ni comentarios."
    );
  }
  const rows = payload.rows || [];
  const fewer = rows.length < Number(payload.requested_n || rows.length);
  const lines = [
    `Delta Ingreso Forecast (MXN) de ${payload.planta_nombre || "la planta"}: ${titleMonth(payload.yearA, payload.monthA)} real vs ${FORECAST_MONTH_NAMES[payload.monthB] || payload.periodo_b} ${payload.yearB} forecast.`,
    `Periodo A ${payload.periodo_a} (real) vs Periodo B ${payload.periodo_b} (forecast). Fuente: Clientes por mes (computeClientesDescuentoMes + ingresoClienteMarginal). No es OLS de computeDeltaIngresoForecast ni Delta Ingreso histórico M9.`,
    `Clientes con Delta Ingreso negativo: ${payload.list_total_negative}. Top ${payload.top_n} mostrado${fewer ? ` (hay menos negativos que los ${payload.requested_n} pedidos)` : ""}.`,
    `Estos ${payload.top_n} clientes acumulan un Delta Ingreso de ${formatDeltaMxn(payload.impacto_top_n)}.`,
  ];
  rows.forEach((r, i) => {
    lines.push(`${i + 1}. ${r.cliente}: ${formatDeltaMxn(r.delta_ingreso)} MXN`);
    lines.push(`   ${formatForecastCommentLine(r.comment)}`);
  });
  return lines.join("\n");
}

function buildDeltaIngresoForecastChatResult(payload, opts = {}) {
  const planta_id = opts.planta_id != null ? opts.planta_id : payload && payload.planta_id;
  const answer = buildDeltaIngresoForecastAnswer(payload);
  ...
  return {
    ...
    context_meta: {
      ...
      requested_domain: "delta_ingreso_forecast",
      openai_called: false,
      period_kind: "forecast_compare",
    ...
    delta_ingreso_forecast: ok
      ? {
          ...
          igf_venta_ton: payload.igf_venta_ton,
          igf_financial_state: payload.igf_financial_state,
          igf_version_number: payload.igf_version_number,
          target_source: payload.target_source,
          upload_day: payload.upload_day,
        }
      : null,
  };
}
...
function isCutoffExplainQuestion(question) {
  ...
  return (
    ...
    /de que corte es esa cifra/.test(t) ||
    /de que corte es esa/.test(t)
  );
}
...
function classifyConversationalIntent(question) {
  ...
  if (greetingPatterns.some((re) => re.test(t))) {
    return { mode: "smalltalk", greeting_kind: classifyGreetingKind(t) };
  }

  return null;
}

function buildConversationalAnswer(mode, plantLabel, extra) {
...
- ¿Qué debe hacer el gerente esta semana?`;
  ...
  return buildNeutralGreeting({
    greeting_kind: extra && extra.greeting_kind,
    nombre_persona: extra && extra.nombre_persona,
  });
}
...
function parseInheritedYyyymm(raw) {
  const m = String(raw || "").trim().match(/^(\d{4})-(\d{2})$/);
  if (!m) return null;
  return { year: Number(m[1]), month: Number(m[2]) };
}

async function loadInheritedExpenseKpis(incoming, plantLabel, plantCode, req) {
  const months = Array.isArray(incoming && incoming.active_period_months)
    ? incoming.active_period_months
    : [];
  const a = parseInheritedYyyymm(months[0]);
  const b = parseInheritedYyyymm(months[1]);
  if (!a || !b || typeof loadKpiForMonth !== "function") return null;
  const deps = {
    now: chatDeps.now,
    pool: chatDeps.pool,
    upload_day: (req && req.body && req.body.upload_day) || null,
    loadRentabilidadKpis: chatDeps.loadRentabilidadKpis,
    loadIgfForecastMiniPayload: chatDeps.loadIgfForecastMiniPayload,
  };
  const kpiA = await loadKpiForMonth(deps, plantLabel, plantCode, a.year, a.month);
  const kpiB = await loadKpiForMonth(deps, plantLabel, plantCode, b.year, b.month);
  return { kpiA, kpiB };
}
...
async function resolveExplicitFolioForOpen(deps, plantaId, numeroFolio, req) {
  const numero = String(numeroFolio || "").trim().toUpperCase();
  if (!numero) return { ok: false, error: "No indicaste un número de folio." };
  const auth = (req && req.dashboardAuth) || {};
  const denied = assertFolioStatusAccess(auth, Number(plantaId));
  if (!denied.ok) return { ok: false, error: denied.error || "No tienes acceso a esa planta." };
  const items = Array.isArray(deps.folioItems) ? deps.folioItems : [];
  const fromItems = items.find((row) => String(row.numero_folio || row.folio_codigo || "").toUpperCase() === numero);
  if (fromItems) {
    const rowPlant = fromItems.planta_id != null ? Number(fromItems.planta_id) : Number(plantaId);
    if (Number.isFinite(rowPlant) && rowPlant > 0 && rowPlant !== Number(plantaId)) {
      return { ok: false, error: `El folio ${numero} no pertenece a la planta actual. No abro un folio de otra planta.` };
    }
    return {
      ok: true,
      numero_folio: fromItems.numero_folio || numero,
      folio_id: fromItems.folio_id != null ? fromItems.folio_id : fromItems.id,
      id: fromItems.id,
    };
  }
  const lookup = typeof deps.getFolioByNumero === "function" ? deps.getFolioByNumero : getFolioByNumero;
  if (typeof lookup !== "function") return { ok: false, error: `No encontré el folio ${numero}.` };
  try {
    const row = await lookup(deps.pool, numero);
    if (!row) return { ok: false, error: `No encontré el folio ${numero} en la planta actual.` };
    const rowPlant = row.planta_id != null ? Number(row.planta_id) : null;
    if (rowPlant != null && rowPlant !== Number(plantaId)) {
      return { ok: false, error: `El folio ${numero} no pertenece a la planta actual. No abro un folio de otra planta.` };
    }
    return { ok: true, numero_folio: row.numero_folio || numero, folio_id: row.id, id: row.id };
  } catch (e) {
    return { ok: false, error: `No pude validar el folio ${numero}. No invento el detalle.` };
  }
}

function priorCategoryFromState(state) {
  if (!state || typeof state !== "object") return null;
  const sub = state.active_subtopic;
  const pending = runtime007.sanitizePendingUiAction(state.pending_ui_action);
  const category =
    sub === "CASA" || sub === "COMISIONISTA"
      ? sub
      : pending && pending.category
        ? pending.category
        : null;
  const metric = sub === "COMMISSION" ? "COMMISSION" : null;
  if (!category && !metric && state.parent_intent !== "category_commission") return null;
  return {
    category,
    metric,
    parent_intent: state.parent_intent || null,
    pending_ui_action: pending,
  };
}
...
function conversationStateForIntent(opts, incoming, newIntent, restorePrevious) {
  ...
  const forecast_magnitudes =
    opts && Object.prototype.hasOwnProperty.call(opts, "forecast_magnitudes")
      ? opts.forecast_magnitudes
      : incoming && incoming.forecast_magnitudes;
...
function resolveEntityAgainstAssembled(turn, assembled) {
  const hints =
    turn && Array.isArray(turn.entity_hint_candidates) && turn.entity_hint_candidates.length
      ? turn.entity_hint_candidates
      : turn && turn.entity_hint
        ? [turn.entity_hint]
        : [];
  if (!hints.length) return { status: "none" };
  const extra =
  ...
  return resolveUniqueEntityFromHints(hints, candidates, {
    requires_canonical_evidence: Boolean(
      turn && (turn.leading_y_requires_canonical || turn.embedded_client_requires_canonical)
    ),
  });
}
...
async function handleExecutiveStatusForChat(opts) {
  ...
  let miniPayloadForPack = null;
  const { buildAuthoritativeForecastRunPack } = require("./director-ia-authoritative-forecast-run-pack");
  const ymForPack = (forecastParity && forecastParity.period) || parseYearMonth(assembled) || {};
  const plantNameForPack = (assembled.plant && assembled.plant.planta_nombre) || plantLabel;
  if (typeof loadMini === "function" && chatDeps.pool && resolvedCutoff) {
    try {
      ...
      const miniPayload = await loadMini(chatDeps.pool, {
        year: ymForPack.year,
        month: ymForPack.month,
        plantName: plantNameForPack,
        upload_day: resolvedCutoff,
      });
      miniLoaded = true;
      miniPayloadForPack = miniPayload;
      if (!forecastParity || typeof forecastParity !== "object") {
        forecastParity = { ok: true, reachable: false };
      }
      forecastParity.mini = readIgfForecastMiniAuthoritative(miniPayload, plantNameForPack, plantCode);
  ...
  const igfPayload = assembled.sources && assembled.sources.igf && assembled.sources.igf.payload;
  const authoritativeForecast = buildAuthoritativeForecastRunPack({
    plant_code: plantCode,
    plant_label: plantNameForPack,
    year: ymForPack.year,
    month: ymForPack.month,
    upload_day: resolvedCutoff,
    cutoff_origin: mapCutoffSourceToOrigin(cutoffSource),
    miniPayload: miniPayloadForPack,
    mini_loader_invoked: miniLoaded,
    actual_to_date: forecastParity && forecastParity.actual_to_date,
    igf_version_id: igfPayload && igfPayload.version_id,
    igf_version_number: igfPayload && igfPayload.version_number,
  });
  if (forecastParity && typeof forecastParity === "object") {
    forecastParity.authoritative_pack = authoritativeForecast;
  }
  ...
  const state = conversationStateForIntent(
    {
      ...
      forecast_magnitudes: forecastMagnitudesFromAuthoritativePack(authoritativeForecast, forecastRun),
    },
    echoedState,
    "plant_diagnosis",
    Boolean(continuityTurn && continuityTurn.restore_previous)
  ...
  if (result && result.context_meta) {
    ...
    const packAvailable = Boolean(
      authoritativeForecast && authoritativeForecast.status === "AVAILABLE"
    );
    ...
    result.context_meta.authoritative_forecast_status =
      (authoritativeForecast && authoritativeForecast.status) || "UNAVAILABLE";
    result.context_meta.authoritative_source = packAvailable
      ? "computeIgfForecastMiniPayload"
      : null;
    result.context_meta.fallback_used = !packAvailable;
    ...
    result.context_meta.forecast_magnitudes = state.forecast_magnitudes;
  }
  return result;
}

function cutoffSourceForFollowUp(questionCutoff, bodyCutoff, previousRun) {
  if (questionCutoff) return "question.explicit_cutoff";
  if (bodyCutoff) return "req.body.upload_day";
  if (previousRun && previousRun.cutoff_origin === "EXPLICIT_QUESTION") return "question.explicit_cutoff";
  if (previousRun && previousRun.cutoff_origin === "REQUEST_UPLOAD_DAY") return "req.body.upload_day";
  if (previousRun && previousRun.cutoff_origin === "PLANT_LAST_UPLOAD") return "arr.upload_log.plant";
  return null;
}

function yearMonthFromCutoffYmd(cutoff) {
  const s = String(cutoff || "").trim().slice(0, 10);
  const m = s.match(/^(\d{4})-(\d{2})-\d{2}$/);
  if (!m) return null;
...
async function handleForecastMagnitudeFollowUpForChat({ req, planta_id, echoedState, question, kind }) {
  const sanitized = sanitizeEchoedState(echoedState, planta_id);
  const previousRun =
    sanitized.forecast_run && sanitized.forecast_run.effective_cutoff_date ? sanitized.forecast_run : null;
  const { parseCutoffYmd } = require("./director-ia-dashboard-forecast-adapter");
  const { buildAuthoritativeForecastRunPack } = require("./director-ia-authoritative-forecast-run-pack");
  const body = (req && req.body) || {};
  const namedPeriod = namedCalendarPeriodFromQuestion(question);
  const bodyYear = Number(body.year);
  const bodyMonth = Number(body.month);
  const questionCutoff = parseExplicitCutoffFromQuestion(question);
  const bodyCutoff = parseCutoffYmd(body.upload_day || body.cutoff_date);
  const periodFromQuestionDiffers = Boolean(
    previousRun &&
      namedPeriod &&
      ((namedPeriod.month != null && namedPeriod.month !== previousRun.month) ||
        (namedPeriod.year != null && namedPeriod.year !== previousRun.year))
  );
  let year = Number.isFinite(bodyYear) ? bodyYear : previousRun && previousRun.year;
  let month =
    Number.isFinite(bodyMonth) && bodyMonth >= 1 && bodyMonth <= 12 ? bodyMonth : previousRun && previousRun.month;
  if (namedPeriod && namedPeriod.year != null) year = namedPeriod.year;
  if (namedPeriod && namedPeriod.month != null) month = namedPeriod.month;
  let cutoff = questionCutoff || bodyCutoff || null;
  if ((!Number.isFinite(year) || !Number.isFinite(month)) && cutoff) {
    const fromCutoff = yearMonthFromCutoffYmd(cutoff);
    if (fromCutoff) {
      if (!Number.isFinite(year)) year = fromCutoff.year;
      if (!Number.isFinite(month)) month = fromCutoff.month;
    }
  }
  if (
    periodFromQuestionDiffers &&
    bodyCutoff &&
    previousRun &&
    bodyCutoff === previousRun.effective_cutoff_date &&
    !questionCutoff
  ) {
    cutoff = null;
  }
  if (!cutoff && previousRun && !periodFromQuestionDiffers && !sanitized.plant_mismatch) {
    cutoff = previousRun.effective_cutoff_date;
  }
  const catalog = await loadPlantCatalog(chatDeps.pool, chatDeps.plantCatalog);
  const plantEntry = (Array.isArray(catalog) ? catalog : []).find(
    (p) => Number(p.planta_id != null ? p.planta_id : p.id) === Number(planta_id)
  );
  const plantLabel =
    (body.planta_nombre && String(body.planta_nombre).trim()) ||
    (plantEntry && (plantEntry.nombre || plantEntry.planta_nombre)) ||
    null;
  const plantCode =
    (previousRun && !sanitized.plant_mismatch && previousRun.plant_code) ||
    (plantEntry && (plantEntry.clave || plantEntry.plant_code)) ||
    null;
  const identityUnchanged = Boolean(
    previousRun &&
      cutoff &&
      cutoff === previousRun.effective_cutoff_date &&
      year === previousRun.year &&
      month === previousRun.month &&
      !sanitized.plant_mismatch
  );
  let miniLoaded = false;
  let miniPayload = null;
  const loadMini = chatDeps.loadIgfForecastMiniPayload;
  if (typeof loadMini === "function" && chatDeps.pool && cutoff && year && month && plantLabel) {
    try {
      miniPayload = await loadMini(chatDeps.pool, {
        year,
        month,
      ...
      directorIaDebug("[DIRECTOR_IA] forecast_magnitude_followup mini:", e && e.message);
      miniLoaded = true;
    }
  }
  const pack = buildAuthoritativeForecastRunPack({
    ...
    month,
    upload_day: cutoff,
    cutoff_origin: questionCutoff
      ? "EXPLICIT_QUESTION"
      : bodyCutoff
        ? "REQUEST_UPLOAD_DAY"
        : (previousRun && previousRun.cutoff_origin) || "UNAVAILABLE",
    miniPayload,
    mini_loader_invoked: miniLoaded,
  });
  const forecastRun = identityUnchanged
    ? previousRun
    : buildForecastRunIdentity({
        ...
        upload_day: cutoff,
        cutoff,
        source: cutoffSourceForFollowUp(questionCutoff, bodyCutoff, previousRun),
      });
  const magnitudes = forecastMagnitudesFromAuthoritativePack(pack, forecastRun);
  const outgoingRun = pack.status === "AVAILABLE" ? forecastRun : identityUnchanged ? previousRun : forecastRun;
  const state = conversationStateForIntent(
    {
      plantaId: planta_id,
      ...
      forecast_run: outgoingRun,
      forecast_magnitudes: magnitudes,
    },
    echoedState,
    (echoedState && echoedState.parent_intent) || "plant_diagnosis",
    false
  );
  const answer = formatForecastMagnitudeFollowUpAnswer(kind, pack);
  const dateEs = cutoff ? formatForecastCutoffDateEs(cutoff) : null;
  return {
    ok: true,
    answer,
    sources: pack.status === "AVAILABLE" ? ["authoritative_forecast_run_pack"] : [],
    context_meta: {
      mode: "forecast_magnitude_followup",
      semantic_need: "FORECAST_MAGNITUDE_FOLLOWUP",
      prompt_mode: "forecast_magnitude_followup",
      openai_called: false,
      openai_call_count: 0,
      ...
      timestamp: new Date().toISOString(),
      conversation_state: state,
      forecast_run: state.forecast_run,
      forecast_magnitudes: state.forecast_magnitudes,
      authoritative_forecast_status: pack.status,
      authoritative_source: pack.status === "AVAILABLE" ? "computeIgfForecastMiniPayload" : null,
      run_identity: pack.run_identity,
      answered_metric: kind,
      mini_loader_invoked: miniLoaded,
      used_plant_diagnosis: false,
      used_materialidad: false,
      used_arr_legacy: false,
      used_forecast_stored: false,
      computePronosticoProyByPlant: false,
      fallback_used: pack.status !== "AVAILABLE",
      effective_cutoff_date: cutoff || null,
      cutoff_label: dateEs,
    },
  };
}
 ...
 */
function buildDiscountPeriodClarificationResult(opts) {
  const span = opts && opts.clientSpan;
  const incoming = (opts && opts.incomingState) || null;
  const carried = !span && incoming ? incoming.active_entities : null;
  const entities = span
    ? [{ kind: "client", display: span, cliente_key: null, cliente_keys: [] }]
    : Array.isArray(carried)
      ? carried
      : [];
  const state = conversationStateForIntent(
    {
      plantaId: opts.planta_id,
      parent_intent: "client_profile",
      last_evidence_bundle_type: "descuento",
  ...
  const reason = span
    ? `Pediste descuento de un periodo para «${span}». No invento el valor si falta evidencia canónica.`
    : entities[0] && entities[0].display
      ? `Pediste descuento de un periodo. Conservo «${entities[0].display}» como cliente; precisa si el alcance es otro. No invento el valor.`
      : "Pediste descuento de un periodo. Precisa el cliente o el alcance. No invento el valor.";
  return {
    ok: true,
    answer: reason,
    sources: [],
    context_meta: {
      mode: "descuento_period_clarification",
...
function buildHistoricalMarginClientScopeResult(opts) {
  const span = opts.clientSpan;
  const state = conversationStateForIntent(
    {
      plantaId: opts.planta_id,
      parent_intent: "historical_margin",
      last_evidence_bundle_type: "historical_margin",
      active_entities: [{ kind: "client", display: span, cliente_key: null, cliente_keys: [] }],
    },
    (opts && opts.incomingState) || null,
    "historical_margin",
    false
  );
  return {
    ok: true,
    answer:
      `El margen histórico FINAL es de planta, no de un cliente. «${span}» queda anclado como cliente. ` +
      "No comparo meses de planta como si fueran de ese cliente.",
    sources: [],
    context_meta: {
...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  let q = expandQuestionFromChatHistory(rawQuestion, chatHistory);
  if (!q) {
  ...
  const pendingUiEarly = runtime007.sanitizePendingUiAction(echoedState && echoedState.pending_ui_action);
  const skipConversationalForPendingUi =
    Boolean(pendingUiEarly) && runtime007.isUiConfirmQuestion(rawQuestion || q);
  const convo = skipConversationalForPendingUi ? null : classifyConversationalIntent(q);
  if (convo) {
    let plantLabel = "";
    if (convo.mode === "help") {
      plantLabel = await resolvePlantaLabelForChat(planta_id, req);
    }
    let nombre_persona = null;
    if (convo.mode === "smalltalk") {
      const identity = await resolveGreetingIdentity(req, chatDeps.pool);
      nombre_persona = identity.nombre_persona;
    }
    directorIaDebug("[DIRECTOR_IA] conversational_intent:", convo.mode, convo.greeting_kind || null);
    return {
      ok: true,
      answer: buildConversationalAnswer(convo.mode, plantLabel, {
        greeting_kind: convo.greeting_kind,
        nombre_persona,
      }),
      sources: [],
      context_meta: {
  ...
  const planOptions = {};
  const completedPending = pendingCompletion.completePendingFrame(
    echoedState && echoedState.pending_information_gap,
    rawQuestion,
    chatDeps.now
  );
  if (completedPending && completedPending.ok && completedPending.parent_intent) {
    planOptions.forceIntent = completedPending.parent_intent;
    if (completedPending.original_question) {
      const extra = String(rawQuestion || "").trim();
      const base = String(completedPending.original_question).trim();
      q = extra && extra.toLowerCase() !== base.toLowerCase() ? `${base} ${extra}` : base;
    }
  }
  if (continuityTurn.inherit && continuityTurn.inherit_parent_intent) {
    if (continuityTurn.inherit_parent_intent === "daily_sales_deviation") {
      ...
      if (!(typeof isCommercialTrendQuestion === "function" && isCommercialTrendQuestion(q))) {
    ...
    } else if (continuityTurn.inherit_parent_intent === "seh_operation_status") {
      planOptions.forceIntent = "seh_operation_status";
    } else if (continuityTurn.inherit_parent_intent !== "folio_search") {
      planOptions.inheritParentIntent = continuityTurn.inherit_parent_intent;
    }
  } else if (
    resumeItem &&
    resumeItem.parent_intent &&
    resumeItem.parent_intent !== "folio_search" &&
    !continuityTurn.standalone
  ) {
  ...
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "seh_operation_status" &&
    isSehFollowUp(q)
  ) {
    planOptions.forceIntent = "seh_operation_status";
  }
  if (
    !planOptions.forceIntent &&
    (backlog.isExplicitFolioIdQuestion(q) ||
      backlog.isFolioIndexNavigationQuestion(q) ||
      (echoedState &&
        echoedState.parent_intent === "folio_search" &&
        backlog.isFolioResultNavigationQuestion(q, echoedState.active_entities)))
  ) {
    planOptions.forceIntent = "folio_search";
  }
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "folio_search" &&
    (isFolioExistenceFollowUp(q) ||
      (echoedState.folio_search_spec &&
        echoedState.folio_search_spec.period_mode === "ANY" &&
        /\bsolo\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/.test(
          String(q || "")
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
  ...
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "client_movement"
  ) {
    const priorMove = priorMovementFromState(echoedState);
    const domainSwitch = runtime004.isCommercialDomainSwitch(q, {
      family: (priorMove && priorMove.movement === "NUEVOS" && "NEW_CLIENTS") ||
        (priorMove && priorMove.movement === "DEJARON_DE_COMPRAR" && "LOST_CLIENTS") ||
        (priorMove && priorMove.movement === "DISMINUYERON" && "LOST_CLIENTS") ||
        "LOST_CLIENTS",
      movement: priorMove && priorMove.movement,
    });
    if (!domainSwitch && runtime004.isChannelForecastGuardrailQuestion(q)) {
      // keep unforced so guardrail wins
    } else if (
      !domainSwitch &&
      !runtime012.classify012Family(q, echoedState) &&
      (pendingCompletion.isPeriodOnlyAnswer(q, chatDeps.now) ||
        runtime004.shouldInheritMovementContext(q, {
          family: priorMove && priorMove.movement === "NUEVOS" ? "NEW_CLIENTS" : "LOST_CLIENTS",
          movement: priorMove && priorMove.movement,
        }) ||
        runtime005.isContextualNewClientCountQuestion(q) ||
        runtime005.isNewClientDiscountEvidenceQuestion(q) ||
        runtime005.isLostVolumeContextualAggregateQuestion(q) ||
        runtime006.isResultSetPronounReference(q) ||
        backlog.extractChannel(backlog.normalize(q)) !== "ALL" ||
        /\btop\s+\d+\b/.test(backlog.normalize(q)) ||
        backlog.isEntitySetFollowUp(
          q,
          (echoedState.active_entities || []).flatMap((ent) => ent.ranked_names || ent.names || ent.result_set || [])
        ))
    ) {
      planOptions.forceIntent = "client_movement";
    }
  }
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "historical_new_clients" &&
    (runtime005.isContextualNewClientCountQuestion(q) ||
      runtime005.isNewClientDiscountEvidenceQuestion(q) ||
      runtime004.isNewClientDiscountQuestion(q) ||
      runtime004.isNewClientPurchaseTotalQuestion(q))
  ) {
    planOptions.forceIntent = "client_movement";
  }
  const priorForPlan = priorPredictiveFromState(echoedState) || priorMovementFromState(echoedState);
  const priorCategory = priorCategoryFromState(echoedState);
  if (priorForPlan || priorCategory) {
    planOptions.prior = { ...(priorForPlan || {}), ...(priorCategory || {}) };
  }
  if (
    !planOptions.forceIntent &&
    (runtime008.isRegulationStatusQuestion(q, echoedState) ||
      runtime008.isRegulationPlantDetailQuestion(q, echoedState) ||
      (echoedState &&
        echoedState.parent_intent === "seh_regulation" &&
        /^(y\s+)?(en\s+)?(planta|estacion|autotanque|sin\s+estado|vigentes?|vencid|tramite)\b/.test(
          String(q || "")
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
    ...
    planOptions.forceIntent = "seh_regulation";
  }
  if (
    !planOptions.forceIntent &&
    (runtime008.isTallerExpenseQuestion(q) ||
      (echoedState &&
        echoedState.parent_intent === "expense_analytics" &&
        pendingCompletion.isPeriodOnlyAnswer(q, chatDeps.now)))
  ) {
    planOptions.forceIntent = "expense_analytics";
  }
  if (
    !planOptions.forceIntent &&
    echoedState &&
    (echoedState.parent_intent === "client_ranking" || echoedState.parent_intent === "predictive_commercial") &&
    (isPredictiveCommercialQuestion(q, priorPredictiveFromState(echoedState)) ||
      (echoedState.parent_intent === "predictive_commercial" && isPredictiveFollowUp(q)))
  ) {
    planOptions.forceIntent = "predictive_commercial";
  }
  if (
    !planOptions.forceIntent &&
    runtime007.isOpenCategoryMovementQuestion(q, priorCategory)
  ) {
    planOptions.forceIntent = "open_category_movement";
  }
  if (
    !planOptions.forceIntent &&
    runtime007.isCategoryCommissionQuestion(q, priorCategory)
  ) {
    planOptions.forceIntent = "category_commission";
  }
  const rankingParentIntent =
    echoedState &&
    (echoedState.parent_intent === "client_ranking" || echoedState.parent_intent === "client_discount_ranking")
      ? echoedState.parent_intent
      : null;
  const rankingContinuityFollow =
    Boolean(rankingParentIntent) &&
    (isClientRankingFollowUp(q) ||
      backlog.isEntitySetFollowUp(
        q,
        (echoedState.active_entities || []).flatMap((ent) => ent.ranked_names || ent.names || [])
      )) &&
    !inheritBlockedByExplicitMetric(q, rankingParentIntent);
  if (
    !planOptions.forceIntent &&
    runtime006.isIgfDirectMetricQuestion(q) &&
    !rankingContinuityFollow
  ) {
    planOptions.forceIntent = "igf_direct_metric";
  }
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "igf_direct_metric" &&
    /^(y|e)\s+/.test(String(q || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()) &&
    !rankingContinuityFollow
  ) {
    planOptions.forceIntent = "igf_direct_metric";
  }
  if (!planOptions.forceIntent && rankingContinuityFollow) {
    planOptions.forceIntent = rankingParentIntent;
  }
  if (pendingUiEarly && runtime007.isUiConfirmQuestion(rawQuestion || q)) {
    if (pendingUiEarly.type === runtime007.UI_ACTIONS.OPEN_CATEGORY_MOVEMENT) {
      planOptions.forceIntent = "open_category_movement";
    } else if (
      pendingUiEarly.type === runtime007.UI_ACTIONS.OPEN_IGF_PRONOSTICO_MODAL ||
      pendingUiEarly.type === runtime007.UI_ACTIONS.OPEN_PRONOSTICO
    ) {
      planOptions.forceIntent = "open_pronostico";
    }
  }
  const directorIaPlan = planDirectorIaQuestion(q, planOptions);
  if (directorIaPlan.intent === "executive_sales_context") {
    const family012 = runtime012.classify012Family(q, echoedState);
    const family = family012 === "CHANNEL_SALES_STATUS" ? "CHANNEL_SALES_STATUS" : runtime010.classifyExecutiveContextFamily(q) || "SALES_STATUS";
    if (!planta_id) {
      return buildScopeClarificationResult({
        clarification: runtime010.plantClarification(),
        original_question: q,
        parent_intent: "executive_sales_context",
        family,
      });
    }
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    const salesLoaded = await resolve012SalesRows(plantLabel);
    const salesRows = salesLoaded.rows;
    if (family === "CHANNEL_SALES_STATUS") {
      const priorDims = (echoedState && echoedState.active_entities || []).find((e) => e && e.kind === "channel_sales") || {};
      const payload = runtime012.analyzeChannelSales(q, {
        salesRows,
        now: chatDeps.now,
        plant_label: plantLabel,
        prior: { channel: priorDims.channel, subcategory: priorDims.subcategory, period: priorDims.period, plant: plantLabel, family: "CHANNEL_SALES_STATUS" },
      });
      return {
        ok: true,
        answer: runtime012.buildChannelSalesAnswer(payload),
        sources: [salesLoaded.source_mode === "injected_test_rows" ? "injected_test_rows" : "arr.ventas_diarias_cliente"],
        context_meta: {
          mode: "channel_sales_status",
          family,
          source_mode: salesLoaded.source_mode,
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: "executive_sales_context",
              active_subtopic: "channel_sales",
              active_entities: [
                {
                  kind: "channel_sales",
                  display: payload.dims.channel || "CASA",
                  channel: payload.dims.channel,
                  subcategory: payload.dims.subcategory,
                  period: payload.dims.period,
                  operation: payload.dims.operation,
                },
              ],
            },
            echoedState,
            "executive_sales_context",
            false
          ),
        },
      };
    }
    const payload = runtime010.analyzeSalesFamily(family, q, {
      salesRows,
      forecastKg: chatDeps.predictivePlantForecastKg,
      metaKg: chatDeps.executiveMetaKg,
      forecastByMonth: chatDeps.executiveForecastByMonth,
      now: chatDeps.now,
      plant_label: plantLabel,
    });
    return {
      ok: true,
      answer: runtime010.buildExecutiveSalesAnswer(payload),
      sources: [salesLoaded.source_mode === "injected_test_rows" ? "injected_test_rows" : "arr.ventas_diarias_cliente"],
      context_meta: {
        mode: "executive_sales_context",
        family,
        source_mode: salesLoaded.source_mode,
        conversation_state: conversationStateForIntent(
          {
  ...
  if (directorIaPlan.intent === "client_comments") {
    const commentsLoaded = await resolve012Comments(planta_id);
    const filtered = runtime012.filterComments(commentsLoaded.rows, q, echoedState, chatDeps.now);
    return {
      ok: true,
      answer: runtime012.buildCommentsAnswer(filtered.rows, filtered),
      sources: [commentsLoaded.source_mode === "injected_test_rows" ? "injected_test_rows" : "arr.cliente_comentarios"],
      context_meta: {
        mode: "client_comments",
        family: "CLIENT_COMMENTS_PERIOD",
        source_mode: commentsLoaded.source_mode,
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: "client_comments",
            active_subtopic: "client_comments",
            active_entities: [
              {
                kind: "client_comments",
                display: "comentarios",
                period: filtered.period,
                channel: filtered.channel,
                result_set: filtered.rows.map((r) => r.cliente_nombre || r.cliente).filter(Boolean).slice(0, 40),
              },
            ],
          },
          echoedState,
  ...
  if (directorIaPlan.intent === "action_register_recent" || (directorIaPlan.intent === "action_status" && runtime012.classify012Family(q, echoedState) === "ACTION_REGISTER_DIRECT")) {
    const actionsLoaded = await resolve012ActionItems(req, planta_id);
    const items = actionsLoaded.items;
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    const priorAct = (echoedState && echoedState.active_entities || []).find((e) => e && e.kind === "action_register") || {};
    const filtered = runtime012.filterActionsByTheme(items, q, { theme: priorAct.theme || priorAct.display });
    const recent = directorIaPlan.intent === "action_register_recent";
    const answer = recent
      ? runtime012.buildActionRecentAnswer(filtered.items, runtime012.extractLimit(q, priorAct.limit || 10))
      : runtime012.buildActionDirectAnswer(filtered.items, plantLabel, q);
    return {
      ok: true,
      answer,
      sources: [actionsLoaded.source_mode === "injected_test_rows" ? "injected_test_rows" : "arr.action_register_items"],
      context_meta: {
        mode: recent ? "action_register_recent" : "action_register_direct",
        source_mode: actionsLoaded.source_mode,
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: recent ? "action_register_recent" : "action_status",
            active_subtopic: recent ? "action_recent" : "action_direct",
            active_entities: [
              {
                kind: "action_register",
                display: filtered.theme || "Action Register",
                sort: recent ? "RECENT" : null,
                limit: runtime012.extractLimit(q, priorAct.limit || 10),
                result_set: filtered.items.map((it) => it.titulo || it.title).filter(Boolean).slice(0, 40),
              },
            ],
          },
          echoedState,
          recent ? "action_register_recent" : "action_status",
          false
        ),
      },
    };
  }
  if (directorIaPlan.intent === "bitacora_topic_lookup") {
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    const priorBit = (echoedState && echoedState.active_entities || []).find((e) => e && e.kind === "bitacora_topic") || {};
    const topic = runtime012.extractTopic(q) || priorBit.topic;
    const n = runtime012.nq(q);
    const bitLoaded = await resolve012BitacoraEntries(planta_id);
    const entries = bitLoaded.entries;
    const hits = runtime012.searchBitacoraTopic(entries, topic);
    const summarize = /\bresumelo\b/.test(n) || /\bresumelos\b/.test(n);
    return {
      ok: true,
      answer: summarize
        ? runtime012.summarizeBitacoraHits(hits.length ? hits : priorBit.result_set || [], topic)
        : runtime012.buildBitacoraTopicAnswer(hits, topic, plantLabel),
      sources: [bitLoaded.source_mode === "injected_test_rows" ? "injected_test_rows" : "arr.director_ia_bitacora"],
      context_meta: {
        mode: "bitacora_topic_lookup",
        family: "BITACORA_TOPIC_LOOKUP",
        source_mode: bitLoaded.source_mode,
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: "bitacora_topic_lookup",
            active_subtopic: "bitacora_topic",
            active_entities: [
              {
                kind: "bitacora_topic",
                display: topic || "tema",
                topic,
                result_set: hits.map((h) => h.titulo).filter(Boolean).slice(0, 40),
              },
            ],
          },
          echoedState,
  ...
  if (
    directorIaPlan.intent === "client_inactivity" ||
    directorIaPlan.intent === "client_direct_purchase" ||
    directorIaPlan.intent === "open_client_delta_forecast"
  ) {
    const family011 = runtime011.classify011Family(q, echoedState) || directorIaPlan.signals && directorIaPlan.signals[0] && directorIaPlan.signals[0].value;
    let rows = Array.isArray(chatDeps.predictiveDicfRows) ? chatDeps.predictiveDicfRows.slice() : [];
    if ((!rows || !rows.length) && typeof chatDeps.computeDicf === "function") {
      const pack = await chatDeps.computeDicf();
      const buckets = ["aumentaron", "disminuyeron", "dejaron", "nuevos", "clientes"];
      for (const key of buckets) {
        const block = pack && pack[key];
        if (block && Array.isArray(block.clientes)) rows.push(...block.clientes);
        else if (Array.isArray(block)) rows.push(...block);
      }
    }
    if (rows.length) {
      rows = await runtime009.enrichDicfClientsLastPurchase(rows, {
        db: chatDeps.pool,
        plantCodes: chatDeps.predictivePlantCodes,
        now: chatDeps.now,
        dicfCacheRows: chatDeps.dicfCacheRows,
        ventasMaxByClient: chatDeps.ventasMaxByClient,
      });
    }
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    const period =
      (echoedState && echoedState.period) ||
      (req && req.body && req.body.igf_period) ||
      null;
    const selectionQuestion = rawQuestion || q;
    const selectedEarly = runtime011.selectUniqueClientFromPrior(selectionQuestion, echoedState, rows);
    if (selectedEarly && !runtime011.classify011Family(selectionQuestion, echoedState)) {
      return {
        ok: true,
        answer: `${selectedEarly} queda como cliente activo.`,
        sources: [],
        context_meta: {
          ...
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: (echoedState && echoedState.parent_intent) || "client_inactivity",
              active_subtopic: "client_selection",
              active_entities: [
                {
                  kind: "CLIENT",
                  display: selectedEarly,
                  canonical_name: selectedEarly,
                },
              ],
            },
            echoedState,
            (echoedState && echoedState.parent_intent) || "client_inactivity",
            false
          ),
        },
      };
    }
    if (directorIaPlan.intent === "open_client_delta_forecast") {
      const finalRes = runtime011.resolveOpenClientTarget(q, echoedState, rows);
      const answer = runtime011.buildOpenClientAnswer(finalRes, plantLabel);
      const ui_action =
        finalRes.status === "unique" ? runtime011.buildOpenClientUiAction({
          client: finalRes.cliente,
          plant: plantLabel,
          plant_id: planta_id,
          period,
        }) : null;
      return {
        ...
        answer,
        ui_action,
        sources: ["arr.dicf_cliente_mes"],
        context_meta: {
          mode: "open_client_delta_forecast",
          family: family011,
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: "open_client_delta_forecast",
              active_subtopic: "OPEN_CLIENT_DELTA_FORECAST",
              active_entities: finalRes.status === "unique"
                ? [{ kind: "CLIENT", display: finalRes.cliente, canonical_name: finalRes.cliente }]
                : runtime011.preserveResultSetEntities(echoedState),
    ...
    if (directorIaPlan.intent === "client_inactivity") {
      const list = runtime011.listInactiveClients(rows, { now: chatDeps.now, source: "computeDicf" });
      const answer = runtime011.buildInactiveClientsAnswer(list);
      const inherited = runtime010.resolveActiveClient({ conversation_state: echoedState });
      return {
        ok: true,
        answer,
        sources: ["computeDicf", "arr.dicf_cliente_mes", "arr.ventas_diarias_cliente"],
        context_meta: {
          mode: "client_inactivity",
          family: "INACTIVE_CLIENTS",
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: "client_inactivity",
              active_subtopic: "INACTIVE_CLIENTS",
              active_entities: runtime011.activeEntitiesForInactiveList(list, inherited),
            },
            echoedState,
            "client_inactivity",
            false
    ...
    const hint = runtime011.extractClientHint(q, echoedState, rows);
    const named = String(q || "").match(/\b([A-ZÁÉÍÓÚÑ0-9][A-ZÁÉÍÓÚÑ0-9 .'-]{2,})\b/);
    const clientHint =
      hint ||
      (named && !runtime010.isNonPlantBusinessToken(named[1]) ? named[1] : null) ||
      (runtime010.resolveActiveClient({ conversation_state: echoedState }) || {}).canonical_name;
    if (!clientHint) {
      return {
        ok: true,
        ...
        sources: [],
        context_meta: {
          mode: "client_direct_purchase",
          requires_clarification: true,
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: "client_direct_purchase",
              pending_information_gap: pendingCompletion.buildPendingGap({
                parent_intent: "client_direct_purchase",
                missing_fields: ["entity"],
                frame: { family: family011 },
                original_question: q,
                why_blocks: "Falta el cliente para la operación directa.",
              }),
            },
            echoedState,
            "client_direct_purchase",
            false
          ),
        },
      };
    }
    const resolved = runtime011.resolveClientInCatalog(clientHint, rows);
    const row =
      (resolved.status === "unique" && resolved.row) ||
      rows.find((r) => runtime009.nclient(r.cliente) === runtime009.nclient(clientHint)) ||
      { cliente: clientHint };
    const enriched = (await runtime009.enrichDicfClientsLastPurchase([row], {
      db: chatDeps.pool,
      plantCodes: chatDeps.predictivePlantCodes,
      now: chatDeps.now,
      dicfCacheRows: chatDeps.dicfCacheRows,
    ...
    const answer = runtime011.buildDirectClientAnswer(enriched, q, chatDeps.now);
    return {
      ok: true,
      answer,
      sources: ["computeDicf", "arr.dicf_cliente_mes", "arr.ventas_diarias_cliente"],
        ...
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: "client_direct_purchase",
            active_subtopic: family011,
            active_entities: [
              {
                kind: "CLIENT",
                display: enriched.cliente,
                canonical_name: enriched.cliente,
              },
            ],
          },
          echoedState,
  ...
  const selectedFromList = runtime011.selectUniqueClientFromPrior(
    q,
    echoedState,
    Array.isArray(chatDeps.predictiveDicfRows) ? chatDeps.predictiveDicfRows : []
  );
  if (selectedFromList) {
    return {
      ok: true,
      answer: `${selectedFromList} queda como cliente activo.`,
      sources: [],
      context_meta: {
        mode: "client_selection",
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: (echoedState && echoedState.parent_intent) || "client_inactivity",
            active_subtopic: "CLIENT",
            active_entities: [
              {
                kind: "CLIENT",
                display: selectedFromList,
                canonical_name: selectedFromList,
              },
            ],
          },
          echoedState,
  ...
  if (shouldHandleDiagnosisObservationRisk(executiveNeed, continuityTurn, directorIaPlan.intent)) {
    directorIaDebug("[DIRECTOR_IA] executive_diagnosis observation/deviation/risk");
    return handleExecutiveDiagnosisForChat({
      req,
      uiPlantaId: planta_id,
      question: q,
      pool: chatDeps.pool,
      plantCatalog: chatDeps.plantCatalog,
      composeExecutiveCycle: chatDeps.composeExecutiveCycle,
      composeOpts: {
        question: q,
  ...
  const sanitizedForMagnitude = sanitizeEchoedState(echoedState, planta_id);
  const magnitudeFollowUp = classifyForecastMagnitudeFollowUp(rawQuestion, {
    has_authoritative_run: Boolean(
      sanitizedForMagnitude.forecast_run && sanitizedForMagnitude.forecast_run.effective_cutoff_date
    ),
    executive_hilo:
      sanitizedForMagnitude.parent_intent === "plant_diagnosis" ||
      sanitizedForMagnitude.last_evidence_bundle_type === "plant_diagnosis",
  });
  if (magnitudeFollowUp && directorIaPlan.intent !== "igf_status") {
    directorIaDebug("[DIRECTOR_IA] forecast_magnitude_followup from authoritative run");
    return handleForecastMagnitudeFollowUpForChat({
      req,
      planta_id,
      echoedState,
      question: rawQuestion,
      kind: magnitudeFollowUp.kind,
    });
  }
  const incomingFolioState = sanitizeEchoedState(echoedState, planta_id);
  const folioAggregateFollowUp =
    incomingFolioState.parent_intent === "folio_search" &&
    isValidFolioSearchSpec(incomingFolioState.folio_search_spec, planta_id) &&
    isFolioSearchAggregateFollowUp(rawQuestion);
  if (folioAggregateFollowUp) {
    directorIaDebug("[DIRECTOR_IA] folio_search aggregate follow-up");
    const loadFn = chatDeps.loadFolioSearchForChat || loadFolioSearchForChat;
    const folioSearchPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: rawQuestion,
      now: chatDeps.now,
      queryPublicFolios: chatDeps.queryPublicFolios,
      resolveEquivalentIds: chatDeps.resolveEquivalentIds,
      inheritedFilters: incomingFolioState.folio_search_spec,
      analysisOverride: {
    ...
    if (folioSearchPayload && folioSearchPayload.ok === false && folioSearchPayload.status) {
      return {
        ok: false,
        status: folioSearchPayload.status,
        code: folioSearchPayload.code,
        error: folioSearchPayload.error || "No se pudo consultar folios",
      };
    }
    return buildFolioSearchChatResult(folioSearchPayload, { planta_id });
  }
  ...
  if (continuityTurn.inherit && continuityTurn.inherit_parent_intent === "profitability_deterioro_snapshot") {
    const incoming = sanitizeEchoedState(echoedState, planta_id);
    const resolved = resolveProfitabilitySubtopicTurn({
      parent_intent: continuityTurn.inherit_parent_intent,
      active_subtopic: incoming.active_subtopic,
      question: rawQuestion || q,
    });
    if (resolved.handled) {
      directorIaDebug("[DIRECTOR_IA] profitability_subtopic_followup in-process", resolved.kind, resolved.focus);
      const plantLabel =
        (req && req.body && req.body.planta_nombre) || (await resolvePlantaLabelForChat(planta_id, req));
      let expenseCompare = null;
      let resolvedKind = resolved.kind;
      if (resolved.kind === "probe_unavailable") {
        try {
          const kpis = await loadInheritedExpenseKpis(incoming, plantLabel, plantLabel, req);
          expenseCompare = buildExpenseCompare(kpis && kpis.kpiA, kpis && kpis.kpiB, resolved.focus);
          if (expenseCompare) resolvedKind = "probe_compare";
        } catch (e) {
          directorIaDebug("[DIRECTOR_IA] profitability expense compare load error:", e && e.message);
        }
      }
      const follow =
        resolvedKind === "open_branch" || (resolvedKind === "stay" && resolved.focus === "expense")
          ? buildProfitabilityExpenseFollowUpChatResult({
              planta_id,
              planta_nombre: plantLabel,
              active_period_months: incoming.active_period_months,
            })
          : buildProfitabilitySubtopicFollowUpChatResult({
              planta_id,
              planta_nombre: plantLabel,
              active_period_months: incoming.active_period_months,
              next_subtopic: resolved.next_subtopic,
              focus: resolved.focus,
              kind: resolvedKind,
              expense_compare: expenseCompare,
            });
      const snapState = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "profitability_deterioro_snapshot",
          last_evidence_bundle_type: incoming.last_evidence_bundle_type || "profitability_deterioro_snapshot",
          active_period_months: incoming.active_period_months,
          active_subtopic: resolved.next_subtopic || null,
        },
        echoedState,
        "profitability_deterioro_snapshot",
        Boolean(continuityTurn.restore_previous)
      );
      if (follow && follow.context_meta) {
        follow.context_meta.conversation_state = snapState;
        follow.context_meta.inherit = true;
        follow.context_meta.active_subtopic = resolved.next_subtopic || null;
        follow.context_meta.subtopic_focus = resolved.focus || null;
      }
      return follow;
    }
  }
  ...
  if (directorIaPlan.intent === "seh_operation_status") {
    directorIaDebug("[DIRECTOR_IA] seh_operation_status in-process");
    const loadFn = chatDeps.loadSehOperationStatusForChat || loadSehOperationStatusForChat;
    const sehPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      querySehEquipos: chatDeps.querySehEquipos,
      items: chatDeps.sehItems,
      plant_label: req && req.body && req.body.planta_nombre,
      priorSpec: priorSpecFromState(echoedState),
      resolvePlantByNombre: chatDeps.resolveSehPlantByNombre || chatDeps.resolveHistoricalMarginPlantByNombre,
      plantCatalog: chatDeps.sehPlantCatalog,
    });
    if (sehPayload && sehPayload.ok === false && sehPayload.status) {
      return {
        ok: false,
        status: sehPayload.status,
        code: sehPayload.code,
        error: sehPayload.error || "No se pudo consultar SEH",
      };
    }
    const resolvedSehPlant = sehPayload && sehPayload.planta_id != null ? sehPayload.planta_id : planta_id;
    const sehResult = buildSehOperationStatusChatResult(sehPayload, { planta_id: resolvedSehPlant });
    if (sehResult && sehResult.context_meta) {
      sehResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: resolvedSehPlant,
          parent_intent: "seh_operation_status",
          active_subtopic: sehPayload && sehPayload.spec && sehPayload.spec.scope,
          active_entities: sehResult.context_meta.conversation_state.active_entities,
        },
        echoedState,
  ...
  if (directorIaPlan.intent === "seh_regulation") {
    directorIaDebug("[DIRECTOR_IA] seh_regulation in-process");
    const loadReg = chatDeps.loadSehRegulationForChat || runtime008.loadSehRegulationForChat;
    const sehReg = await loadReg(chatDeps.pool, planta_id, req, {
      question: q,
      prior: echoedState,
      plant_label: req && req.body && req.body.planta_nombre,
      resolvePlantByNombre: chatDeps.resolveSehPlantByNombre || chatDeps.resolveHistoricalMarginPlantByNombre,
      plantCatalog: chatDeps.sehPlantCatalog,
      equipoRows: chatDeps.sehEquipoRows,
      carpetaRows: chatDeps.sehCarpetaRows,
    });
    if (sehReg && sehReg.ok === false && sehReg.status) {
      return {
        ok: false,
        status: sehReg.status,
        code: sehReg.code,
        error: sehReg.error || "No se pudo consultar SEH",
      };
    }
    if (sehReg && sehReg.clarification) {
      return {
        ok: true,
        answer: sehReg.clarification,
        sources: [],
        context_meta: {
          mode: "seh_regulation",
          openai_called: false,
          ...
          planta_id,
          conversation_state: conversationStateForIntent(
            { plantaId: planta_id, parent_intent: "seh_regulation", active_subtopic: sehReg.family },
            echoedState,
            "seh_regulation",
            false
          ),
    ...
    const family = sehReg && sehReg.family;
    const answer =
      family === "REGULATION_PLANT_DETAIL"
        ? runtime008.buildLegalDetailAnswer(sehReg.carpetas, q, sehReg.plant_label)
        : runtime008.buildSehRegulationAnswer(sehReg);
    const resolvedRegPlant = sehReg && sehReg.plant_id != null ? sehReg.plant_id : planta_id;
    return {
      ok: true,
      answer,
      sources: ["GET /api/seh/cumplimiento", "public.seh_equipos", "public.seh_carpetas_legales"],
      context_meta: {
        mode: "seh_regulation",
        openai_called: false,
        requested_domain: "seh_regulation",
        family,
        planta_id: resolvedRegPlant,
        conversation_state: conversationStateForIntent(
          {
            plantaId: resolvedRegPlant,
            parent_intent: "seh_regulation",
            active_subtopic: family,
          },
          echoedState,
          "seh_regulation",
  ...
  if (directorIaPlan.intent === "client_movement") {
    directorIaDebug("[DIRECTOR_IA] client_movement in-process");
    const priorMove = priorMovementFromState(echoedState);
    const entityNames = priorMove && priorMove.ranked_names ? priorMove.ranked_names : [];
    const nFollow = backlog.normalize(q);
    const recomputeMovementFollowUp =
      runtime004.shouldInheritMovementContext(q, {
        family: priorMove && priorMove.movement === "NUEVOS" ? "NEW_CLIENTS" : "LOST_CLIENTS",
        movement: priorMove && priorMove.movement,
      }) ||
      /\bperdimos\b/.test(nFollow) ||
      /\bpor\s+ellos\b/.test(nFollow) ||
      runtime006.isResultSetPronounReference(q) ||
      /\btoneladas\b/.test(nFollow) ||
      /\bdescuento/.test(nFollow) ||
      /\bcomisionistas?\b/.test(nFollow) ||
      /\bmas\s+importante\b/.test(nFollow);
    if (entityNames.length && backlog.isEntitySetFollowUp(q, entityNames) && !recomputeMovementFollowUp) {
      return {
        ok: true,
        answer: buildEntitySetFollowUpAnswer(entityNames, q),
        sources: ["arr.ventas_diarias_cliente"],
        context_meta: {
          conversation_state: conversationStateForIntent(
            {
              plantaId: planta_id,
              parent_intent: "client_movement",
              active_subtopic: "entity_set",
              active_entities: echoedState && echoedState.active_entities ? echoedState.active_entities : [],
            },
            echoedState,
            "client_movement",
            false
          ),
          scoped_entities: entityNames,
        },
      };
    }
    const loadMove = chatDeps.loadClientMovementForChat || loadClientMovementForChat;
    const movementPayload = await loadMove(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      plant_label: req && req.body && req.body.planta_nombre,
      selectedPeriod:
        (completedPending && completedPending.patch && (completedPending.patch.period_month || completedPending.patch.period)) ||
        (req && req.body && req.body.arr_period),
      priorSpec:
        completedPending && completedPending.parent_intent === "client_movement"
          ? {
              ok: true,
              ...(priorMove || {}),
              ...completedPending.frame,
              period:
                completedPending.frame.period_month ||
                (completedPending.frame.period && !String(completedPending.frame.period).includes("..")
                  ? completedPending.frame.period
                  : priorMove && priorMove.period),
              movement: completedPending.frame.movement || (priorMove && priorMove.movement),
              metric: completedPending.frame.metric || (priorMove && priorMove.metric),
              direction: completedPending.frame.direction || (priorMove && priorMove.direction),
              limit: completedPending.frame.limit || (priorMove && priorMove.limit),
              channel: completedPending.frame.channel || (priorMove && priorMove.channel),
              subcategory: completedPending.frame.subcategory || (priorMove && priorMove.subcategory) || null,
              want_aggregate: completedPending.frame.want_aggregate || (priorMove && priorMove.want_aggregate),
              want_discount: completedPending.frame.want_discount || (priorMove && priorMove.want_discount),
              want_count: completedPending.frame.want_count || (priorMove && priorMove.want_count),
              pending_operation: completedPending.frame.pending_operation || (priorMove && priorMove.pending_operation),
              pending_family: completedPending.frame.pending_family || (priorMove && priorMove.pending_family),
              pending_intent: completedPending.frame.pending_intent || (priorMove && priorMove.pending_intent) || "client_movement",
              result_set_family: completedPending.frame.result_set_family || (priorMove && priorMove.result_set_family),
              result_set_period: completedPending.frame.result_set_period || (priorMove && priorMove.result_set_period),
              result_set_ids: completedPending.frame.result_set_ids || (priorMove && priorMove.result_set_ids),
              result_set_operation: completedPending.frame.result_set_operation || (priorMove && priorMove.result_set_operation),
            }
          : priorMove,
      movementRows: chatDeps.clientMovementRows,
      computeClientesDescuentoMes: chatDeps.computeClientesDescuentoMes,
      arrPlantCode: chatDeps.arrPlantCode,
    });
    if (movementPayload && movementPayload.ok === false && movementPayload.status) {
      return {
        ok: false,
        status: movementPayload.status,
        code: movementPayload.code,
        error: movementPayload.error || "No se pudo consultar el movimiento de clientes",
      };
    }
    const movePlant = movementPayload && movementPayload.planta_id != null ? movementPayload.planta_id : planta_id;
    const moveResult = buildClientMovementChatResult(movementPayload, { planta_id: movePlant });
    if (
      moveResult &&
      runtime011.isInactiveClientsQuestion(q) &&
      !runtime011.isVolumeOrShareQuestion(runtime011.nq(q))
    ) {
      const moveRows = (movementPayload && (movementPayload.rows || movementPayload.clients)) || [];
      const mapped = moveRows.map((r) => ({
        cliente: r.cliente || r.display,
        lastPurchaseDate: r.lastPurchaseDate || r.last_date,
        freqDays: r.freqDays,
        movement: r.movement || "DEJARON_DE_COMPRAR",
      }));
      const list = runtime011.listInactiveClients(mapped.length ? mapped : moveRows, {
        now: chatDeps.now,
        source: "commercial_state",
      });
      if (list.length || mapped.length) {
        const fallback = (mapped.length ? mapped : moveRows).map((r) => ({
          cliente: r.cliente,
          lastPurchaseDate: r.lastPurchaseDate || null,
          days_since_last_purchase: r.lastPurchaseDate
            ? runtime011.daysBetween(r.lastPurchaseDate, chatDeps.now)
            : null,
          freqDays: r.freqDays || null,
        }));
        moveResult.answer = runtime011.buildInactiveClientsAnswer(list.length ? list : fallback);
        const inheritedMove = runtime010.resolveActiveClient({ conversation_state: echoedState });
        moveResult.context_meta.conversation_state.active_entities =
          runtime011.activeEntitiesForInactiveList(list.length ? list : fallback, inheritedMove);
      }
    }
    if (moveResult && moveResult.context_meta) {
      moveResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: movePlant,
          parent_intent: "client_movement",
          active_subtopic: "client_movement",
          active_entities: moveResult.context_meta.conversation_state.active_entities,
          pending_information_gap:
            moveResult.context_meta.conversation_state.pending_information_gap || null,
        },
        echoedState,
        "client_movement",
        false
  ...
  if (directorIaPlan.intent === "predictive_commercial") {
    directorIaDebug("[DIRECTOR_IA] predictive_commercial in-process");
    const priorPred = priorPredictiveFromState(echoedState) || priorRankingFromState(echoedState);
    const predictivePayload = await (chatDeps.loadPredictiveForChat || loadPredictiveForChat)(
      chatDeps.pool,
      planta_id,
      req,
      {
        ...
        plant_label: req && req.body && req.body.planta_nombre,
        priorSpec:
          completedPending && completedPending.parent_intent === "predictive_commercial"
            ? {
                ok: true,
                ...(priorPred || {}),
                ...completedPending.frame,
                period:
                  ...
                  (completedPending.frame.period && !String(completedPending.frame.period).includes("..")
                    ? completedPending.frame.period
                    : priorPred && priorPred.period),
                family: completedPending.frame.family || (priorPred && priorPred.family),
                channel: completedPending.frame.channel || (priorPred && priorPred.channel),
                limit: completedPending.frame.limit || (priorPred && priorPred.limit),
                denominator: completedPending.frame.denominator || (priorPred && priorPred.denominator),
                data_semantics: completedPending.frame.data_semantics || (priorPred && priorPred.data_semantics),
              }
            : priorPred && (priorPred.ok || priorPred.period || priorPred.family || priorPred.channel)
              ? { ok: true, ...priorPred }
              : null,
        salesRows: chatDeps.predictiveSalesRows || chatDeps.clientRankingSalesRows,
        priorRows: chatDeps.predictivePriorRows,
        plantForecastKg: chatDeps.predictivePlantForecastKg,
        movementPack: chatDeps.predictiveMovementPack,
        dicfRows: chatDeps.predictiveDicfRows,
        computeDicf: chatDeps.computeDicf,
        dicfCacheRows: chatDeps.dicfCacheRows,
        ventasMaxByClient: chatDeps.ventasMaxByClient,
        conversation_state: echoedState,
        priorClient: priorPred && (priorPred.canonical_name || (priorPred.ranked_names && priorPred.ranked_names[0])),
        maxFecha: chatDeps.predictiveMaxFecha,
        plantCodes: chatDeps.predictivePlantCodes || chatDeps.clientRankingPlantCodes,
        queryMonthlySales: chatDeps.queryPredictiveSales || chatDeps.queryClientRankingSales,
        computeClientesDescuentoMes: chatDeps.computeClientesDescuentoMes,
        arrPlantCode: chatDeps.arrPlantCode,
      }
    );
    if (predictivePayload && predictivePayload.ok === false && predictivePayload.status) {
      return {
        ok: false,
        status: predictivePayload.status,
        code: predictivePayload.code,
        error: predictivePayload.error || "No se pudo consultar la lectura comercial",
      };
    }
    const predPlant = predictivePayload && predictivePayload.planta_id != null ? predictivePayload.planta_id : planta_id;
    const predResult = buildPredictiveChatResult(predictivePayload, { planta_id: predPlant });
    if (predResult && predResult.context_meta) {
      predResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: predPlant,
          parent_intent: "predictive_commercial",
          active_subtopic: predictivePayload && predictivePayload.family,
          active_entities: predResult.context_meta.conversation_state.active_entities,
          pending_information_gap:
            predResult.context_meta.conversation_state.pending_information_gap || null,
        },
        echoedState,
  ...
  if (directorIaPlan.intent === "client_ranking") {
    directorIaDebug("[DIRECTOR_IA] client_ranking in-process");
    const priorRank = priorRankingFromState(echoedState);
    const entityNames = priorRank && priorRank.ranked_names ? priorRank.ranked_names : [];
    if (entityNames.length && backlog.isEntitySetFollowUp(q, entityNames)) {
      return {
        ok: true,
        answer: buildEntitySetFollowUpAnswer(entityNames, q),
        sources: ["arr.ventas_diarias_cliente"],
    ...
    const loadFn = chatDeps.loadClientRankingForChat || loadClientRankingForChat;
    const rankingPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      plant_label: req && req.body && req.body.planta_nombre,
      selectedPeriod: req && req.body && req.body.arr_period,
      priorSpec:
        completedPending && completedPending.parent_intent === "client_ranking"
          ? {
              ok: true,
              ...(priorRank || {}),
              ...completedPending.frame,
              period_kind:
                completedPending.frame.period_kind ||
                (completedPending.frame.period_mode === "RANGE" ||
                (completedPending.frame.period_start && completedPending.frame.period_end)
                  ? "RANGE"
                  : "SINGLE"),
              period_start: completedPending.frame.period_start || null,
              period_end: completedPending.frame.period_end || null,
              period:
                completedPending.frame.period_month ||
                (completedPending.frame.period && !String(completedPending.frame.period).includes("..")
                  ? completedPending.frame.period
                  : priorRank && priorRank.period),
            }
          : priorRank,
      resolvePlantByNombre: chatDeps.resolveClientRankingPlantByNombre || chatDeps.resolveHistoricalMarginPlantByNombre,
      plantCatalog: chatDeps.clientRankingPlantCatalog,
      salesRows: chatDeps.clientRankingSalesRows,
      lastSafeCutSalesRows: chatDeps.clientRankingLastSafeCutSalesRows,
      lastSafeCutSalesRowsByPeriod: chatDeps.clientRankingLastSafeCutSalesRowsByPeriod,
      lastSafeCutPeriod: chatDeps.clientRankingLastSafeCutPeriod,
      uniformProjectionFactor: chatDeps.clientRankingUniformProjectionFactor,
      plantCodes: chatDeps.clientRankingPlantCodes,
      queryMonthlySales: chatDeps.queryClientRankingSales,
      resolvePlantCodes: chatDeps.resolveClientRankingPlantCodes,
      discountByClient: chatDeps.clientRankingDiscountByClient,
    });
    if (rankingPayload && rankingPayload.ok === false && rankingPayload.status) {
      return {
        ok: false,
        status: rankingPayload.status,
        code: rankingPayload.code,
        error: rankingPayload.error || "No se pudo consultar el ranking de clientes",
      };
    }
    const rankingPlant = rankingPayload && rankingPayload.planta_id != null ? rankingPayload.planta_id : planta_id;
    const rankingResult = buildClientRankingChatResult(rankingPayload, { planta_id: rankingPlant });
    if (rankingResult && rankingResult.context_meta) {
      rankingResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: rankingPlant,
          parent_intent: "client_ranking",
          active_subtopic: "client_ranking",
          active_entities: rankingResult.context_meta.conversation_state.active_entities,
          pending_information_gap:
            rankingResult.context_meta.conversation_state.pending_information_gap || null,
        },
        echoedState,
        "client_ranking",
        false
  ...
  if (directorIaPlan.intent === "client_discount_ranking") {
    const loadFn = chatDeps.loadClientRankingForChat || loadClientRankingForChat;
    const rankingPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      ...
      selectedPeriod: req && req.body && req.body.arr_period,
      priorSpec:
        completedPending && completedPending.parent_intent === "client_discount_ranking"
          ? {
              ok: true,
              ...(priorRankingFromState(echoedState) || {}),
              ...completedPending.frame,
              metric: "DISCOUNT",
      ...
      plantCatalog: chatDeps.clientRankingPlantCatalog,
      plantCodes: chatDeps.clientRankingPlantCodes,
      queryMonthlyDiscount: chatDeps.queryClientRankingDiscount || chatDeps.queryMonthCloseDiscount,
      resolvePlantCodes: chatDeps.resolveClientRankingPlantCodes,
      discountRows: chatDeps.clientRankingDiscountRows,
      lastSafeCutDiscountRows: chatDeps.clientRankingLastSafeCutDiscountRows,
      lastSafeCutDiscountRowsByPeriod: chatDeps.clientRankingLastSafeCutDiscountRowsByPeriod,
      lastSafeCutPeriod: chatDeps.clientRankingLastSafeCutPeriod,
      forceDiscount: true,
    });
    if (rankingPayload && rankingPayload.ok === false && rankingPayload.status) {
      return {
        ...
        status: rankingPayload.status,
        code: rankingPayload.code,
        error: rankingPayload.error || "No se pudo consultar el ranking de descuento",
      };
    }
    const rankingPlant = rankingPayload && rankingPayload.planta_id != null ? rankingPayload.planta_id : planta_id;
    const rankingResult = buildClientRankingChatResult(rankingPayload, { planta_id: rankingPlant });
  ...
  if (directorIaPlan.intent === "client_contact_lookup") {
    const spec = backlog.extractClientContactLookupSpec(q);
    let answer =
      "Necesito un cliente inequívoco y una planta autorizada para buscar contacto. No cruzo plantas ni invento teléfono/correo.";
    if (spec.ok && spec.name_hint && chatDeps.pool) {
      try {
        const found = await searchClienteContactosByPlant(chatDeps.pool, {
          planta_id,
          nombre_hint: spec.name_hint,
        });
        const matches = (found && found.matches) || [];
        if (matches.length > 1) {
          answer = `Hay ${matches.length} clientes en la planta autorizada que coinciden con "${spec.name_hint}": ${matches
            .map((row) => row.cliente_nombre)
            .join(", ")}. Indica el nombre completo. No cruzo plantas.`;
        } else if (matches.length === 1) {
          const row = matches[0];
          const bits = [];
          if (spec.field === "telefono" || spec.field === "contacto") {
            bits.push(row.telefono ? `teléfono ${row.telefono}` : "teléfono no registrado");
          }
          if (spec.field === "correo" || spec.field === "contacto") {
            bits.push(row.correo ? `correo ${row.correo}` : "correo no registrado");
          }
          if (row.nombre_contacto) bits.push(`contacto ${row.nombre_contacto}`);
          answer = `${row.cliente_nombre}: ${bits.join("; ")}. Fuente arr.cliente_contactos de la planta autorizada. No invento datos vacíos.`;
        } else {
          answer = `No hay contacto registrado para "${spec.name_hint}" en la planta autorizada. No invento teléfono/correo ni busco en otra planta.`;
        }
      } catch (e) {
        answer =
          "No pude leer arr.cliente_contactos. No invento teléfono/correo. Si el nombre es ambiguo, hay que aclararlo dentro de la misma planta.";
      }
    }
    return {
      ok: true,
      answer,
      sources: ["arr.cliente_contactos"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "client_contact_lookup" },
          echoedState,
          "client_contact_lookup",
          false
        ),
        spec,
      },
  ...
  if (directorIaPlan.intent === "profitability_period_comparison") {
    return {
      ok: true,
      answer:
        "Compararé el periodo heredado contra el actual con verdad ARR/IGF. Etiqueto FINAL/FORECAST/OPEN. No invento causalidad: comentarios son declaraciones, no causa demostrada. Conservo planta y métrica compatibles; solo reemplazo la dimensión pedida.",
      sources: ["igf", "arr"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "profitability_period_comparison" },
          echoedState,
          "profitability_period_comparison",
          false
        ),
      },
    };
  ...
  if (directorIaPlan.intent === "profitability_period_ranking") {
    const spec = backlog.extractProfitabilityPeriodRankingSpec(q);
    return {
      ok: true,
      answer:
        "Compararé meses históricos comparables/cerrados de esa planta. El forecast del mes abierto no se presenta como el mejor mes. Si no hay meses FINAL suficientes, lo declaro.",
      sources: ["igf", "arr"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "profitability_period_ranking" },
          echoedState,
          "profitability_period_ranking",
          false
        ),
        spec,
      },
  ...
  if (directorIaPlan.intent === "plant_metric_comparison") {
    const spec = backlog.extractPlantComparisonSpec(q);
    return {
      ok: true,
      answer:
        spec.ok && spec.metric === "DISCOUNT"
          ? "Comparación de descuento a nivel planta (sin pedir cliente). Si un lado es FORECAST y el otro FINAL, etiqueto cada dato y no los presento como dos cierres."
          : "Comparación de margen a nivel planta. Forecast y cierre se etiquetan por separado.",
      sources: ["arr", "igf"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "plant_metric_comparison" },
          echoedState,
          "plant_metric_comparison",
          false
        ),
        spec,
      },
  ...
  if (directorIaPlan.intent === "profitability_action_plan") {
    return {
      ok: true,
      answer:
        "Plan ejecutivo a partir de evidencia existente: resultado actual, recuperación comercial, clientes en riesgo, clientes nuevos, cobertura de acciones, oportunidades de costo y escenarios. Separo HECHO, DESVIACION, OPORTUNIDAD, ESCENARIO, ACCION_EXISTENTE y HUECO_DE_ACCION. No afirmo que recuperar un cliente garantiza un monto.",
      sources: ["igf", "arr", "dicf", "action_register"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "profitability_action_plan" },
          echoedState,
          "profitability_action_plan",
          false
        ),
      },
    };
  ...
  if (directorIaPlan.intent === "open_pronostico") {
    const spec = extractOpenPronosticoSpec(q);
    const pendingPronostico = runtime007.sanitizePendingUiAction(echoedState && echoedState.pending_ui_action);
    const plant =
      (spec && spec.plant) ||
      (pendingPronostico && pendingPronostico.plant) ||
      (req && req.body && req.body.planta_nombre) ||
      null;
    const actionType =
      pendingPronostico && pendingPronostico.type === runtime007.UI_ACTIONS.OPEN_IGF_PRONOSTICO_MODAL
        ? runtime007.UI_ACTIONS.OPEN_IGF_PRONOSTICO_MODAL
        : runtime007.UI_ACTIONS.OPEN_PRONOSTICO;
    return {
      ok: true,
      answer: plant
        ? `Abro el modal Pronóstico de ${plant}.`
        : "Abro el modal Pronóstico de la planta autorizada.",
      ui_action: {
        type: actionType,
        plant: plant || null,
        year: pendingPronostico && pendingPronostico.year,
        month: pendingPronostico && pendingPronostico.month,
        do_not_pretend_opened: false,
      },
      sources: ["igf"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "open_pronostico", pending_ui_action: null },
          echoedState,
          "open_pronostico",
          false
        ),
  ...
  if (directorIaPlan.intent === "executive_coverage") {
    const family = classifyExecutiveFamily(q);
    const limitation = limitationForFamily(family);
    return {
      ok: true,
      answer:
        limitation ||
        (family
          ? `Reconocí la pregunta como ${family}. No invento la cifra si falta la fuente contractual.`
          : "Reconocí una pregunta ejecutiva, pero no tengo fuente contractual para responderla con evidencia."),
      sources: [],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "executive_coverage", active_subtopic: family },
          echoedState,
          "executive_coverage",
          false
        ),
        family,
        no_data_policy: limitation ? "SPECIFIC_LIMITATION" : "NO_SOURCE",
      },
    };
  }

  if (directorIaPlan.intent === "open_daily_sales_view") {
    const spec = backlog.extractOpenDailySalesViewSpec(q);
    const plant = (spec && spec.plant) || (req && req.body && req.body.planta_nombre) || null;
    const pendingPronostico = runtime007.pendingUiAction(runtime007.UI_ACTIONS.OPEN_IGF_PRONOSTICO_MODAL, {
      plant,
    });
    if (runtime007.isSoftDailySalesAsk(q) && !runtime007.isImperativeDailySalesOpen(q)) {
      return {
        ok: true,
        answer: "¿Quieres abrir la tabla de Pronóstico?",
        sources: ["igf"],
        context_meta: {
          conversation_state: conversationStateForIntent(
            {
              ...
              pending_ui_action: pendingPronostico,
            },
            echoedState,
          ...
          spec,
          offered_ui_action: pendingPronostico,
        },
      };
    }
    return {
      ...
      answer: plant
        ? `Abro el modal Pronóstico de ${plant}.`
        : "Abro el modal Pronóstico.",
      ui_action: {
        type: runtime007.UI_ACTIONS.OPEN_IGF_PRONOSTICO_MODAL,
        plant: plant || null,
        do_not_pretend_opened: false,
      },
      sources: ["igf"],
      context_meta: {
        conversation_state: conversationStateForIntent(
          { plantaId: planta_id, parent_intent: "open_daily_sales_view", pending_ui_action: null },
          echoedState,
          "open_daily_sales_view",
          false
        ),
  ...
  if (directorIaPlan.intent === "category_commission") {
    directorIaDebug("[DIRECTOR_IA] category_commission in-process");
    const priorCat = priorCategoryFromState(echoedState);
    const plantLabel = (req && req.body && req.body.planta_nombre) || null;
    const loadFn = chatDeps.loadCategoryCommissionForChat || runtime007.loadCategoryCommissionForChat;
    let loaded;
    try {
      loaded = await loadFn(chatDeps.pool, planta_id, req, {
        question: q,
        prior: priorCat,
        now: chatDeps.now,
        categoryRows: chatDeps.categoryCommissionRows,
        plant_label: plantLabel,
        arrPlantCode: chatDeps.arrPlantCode,
        db: chatDeps.pool,
      });
    } catch (e) {
      directorIaDebug("[DIRECTOR_IA] category_commission load error:", e && e.message);
      loaded = { ok: false, pack: null, category: priorCat && priorCat.category };
    }
    const category = (loaded && loaded.category) || (priorCat && priorCat.category) || "CASA";
    const answer = runtime007.buildCategoryCommissionAnswer(loaded && loaded.pack, {
      subcategory: loaded && loaded.subcategory,
      period: loaded && loaded.period,
      period_label: loaded && loaded.period_label,
    });
    const pending = runtime007.pendingUiAction(runtime007.UI_ACTIONS.OPEN_CATEGORY_MOVEMENT, {
      category,
      plant: plantLabel,
    });
    return {
      ok: true,
      answer,
      sources: ["arr.clientes_mes", "movimiento_por_categoria"],
      context_meta: {
        mode: "category_commission",
        family: loaded && loaded.family,
        category,
        subcategory: loaded && loaded.subcategory,
        conversation_state: conversationStateForIntent(
          {
            ...
            active_subtopic: category,
            pending_ui_action: pending,
            active_entities: [
              {
                kind: "client",
                display: category,
                cliente_nombre: category,
              },
            ],
          },
          echoedState,
  ...
  if (directorIaPlan.intent === "open_category_movement") {
    const priorCat = priorCategoryFromState(echoedState);
    const pending = runtime007.sanitizePendingUiAction(echoedState && echoedState.pending_ui_action);
    const category =
      (pending && pending.category) ||
      runtime007.extractCategoryFromQuestion(q, priorCat) ||
      "CASA";
    const plant = (pending && pending.plant) || (req && req.body && req.body.planta_nombre) || null;
    return {
      ok: true,
      answer: `Abro el modal Movimiento por categoría en ${category}.`,
      ui_action: {
        type: runtime007.UI_ACTIONS.OPEN_CATEGORY_MOVEMENT,
        category,
        plant,
        do_not_pretend_opened: false,
      },
      sources: ["arr", "igf"],
      context_meta: {
        mode: "open_category_movement",
        category,
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: "open_category_movement",
            active_subtopic: category,
            pending_ui_action: null,
          },
          echoedState,
          "open_category_movement",
          false
        ),
      },
    };
  ...
  if (directorIaPlan.intent === "expense_analytics") {
    directorIaDebug("[DIRECTOR_IA] expense_analytics in-process");
    const loadFn = chatDeps.loadExpenseAnalyticsForChat || loadExpenseAnalyticsForChat;
    let expenseQuestion = q;
    const pendingExpense =
      (completedPending && completedPending.parent_intent === "expense_analytics" && completedPending) ||
      (echoedState && echoedState.parent_intent === "expense_analytics" ? echoedState : null);
    if (pendingExpense && pendingExpense.original_question) {
      expenseQuestion = `${pendingExpense.original_question} ${q}`;
    } else if (
      echoedState &&
      echoedState.pending_information_gap &&
      echoedState.pending_information_gap.original_question &&
      echoedState.parent_intent === "expense_analytics"
    ) {
      expenseQuestion = `${echoedState.pending_information_gap.original_question} ${q}`;
    }
    const inheritedDomain =
      (pendingExpense && pendingExpense.frame && pendingExpense.frame.domain) ||
      (echoedState && echoedState.active_subtopic) ||
      null;
    const expensePayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: expenseQuestion,
      now: chatDeps.now,
      queryPublicFolios: chatDeps.queryPublicFolios,
      resolveEquivalentIds: chatDeps.resolveEquivalentIds,
      inheritedDomain: inheritedDomain === "TALLER" || inheritedDomain === "GASTOS" || inheritedDomain === "INVERSIONES" ? inheritedDomain : inheritedDomain,
      inheritedMetric: "SUM",
    });
    if (expensePayload && expensePayload.ok === false && expensePayload.status) {
      return {
        ok: false,
        status: expensePayload.status,
        code: expensePayload.code,
        error: expensePayload.error || "No se pudo consultar Expense Analytics",
      };
    }
    const expenseResult = buildExpenseAnalyticsChatResult(expensePayload, { planta_id });
    const missingPeriod = Boolean(expensePayload && expensePayload.missing_period);
    if (expenseResult && expenseResult.context_meta) {
      expenseResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "expense_analytics",
          active_subtopic: (expensePayload && expensePayload.spec && expensePayload.spec.domain) || inheritedDomain || "TALLER",
          pending_information_gap: missingPeriod
            ? pendingCompletion.buildPendingGap({
                parent_intent: "expense_analytics",
                missing_fields: ["period"],
                frame: {
                  domain: (expensePayload && expensePayload.spec && expensePayload.spec.domain) || "TALLER",
                  metric: "SUM",
                  family: (expensePayload && expensePayload.spec && expensePayload.spec.keyword) ? "KEYWORD_EXPENSE" : "TALLER_EXPENSE",
                },
                original_question: expenseQuestion,
                why_blocks: "Falta mes o periodo para sumar el gasto.",
              })
            : null,
          active_entities:
            expensePayload && expensePayload.spec && expensePayload.spec.keyword
              ? [
                  {
                    kind: "FOLIO_KEYWORD",
                    display: expensePayload.spec.keyword,
                    keyword: runtime010.normalizeKeywordConcept(expensePayload.spec.keyword),
                    folio_ids: (expensePayload.records || []).map((r) => r.id || r.folio_id || r.numero_folio).filter(Boolean),
                    period_start: expensePayload.spec.period_start,
                    period_end: expensePayload.spec.period_end,
                    operation: "AGGREGATE",
                    result_set_family: "FOLIO_KEYWORD",
                  },
                ]
              : undefined,
        },
  ...
  if (directorIaPlan.intent === "folio_search") {
    directorIaDebug("[DIRECTOR_IA] folio_search in-process");
    const incomingForOrdinal = sanitizeEchoedState(echoedState, planta_id);
    const ordinalSel = backlog.extractOrdinal(q);
    const priorFolioSet = (incomingForOrdinal.active_entities || []).find((ent) => ent && ent.kind === "folio_result_set");
    const navWantsOpen =
      backlog.isExplicitFolioIdQuestion(q) ||
      backlog.isFolioIndexNavigationQuestion(q) ||
      (incomingForOrdinal.parent_intent === "folio_search" &&
        backlog.isFolioResultNavigationQuestion(q, priorFolioSet && priorFolioSet.items));
    if (navWantsOpen && ordinalSel) {
      const keepState = () =>
        conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: incomingForOrdinal.parent_intent || "folio_search",
            folio_search_spec: incomingForOrdinal.folio_search_spec,
            active_entities: incomingForOrdinal.active_entities,
          },
          echoedState,
          incomingForOrdinal.parent_intent || "folio_search",
          false
      ...
      const labelFor = (sel) => {
        if (!sel || sel.kind === "folio_id") return "folio";
        if (sel.index === 1) return "primer folio";
        if (sel.index === 2) return "segundo folio";
        if (sel.index === 3) return "tercer folio";
        if (sel.index === -1) return "último folio";
        if (sel.index === -2) return "penúltimo folio";
        return `folio número ${sel.index}`;
      };
      const openHit = (hit) => ({
        ok: true,
        answer: `El ${labelFor(ordinalSel)} es ${hit.numero_folio}.`,
        ui_action: {
          type: "OPEN_FOLIO",
          numero_folio: hit.numero_folio,
          folio_id: hit.folio_id != null ? hit.folio_id : hit.id,
          do_not_pretend_opened: true,
        },
        sources: ["public.folios"],
        context_meta: { conversation_state: keepState() },
      });
      const fromSet = backlog.selectFromDisplayedResultSet(priorFolioSet && priorFolioSet.items, ordinalSel);
      if (fromSet && fromSet.ok && fromSet.hit) return openHit(fromSet.hit);
      if (fromSet && fromSet.code === "OUT_OF_RANGE") {
        return {
          ok: true,
          answer: `La lista actual tiene ${fromSet.size} folios. Elige un número del 1 al ${fromSet.size}.`,
          sources: ["public.folios"],
          context_meta: { conversation_state: keepState() },
        };
      }
      if (ordinalSel.kind === "ordinal" && (!priorFolioSet || !priorFolioSet.items || !priorFolioSet.items.length)) {
        return {
          ok: true,
          answer:
            "No hay una lista de folios en este hilo. Pide primero el listado o indica el número de folio F-######-###.",
          sources: [],
          context_meta: { conversation_state: keepState() },
        };
      }
      if (ordinalSel.kind === "folio_id") {
        const resolved = await resolveExplicitFolioForOpen(chatDeps, planta_id, ordinalSel.value, req);
        if (resolved && resolved.ok) return openHit(resolved);
        return {
          ok: true,
          answer: resolved && resolved.error
            ? resolved.error
            : `No encontré el folio ${ordinalSel.value} en la planta actual. No abro un folio distinto.`,
          sources: ["public.folios"],
          context_meta: { conversation_state: keepState() },
        };
      }
    ...
    const loadFn = chatDeps.loadFolioSearchForChat || loadFolioSearchForChat;
    const incomingFolio = sanitizeEchoedState(echoedState, planta_id);
    const inheritFolio =
      incomingFolio.parent_intent === "folio_search" &&
      isValidFolioSearchSpec(incomingFolio.folio_search_spec, planta_id) &&
      (isFolioExistenceFollowUp(q) || incomingFolio.folio_search_spec.period_mode === "ANY");
    const folioSearchPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      queryPublicFolios: chatDeps.queryPublicFolios,
      queryPublicFoliosByPlant: chatDeps.queryPublicFoliosByPlant,
      folioItems: chatDeps.folioItems,
      resolveEquivalentIds: chatDeps.resolveEquivalentIds,
      inheritedFilters:
        completedPending && completedPending.parent_intent === "folio_search"
          ? {
              version: 1,
              planta_id,
              scope: completedPending.frame.scope || "ALL_PUBLIC_FOLIOS",
              period_mode: completedPending.frame.period_mode || "SINGLE",
              period_month: completedPending.frame.period_month || null,
              period_start: completedPending.frame.period_start || null,
              period_end: completedPending.frame.period_end || null,
              period_field: "mes_cargo",
              concept_mode: completedPending.frame.concept_mode || "SINGLE",
              concept_query: completedPending.frame.concept_query,
              operation: completedPending.frame.operation || "concept_sequence",
            }
          : inheritFolio &&
              (isFolioExistenceFollowUp(q) ||
                (incomingFolio.folio_search_spec &&
                  incomingFolio.folio_search_spec.period_mode === "ANY" &&
                  /\bsolo\s+(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/.test(
                    String(q || "")
                      .normalize("NFD")
                      .replace(/[\u0300-\u036f]/g, "")
  ...
  if (directorIaPlan.intent === "month_close_result") {
    ...
    const presentation = assembled.financial && assembled.financial.presentation;
    const useDeterministicClose = Boolean(presentation && presentation.state);
    ...
    let answer;
    let openaiCalled = false;
    if (useDeterministicClose) {
      answer = formatFinancialPresentationAnswer(assembled);
    } else {
      ...
      const userContent = prependHiloToUserContent(prompt.userContent, state);
      answer = await openaiDirectorIaChat(prompt.systemPrompt, userContent);
      openaiCalled = true;
    }
    if (!answer) {
      return { ok: false, error: "No se pudo obtener respuesta del modelo", status: 502 };
  ...
  if (directorIaPlan.intent === "historical_margin") {
    directorIaDebug("[DIRECTOR_IA] historical_margin in-process");
    const clientSpan = explicitClientScopeSpan(rawQuestion);
    if (clientSpan) {
      return buildHistoricalMarginClientScopeResult({
        planta_id,
        clientSpan,
        incomingState: echoedState,
      });
    }
    const loadFn = chatDeps.loadHistoricalMarginForChat || loadHistoricalMarginForChat;
    const payload = await loadFn(chatDeps.pool, planta_id, req, {
      question: rawQuestion,
      now: chatDeps.now,
      resolvePlanta: chatDeps.resolveHistoricalMarginPlanta,
      resolvePlantByNombre: chatDeps.resolveHistoricalMarginPlantByNombre,
      queryVersions: chatDeps.queryHistoricalMarginVersions,
      queryLatestVersion: chatDeps.queryHistoricalMarginLatestVersion,
      queryLines: chatDeps.queryHistoricalMarginLines,
    });
    const state = conversationStateForIntent(
      {
        plantaId: planta_id,
    ...
    const result = buildHistoricalMarginChatResult(payload, { planta_id, conversation_state: state });
    if (result && result.context_meta) {
      result.context_meta.conversation_state = state;
    }
    return result;
  ...
  if (directorIaPlan.intent === "historical_new_clients") {
    directorIaDebug("[DIRECTOR_IA] historical_new_clients in-process");
    const loadFn = chatDeps.loadHistoricalNewClientsForChat || loadHistoricalNewClientsForChat;
    const payload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      resolvePlanta: chatDeps.resolveHistoricalNewClientsPlanta,
      resolvePlantByNombre: chatDeps.resolveHistoricalNewClientsPlantByNombre,
      loadMonthlyFacts: chatDeps.loadHistoricalNewClientsFacts,
      getMargenKgPorPeriodo: chatDeps.getHistoricalNewClientsMargen,
    });
    const histResult = buildHistoricalNewClientsChatResult(payload, { planta_id });
    if (histResult && histResult.context_meta) {
      const histState =
        histResult.context_meta.conversation_state || histResult.conversation_state || null;
      histResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "historical_new_clients",
          last_evidence_bundle_type: "historical_new_clients",
          active_entities: histState && histState.active_entities,
          periodoB: (payload && payload.periodoB) || (histState && histState.periodoB),
        },
        echoedState,
        "historical_new_clients",
        false
      );
      if (histResult.context_meta.conversation_state) {
        histResult.context_meta.conversation_state.periodoB = payload && payload.periodoB;
        histResult.context_meta.conversation_state.periodoA = payload && payload.periodoA;
      }
    }
    return histResult;
  }
  ...
  if (directorIaPlan.intent === "commercial_trend") {
    ...
    try {
      assembled = await loadFn(chatDeps.pool, planta_id, req, {
        ...
        queryCalendarKg: chatDeps.queryCommercialTrendCalendarKg,
    ...
    if (assembled.period_kind === "calendar_compare" && assembled.deterministic_answer) {
      const pendingGap = assembled.pending_information_gap || null;
      const state = conversationStateForIntent(
        {
          plantaId: planta_id,
      ...
      const result = buildCommercialTrendChatResult(assembled, {
        answer: assembled.deterministic_answer,
        planta_id,
        openai_called: false,
      });
      if (result && result.context_meta) {
        result.context_meta.conversation_state = state;
        result.context_meta.pending_information_gap = pendingGap;
        result.context_meta.period_kind = "calendar_compare";
      }
      return result;
    }
  ...
  if (directorIaPlan.intent === "client_profile") {
    ...
    const discountPeriod = isDiscountPeriodQuestion(rawQuestion);
    const profileHint = continuityTurn.entity_hint || (active && active.display) || explicitClientScopeSpan(rawQuestion);
    if (discountPeriod && !profileHint) {
      return buildDiscountPeriodClarificationResult({
        planta_id,
        incomingState: echoedState,
      });
    }
    if (discountPeriod && directorIaPlan.requires_clarification && !profileHint) {
      return buildDiscountPeriodClarificationResult({
        planta_id,
        incomingState: echoedState,
      });
    ...
    try {
      assembled = await loadFn(chatDeps.pool, planta_id, req, {
        question: q,
        entity_hint: profileHint || null,
        entity_hint_candidates: continuityTurn.entity_hint_candidates,
        leading_y_requires_canonical: continuityTurn.leading_y_requires_canonical,
        embedded_client_requires_canonical: continuityTurn.embedded_client_requires_canonical,
    ...
    if (assembled && assembled.needs_clarification) {
      if (discountPeriod) {
        return buildDiscountPeriodClarificationResult({
          planta_id,
          clientSpan: profileHint || (assembled.clarification && assembled.clarification.hint),
          incomingState: echoedState,
        });
      }
  ...
  if (directorIaPlan.intent === "igf_direct_metric") {
    directorIaDebug("[DIRECTOR_IA] igf_direct_metric in-process");
    const metricSpec = runtime006.extractIgfDirectMetric(rawQuestion || q, { now: chatDeps.now });
    const loadFn = chatDeps.loadIgfArrSourceBlocksForChat || loadIgfArrSourceBlocksForChat;
    let assembled;
    try {
      assembled = await loadFn(chatDeps.pool, planta_id, req, q);
    } catch (e) {
      directorIaDebug("[DIRECTOR_IA] igf_direct_metric load error:", e && e.message);
      assembled = { ok: false, status: 500, error: (e && e.message) || "No se pudo cargar IGF" };
    }
    if (assembled && assembled.abort) {
      return {
        ok: false,
        status: assembled.status || 403,
        code: assembled.code || "SOURCE_RESTRICTED",
        error: assembled.error || "Sin acceso a IGF",
      };
    }
    const now = chatDeps.now instanceof Date && !Number.isNaN(chatDeps.now.getTime()) ? chatDeps.now : new Date();
    const sourceSel = selectIgfStatusSourceMode(rawQuestion || q, {
      now,
      year: assembled && assembled.year,
      month: assembled && assembled.month,
    });
    let values = runtime006.valuesFromSnapshot(assembled);
    if (sourceSel.mode === IGF_STATUS_SOURCE_MODE.MINI_FORECAST_PROY) {
      const loadMini = chatDeps.loadIgfForecastMiniPayload;
      if (typeof loadMini === "function") {
        try {
          const cutoffRes = resolveDirectorIaEffectiveCutoff({
          ...
          const mini = await loadMini(chatDeps.pool, {
            year: assembled && assembled.year,
            month: assembled && assembled.month,
            upload_day: cutoffRes && cutoffRes.cutoff ? cutoffRes.cutoff : null,
            plantName: assembled && assembled.plant && assembled.plant.planta_nombre,
          });
          if (mini) {
            const adapter = require("./director-ia-dashboard-forecast-adapter");
            ...
            const auth = adapter.readIgfForecastMiniAuthoritative(mini, plantName, plantCode);
            const row = adapter.findMiniRowForPlant(mini.rows, plantName, plantCode);
            values = { ...values, ...runtime006.valuesFromMiniRow(row, auth), projected: runtime006.valuesFromMiniRow(row, auth) };
          }
        } catch (_e) {
    ...
    const period =
      Number.isFinite(Number(year)) && Number.isFinite(Number(month))
        ? `${Number(year)}-${String(Number(month)).padStart(2, "0")}`
        : metricSpec.period;
    const answer = runtime006.buildIgfDirectMetricAnswer(
      { ...metricSpec, period, period_label: runtime006.formatPeriodLabel(period) },
      values
    );
    return {
      ok: true,
      answer,
      sources: ["igf.compromiso_lines", "computeIgfForecastMiniPayload"],
      context_meta: {
        mode: "igf_direct_metric",
        conversation_state: conversationStateForIntent(
          {
            plantaId: planta_id,
            parent_intent: "igf_direct_metric",
            active_subtopic: metricSpec.metric,
            active_entities: [
              {
                kind: "client",
                display: metricSpec.metric || "igf_direct_metric",
                cliente_nombre: metricSpec.metric || "igf_direct_metric",
              },
            ],
          },
          echoedState,
  ...
  if (directorIaPlan.intent === "igf_status") {
    directorIaDebug("[DIRECTOR_IA] igf_status in-process");
    const loadFn = chatDeps.loadIgfArrSourceBlocksForChat || loadIgfArrSourceBlocksForChat;
    let assembled;
    try {
      assembled = await loadFn(chatDeps.pool, planta_id, req, q);
    } catch (e) {
      directorIaDebug("[DIRECTOR_IA] igf_status load error:", e && e.message);
      assembled = {
        ok: false,
        status: 500,
        error: (e && e.message) || "No se pudo cargar el snapshot IGF",
      };
    }
    if (assembled && assembled.abort) {
      return {
    ...
    let igfStatusResult;
    if (sourceSel.mode === IGF_STATUS_SOURCE_MODE.MINI_FORECAST_PROY) {
      igfStatusResult = await buildIgfStatusMiniForecastChatResult(assembled, {
        planta_id,
        question: rawQuestion || q,
        req,
      });
    } else {
      igfStatusResult = buildIgfStatusSnapshotChatResult(assembled, {
        planta_id,
        question: rawQuestion || q,
        budget: sourceSel.mode === IGF_STATUS_SOURCE_MODE.IGF_COMMIT_SNAPSHOT,
        source_mode: sourceSel.mode,
      });
    }
    if (igfStatusResult && igfStatusResult.context_meta) {
      igfStatusResult.context_meta.source_reason = sourceSel.reason;
      igfStatusResult.context_meta.conversation_state = conversationStateForIntent(
        { plantaId: planta_id, parent_intent: "igf_status" },
        echoedState,
        "igf_status",
        false
      );
    }
    return igfStatusResult;
  }

  if (directorIaPlan.intent === "financial_diagnosis") {
    const rentEvidence = (directorIaPlan.evidence || []).some(
      (e) => String((e && e.value) || e) === "rentabilidad_deterioro_snapshot"
    );
    if (rentEvidence || isRentabilidadDeterioroSnapshotQuestion(rawQuestion || q)) {
      directorIaDebug("[DIRECTOR_IA] rentabilidad_deterioro_snapshot in-process");
      let snapPayload;
      try {
        snapPayload = await assembleRentabilidadDeterioroSnapshot({
          question: rawQuestion || q,
          now: chatDeps.now,
          pool: chatDeps.pool,
          upload_day: (req && req.body && req.body.upload_day) || null,
          loadRentabilidadKpis: chatDeps.loadRentabilidadKpis,
          loadIgfForecastMiniPayload: chatDeps.loadIgfForecastMiniPayload,
          loadDeltaTopN: async (months) =>
            loadDeltaIngresoForecastNegativeTopN(chatDeps.pool, planta_id, req, {
              question: rawQuestion || q,
              now: chatDeps.now,
              months,
            }),
        ...
        directorIaDebug("[DIRECTOR_IA] rentabilidad_deterioro_snapshot load error:", e && e.message);
        snapPayload = {
          ok: false,
          error: "No pude armar el snapshot de rentabilidad. No invento valores.",
        };
      }
      if (snapPayload && snapPayload.clarification) {
        return buildUnknownClarificationResult({
          planta_id,
          reason: snapPayload.error,
          conversation_state: preserveFramesOnClarify(echoedState, planta_id),
        });
      }
      const snapResult = buildRentabilidadDeterioroSnapshotChatResult(snapPayload, { planta_id });
      const snapState = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "profitability_deterioro_snapshot",
          ...
          active_period_months:
            snapPayload && snapPayload.ok
              ? [snapPayload.periodo_a, snapPayload.periodo_b].filter(Boolean)
              : [],
        },
        echoedState,
        "profitability_deterioro_snapshot",
      ...
      if (snapResult && snapResult.context_meta) {
        snapResult.context_meta.conversation_state = snapState;
      }
      return snapResult;
    }
  ...
  if (directorIaPlan.intent === "delta_income") {
    if (isDeltaIngresoForecastQuestion(rawQuestion || q)) {
      directorIaDebug("[DIRECTOR_IA] delta_income_forecast in-process");
      let forecastPayload;
      try {
        forecastPayload = await loadDeltaIngresoForecastNegativeTopN(chatDeps.pool, planta_id, req, {
          question: rawQuestion || q,
          now: chatDeps.now,
        });
      } catch (e) {
        directorIaDebug("[DIRECTOR_IA] delta_income_forecast load error:", e && e.message);
        forecastPayload = {
          ok: false,
          error: "No pude completar Delta Ingreso Forecast por un error de fuente. No invento valores.",
        };
      }
      const forecastResult = buildDeltaIngresoForecastChatResult(forecastPayload, { planta_id });
      const forecastState = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "delta_income_forecast",
          last_evidence_bundle_type: "delta_income_forecast",
          keepIncomingPreviousFrame: keepDailyPreviousFrame(continuityTurn, echoedState),
        },
        echoedState,
        "delta_income_forecast",
        Boolean(continuityTurn.restore_previous)
      );
      if (forecastResult && forecastResult.context_meta) {
        forecastResult.context_meta.conversation_state = forecastState;
      }
...
module.exports = {
  ...
  selectIgfStatusSourceMode,
  IGF_STATUS_SOURCE_MODE,
