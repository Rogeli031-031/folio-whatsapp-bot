...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (
    !planOptions.forceIntent &&
    echoedState &&
    echoedState.parent_intent === "client_ranking" &&
    isClientRankingFollowUp(q) &&
    !(typeof isHistoricalNewClientsQuestion === "function" && isHistoricalNewClientsQuestion(q))
  ) {
  ...
  if (
    !planOptions.forceIntent &&
    ((echoedState &&
      (echoedState.parent_intent === "client_movement" || echoedState.parent_intent === "client_ranking") &&
      isClientMovementFollowUp(q)) ||
      (isClientMovementQuestion(q) &&
        !(typeof isHistoricalNewClientsQuestion === "function" && isHistoricalNewClientsQuestion(q)) &&
        !(echoedState && echoedState.parent_intent === "commercial_trend") &&
        !/\b(por\s+que|porque)\b/.test(
          String(q || "")
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
  ...
  if (
    !planOptions.forceIntent &&
    !(typeof isExpedienteComercialQuestion === "function" && isExpedienteComercialQuestion(q)) &&
    (isClientEvidenceQuestion(q) ||
      (echoedState && echoedState.parent_intent === "client_evidence" && isClientEvidenceFollowUp(q)))
  ) {
    planOptions.forceIntent = "client_evidence";
  }
  if (!planOptions.forceIntent && isProcurementByConceptQuestion(q)) {
    const { extractProcurementSpec } = require("./director-ia-procurement-by-concept");
    const proc = extractProcurementSpec(q, null);
    if (proc && proc.ok && (proc.operation === "SUPPLIERS" || proc.operation === "BENEFICIARIES" || proc.operation === "UNIT_COST")) {
      planOptions.forceIntent = "procurement_by_concept";
    }
  }
  ...
  if (directorIaPlan.intent === "client_movement") {
    ...
    const loadFn = chatDeps.loadClientMovementForChat || loadClientMovementForChat;
    const movementPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      plant_label: req && req.body && req.body.planta_nombre,
      selectedPeriod: req && req.body && req.body.arr_period,
      latestArrPeriod: chatDeps.latestArrPeriod,
      priorSpec: priorMovementFromState(echoedState) || priorRankingFromState(echoedState),
      resolvePlantByNombre: chatDeps.resolveClientRankingPlantByNombre || chatDeps.resolveHistoricalMarginPlantByNombre,
      currentSalesRows: chatDeps.clientMovementCurrentSalesRows,
      previousSalesRows: chatDeps.clientMovementPreviousSalesRows,
      currentDiscountRows: chatDeps.clientMovementCurrentDiscountRows,
      previousDiscountRows: chatDeps.clientMovementPreviousDiscountRows,
      plantCodes: chatDeps.clientRankingPlantCodes,
    });
    if (movementPayload && movementPayload.ok === false && movementPayload.status) {
      return {
        ...
        error: movementPayload.error || "No se pudo consultar el movimiento comercial",
      };
    }
    const movementPlant = movementPayload && movementPayload.planta_id != null ? movementPayload.planta_id : planta_id;
    const movementResult = buildClientMovementChatResult(movementPayload, { planta_id: movementPlant });
    if (movementResult && movementResult.context_meta) {
      movementResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: movementPlant,
          parent_intent: "client_movement",
          active_subtopic: "client_movement",
          active_entities: movementResult.context_meta.conversation_state.active_entities,
        },
        echoedState,
        "client_movement",
        false
  ...
  if (directorIaPlan.intent === "client_evidence") {
    directorIaDebug("[DIRECTOR_IA] client_evidence in-process");
    const priorMove = priorMovementFromState(echoedState) || priorRankingFromState(echoedState);
    const loadFn = chatDeps.loadClientEvidenceForChat || loadClientEvidenceForChat;
    const evidencePayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      priorSpec: {
        ok: Boolean(priorMove),
        entity_names: (priorMove && priorMove.entity_names) || [],
      },
      evidenceItems: chatDeps.clientEvidenceItems,
    });
    const evidenceResult = buildClientEvidenceChatResult(evidencePayload, { planta_id });
    if (evidenceResult && evidenceResult.context_meta) {
      evidenceResult.context_meta.conversation_state = conversationStateForIntent(
        {
          plantaId: planta_id,
          parent_intent: "client_evidence",
          active_subtopic: "client_evidence",
          active_entities: evidenceResult.context_meta.conversation_state.active_entities,
        },
        echoedState,
        "client_evidence",
        false
  ...
  if (directorIaPlan.intent === "procurement_by_concept") {
    directorIaDebug("[DIRECTOR_IA] procurement_by_concept in-process");
    const loadFn = chatDeps.loadProcurementForChat || loadProcurementForChat;
    const procurementPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      folioItems: chatDeps.folioItems,
    });
    return buildProcurementChatResult(procurementPayload, { planta_id });
  }
  ...
  if (directorIaPlan.intent === "folio_search") {
    ...
    const inheritFolio =
      ...
      (isFolioResultSetFollowUp(q) || isFolioExistenceFollowUp(q) || incomingFolio.folio_search_spec.period_mode === "ANY");
    const folioSearchPayload = await loadFn(chatDeps.pool, planta_id, req, {
      question: q,
      now: chatDeps.now,
      queryPublicFolios: chatDeps.queryPublicFolios,
