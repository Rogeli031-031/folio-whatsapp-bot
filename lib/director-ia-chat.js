...
async function handleExecutiveStatusForChat(opts) {
  ...
  try {
    const { loadDashboardForecastParity } = require("./director-ia-dashboard-forecast-adapter");
    forecastParity = await loadDashboardForecastParity(chatDeps.pool, { assembled });
    const igfRow = assembled.sources && assembled.sources.igf && assembled.sources.igf.payload && assembled.sources.igf.payload.row;
    if (igfRow && chatDeps.pool) {
      const { loadDashboardIgfForecastEconomics } = require("./director-ia-dashboard-forecast-adapter");
      const ym = (forecastParity && forecastParity.period) || {};
      forecastParity.economics = await loadDashboardIgfForecastEconomics(chatDeps.pool, {
        year: ym.year,
        month: ym.month,
        plantCode: assembled.plant && assembled.plant.plant_code,
        plantName: assembled.plant && assembled.plant.planta_nombre,
        igfRow,
      });
    }
  } catch (e) {
