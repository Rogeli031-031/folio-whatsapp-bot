...
async function handleExecutiveStatusForChat(opts) {
  ...
  let resolvedCutoff = parseCutoffYmd(
    (req && req.body && (req.body.upload_day || req.body.cutoff_date)) || null
  );
  let cutoffSource = resolvedCutoff ? "req.body.upload_day" : null;
  if (!resolvedCutoff) {
    ...
    if (typeof loadLast === "function" && chatDeps.pool && ymAssembled) {
      try {
        const last = await loadLast(chatDeps.pool, ymAssembled.year, ymAssembled.month);
        resolvedCutoff = parseCutoffYmd(last && (last.upload_day || last.uploaded_day));
        if (resolvedCutoff) cutoffSource = "arr.upload_log";
      } catch (e) {
  ...
  if (typeof loadMini === "function" && chatDeps.pool) {
    try {
      ...
      const miniCutoff = mini.cutoff_date || resolvedCutoff;
      if (miniCutoff && !mini.cutoff_date) mini.cutoff_date = miniCutoff;
      if (forecastParity.period) {
        forecastParity.period.cutoff_date = forecastParity.period.cutoff_date || miniCutoff;
      ...
      if (mini.venta_ton != null || mini.desc_kg != null) {
        ...
        forecastParity.forecast = {
          ...
          cutoff_date: miniCutoff || prev.cutoff_date || null,
          source: mini.source,
