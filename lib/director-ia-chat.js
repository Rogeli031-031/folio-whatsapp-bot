...
async function askDirectorIa(req, plantaId, question, _user) {
  ...
  if (
    continuityTurn.inherit &&
    continuityTurn.inherit_parent_intent === "profitability_deterioro_snapshot" &&
    isProfitabilityExpenseFollowUp(rawQuestion || q)
  ) {
    directorIaDebug("[DIRECTOR_IA] profitability_expense_followup in-process");
    ...
    const follow = buildProfitabilityExpenseFollowUpChatResult({
      planta_id,
