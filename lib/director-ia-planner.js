...
const { isClientMovementQuestion } = require("./director-ia-client-movement");
const { isClientEvidenceQuestion, isActionRegisterThemeQuestion } = require("./director-ia-client-evidence");
const { isProcurementByConceptQuestion, extractProcurementSpec } = require("./director-ia-procurement-by-concept");
...
const INTENT_LABELS = Object.freeze({
  ...
  client_ranking: "Ranking comercial de clientes por VENTA_TON o DESCUENTO_POR_KG",
  client_movement: "Movimiento comercial de clientes (crudo, no forecast)",
  client_evidence: "Evidencia y acciones de cliente por fuente",
  procurement_by_concept: "Compras por concepto (gasto relacionado, beneficiarios, costo unitario)",
...
const INTENT_DOMAIN_MAP = Object.freeze({
  ...
  client_movement: ["client_movement", "arr"],
  client_evidence: ["dicf", "cliente_comentarios", "action_register", "bitacora"],
  procurement_by_concept: ["folio_search"],
...
function detectDirectorIaIntent(question) {
  ...
  if (typeof isProcurementByConceptQuestion === "function" && isProcurementByConceptQuestion(question)) {
    const proc = extractProcurementSpec(question, null);
    if (proc && proc.ok && (proc.operation === "SUPPLIERS" || proc.operation === "BENEFICIARIES" || proc.operation === "UNIT_COST")) {
      return makeIntent("procurement_by_concept", [{ value: `procurement_${String(proc.operation).toLowerCase()}` }], 0.9);
    }
  }
  ...
  if (typeof isClientMovementQuestion === "function" && isClientMovementQuestion(question)) {
    return makeIntent("client_movement", [{ value: "client_movement" }], 0.9);
  }
  if (isExpedienteComercialQuestion(question)) {
    return makeIntent("expediente_comercial", [{ value: "expediente_comercial" }], 0.92);
  }
  if (
    typeof isClientEvidenceQuestion === "function" &&
    isClientEvidenceQuestion(question) &&
    !(typeof isActionRegisterThemeQuestion === "function" && isActionRegisterThemeQuestion(q))
  ) {
    return makeIntent("client_evidence", [{ value: "client_evidence_and_actions" }], 0.9);
  }
