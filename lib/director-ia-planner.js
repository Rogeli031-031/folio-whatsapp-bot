...
function isDailySalesDeviationQuestion(q) {
  ...
  if (/\bdescuento\b/.test(q) && !/\bventa\b/.test(q) && !/\bvendim/.test(q) && !/\bvendio\b/.test(q)) {
    return false;
  }
  return /\bventa\b/.test(q) || /\bvendimos\b/.test(q) || /\bvendio\b/.test(q) || /\bvendi\b/.test(q);
}
...
function detectDirectorIaIntent(question) {
  ...
  if (/\bacciones?\b/.test(q) && /\b(?:at|pt|s|c|u)[-\s]?\d{1,4}\b/.test(q) && !/\btaller\b/.test(q)) {
