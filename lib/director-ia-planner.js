...
const INTENT_DOMAIN_MAP = Object.freeze({
  ...
  delta_sales: ["delta_venta", "arr"],
  delta_discount: ["delta_descuento", "arr"],
  delta_income: ["delta_ingreso", "arr", "igf"],
...
function detectDirectorIaIntent(question) {
  ...
  if (
    ...
    (/\bfalt/.test(q) || /\badjunt/.test(q) || /\bfolio/.test(q) || /\bcotizacion\b/.test(q) || /\bfacturas?\b/.test(q))
  ) {
  ...
  if (/\bproyectos?\b/.test(q)) {
    ...
    if (
      ...
      return makeIntent("project_status", [{ value: "proyectos_modulo" }], 0.78, {
        requires_clarification: true,
        clarification_reason:
          "Podría referirse al módulo Proyectos o a iniciativas registradas en Action Register",
      });
    }
  }
  ...
  if (
    (/\bgastos?\b/.test(q) && /\b(folio|excel|categoria|export|listad|rango\s+de\s+meses)\b/.test(q)) ||
  ...
  // Taller por AT explícito (no "cómo va Taller")
  if (
    /\btaller\s+por\s+at\b/.test(q) ||
    /\btaller\s+at\b/.test(q) ||
    (/\btaller\b/.test(q) && /\b(unidad|at-\d|por\s+at|excel\s+taller)\b/.test(q))
  ) {
    return makeIntent("expense_analysis", [{ value: "taller_at_as_expense_domain" }], 0.7, {
      domain_override: ["taller_at", "gastos", "folios"],
  ...
  // 8. Action Register
  if (/\bacciones?\b/.test(q) && /\b(vencid|atrasad|overdue)/.test(q)) {
  ...
  if (unsupported) {
    const byDomain = {
      ...
      taller_at: "expense_analysis",
    };
    const mapped = byDomain[unsupported.id];
    if (mapped) {
      return makeIntent(
        mapped,
        [{ value: `capabilities_unsupported:${unsupported.id}` }],
        0.75,
        mapped === "expense_analysis" && unsupported.id === "taller_at"
          ? { domain_override: ["taller_at", "gastos", "folios"] }
          : undefined
      );
    }
...
/**
 * @param {string} question
 * @param {{ forceIntent?: string }} [options]
 */
function planDirectorIaQuestion(question, options = {}) {
  ...
  const plan = {
    ...
    clarification_reason,
    confidence: clampConfidence(confidence),
    evidence: (detected.evidence || []).map((e) => ({ type: "rule", value: e.value })),
  };
