...
const { isFolioSearchQuestion, isFolioExistenceQuestion, looksLikePaidFolioQuestion } = require("./director-ia-folio-search");
const { isExpenseAnalyticsQuestion } = require("./director-ia-expense-analytics");
const { isSehOperationStatusQuestion } = require("./director-ia-seh-operation-status");
const { isClientRankingQuestion } = require("./director-ia-client-ranking");
const {
...
const { isClientProfileQuestion, extractEmbeddedClientHintCandidates } = require("./director-ia-client-profile");
...
const { isHistoricalNewClientsQuestion } = require("./director-ia-new-clients");
const { isHistoricalMarginQuestion, extractNamedPlant } = require("./director-ia-historical-margin");
const backlog = require("./director-ia-executive-backlog");
const coverage = require("./director-ia-executive-coverage");
...
const INTENT_LABELS = Object.freeze({
  ...
  igf_direct_metric: "Métrica directa IGF (una cifra, respuesta corta)",
  category_commission: "Comisión por categoría/subcategoría (Movimiento por categoría)",
  open_category_movement: "Abrir modal Movimiento por categoría",
  ...
  historical_new_clients: "Clientes nuevos de un mes calendario cerrado",
  historical_margin: "Margen histórico $/kg (FINAL / forecast abierto)",
  client_profile: "Perfil longitudinal de cliente",
  bitacora_lookup: "Consulta de bitácora",
  mejora_continua: "Mejora Continua",
  folio_status: "Etapa / estatus de folio",
  folio_search: "Búsqueda de folios / apoyos",
  ...
  expense_analytics: "Analítica de gasto por categoría",
  seh_operation_status: "Estado operativo SEH",
  seh_regulation: "Cumplimiento SEH / regulación",
  client_ranking: "Ranking comercial de clientes por VENTA_TON",
  client_movement: "Movimiento de clientes ARR (dejaron / disminuyeron / aumentaron / nuevos)",
  open_pronostico: "Navegación al modal Pronóstico IGF",
  predictive_commercial: "Cobertura predictiva/comercial (share, retención, concentración proyectada)",
  executive_coverage: "Familia ejecutiva con evidencia o limitación explícita",
  ...
  taller_mayor: "Taller Mayor por unidad",
  pre_meeting_brief: "Preparación de junta / pre-cierre",
  month_close_result: "Resultado mensual de cierre",
  profitability_deterioro_snapshot: "Snapshot de deterioro de rentabilidad",
  client_discount_ranking: "Ranking de clientes por descuento",
  client_contact_lookup: "Contacto comercial de cliente",
  profitability_period_ranking: "Ranking histórico de rentabilidad por mes",
  profitability_period_comparison: "Comparación de periodos de rentabilidad",
  plant_metric_comparison: "Comparación de descuento/margen de planta",
  profitability_action_plan: "Plan ejecutivo de rentabilidad",
  executive_sales_context: "Lectura ejecutiva de venta / comparativos / tendencia",
  client_inactivity: "Clientes sin compra / inactivos",
  client_direct_purchase: "Última compra y operaciones directas de cliente",
  open_client_delta_forecast: "Abrir Delta Ingreso Cliente Forecast",
  client_comments: "Comentarios físicos de clientes",
  action_register_recent: "Action Register por recencia de registro",
  bitacora_topic_lookup: "Búsqueda temática en bitácoras",
  open_daily_sales_view: "Navegación a venta diaria",
...
const INTENT_DOMAIN_MAP = Object.freeze({
  ...
  arr_status: ["arr"],
  igf_status: ["igf"],
  igf_direct_metric: ["igf"],
  category_commission: ["arr", "igf"],
  open_category_movement: ["arr", "igf"],
  ...
  historical_new_clients: ["historical_new_clients"],
  historical_margin: ["historical_margin"],
  client_profile: ["arr", "dicf", "cliente_comentarios"],
  ...
  expense_analytics: ["folios"],
  seh_operation_status: ["seh_equipos"],
  seh_regulation: ["seh_equipos"],
  client_ranking: ["client_ranking"],
  client_movement: ["arr"],
  open_pronostico: ["igf"],
  predictive_commercial: ["arr"],
  executive_sales_context: ["arr"],
  client_inactivity: ["arr", "dicf"],
  client_direct_purchase: ["arr", "dicf"],
  open_client_delta_forecast: ["arr", "dicf"],
  client_comments: ["cliente_comentarios"],
  action_register_recent: ["action_register"],
  bitacora_topic_lookup: ["bitacora"],
  executive_coverage: ["arr", "igf"],
  investment_analysis: ["inversiones", "folios"],
  clasificacion_apoyos_query: ["clasificacion_apoyos"],
  igf_reviewable_supports: ["folios", "igf"],
  taller_at: ["taller_at"],
  taller_mayor: ["folios"],
  pre_meeting_brief: ["arr", "dicf", "cliente_comentarios", "igf", "folios", "action_register"],
  month_close_result: ["arr", "igf", "dicf", "cliente_comentarios", "action_register"],
  profitability_deterioro_snapshot: ["igf", "delta_ingreso"],
  client_discount_ranking: ["arr"],
  client_contact_lookup: ["entidades_comerciales"],
  profitability_period_ranking: ["igf", "arr"],
  profitability_period_comparison: ["igf", "arr"],
  plant_metric_comparison: ["arr", "igf"],
  profitability_action_plan: ["igf", "arr", "dicf", "action_register"],
  open_daily_sales_view: ["arr"],
...
const INHERITED_INTENT_METRIC_FAMILY = Object.freeze({
  historical_margin: "margen",
  daily_discount_deviation: "descuento",
  delta_discount: "descuento",
  daily_sales_deviation: "venta",
  delta_venta: "venta",
  client_discount_ranking: "descuento",
  client_ranking: "venta",
});

const INCOMPATIBLE_EXPLICIT_METRIC_PARENTS = Object.freeze({
  plant_diagnosis: true,
  daily_executive_brief: true,
  pre_meeting_brief: true,
  commercial_trend: true,
  month_close_result: true,
});

const CALENDAR_MONTH_RE =
  /\b(enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|setiembre|octubre|noviembre|diciembre)\b/;

function normalizeSpanKey(raw) {
  return String(raw || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
function explicitClientScopeSpan(question) {
  const embedded = extractEmbeddedClientHintCandidates(question);
  const span = embedded && embedded.longest;
  if (!span) return null;
  const plant = typeof extractNamedPlant === "function" ? extractNamedPlant(question) : null;
  if (plant && normalizeSpanKey(span) === normalizeSpanKey(plant)) return null;
  return span;
}

function hasNamedCalendarPeriod(q) {
  return (
    CALENDAR_MONTH_RE.test(q) ||
    /\bmes(es)?\b/.test(q) ||
    /\bmensual\b/.test(q) ||
    /\b(20\d{2})-(\d{1,2})\b/.test(q)
  );
}

function isHistoricalDeltaIngresoQuestion(q) {
  return (
    (/\b(cambio|cambi|vario|variacion)\b/.test(q) || /\bcomo\s+cambi/.test(q)) &&
    /\bingreso\b/.test(q) &&
    !/\bimpacto\b/.test(q) &&
    !/\bdeterior/.test(q) &&
    !/\bforecast\b/.test(q)
...
function isRentabilidadDeterioroSnapshotQuestion(raw) {
  const q = normalizeQuestion(raw);
  if (!q) return false;
  if (!/\brentabilidad\b/.test(q)) return false;
  if (!/\bdeterioro\b/.test(q) && !/\bprovocando\b/.test(q)) return false;
  if (/\bgastos?\b/.test(q) && !/\bdeterioro\b/.test(q)) return false;
  return true;
}

function isIgfStatusFinancialSnapshotQuestion(raw) {
  const q = normalizeQuestion(raw);
  if (!q) return false;
  if (isRentabilidadDeterioroSnapshotQuestion(raw)) return false;
  if (/\bmargen\b/.test(q) && !/\b(rentabilidad|utilidad|resultado\s+final)\b/.test(q)) return false;
  if (/\brentabilidad\b/.test(q)) return true;
  if (/\butilidad\s+operativ/.test(q)) return true;
  if (/\bresultado\s+final\b/.test(q)) return true;
  return false;
}

function isDeltaIngresoForecastQuestion(raw) {
  const q = normalizeQuestion(raw);
  if (!q || !/\bingreso\b/.test(q)) return false;
  if (isHistoricalDeltaIngresoQuestion(q)) return false;
  const impact =
    /\bimpacto\s+negativ/.test(q) ||
    /\bmayor\s+impacto\b/.test(q) ||
    /\bdeterior/.test(q) ||
    (/\bafectan\b/.test(q) && /\bingreso\b/.test(q)) ||
    /\bdelta\s+ingreso\b/.test(q) ||
    (/\bdelta\b/.test(q) && /\bingreso\b/.test(q));
  return impact && hasNamedCalendarPeriod(q);
}

function isDiscountPeriodQuestion(question) {
  const q = normalizeQuestion(question);
  if (!q) return false;
  if (!/\bdescuento/.test(q)) return false;
  if (/\bnuevos?\b/.test(q) && /\bclientes?\b/.test(q)) return false;
  if (backlog.isClientDiscountRankingQuestion(question)) return false;
  if (isDailyDiscountDeviationQuestion(q)) return false;
  if (
  ...
  return hasNamedCalendarPeriod(q);
}

function explicitTurnMetricFamily(question) {
  const q = normalizeQuestion(question);
  if (!q) return null;
  const discount = /\bdescuento/.test(q);
  const margin = /\bmargen\b/.test(q);
  const venta = namesDailySalesMetric(q) || /\bkg\b/.test(q);
  const hits = [];
  if (discount) hits.push("descuento");
  if (margin) hits.push("margen");
  if (venta) hits.push("venta");
  if (hits.length !== 1) return null;
  return hits[0];
}

function isBareFolioAggregateFollowUp(q) {
  const n = String(q || "");
  if (!n || /\bcuantos\s+fueron\b/.test(n) || /^y\s+solo\b/.test(n)) return false;
  if (!/\bpor\s+mes\b/.test(n)) return false;
  return (
    /\bsumarlos\b/.test(n) ||
    /\bsumalos\b/.test(n) ||
    /\bsuma\s+(los|esos|el)\b/.test(n) ||
    /\bpuedes\s+sumar/.test(n) ||
    /\bdame\s+(el|un)\s+total\b/.test(n) ||
    /\bel\s+total\s+por\s+mes\b/.test(n)
  );
}

function inheritBlockedByExplicitMetric(question, inheritParentIntent) {
  const current = explicitTurnMetricFamily(question);
  if (!current) return false;
  const inherited = INHERITED_INTENT_METRIC_FAMILY[inheritParentIntent];
  if (inherited) return current !== inherited;
  return Boolean(INCOMPATIBLE_EXPLICIT_METRIC_PARENTS[inheritParentIntent]);
}

function isGenericDiscountFallbackIntent(intent, question) {
  if (intent === "unknown") return true;
  if (intent === "igf_direct_metric") return runtime006.isIgfDiscountQuestion(question);
  if (intent === "client_profile") return isDiscountPeriodQuestion(question);
  return false;
}

function priorIsDiscountRanking(prior) {
  if (!prior) return false;
  return (
    prior.parent_intent === "client_discount_ranking" ||
    prior.active_subtopic === "client_discount_ranking" ||
    prior.metric === "DISCOUNT"
  );
}
 ...
 * @param {{ prior?: object }} [options]
 * @returns {DetectedIntent}
 */
function detectDirectorIaIntent(question, options = {}) {
  const q = normalizeQuestion(question);
  const prior = options && options.prior ? options.prior : null;
  if (!q) {
  ...
  if (
    priorIsDiscountRanking(prior) &&
    !inheritBlockedByExplicitMetric(question, "client_discount_ranking")
  ) {
    const { isClientRankingFollowUp } = require("./director-ia-client-ranking");
    if (isClientRankingFollowUp(question) || backlog.isClientDiscountRankingQuestion(question)) {
      return makeIntent("client_discount_ranking", [{ value: "discount_ranking_continuity" }], 0.93);
    }
  }
  ...
  if (
    /\bcomentarios?\b/.test(q) &&
    /\bclientes?\b/.test(q) &&
    !isDeltaIngresoForecastQuestion(question) &&
    !(typeof isCommercialTrendQuestion === "function" && isCommercialTrendQuestion(question))
  ) {
  ...
  const family011 = runtime011.classify011Family(question, prior);
  if (family011 === "OPEN_CLIENT_DELTA_FORECAST") {
    return makeIntent("open_client_delta_forecast", [{ value: "open_client_delta_forecast" }], 0.97);
  }
  if (family011 === "LAST_PURCHASE_DIRECT" || family011 === "DIRECT_ANSWER_COMPRESSION") {
    const { isExpectedNextPurchaseQuestion } = require("./director-ia-predictive-commercial");
    if (typeof isExpectedNextPurchaseQuestion === "function" && isExpectedNextPurchaseQuestion(question) && family011 !== "LAST_PURCHASE_DIRECT") {
      // expected-next nombrado sigue en predictive_commercial (009/008).
    } else {
      return makeIntent("client_direct_purchase", [{ value: family011 }], 0.96);
    }
  }
  const family012 = runtime012.classify012Family(question, prior);
  if (family012 === "CLIENT_COMMENTS_PERIOD") {
    return makeIntent("client_comments", [{ value: family012 }], 0.97);
  }
  if (family012 === "ACTION_REGISTER_RECENT") {
    return makeIntent("action_register_recent", [{ value: family012 }], 0.97);
  }
  if (family012 === "ACTION_REGISTER_DIRECT") {
    return makeIntent("action_status", [{ value: family012 }], 0.96);
  }
  if (family012 === "CLIENT_MOVEMENT_DEJARON") {
    return makeIntent("client_movement", [{ value: family012 }], 0.97);
  }
  if (family012 === "CHANNEL_SALES_STATUS") {
    return makeIntent("executive_sales_context", [{ value: family012 }], 0.97);
  }

  if (family011 === "INACTIVE_CLIENTS") {
    if (runtime004.isLostClientsListQuestion(question)) {
      return makeIntent("client_movement", [{ value: "inactive_lost_list" }], 0.95);
    }
    return makeIntent("client_inactivity", [{ value: "inactive_clients" }], 0.96);
  }

  const family010 = runtime010.classifyExecutiveContextFamily(question, prior);
  if (family010 === "KEYWORD_EXPENSE") {
    return makeIntent("expense_analytics", [{ value: "keyword_expense" }], 0.97);
  }
  if (runtime010.isSalesContextFamily(family010)) {
    return makeIntent("executive_sales_context", [{ value: family010 }], 0.96);
  }
  if (family010 === "PLANT_DIAGNOSIS_CURRENT") {
    return makeIntent("plant_diagnosis", [{ value: "plant_diagnosis_current" }], 0.95);
  }

  if (runtime008.isRegulationStatusQuestion(question, prior) || runtime008.isRegulationPlantDetailQuestion(question, prior)) {
    return makeIntent("seh_regulation", [{ value: runtime008.classifyRegulationFamily(question, prior) || "REGULATION_STATUS" }], 0.94);
  }

  if (typeof isSehOperationStatusQuestion === "function" && isSehOperationStatusQuestion(question)) {
    return makeIntent("seh_operation_status", [{ value: "seh_operation_status" }], 0.9);
  }

  if (
    (runtime008.isTallerExpenseQuestion(question) || runtime008.isTallerExpenseFollowUp(question, prior)) &&
    !(typeof isM5TallerAtQuery === "function" && isM5TallerAtQuery(q))
  ) {
    return makeIntent("expense_analytics", [{ value: "taller_expense" }], 0.95);
  }
  if (family012 === "BITACORA_TOPIC_LOOKUP") {
    return makeIntent("bitacora_topic_lookup", [{ value: family012 }], 0.94);
  }

  if (typeof isFolioSearchQuestion === "function" && isFolioExistenceQuestion(question)) {
    return makeIntent("folio_search", [{ value: "folio_existence_by_concept" }], 0.9);
  }

  if (typeof looksLikePaidFolioQuestion === "function" && looksLikePaidFolioQuestion(question)) {
    return makeIntent("folio_search", [{ value: "folio_paid_status_period" }], 0.9);
  }

  if (typeof isChannelForecastGuardrailQuestion === "function" && isChannelForecastGuardrailQuestion(question)) {
    return makeIntent("predictive_commercial", [{ value: "channel_forecast_guardrail" }], 0.96);
  }
  if (runtime005.isChannelForecastProjectionGuardrailQuestion(question)) {
    return makeIntent("predictive_commercial", [{ value: "channel_forecast_projection_guardrail" }], 0.96);
  }
  if (runtime004.isObservedChannelSalesQuestion(question) && !runtime004.isCasaAsClientName(question)) {
    return makeIntent("predictive_commercial", [{ value: "observed_channel_sales" }], 0.93);
  }
  if (typeof isPredictiveCommercialQuestion === "function" && isPredictiveCommercialQuestion(question, prior)) {
    return makeIntent("predictive_commercial", [{ value: "predictive_commercial" }], 0.93);
  }
  if (
    typeof isHistoricalNewClientsQuestion === "function" &&
    isHistoricalNewClientsQuestion(question) &&
    (/\bentraron\b/.test(q) || /\bhubo\b/.test(q) || /\bdime\s+los\b/.test(q) || /\bque\s+clientes\s+nuevos\b/.test(q))
  ) {
    return makeIntent("historical_new_clients", [{ value: "historical_new_clients_month" }], 0.93);
  }
  if (runtime004.isNewClientDiscountQuestion(question)) {
    return makeIntent("client_movement", [{ value: "new_client_discount" }], 0.94);
  }
  if (
    (runtime005.isContextualNewClientCountQuestion(question) || runtime006.isContextualNewClientCountQuestion(question)) &&
    (/\bnuevos?\b/.test(q) || /\bcuentas?\s+nuev/.test(q))
  ) {
    return makeIntent("client_movement", [{ value: "contextual_new_client_count" }], 0.95);
  }
  if (runtime006.isResultSetPronounReference(question) && prior && (prior.movement || prior.result_set_family)) {
    return makeIntent("client_movement", [{ value: "result_set_pronoun" }], 0.95);
  }
  if (runtime007.isOpenCategoryMovementQuestion(question, prior)) {
    return makeIntent("open_category_movement", [{ value: "open_category_movement" }], 0.96);
  }
  if (runtime007.isCategoryCommissionQuestion(question, prior)) {
    return makeIntent("category_commission", [{ value: "category_commission" }], 0.96);
  }
  if (runtime006.isIgfDirectMetricQuestion(question)) {
    return makeIntent("igf_direct_metric", [{ value: "igf_direct_metric" }], 0.95);
  }

  if (
    typeof isExpenseAnalyticsQuestion === "function" &&
    isExpenseAnalyticsQuestion(question) &&
    !isBareFolioAggregateFollowUp(q)
  ) {
    return makeIntent("expense_analytics", [{ value: "expense_analytics_core" }], 0.9);
  }

  if (coverage.isOpenPronosticoQuestion(question)) {
    return makeIntent("open_pronostico", [{ value: "open_pronostico" }], 0.95);
  }
  if (backlog.isClientMovementQuestion(question)) {
    return makeIntent("client_movement", [{ value: "client_movement" }], 0.94);
  }
  if (backlog.isIndividualDiscountLookupQuestion(question)) {
    return makeIntent("client_profile", [{ value: "client_discount_individual" }], 0.93);
  }
  if (backlog.isClientDiscountRankingQuestion(question)) {
    return makeIntent("client_discount_ranking", [{ value: "client_discount_ranking" }], 0.94);
  }
  if (typeof isClientRankingQuestion === "function" && isClientRankingQuestion(question)) {
    return makeIntent("client_ranking", [{ value: "client_ranking" }], 0.9);
  }

  if (typeof isTallerMayorQuestion === "function" && isTallerMayorQuestion(question)) {
    return makeIntent("taller_mayor", [{ value: "taller_mayor_unidad" }], 0.88);
  }
  ...
  if (typeof isFolioSearchQuestion === "function" && isFolioSearchQuestion(question)) {
    return makeIntent("folio_search", [{ value: "folio_search" }], 0.88);
  }
  ...
  if (isRentabilidadDeterioroSnapshotQuestion(question)) {
    return makeIntent("financial_diagnosis", [{ value: "rentabilidad_deterioro_snapshot" }], 0.91, {
      domain_override: ["igf", "delta_ingreso"],
    });
  }
  if (backlog.isOpenDailySalesViewQuestion(question)) {
    return makeIntent("open_daily_sales_view", [{ value: "open_daily_sales_view" }], 0.93);
  }
  if (backlog.isClientDiscountRankingQuestion(question)) {
    return makeIntent("client_discount_ranking", [{ value: "client_discount_ranking" }], 0.93);
  }
  if (backlog.isClientContactLookupQuestion(question)) {
    return makeIntent("client_contact_lookup", [{ value: "client_contact_lookup" }], 0.92);
  }
  if (backlog.isProfitabilityPeriodRankingQuestion(question)) {
    return makeIntent("profitability_period_ranking", [{ value: "profitability_period_ranking" }], 0.94);
  }
  if (backlog.isProfitabilityPeriodComparisonQuestion(question)) {
    return makeIntent("profitability_period_comparison", [{ value: "profitability_period_comparison" }], 0.93);
  }
  if (backlog.isProfitabilityActionPlanQuestion(question)) {
    return makeIntent("profitability_action_plan", [{ value: "profitability_action_plan" }], 0.93);
  }
  if (backlog.isPlantDiscountOrMarginComparisonQuestion(question)) {
    return makeIntent("plant_metric_comparison", [{ value: "plant_metric_comparison" }], 0.92);
  }
  if (backlog.isFolioFinancialByPlantQuestion(question)) {
    return makeIntent("folio_search", [{ value: "folio_financial_by_plant" }], 0.91);
  }
  if (backlog.isAutotanqueGroupQuestion(question)) {
    return makeIntent("expense_analytics", [{ value: "taller_autotanque_group" }], 0.91);
  }
  if (backlog.isProcurementQuestion(question)) {
    return makeIntent("expense_analytics", [{ value: "procurement_by_concept" }], 0.9);
  }
  if (isIgfStatusFinancialSnapshotQuestion(question)) {
    return makeIntent("igf_status", [{ value: "igf_financial_snapshot" }], 0.92);
  }
  if (typeof isPreCloseQuestion === "function" && isPreCloseQuestion(question)) {
    return makeIntent("pre_meeting_brief", [{ value: "pre_close_compose" }], 0.9);
  }
  if (isDailyExecutiveBriefQuestion(q)) {
    return makeIntent("daily_executive_brief", [{ value: "daily_overview" }], 0.9);
  }
  if (typeof isHistoricalMarginQuestion === "function" && isHistoricalMarginQuestion(question)) {
    if (explicitClientScopeSpan(question)) {
      return makeIntent("client_profile", [{ value: "client_margin_as_historical_discount" }], 0.93);
    }
    return makeIntent("historical_margin", [{ value: "historical_margin_period" }], 0.94);
  }
  if (typeof isHistoricalNewClientsQuestion === "function" && isHistoricalNewClientsQuestion(question)) {
    return makeIntent("historical_new_clients", [{ value: "historical_new_clients_month" }], 0.93);
  }
  if (isDeltaIngresoForecastQuestion(question)) {
    return makeIntent("delta_income", [{ value: "delta_ingreso_forecast" }], 0.92);
  }
  ...
  if (typeof isClientProfileQuestion === "function" && isClientProfileQuestion(question)) {
    return makeIntent("client_profile", [{ value: "client_profile_semantic" }], 0.88);
  }
  if (isDiscountPeriodQuestion(question)) {
    const hasClient = Boolean(explicitClientScopeSpan(question));
    return makeIntent("client_profile", [{ value: "discount_period" }], hasClient ? 0.86 : 0.84, {
      requires_clarification: !hasClient,
      clarification_reason: hasClient
        ? null
        : "Pediste descuento de un periodo. Precisa el cliente o el alcance. No invento el valor.",
    });
  }
  if (typeof isMonthCloseQuestion === "function" && isMonthCloseQuestion(question)) {
    return makeIntent("month_close_result", [{ value: "month_close_compose" }], 0.88);
  }
  if (typeof isPreMeetingQuestion === "function" && isPreMeetingQuestion(question)) {
    return makeIntent("pre_meeting_brief", [{ value: "pre_meeting_compose" }], 0.88);
  }
  ...
  if (
    ...
    /\bventa\b/.test(q) &&
    !/\b(mezcla|mix|casa|comisionista)\b/.test(q)
  ) {
  ...
  if (backlog.isExplicitFolioIdQuestion(question) || backlog.isFolioIndexNavigationQuestion(question)) {
    return makeIntent("folio_search", [{ value: "folio_navigation" }], 0.91);
  }

  // Estado ambiguo
  if (
    /^(estado|como\s+esta|como\s+van)$/.test(q) ||
    (/\bestado\b/.test(q) &&
  ...
  const executiveFamily = coverage.classifyExecutiveFamily(question);
  if (executiveFamily) {
    return makeIntent("executive_coverage", [{ value: executiveFamily }], 0.86, {
      family: executiveFamily,
    });
  }
...
function planDirectorIaQuestion(question, options = {}) {
  const detected = options.forceIntent
    ...
    : detectDirectorIaIntent(question, { prior: options.prior || null });

  const inheritParent =
    !options.forceIntent &&
    options.inheritParentIntent &&
    INTENT_DOMAIN_MAP[options.inheritParentIntent] &&
    !inheritBlockedByExplicitMetric(question, options.inheritParentIntent) &&
    (detected.intent === "unknown" ||
      (options.inheritParentIntent === "client_discount_ranking" &&
        isGenericDiscountFallbackIntent(detected.intent, question)));
