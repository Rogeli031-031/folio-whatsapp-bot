...
const { resolveUploadDayLikeClientesPorMes, parseUploadDayYmd } = require("./igf-effective-proy-target");
...
/** Misma rama sin corte que isIgfMesCerradoPorCorte: mes anterior al calendario de now. */
function isCalendarMonthClosed(year, month, now) {
...
async function resolveOpenMonthUploadDay(deps, year, month) {
  if (typeof deps.resolveUploadDay === "function") {
    const raw = await deps.resolveUploadDay(deps.client || deps.pool, year, month, {
      upload_day: deps.upload_day || null,
    });
    if (raw && typeof raw === "object") return parseUploadDayYmd(raw.upload_day) || null;
    return parseUploadDayYmd(raw);
  }
...
async function loadKpiForMonth(deps, plantLabel, plantCode, year, month) {
  ...
  const closed = isCalendarMonthClosed(year, month, deps.now);
  let uploadDay = null;
  if (!closed) {
    uploadDay = await resolveOpenMonthUploadDay(deps, year, month);
  }
  ...
  return {
    ...
    operativos: numOrNull(row.operativos),
    corporativos: numOrNull(row.corporativos),
    gasto: numOrNull(row.gasto),
...
function buildProfitabilityExpenseFollowUpAnswer(opts) {
  ...
  return [
    ...
    "Si quieres, el siguiente paso puede ser seguir por la presión comercial ya observada o precisar qué comparativo de gasto sí está disponible, sin inventar causalidad.",
  ].join(" ");
}

function buildProfitabilityExpenseFollowUpChatResult(opts = {}) {
  const planta_id = opts.planta_id;
  return {
    ok: true,
    answer: buildProfitabilityExpenseFollowUpAnswer(opts),
    sources: [],
    context_meta: {
      mode: "profitability_expense_followup",
      requested_domain: "profitability_deterioro_snapshot",
      inherit: true,
      inherit_parent_intent: "profitability_deterioro_snapshot",
      openai_called: false,
      planta_id,
...
module.exports = {
  ...
  buildProfitabilityExpenseFollowUpAnswer,
