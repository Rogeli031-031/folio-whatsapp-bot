...
function writeDay(ws, r, day, ctx) {
  ...
  if (ownCosto || (historical && ctx.prior.some((item) => item.costo))) {
    ...
    if (historical && priors.length) markCarry(ws, ws.getCell(r, 6), ownCosto);
  }
  if (ownFlete || (historical && ctx.prior.some((item) => item.flete))) {
    ...
    if (historical && priors.length) markCarry(ws, ws.getCell(r, 7), ownFlete);
  }
...
function markCarry(ws, cell, own) {
  const addr = cell.address;
  const usingPrior = own
    ? `AND(ISNUMBER(${addr}),OR(NOT(ISNUMBER(${own})),${own}=0))`
    : `ISNUMBER(${addr})`;
  ws.addConditionalFormatting({
    ...
    rules: [{
      type: "expression",
      formulae: [usingPrior],
      style: { fill: { type: "pattern", pattern: "solid", bgColor: { argb: YELLOW } } },
    }],
  });
}
