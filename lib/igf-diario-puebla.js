...
function fillIgfDiarioPuebla(wb, opts) {
  ...
  weeks.forEach((week, index) => {
    ...
    for (const day of week) {
      ...
      writeDay(ws, r, day, {
        year, month, habiles, venta, comisiones, compras, canals, cols, precioRows, compraRows, corte, prior,
      });
      dayRows.push(r);
      history.push({
        fecha: day.fecha,
        row: r,
        costo: compraRows.get(day.fecha) && cols.costo ? ref("CONTROL DE COMPRAS", cols.costo, compraRows.get(day.fecha)) : null,
        flete: compraRows.get(day.fecha) && cols.tarifa ? ref("CONTROL DE COMPRAS", cols.tarifa, compraRows.get(day.fecha)) : null,
      });
      r += 1;
    }
    if (index === weeks.length - 1) r += 1;
    writeWeek(ws, r, index + 1, start, start + week.length - 1, index === 0);
    weekRows.push(r);
    r += 2;
  });
...
function writeDay(ws, r, day, ctx) {
  ...
  if (ownCosto || (historical && ctx.prior.some((item) => item.costo))) {
    ...
    if (historical && !rawNonZero(ctx.compras, compraRow, ctx.cols.costo) && priors.some((addr) => rawNonZeroAddr(ctx.compras, addr))) {
      paint(ws.getCell(r, 6), YELLOW);
    }
  }
  if (ownFlete || (historical && ctx.prior.some((item) => item.flete))) {
    ...
    if (historical && !rawNonZero(ctx.compras, compraRow, ctx.cols.tarifa) && priors.some((addr) => rawNonZeroAddr(ctx.compras, addr))) {
      paint(ws.getCell(r, 7), YELLOW);
    }
  }
...
function writeWeek(ws, r, n, start, end, first) {
  ...
  const fill = first ? WEEK_BLACK : WEEK_GRAY;
  const font = { bold: true, color: { argb: first ? WHITE : BLACK }, name: "Calibri", size: 11 };
  for (let c = 1; c <= 34; c += 1) paint(ws.getCell(r, c), fill, font);
...
function writeTotal(ws, r, dayRows, weekRows) {
  ...
  ws.getCell(r, 31).value = { formula: `IF(OR(NOT(ISNUMBER(B${r})),B${r}=0),"",AF${r}/B${r})` };
  for (let c = 1; c <= 34; c += 1) {
    paint(ws.getCell(r, c), WEEK_BLACK, { bold: true, color: { argb: WHITE }, name: "Calibri", size: 11 });
  }
  ws.getRow(r).height = 18.6;
  ws.getCell(r, 2).numFmt = FMT_INT;
  for (const c of [3, 4, 6, 7, 8, 13, 15, 20, 22, 24, 25, 27, 29, 31, 32]) ws.getCell(r, c).numFmt = FMT_2;
