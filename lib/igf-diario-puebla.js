...
function sheetNameFor(plant) {
  const name = String(plant || "Puebla").trim() || "Puebla";
  return `IGF Diario ${name}`;
}
...
function canalCols(ws, plant) {
  const token = foldPlant(plant || "Puebla");
  let casa = null;
  let com = null;
  for (let c = 1; c <= Math.max(ws.columnCount || 0, 40); c += 1) {
    const text = foldPlant(headerText(ws.getCell(1, c).value));
...
function findPlantSheet(wb, plant) {
  const wanted = foldPlant(sheetNameFor(plant));
  for (const ws of wb.worksheets || []) {
    if (foldPlant(ws.name) === wanted) return ws;
  }
  return null;
}

function reserveSheet(wb, plant) {
  const existing = findPlantSheet(wb, plant);
  if (existing) return existing;
  wb.addWorksheet(sheetNameFor(plant));
  return findPlantSheet(wb, plant);
}

function fillIgfDiarioPuebla(wb, opts) {
  const year = Number(opts && opts.year);
  const month = Number(opts && opts.month);
  const plantName = String((opts && opts.plantName) || "Puebla").trim() || "Puebla";
  const ws = reserveSheet(wb, plantName);
  ...
  const canals = venta ? canalCols(venta, plantName) : { casa: null, com: null };
  ...
  ws.getCell(2, 1).value = `PLANTA ${plantName.toUpperCase()}`;
