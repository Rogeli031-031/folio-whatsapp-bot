/**
 * Evidence Builder — runtime mínimo (02 v2.1, D1–D15 + N3 Rules D1–D16 + N4 Rules D1–D17).
...
const N3_CONTRADICTION_RULE_V1 = Object.freeze({
...
const N4_UNRESOLVED_CONFLICT_RULE_V1 = Object.freeze({
...
const N4_UNRESOLVED_CONFLICT_STATEMENT =
  "supported facts remain incompatible under the contradiction rule and the associated conflict remains OPEN";

const RULE_REGISTRY = Object.freeze({
  evidence_rules: Object.freeze([N3_CONTRADICTION_RULE_V1]),
  diagnostic_rules: Object.freeze([N4_UNRESOLVED_CONFLICT_RULE_V1]),
...
function isExcludedEntity(entity) {
  if (!entity) return false;
  return entity.resolution === "UNRESOLVED" || entity.resolution === "AMBIGUOUS";
}

function comparabilityKey(fact) {
  if (!fact || !isNonEmptyString(fact.fact_id)) return null;
  if (isExcludedEntity(fact.entity)) return null;
  const entity = fact.entity;
  let entityPart;
  if (!entity) {
    entityPart = "";
  } else if (isNonEmptyString(entity.entity_id)) {
    entityPart = entity.entity_id;
  } else {
    return null;
  }
  return `${entityPart}|${fact.metric_or_event || ""}|${fact.period || ""}`;
}

function activeContradictionRule() {
  return RULE_REGISTRY.evidence_rules.find(
    (rule) =>
      rule &&
      rule.status === "ACTIVE" &&
      rule.rule_id === N3_CONTRADICTION_RULE_V1.rule_id &&
      rule.rule_version === N3_CONTRADICTION_RULE_V1.rule_version &&
      rule.causal === false
  );
}

function to_n3(n2, context) {
  if (!Array.isArray(n2) || n2.length < 2) return [];
  const rule = activeContradictionRule();
  if (!rule) return [];
  ...
  for (const fact of n2) {
    const key = comparabilityKey(fact);
  ...
  for (const key of keys) {
    ...
    const distinct = new Set(group.map((fact) => stableValue(fact.value)));
    ...
    evidence.push({
      ...
      supporting_fact_ids: group.map((fact) => fact.fact_id).slice().sort(),
      applied_rule: {
        rule_id: N3_CONTRADICTION_RULE_V1.rule_id,
        rule_version: N3_CONTRADICTION_RULE_V1.rule_version,
      },
      materiality: "MATERIALITY_NOT_ASSESSED",
...
function activeUnresolvedConflictRule() {
  return (RULE_REGISTRY.diagnostic_rules || []).find(
    (rule) =>
      rule &&
      rule.status === "ACTIVE" &&
      rule.rule_id === N4_UNRESOLVED_CONFLICT_RULE_V1.rule_id &&
      rule.rule_version === N4_UNRESOLVED_CONFLICT_RULE_V1.rule_version &&
      rule.diagnostic_category === "UNRESOLVED_CONFLICT" &&
      rule.causal === false
  );
}

function sortedIds(ids) {
  return Array.from(new Set((ids || []).filter((id) => isNonEmptyString(id)))).sort();
}
...
function factsExist(ids, facts) {
  const known = new Set(
    (facts || []).filter((fact) => fact && isNonEmptyString(fact.fact_id)).map((fact) => fact.fact_id)
  );
  return ids.length > 0 && ids.every((id) => known.has(id));
}

function isEligibleN3Contradiction(evidence, context) {
  if (!evidence || evidence.evidence_type !== "CONTRADICTION") return false;
  if (evidence.causal_status !== "NON_CAUSAL") return false;
  if (!isNonEmptyString(evidence.evidence_id)) return false;
  const ctxTrace = context && context.trace_id;
  const evTrace = evidence.traceability && evidence.traceability.trace_id;
  if (ctxTrace && evTrace && ctxTrace !== evTrace) return false;
  return true;
}

function isEligibleTypeAOpenConflict(conflict) {
  return Boolean(
    conflict &&
      isNonEmptyString(conflict.conflict_id) &&
...
function to_n4(n3, context) {
  ...
  const rule = activeUnresolvedConflictRule();
  if (!rule) return [];

  const ctx = context || {};
  const facts = Array.isArray(ctx.facts) ? ctx.facts : [];
  const conflicts = Array.isArray(ctx.conflicts) ? ctx.conflicts : [];
  if (facts.length === 0 || conflicts.length === 0) return [];

  const idFactory = ctx.idFactory || defaultIdFactory(ctx.trace_id || "trace");
  const traceId = ctx.trace_id || null;

  const evidenceList = n3.filter((evidence) => isEligibleN3Contradiction(evidence, ctx));
  evidenceList.sort((a, b) => {
    if (a.evidence_id < b.evidence_id) return -1;
    if (a.evidence_id > b.evidence_id) return 1;
    return 0;
  });

  const conflictList = conflicts.filter(isEligibleTypeAOpenConflict);
  conflictList.sort((a, b) => {
    if (a.conflict_id < b.conflict_id) return -1;
    if (a.conflict_id > b.conflict_id) return 1;
    return 0;
  });
  ...
  for (const evidence of evidenceList) {
    const evidenceFacts = sortedIds(evidence.supporting_fact_ids);
    if (!factsExist(evidenceFacts, facts)) continue;
    for (const conflict of conflictList) {
      const tension = sortedIds(conflict.facts_in_tension);
      if (!sameIdSet(evidenceFacts, tension)) continue;
      if (!factsExist(tension, facts)) continue;
      diagnoses.push({
        diagnosis_id: idFactory("diagnosis"),
        diagnostic_category: "UNRESOLVED_CONFLICT",
        statement: N4_UNRESOLVED_CONFLICT_STATEMENT,
        classification_criterion: {
          rule_id: N4_UNRESOLVED_CONFLICT_RULE_V1.rule_id,
          rule_version: N4_UNRESOLVED_CONFLICT_RULE_V1.rule_version,
        },
        supporting_fact_ids: evidenceFacts.slice(),
        supporting_evidence_ids: sortedIds([evidence.evidence_id]),
        supporting_conflict_ids: sortedIds([conflict.conflict_id]),
