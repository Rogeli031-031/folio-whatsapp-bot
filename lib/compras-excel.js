...
const { hgCosto, hgImporte, rawDailyHgCost } = require("./compras-dashboard");
...
function buildComprasDailyEstimateContext(payload) {
  ...
  for (const day of days) {
    ...
    for (const p of providers) {
      ...
      const kgKey = `kg:${p.id}`;
      const impKey = `imp:${p.id}`;
      const kg = kgReal != null ? kgReal : priorMean(kgKey);
      const importe = impReal != null ? impReal : priorMean(impKey);
      if (kgReal != null) remember(kgKey, kgReal);
      if (impReal != null) remember(impKey, impReal);
      providersOut[p.id] = {
        kg,
        importe,
    ...
    const kilos = kilosReal != null ? kilosReal : priorMean("kilos");
    if (kilosReal != null) remember("kilos", kilosReal);
    const importeReal = realCapturedHgImporte(day);
    const hgImporteValue = importeReal != null ? importeReal : priorMean("captured-imp");
...
async function appendComprasWorksheet(wb, payload, opts = {}) {
  ...
  const estimateCtx = buildComprasDailyEstimateContext(payload);
  ...
  function writeHgBlock(row, src, opts) {
    ...
    const costo = daily && raw == null ? (carried === undefined ? aggregated : carried) : aggregated;
    ...
    if (daily && raw != null) {
      ...
      costoCell.value = costo == null ? null : costo;
  ...
  function writeDailyDerived(excelRow, day) {
    ...
    writeFormula(ws.getCell(excelRow, consStart), `IF(COUNT(${kgRefs.join(",")})=0,"",${kgRefs.join("+")})`, "#,##0.0");
    writeFormula(ws.getCell(excelRow, consStart + 2), `IF(COUNT(${impRefs.join(",")})=0,"",${impRefs.join("+")})`, "#,##0.00");
    const kgRef = `${colLetter(consStart)}${excelRow}`;
    const impRef = `${colLetter(consStart + 2)}${excelRow}`;
    writeFormula(ws.getCell(excelRow, consCostoCol), `IF(OR(${kgRef}="",${kgRef}=0),"",${impRef}/${kgRef})`, "0.000");
    providers.forEach((p, i) => {
      const startCol = fleteStarts[i];
      const purchaseRef = `${colLetter(starts[i])}${excelRow}`;
      writeFormula(ws.getCell(excelRow, startCol), `IF(${purchaseRef}="","",${purchaseRef})`, "#,##0.0");
      ...
      writeFormula(ws.getCell(excelRow, startCol + 2), `IF(OR(${kgCellRef}="",${tarifaRef}=""),"",${kgCellRef}*${tarifaRef})`, "#,##0.00");
    });
    ...
    writeFormula(ws.getCell(excelRow, fleteConsStart), `IF(COUNT(${fleteKgRefs.join(",")})=0,"",${fleteKgRefs.join("+")})`, "#,##0.0");
    writeFormula(
      ws.getCell(excelRow, fleteConsStart + 2),
      `IF(OR(${incomplete.join(",")}),"",IF(COUNT(${fleteImpRefs.join(",")})=0,"",${fleteImpRefs.join("+")}))`,
      "#,##0.00"
    );
    writeFormula(ws.getCell(excelRow, fleteConsStart + 1), `IF(OR(${aiRef}="",${aiRef}=0,${akRef}=""),"",${akRef}/${aiRef})`, "0.00");
  }
  ...
  for (const row of payload.grid.rows || []) {
    if (row.type === "day") {
      ...
      providers.forEach((p, i) => {
        ...
        writeFormula(ws.getCell(r, start + 1), `IF(OR(${kgRef}="",${impRef}="",${kgRef}=0),"",${impRef}/${kgRef})`, "0.000");
