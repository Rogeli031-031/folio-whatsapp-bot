...
async function buildComprasWorkbook(payload, opts = {}) {
  ...
  const hgCol = consStart + 4;
  const fleteStarts = blockStarts(providers.length).map((c) => c + hgCol + 1);
  ...
  for (let c = 2; c <= lastCol + 1; c += 1) {
    const inFlete = c >= hgCol + 2;
    const rel = inFlete ? c - (hgCol + 2) : c - 2;
    const isGap = rel % 4 === 3 || c === hgCol + 1;
  ...
  ws.mergeCells(titleRow, hgCol, metricRow, hgCol);
  const hgTitle = ws.getCell(titleRow, hgCol);
  hgTitle.value = "HG EN KILOS";
  paintTitle(hgTitle);
  paint(ws.getCell(metricRow, hgCol), HEADER, { bold: true, color: { argb: WHITE }, size: 8 });
  ...
  function setHg(cell, value) {
    paint(cell, WHITE, { bold: false });
    cell.alignment = { horizontal: "right", vertical: "middle" };
  ...
  function writeFleteTriple(row, startCol, cell, blankKgIfZero) {
    ...
    if (tarifa == null || !Number.isFinite(Number(tarifa))) {
      setNum(ws.getCell(row, startCol + 1), null, "0.00", WHITE, false, false);
    } else {
    ...
    if (importe == null || !Number.isFinite(Number(importe))) {
      setNum(ws.getCell(row, startCol + 2), null, "#,##0.00", WHITE, false, false);
    } else {
  ...
  function writeFleteRow(excelRow, flete, blankKgIfZero) {
    ...
    writeFleteTriple(excelRow, fleteConsStart, (flete && flete.consolidado) || { kg: 0, tarifa: null, importe: 0 }, blankKgIfZero);
  }
  ...
  for (const row of payload.grid.rows || []) {
    if (row.type === "day") {
      ...
      setHg(ws.getCell(r, hgCol), day ? day.hg_kilos : null);
      ...
      setHg(ws.getCell(r, hgCol), week ? week.hg_kilos : null);
      ws.getCell(r, hgCol).font = { bold: true, name: "Calibri", size: 9 };
  ...
  setHg(ws.getCell(r, hgCol), monthRow ? monthRow.hg_kilos : null);
  ws.getCell(r, hgCol).font = { bold: true, name: "Calibri", size: 9 };
