...
async function loadArrFromBuffer(client, plantCode, fileBuffer, options = {}) {
  ...
  for (const v of allVentas) {
    ...
    const subcanal = (v.subcanal != null && v.subcanal !== "") ? v.subcanal : (cat && cat.subcanal) || null;
    await client.query(
      `INSERT INTO arr.ventas_diarias_cliente (plant_code, fecha, cliente_norm, canal, subcanal, kg)
       VALUES ($1, $2::date, $3, $4, $5, $6)
       ON CONFLICT (plant_code, fecha, cliente_norm, canal, COALESCE(subcanal, ''))
       DO UPDATE SET kg = arr.ventas_diarias_cliente.kg + EXCLUDED.kg`,
      [plantCode, v.fecha, v.cliente, canal, subcanal, v.kg]
    );
  }
  ...
  for (const c of catCatalog) {
    await client.query(
      ...
      [plantCode, year, month, c.cliente, c.canal, c.subcanal || null]
    );
  }
