 ...
 * Estrategia: borrar SOLO el mes objetivo e insertar SOLO filas de ese mes
 * (filas de otros meses en el Excel se ignoran; no se corrompen meses previos).
...
/**
 * Detecta mes objetivo a partir de las fechas del archivo (Total + Categoria).
 * Si no hay fechas, devuelve mes actual del sistema.
 */
function detectTargetMonth(workbook, fallbackDate = new Date()) {
  const dates = [];
  for (const sh of ["Total", "Categoria", "Notas", "Factura", "Comision Extra"]) {
    const rows = sheetToRows(workbook, sh);
    for (const row of rows) {
      const f = parseDate(getCol(row, "Fecha", "Fecha de vencimiento"));
      if (f) dates.push(f);
    }
  }
  if (dates.length === 0) {
...
function extractCategoria(rows, todayStr) {
  ...
  for (const row of rows) {
    ...
    catalog.push({ cliente, canal, subcanal });
  }
...
/** True si fecha YYYY-MM-DD cae en year/month. */
function isInYearMonth(fecha, year, month) {
 ...
 * Estrategia: borrar SOLO el mes objetivo e insertar SOLO filas de ese mes
 * (aunque el Excel traiga otros meses, no se tocan meses anteriores).
 ...
 * @returns {Promise<{ year, month, ventasCount, descuentosCount, catalogCount, skippedOtherMonths }>}
 */
async function loadArrFromBuffer(client, plantCode, fileBuffer, options = {}) {
  ...
  const { ventas: totalVentas, descuentos: totalDesc } = extractTotal(totalRows, todayStr);
  const notasDesc = extractNotas(notasRows, todayStr);
  const facturaDesc = extractFactura(facturaRows, todayStr);
  const comisionDesc = extractComisionExtra(comisionRows, todayStr);
  const { ventas: catVentas, catalog: catCatalog } = extractCategoria(categoriaRows, todayStr);

  const mergedDescuentos = mergeDescuentos([totalDesc, notasDesc, facturaDesc, comisionDesc]);
  const mergedVentas = mergeVentas(totalVentas, catVentas);
  const skippedOtherMonths =
    mergedVentas.filter((v) => !isInYearMonth(v.fecha, year, month)).length +
    mergedDescuentos.filter((d) => !isInYearMonth(d.fecha, year, month)).length;
  const allDescuentos = mergedDescuentos.filter((d) => isInYearMonth(d.fecha, year, month));
  const allVentas = mergedVentas.filter((v) => isInYearMonth(v.fecha, year, month));
  ...
  for (const v of allVentas) {
    ...
    await client.query(
      `INSERT INTO arr.ventas_diarias_cliente (plant_code, fecha, cliente_norm, canal, subcanal, kg)
       ...
       DO UPDATE SET kg = arr.ventas_diarias_cliente.kg + EXCLUDED.kg`,
  ...
  return {
    ...
    catalogCount: catCatalog.length,
    skippedOtherMonths,
  };
}
