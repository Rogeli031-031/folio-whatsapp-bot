...
async function tryLatestForecastContext(deps, plant, year, month) {
  if (!deps || typeof deps.queryLatestVersion !== "function" || typeof deps.queryLines !== "function") {
    return null;
  }
  const version = await deps.queryLatestVersion(deps.client, year, month);
  if (!version || String(version.financial_state || "") !== "FORECAST") return null;
  ...
  if (match.ambiguous || !match.ok || !match.row) return null;
...
async function loadClosedMonth(deps, plant, year, month) {
  ...
  if (finals.length === 0) {
    ...
    const forecast = await tryLatestForecastContext(deps, plant, year, month);
    if (forecast) {
      missing.forecast_context = forecast;
      missing.forecast_used = true;
    }
    return missing;
  }
  if (finals.length > 1) {
...
function buildSingleAnswer(ev, plantaNombre) {
  ...
  if (ev.status !== "valid") {
    ...
    const forecast = ev.forecast_context;
    if (
      ev.reason === "NOT_FINAL" &&
      forecast &&
      forecast.status === "valid" &&
      forecast.margin_kg != null
    ) {
      return [
        `No hay un margen histórico FINAL defendible para ${monthBare(ev.month)} ${ev.year}.`,
        `La última proyección disponible (FORECAST, vista vigente) muestra ${formatMarginKg(forecast.margin_kg)} $/kg.`,
        `No lo presento como cierre real.`,
      ].join(" ");
    }
...
function usableForecastContext(ev) {
  const fc = ev && ev.forecast_context;
  if (!fc || fc.status !== "valid" || fc.margin_kg == null) return null;
  return fc;
}

function hasUsableCompareEvidence(ev) {
  return Boolean(ev && (ev.status === "valid" || usableForecastContext(ev)));
}

function comparePeriodLines(ev) {
  if (ev.status === "valid") {
    const tag = ev.truth_class === "FORECAST" ? " (forecast, mes abierto)" : "";
    return [`${monthTitle(ev.year, ev.month)}: ${formatMarginKg(ev.margin_kg)} $/kg${tag}`];
  }
  if (ev.status === "error") return [periodErrorLine(ev)];
  if (ev.period_kind === "future_month") {
    return [`${monthTitle(ev.year, ev.month)}: periodo futuro. Sin margen histórico.`];
  }
  const fc = usableForecastContext(ev);
  if (fc) {
    return [
      `${monthTitle(ev.year, ev.month)} no tiene un margen FINAL defendible.`,
      `Existe contexto FORECAST vigente de ${formatMarginKg(fc.margin_kg)} $/kg. No lo presento como cierre real.`,
    ];
  }
  return [`${monthTitle(ev.year, ev.month)}: sin margen FINAL defendible.`];
}
...
function buildCompareAnswer(a, b) {
  ...
  const lines = [...comparePeriodLines(a), ...comparePeriodLines(b)];
  if (!comparable) {
...
async function loadHistoricalMarginForChat(pool, plantaId, req, opts = {}) {
  ...
  const run = async (client) => {
    ...
    if (requested.operation === "compare_months") {
      ...
      const usableA = hasUsableCompareEvidence(a);
      const usableB = hasUsableCompareEvidence(b);
      const bothValidComparable = compared.comparable;
      let ok;
      let status;
      let code;
      if (anyError && !usableA && !usableB) {
        ok = false;
        status = 500;
        code = DIRECTOR_IA_VERACITY.SOURCE_ERROR;
      } else if (anyError) {
        ...
        ok = true;
        status = 200;
        code = DIRECTOR_IA_VERACITY.SOURCE_AVAILABLE;
      } else if (usableA || usableB) {
        ok = true;
        status = 200;
        code = DIRECTOR_IA_VERACITY.SOURCE_PARTIAL;
      } else {
      ...
      return {
        ...
        forecast_used: Boolean(a.forecast_used || b.forecast_used || usableForecastContext(a) || usableForecastContext(b)),
