...
async function executeDeltaIngresoCommand(cmd, opts) {
  ...
  if (cmd.type === "pendientes") {
    if (!aiDb || !client) return "No disponible.";
    const list = await aiDb.getPendientes(client, pa, pb);
    if (!list.length) return "No hay acciones pendientes.";
    const lines = ["📋 Pendientes (por impacto)", ...list.slice(0, 10).map((a, i) => `${i + 1} ${a.plant_code} ${a.cliente_norm}: ${a.action_status}${a.how_much_impact_mxn != null ? " " + fmtMxn(a.how_much_impact_mxn) : ""}`)];
    return lines.join("\n");
  }
...
async function handleDeltaIngresoCommand(opts) {
  ...
  const pa = PERIODO_AI_A;
  const pb = PERIODO_AI_B;
  ...
  const out = (replyText || "").substring(0, 400);
