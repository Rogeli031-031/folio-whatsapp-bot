...
function classifyPair(a, b, channel) {
  ...
  return {
    ...
    descKg: b.descKg != null ? Number(b.descKg) : kgB > 0 ? Math.abs(Number(b.monto) || 0) / kgB : null,
    movement,
  };
}

function computeMovementAggregates(rows, movement) {
  const matched = (rows || []).filter((r) => r && (movement === "SUMMARY" || r.movement === movement));
  let lostKg = 0;
  let boughtKg = 0;
  let monto = 0;
  ...
  return {
    ...
    discount_kg: boughtKg > 0 ? monto / boughtKg : null,
...
async function loadClientMovementForChat(pool, plantaId, req, opts = {}) {
  ...
  if (Array.isArray(opts.movementRows)) {
    ...
    const aggregates = computeMovementAggregates(opts.movementRows, spec.movement);
    return {
      ok: true,
      spec,
      ranked,
  ...
  const aggregates = computeMovementAggregates(classified, spec.movement);
  return {
    ok: true,
    spec,
    ranked,
...
function buildClientMovementAnswer(payload) {
  ...
  if (wantAgg && spec.movement === "NUEVOS") {
    ...
    if (spec.want_discount) {
      if (agg.has_discount_evidence && agg.discount_kg != null) {
        header.push(`Descuento ARR (ABS(SUM(monto_descuento))/SUM(kg)): ${agg.discount_kg.toFixed(4)} $/kg.`);
      } else {
        header.push("No tengo monto_descuento y kg suficientes para calcular descuento_kg. No invento el descuento.");
      }
    } else {
  ...
  const lines = ranked.map((row, i) => {
    ...
    const extra =
      spec.want_discount && row.descKg != null ? ` — descuento ${Number(row.descKg).toFixed(4)} $/kg` : "";
        ...
        active_entities: [
          {
            ...
            result_set: ((payload && (payload.matched || payload.ranked)) || []).map((r) => r.cliente).slice(0, 50),
          },
        ],
