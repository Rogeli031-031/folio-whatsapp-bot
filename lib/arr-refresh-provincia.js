 ...
 * - Si ARR_ZONA_PROVINCIA está definida, inserta esa lista y además asegura las 6 plantas provincia de public.plantas (para no perder ej. Tehuacán).
 * - Si no, rellena solo desde public.plantas: primeras 6 por id, excluyendo Corporativo.
...
async function syncProvinciaPlants(client) {
  ...
  // Siempre asegurar las primeras 6 de public.plantas (por id) para que el dashboard muestre todas las columnas
  await client.query(`
  ...
  `);
  // Por si el orden por id no incluye Morelos/Puebla/Tehuacán/etc.: asegurar estas 6 por nombre (si existen en public.plantas)
  const nombresProvincia = ["Acapulco", "Morelos", "Puebla", "Querétaro", "San Luis", "Tehuacán"];
  for (const nom of nombresProvincia) {
    await client.query(
      `INSERT INTO arr.provincia_plants (plant_code) SELECT p.nombre FROM public.plantas p WHERE UPPER(TRIM(COALESCE(p.nombre,''))) = UPPER(TRIM($1)) OR (p.clave IS NOT NULL AND UPPER(TRIM(p.clave)) = UPPER(TRIM($1))) LIMIT 1 ON CONFLICT (plant_code) DO NOTHING`,
      [nom]
    );
  }
}
