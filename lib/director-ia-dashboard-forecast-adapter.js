...
function findMiniRowForPlant(miniRows, plantLabel) {
  const want = normalizeEmpresaLabel(plantLabel);
  if (!want || !Array.isArray(miniRows)) return null;
  return miniRows.find((r) => normalizeEmpresaLabel(r && r.empresa) === want) || null;
}
 ...
 */
function readIgfForecastMiniAuthoritative(miniPayload, plantLabel) {
  const row = findMiniRowForPlant(miniPayload && miniPayload.rows, plantLabel);
  if (!row) {
    return {
      venta_ton: null,
      desc_kg: null,
...
async function loadDashboardForecastParity(pool, opts = {}) {
  ...
  const empty = {
    ...
    helper:
      "loadPronosticoMiniSnapshot + computePronosticoProyByPlant + resolveProyFromPlantMap + getVentaRealTonProvinciaByPlant",
  };
  ...
  try {
    ...
    return {
      ...
      forecast: {
        ...forecast,
        source: "loadPronosticoMiniSnapshot overlay + computePronosticoProyByPlant",
