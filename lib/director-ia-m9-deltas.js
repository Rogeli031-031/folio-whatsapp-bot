...
async function getDeltaIngresoClientes(client, plantaNombre, periodoA, periodoB, opts = {}) {
  ...
  if (!Number.isFinite(yA) || !Number.isFinite(mA) || !Number.isFinite(yB) || !Number.isFinite(mB)) {
    return {
      ...
      margenB: null,
      exact_ingreso: false,
      availability: { margenA: DIRECTOR_IA_VERACITY.DATA_NOT_FOUND, margenB: DIRECTOR_IA_VERACITY.DATA_NOT_FOUND },
      missing_inputs: ["periodo"],
    };
  }
  const loadMargen = opts.getMargenKgPorPeriodo || getMargenKgPorPeriodo;
  const margenA = finiteOrNull(await loadMargen(client, plantaNombre, yA, mA, opts));
  const margenB = finiteOrNull(await loadMargen(client, plantaNombre, yB, mB, opts));
  const r = await client.query(
    `WITH prov_map AS (${SQL_PROV_MAP}),
     desc_mes AS (
       SELECT pm.prov_name AS planta,
  ...
  const missingInputs = [];
  if (margenA == null) missingInputs.push("margenA");
  if (margenB == null) missingInputs.push("margenB");
  const exactIngreso = missingInputs.length === 0;
  ...
  return {
    ...
    margenA,
    margenB,
    exact_ingreso: exactIngreso,
    availability: {
      margenA: margenA != null ? DIRECTOR_IA_VERACITY.SOURCE_AVAILABLE : DIRECTOR_IA_VERACITY.DATA_NOT_FOUND,
      margenB: margenB != null ? DIRECTOR_IA_VERACITY.SOURCE_AVAILABLE : DIRECTOR_IA_VERACITY.DATA_NOT_FOUND,
    },
...
async function getDeltaIngresoDatosInternal(client, planta, periodoA, periodoB, sinRegla8020, opts = {}) {
  ...
  const {
    ...
    margenA,
    margenB,
    exact_ingreso: exactIngreso = true,
    availability = null,
    missing_inputs: missingInputs = [],
  } = await getDeltaIngresoClientes(client, planta.trim(), pa, pb, opts);
  const margenAStr = fmtDescKg(margenA);
  const margenBStr = fmtDescKg(margenB);
  if (exactIngreso === false) {
    const empty = (signPositive) => ({
      totalDeltaIngreso: null,
      totalDeltaIngresoStr: FMT_UNAVAILABLE,
      signPositive,
      clientes: [],
      totalTonA: null,
      totalTonB: null,
      totalTonAStr: FMT_UNAVAILABLE,
      totalTonBStr: FMT_UNAVAILABLE,
    });
    return {
      planta: planta.trim(),
      periodoA: pa,
      ...
      availability,
      missing_inputs: missingInputs,
      totalTonAGeneralStr: FMT_UNAVAILABLE,
      totalTonBGeneralStr: FMT_UNAVAILABLE,
      dejaron: empty(false),
      mas: empty(true),
      disminuyeron: empty(false),
      clientesNuevos: empty(true),
      crecen: empty(true),
      estables: empty(false),
      otrosClientes: empty(false),
    };
  }
  const build = (filterFn, sortFn, totalReduce, signPositive) => {
    const candidatos = (rows || []).filter(filterFn).sort(sortFn);
...
async function loadFamilyForChat(pool, plantaId, req, opts, family) {
  ...
  return runWithClient(pool, { injected: Boolean(opts.resolvePlanta && opts.listPeriodos && opts.loadDatos) }, family.domain, async (client) => {
    ...
    const exactIngreso = datos.exact_ingreso !== false;
    const availabilityStatus = exactIngreso
      ? DIRECTOR_IA_VERACITY.SOURCE_AVAILABLE
      : DIRECTOR_IA_VERACITY.SOURCE_PARTIAL;
    return {
      ok: true,
      ...
      missing_inputs: Array.isArray(datos.missing_inputs) ? datos.missing_inputs : [],
    };
  });
}
...
async function loadDeltaIngresoForChat(pool, plantaId, req, opts = {}) {
  return loadFamilyForChat(pool, plantaId, req, {
    ...
    unit: "MXN",
    source_coercion: "margen IGF ausente → no se calcula ingreso exacto; cliente ausente en mes disponible = 0 kg.",
    not: ["delta_venta", "delta_descuento", "delta_ingreso_forecast", "m19", "igf_annex", "arr_snapshot"],
  });
}

function chatVeracity(payload) {
  if (payload && payload.ok && payload.availability_status === DIRECTOR_IA_VERACITY.SOURCE_PARTIAL) {
    return DIRECTOR_IA_VERACITY.SOURCE_PARTIAL;
  }
...
function buildDeltaIngresoAnswer(payload) {
  ...
  const d = payload.datos || {};
  const scope = `${payload.planta_nombre || `planta ${payload.planta_id}`}`;
  if (payload.availability_status === DIRECTOR_IA_VERACITY.SOURCE_PARTIAL || d.exact_ingreso === false) {
    const missing = Array.isArray(payload.missing_inputs) && payload.missing_inputs.length
      ? payload.missing_inputs.join(", ")
      : "margen IGF";
    return (
      `Delta Ingreso: NO DISPONIBLE para ${scope} (${payload.periodoA} → ${payload.periodoB}). ` +
      `Faltan insumos requeridos (${missing}); no calculo ingreso exacto ni deltaIngreso exacto. ` +
      `null no es 0. No es Delta Venta, Delta Descuento, forecast de ingreso ni M19. ` +
  ...
  return (
    ...
    `Margen A: ${d.margenAStr || FMT_UNAVAILABLE}; Margen B: ${d.margenBStr || FMT_UNAVAILABLE}.\n` +
