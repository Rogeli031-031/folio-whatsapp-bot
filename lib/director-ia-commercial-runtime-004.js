...
function classifyCommercialRuntimeFamily(question, prior) {
  ...
  if (
    prior &&
    (prior.family === "NEW_CLIENTS" || prior.movement === "NUEVOS") &&
    (/\btotal\b/.test(n) && /\bnuevos?\b/.test(n) || /\bcuantos\b/.test(n) && /\b(nuevos?|fueron|entraron|total)\b/.test(n))
  ) {
    return "NEW_CLIENTS";
  }
...
function shouldInheritMovementContext(question, prior) {
  ...
  return Boolean(
    /\bcon\s+ellos\b/.test(n) ||
      ...
      (/\bcuantos\b/.test(n) && /\b(nuevos?|fueron|entraron|total)\b/.test(n)) ||
      /\bdejamos\s+de\s+vender\b/.test(n) ||
      /\bcayeron\s+a\s+cero\b/.test(n) ||
