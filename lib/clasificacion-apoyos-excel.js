...
async function buildClasificacionApoyosWorkbook(rows, mesA, mesB, options) {
  ...
  const detalleByPlanta = (options && options.detalleGastosByPlanta) || {};
  for (const cfg of PLANTAS_GASTOS_SHEETS) {
    addPlantaGastosSheet(wb, detalleByPlanta[cfg.clave] || [], mesA, cfg);
  }
...
/**
 * Hojas de listado de gastos por planta (misma lógica/formato que E7 G).
 * clave = código hoja; prefijoFecha = columna FECHA (ej. PL/JUL).
 */
const PLANTAS_GASTOS_SHEETS = [
  { clave: "E7", sheetName: "E7 G", title: "PUEBLA", prefijoFecha: "PL", ids: [2, 14] },
  { clave: "E8", sheetName: "E8 G", title: "TEHUACÁN", prefijoFecha: "PL", ids: [3, 15] },
  { clave: "E9", sheetName: "E9 G", title: "ACAPULCO", prefijoFecha: "PL", ids: [1, 11, 12] },
  { clave: "E12", sheetName: "E12 G", title: "QUERÉTARO", prefijoFecha: "PL", ids: [4, 16] },
  { clave: "E13", sheetName: "E13 G", title: "SAN LUIS POTOSÍ", prefijoFecha: "PL", ids: [5, 17, 18] },
  { clave: "E15", sheetName: "E15 G", title: "MORELOS", prefijoFecha: "PL", ids: [6, 13] },
];
...
function expandDetalleGastosRows(detalleRows) {
  const out = [];
...
/**
 * Listado de gastos por planta (E7 G, E8 G, …): subcategorías, SUMA, TOTAL, firmas.
 * FECHA = {prefijo}/(mes); FECHA DE ENVÍO = Depósito y cierre; amarillo si ya depositó.
 */
function addPlantaGastosSheet(wb, detalleRows, mesA, plantaCfg) {
  const labelMes = mesLabelEs(mesA);
  const mesAbrev = mesAbrevFromYyyyMm(mesA);
  const prefijo = (plantaCfg && plantaCfg.prefijoFecha) || "XX";
  const sheetName = (plantaCfg && plantaCfg.sheetName) || "G";
  ...
  for (const sc of PLANTA_GASTO_SECTIONS) bySection.set(sc.key, []);
  for (const r of expanded) {
    const key = matchGastoSubcat(r.subcategoria);
  ...
  let row = 4;
  let grandTotal = 0;
  const sectionsWithData = PLANTA_GASTO_SECTIONS.filter((sc) => (bySection.get(sc.key) || []).length > 0);

  if (!sectionsWithData.length) {
    ws.getCell(`B${row}`).value = `(Sin folios de GASTOS para ${titleName} en este mes)`;
  ...
  totLabel.value = `TOTAL GASTOS ${labelMes}`;
