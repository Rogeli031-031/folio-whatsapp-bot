...
function addPlantaTallerSheet(wb, detalleRows, mesA, plantaCfg) {
  ...
  // C11 = SUMA(D11:G11) — el total vive en el bloque B:C (muestra C11)
  ...
  // Exponer también en C para fórmulas D12=D11/$C$11
  ws.getCell(`C${sumaRow}`).value = { formula: `SUM(D${sumaRow}:G${sumaRow})` };
  ws.getCell(`C${sumaRow}`).numFmt = NUM_FMT;
  ws.getCell(`C${sumaRow}`).alignment = { horizontal: "right", vertical: "middle" };
  ...
  ["D", "E", "F", "G"].forEach((col) => {
    ...
    cell.value = { formula: `${col}${sumaRow}/$C$${sumaRow}` };
    cell.numFmt = PCT_FMT;
