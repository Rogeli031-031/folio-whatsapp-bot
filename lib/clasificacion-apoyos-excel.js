...
/**
 * Tras expandDetalleLineasRows: homologa unidad y reparte importe si hay varias pipas.
 * Fila grupo (importe 0) + sublíneas parciales.
 */
function expandTallerUnidadesEnDetalle(detalleRows) {
  const out = [];
  for (const r of detalleRows || []) {
    ...
    if (unidades.length <= 1) {
      const unidad =
        unidades[0] ||
        unidadTaller.normalizeUnidadToken(r.unidad) ||
        String(r.unidad || "").trim() ||
        "—";
      out.push({ ...r, unidad, importe, row_kind: "normal" });
      continue;
    }

    const stored = unidadTaller.formatUnidadesStored(unidades);
    out.push({ ...r, unidad: stored, importe: 0, concepto, row_kind: "grupo" });
    for (const p of unidadTaller.splitImportePorUnidades(importe, unidades)) {
      out.push({
        ...r,
        unidad: p.unidad,
        importe: p.importe,
...
function addPlantaTallerSheet(wb, detalleRows, mesA, plantaCfg) {
  ...
  const idSet = new Set((plantaCfg.ids || []).map(Number));
  const expanded = expandTallerUnidadesEnDetalle(expandDetalleLineasRows(detalleRows)).filter((r) => {
    if (!Number.isFinite(Number(r.importe))) return false;
    if (idSet.size && r.planta_id != null && !idSet.has(Number(r.planta_id))) return false;
    return incluirEnListadoMes(r, mesA);
  });
  ...
  expanded.sort((a, b) => {
    ...
    return String(a.unidad || "").localeCompare(String(b.unidad || ""), "es", { numeric: true });
  });
  ...
  for (const item of expanded) {
    const isGrupo = item.row_kind === "grupo";
    const importe = Math.round(Number(item.importe) * 100) / 100;
    ...
    const flags = folioFlagsDetalle(item);
    // Solo sumar parciales/normales (grupo lleva importe 0).
    if (!isGrupo && cuentaEnSumaListado(item, mesA)) sumRows.push(row);

    const a = ws.getCell(`A${row}`);
    a.value = unidad;
    a.font = { name: FONT, size: isGrupo ? 9 : 10, bold: !!isGrupo };
    a.border = borderAll;
    a.alignment = { horizontal: isGrupo ? "left" : "center", wrapText: !!isGrupo };

    safeMerge(ws, `B${row}:C${row}`);
    const b = ws.getCell(`B${row}`);
    b.value = String(item.concepto || "").trim() || "—";
    b.font = { name: FONT, size: 9, italic: item.row_kind === "parcial" };
    b.border = borderAll;
