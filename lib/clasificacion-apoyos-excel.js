 ...
 * Layout alineado a la plantilla: D5:H5 subcats, I = total renglón, fila 14 totales.
 */
function addInversionesSheet(wb, rows, mesA) {
  ...
  INVERSION_SUBCATS.forEach((sc, i) => {
    const cell = ws.getCell(`${cols[i]}5`);
    cell.value = sc.label;
    applyFont(cell, { bold: true, color: { argb: "FFFFFFFF" }, size: 10 });
    applyFill(cell, "FFFF0000"); // rojo brillante como muestra
    cell.alignment = { horizontal: "center", vertical: "middle", wrapText: true };
    cell.border = thinBorder();
  });

  const hInv = ws.getCell("I5");
  hInv.value = "INVERSION";
  applyFont(hInv, { bold: true, color: { argb: "FFFFFFFF" }, size: 10 });
  applyFill(hInv, "FFC00000"); // rojo oscuro / maroon
  ...
  for (const r of rows || []) {
    ...
    const canon = idToCanon.get(Number(r.planta_id));
    if (canon == null) continue;
    const subKey = matchInversionSubcat(r.subcategoria);
    if (!subKey) continue;
    const imp = Number(r.importe);
    if (!Number.isFinite(imp)) continue;
  ...
  PLANTAS_COMPARATIVO.forEach((p, i) => {
    ...
    INVERSION_SUBCATS.forEach((sc, j) => {
      const col = cols[j];
      const amount = byKey.get(`${i}|${sc.key}`) || 0;
    ...
    const tot = ws.getCell(`I${row}`);
    tot.value = { formula: `SUM(D${row}:H${row})` };
    tot.numFmt = NUM_FMT;
  ...
  const i14 = ws.getCell("I14");
  i14.value = { formula: "SUM(I7:I12)" };
  i14.numFmt = NUM_FMT;
  applyFont(i14, { bold: true });
  applyFill(i14, "FFD9D9D9");
  i14.border = thinBorder();
  i14.alignment = { horizontal: "center" };
