 ...
 * @param {{ detalleE7G?: Array }} [options]
 */
async function buildClasificacionApoyosWorkbook(rows, mesA, mesB, options) {
  ...
  addE7GastosSheet(wb, (options && options.detalleE7G) || [], mesA);
...
/** Orden de secciones en listado E7 G (gastos Puebla). */
const E7_GASTO_SECTIONS = [
...
function formatFechaEnvioMx(val) {
  ...
  // Usar componentes UTC para fechas DATE de Postgres sin corrimiento de zona.
...
function expandDetalleE7Rows(detalleRows) {
  const out = [];
...
/**
 * Hoja "E7 G": listado de gastos planta Puebla por subcategoría (mes principal).
 * FECHA = PL/(mes); FECHA DE ENVÍO = llegada a Depósito y cierre; amarillo si ya depositó.
 */
function addE7GastosSheet(wb, detalleRows, mesA) {
  const labelMes = mesLabelEs(mesA);
  const mesAbrev = mesAbrevFromYyyyMm(mesA);
  const codigoFecha = mesAbrev ? `PL/${mesAbrev}` : "PL";
  const ws = wb.addWorksheet("E7 G", { views: [{ showGridLines: true }] });

  const FONT = "Courier New";
  const NUM_FMT = '$#,##0.00;-$#,##0.00;"-"';
  const HDR_FILL = "FF8497B0"; // azul-gris claro (encabezados)
  const SUMA_FILL = "FF767171"; // gris SUMA
  const TOTAL_FILL = "FFB20000"; // rojo TOTAL GASTOS
  const YELLOW_FILL = "FFFFF2CC"; // amarillo muy claro si ya depositó
  ...
  const expanded = expandDetalleE7Rows(detalleRows).filter((r) => {
    const mes = String(r.mes_cargo || "").trim();
    return mes === mesA && Number.isFinite(Number(r.importe));
  });

  const bySection = new Map();
  for (const sc of E7_GASTO_SECTIONS) bySection.set(sc.key, []);
  ...
  let row = 4;
  let grandTotal = 0;
  const sectionsWithData = E7_GASTO_SECTIONS.filter((sc) => (bySection.get(sc.key) || []).length > 0);

  if (!sectionsWithData.length) {
    ws.getCell(`B${row}`).value = "(Sin folios de GASTOS para Puebla en este mes)";
