...
const FOLIO_SELECT_COLUMNS = `f.id,
            f.numero_folio,
            f.folio_codigo,
            f.planta_id,
            f.mes_cargo,
...
async function queryPublicFoliosByPlant(client, plantaId, opts = {}) {
  const ids = plantScopeIds(plantaId, opts.resolveEquivalentIds);
  const r = await client.query(
    `SELECT ${FOLIO_SELECT_COLUMNS}
       FROM public.folios AS f
       LEFT JOIN public.plantas p ON p.id = f.planta_id
       LEFT JOIN public.proyectos pr ON pr.id = f.proyecto_id
      WHERE f.planta_id = ANY($1::int[])
      ORDER BY f.mes_cargo DESC NULLS LAST, f.id`,
    [ids.length ? ids : [Number(plantaId)]]
  );
  return r.rows || [];
}
