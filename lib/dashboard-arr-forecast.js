...
function applyIgfMiniResumenFormulas(worksheet, row0Header, plantCount, nCols) {
  ...
  for (let i = 0; i < plantCount; i++) {
    ...
    // H = SUM(M:P)*B*1000 (operativos) tomando la fila fuente
    // Nota: en archivos .xlsx las funciones se guardan en inglés (SUM), aunque Excel muestre SUMA en UI.
    excelTheme.setCellFormula(worksheet, dstExcelRow - 1, 7, `=SUM(M${srcExcelRow}:P${srcExcelRow})*B${dstExcelRow}*1000`);
