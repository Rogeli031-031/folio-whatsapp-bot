...
/** Columnas (0-based) E,F,I,J,M,N,O,P — regla de tres respecto a fila IGF y columna B. */
const IGF_RULE_OF_THREE_COL_INDICES = [4, 5, 8, 9, 12, 13, 14, 15];
...
/**
 * Bloque resumen plantilla 18 col: H regla de tres vs tabla superior; K, Q sumas; L,R importes.
 * H29 = H18*G29/G18; K = C+D-E-F+H-I-J; Q = K+M-N-O-P; L = K*B*1000; R = Q*B*1000.
 * Fila Zona Provincia: sin H (sin fila IGF); K,Q,L,R sí.
 */
function applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCols) {
  ...
  const setDerivedForRow = (r0, igfR) => {
    const er = r0 + 1;
    if (igfR != null) {
  ...
  for (let i = 0; i < plantMetaList.length; i++) {
    setDerivedForRow(row0Header + 1 + i, plantMetaList[i].igfMargenRow);
  }
  const rZona = row0Header + 1 + plantMetaList.length;
  setDerivedForRow(rZona, null);
