  ...
  for (const p of plants) {
    ...
    if (ctx.enableLookback && ctx.lookbackStartYmd && ctx.lookbackEndYmd) {
      ...
      for (let i = 0; i < promDayCountDesc; i++) {
        ...
        const ok = selPlant.has(key) ? selPlant.get(key) !== false : inStrict;
        if (!ok) continue;
...
function buildVentaPronosticoSheetLike(year, month, ctx, plantKey, selPlant) {
  ...
  if (ctx.enableLookback && ctx.lookbackStartYmd && ctx.lookbackEndYmd) {
    ...
    for (let i = 0; i < promDayCount; i++) {
      ...
      if (v == null || !Number.isFinite(Number(v))) continue;
      const inStrict = key >= ctx.lookbackStartYmd && key <= ctx.lookbackEndYmd;
      const ok = selPlant && selPlant.has(key) ? selPlant.get(key) !== false : inStrict;
      if (!ok) continue;
      valsByDow[isoDow(d) - 1].push(Number(v));
    }
    for (let k = 0; k < 7; k++) {
...
async function computePronosticoProyByPlant(client, year, month, opts = {}) {
  ...
  for (const p of plants) {
    ...
    if (enableLookback && lookbackStartYmd && lookbackEndYmd) {
      const startDt = parseYmdToDate(lookbackQueryFromYmd || lookbackVisualStartYmd || lookbackStartYmd);
      ...
      for (let i = 0; i < promDayCount; i++) {
        ...
        const v = ventaMap.get(key);
        if (v == null || !Number.isFinite(Number(v))) continue;
        const inStrict = key >= lookbackStartYmd && key <= lookbackEndYmd;
        const ok = selPlant.has(key) ? selPlant.get(key) !== false : inStrict;
        if (!ok) continue;
        valsByDow[isoDow(d) - 1].push(Number(v));
      }
