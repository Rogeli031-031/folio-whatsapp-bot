...
async function appendHojaProyeccionCatSub(client, wb, opts) {
  ...
  data.push([
    ...
    "Categoría",
    "Subcategoría",
    useMtdAcumulado ? "Kg acumulados" : "Kg proyectados",
  ...
  for (const p of plants) {
    ...
    if (useMtdAcumulado) {
      ...
      // Si no hay acumulado (o está en cero), cae al método anterior para tener distribución.
    ...
    if (scaleToForecastTotal && forecastKgByPlant) {
      const totalKgProj = rows.reduce((s, r) => s + (Number(r.kg_proyectado) || 0), 0);
      const targetKg = forecastKgByPlant.get(String(p || "").trim()) || 0;
      const factor = totalKgProj > 0 && targetKg > 0 ? targetKg / totalKgProj : 1;
      // Regla de tres: mantener proporciones por cat/sub y forzar suma al total objetivo.
      if (Number.isFinite(factor) && factor > 0 && factor !== 1) {
        for (const r of rows) {
          const kg = Number(r.kg_proyectado) || 0;
          const mxn = Number(r.descuento_mxn_proyectado) || 0;
          const kg2 = kg * factor;
          const mxn2 = mxn * factor;
          r.kg_proyectado = Math.round(kg2 * 100) / 100;
          r.descuento_mxn_proyectado = Math.round(mxn2 * 100) / 100;
          r.descuento_por_kg_proyectado = kg2 > 0 ? Math.round((mxn2 / kg2) * 1000000) / 1000000 : null;
        }
        // Ajuste final por redondeo (centavos/kg): corrige el último renglón con mayor kg.
        const sumAfter = rows.reduce((s, r) => s + (Number(r.kg_proyectado) || 0), 0);
        const diff = Math.round((targetKg - sumAfter) * 100) / 100;
        if (Math.abs(diff) >= 0.01) {
          let idx = -1;
          let maxKg = -1;
          for (let i = 0; i < rows.length; i++) {
            const kg = Number(rows[i].kg_proyectado) || 0;
            if (kg > maxKg) {
              maxKg = kg;
              idx = i;
            }
          ...
          if (idx >= 0) {
            const r = rows[idx];
            const kgOld = Number(r.kg_proyectado) || 0;
            const mxnOld = Number(r.descuento_mxn_proyectado) || 0;
            const kgNew = Math.max(0, Math.round((kgOld + diff) * 100) / 100);
            // Mantén desc $/kg de ese renglón ajustando el monto.
            const descKg = kgOld > 0 ? mxnOld / kgOld : 0;
            const mxnNew = Math.round((kgNew * descKg) * 100) / 100;
            r.kg_proyectado = kgNew;
            r.descuento_mxn_proyectado = mxnNew;
            r.descuento_por_kg_proyectado = kgNew > 0 ? Math.round((mxnNew / kgNew) * 1000000) / 1000000 : null;
          }
        }
      }
    }
