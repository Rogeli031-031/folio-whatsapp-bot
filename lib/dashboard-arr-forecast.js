...
function buildSemanaDowTableRows(year, month, byDateValue, isSum) {
  ...
  for (let wkMon = startWeekMon, guard = 0; guard < 80 && wkMon <= endWeekSun; guard++, wkMon = addDays(wkMon, 7)) {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (t != null) {
        ...
        cells[col] = v;
        weekVals.push(v);
        // Totales del mes: SOLO se calculan con días dentro del mes objetivo.
        if (inMonth) {
          totalsByDow[col].push(v);
          if (isSum) rowSum += v;
        }
      }
    }
  ...
  rows.push([]);
  if (isSum) {
    const sumsByDow = totalsByDow.map((arr) => arr.reduce((a, b) => a + b, 0));
    const grand = sumsByDow.reduce((a, b) => a + b, 0);
    rows.push(["TOTAL mes (por día de semana)", ...sumsByDow.map((x) => Math.round(x * 100) / 100), Math.round(grand * 100) / 100]);
  } else {
    const meansByDow = totalsByDow.map((arr) => {
      const m = meanNonEmpty(arr);
      return m != null ? Math.round(m * 100) / 100 : "";
    });
    const grandMean = meanNonEmpty(meansByDow.filter((x) => x !== ""));
    rows.push(["PROM mes (por día de semana)", ...meansByDow, grandMean != null ? Math.round(grandMean * 100) / 100 : ""]);
  }
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  // Si el corte está dentro de los primeros 14 días del mes, agregar 2 semanas hacia atrás (visual)
  // con datos reales de la BD aunque pertenezcan al mes anterior. No deben afectar totales del mes.
  const corteDt = parseYmdToDate(fechaCorte);
  const isCorteEnMes = corteDt && corteDt.getFullYear() === year && (corteDt.getMonth() + 1) === month;
  const enableLookback = Boolean(isCorteEnMes && corteDt.getDate() <= 14);
  ...
  for (const p of plants) {
    ...
    const header1 = [];
    header1[0] = `${p} — venta (ton) por semana del mes y día de la semana (L–D)`;
    header1[1] = `${year}-${String(month).padStart(2, "0")}`;
    header1[COLK] = `${p} — descuento ($/kg) por semana del mes y día de la semana (L–D)`;
    header1[COLK + 1] = `${year}-${String(month).padStart(2, "0")}`;
    aoa.push(header1);

    const header2 = [];
    header2[0] = "Filas: semana ISO del año (Lun–Dom). Columnas: Lun a Dom. Cada celda es el valor del día que cae en esa columna.";
    header2[COLK] = "Mismo arreglo, pero con descuento diario ($/kg). Total semana/mes = promedio (no suma).";
    aoa.push(header2);
    aoa.push([]);

    const ventaTable = buildSemanaDowTableRows(year, month, ventaMap, true);
    const descTable = buildSemanaDowTableRows(year, month, descMap, false);
    const height = Math.max(ventaTable.height, descTable.height);

    for (let i = 0; i < height; i++) {
      const row = [];
      const left = ventaTable.rows[i] || [];
      const right = descTable.rows[i] || [];
