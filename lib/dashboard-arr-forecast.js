...
function applyIgfProvinciaResumenFormulas(ws, nCols, row0Header, plantMetaList, pronosticoColI0, pronosticoDescCol0) {
  ...
  for (let i = 0; i < plantMetaList.length; i++) {
    ...
    if (meta.pronosticoProyRow != null) {
      ws[br] = { f: `${sh}!${colI}${meta.pronosticoProyRow}`, t: "n" };
    }
    if (meta.igfMargenRow != null) {
      ws[cr] = { f: `${margenCol}${meta.igfMargenRow}`, t: "n" };
    }
    if (meta.pronosticoProyRow != null) {
      ws[dr] = { f: `${sh}!${colDesc}${meta.pronosticoProyRow}`, t: "n" };
    }
  }
...
function matchPronosticoProyRow(plantToProyExcelRow, label) {
  ...
  for (const [code, excelRow] of plantToProyExcelRow) {
    ...
    if (!k) continue;
    if (k === want || k.includes(want) || want.includes(k)) return excelRow;
  }
  return null;
}
...
function matchIgfMargenRow(empresaToExcelRow, label) {
  ...
  for (const [k, row] of empresaToExcelRow) {
    if (k.includes(want) || want.includes(k)) return row;
  }
  return null;
}

/** Añade filas 28+ (con relleno): títulos, plantas provincia, Zona Provincia. Devuelve fila 0-based del encabezado y mapa empresa→fila margen. */
function appendProvinciaResumenToIgfData(data, nCol, dataRows) {
  ...
  const subHdr = ["Empresa", "Venta", "Margen", "Com. y Desc.", "Gasto", "Impuestos", "HG - %"];
  data.push(padRowToCols(subHdr, nCol));
...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  if (rawIgfRows.length > 0) {
    ...
    const { row0Header, empMap } = appendProvinciaResumenToIgfData(data, nCol, dataRows);
    const ws = XLSX.utils.aoa_to_sheet(data);
    const plantMetaList = IGF_PROVINCIA_RESUMEN_PLANTAS.map((label) => ({
      pronosticoProyRow: matchPronosticoProyRow(pronosticoMeta.plantToProyExcelRow, label),
    ...
    const { row0Header, empMap } = appendProvinciaResumenToIgfData(data, nCol, forecastDataRows);
    const ws = XLSX.utils.aoa_to_sheet(data);
    const plantMetaList = IGF_PROVINCIA_RESUMEN_PLANTAS.map((label) => ({
      pronosticoProyRow: matchPronosticoProyRow(pronosticoMeta.plantToProyExcelRow, label),
      igfMargenRow: matchIgfMargenRow(empMap, label),
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  /** Fila Excel 1-based de la fila "PROY" (venta ton) por planta — para fórmulas en IGF Forecast. */
  const plantToProyExcelRow = new Map();
  ...
  for (const p of plants) {
    ...
    plantToProyExcelRow.set(String(p).trim(), tableStartRow + height);
