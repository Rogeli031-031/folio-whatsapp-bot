 ...
 */
function applyIgfProvinciaResumenFormulas(ws, nCols, row0Header, plantMetaList, pronosticoColI0, pronosticoDescCol0) {
  ...
  for (let i = 0; i < plantMetaList.length; i++) {
    ...
    const excelRow = r0 + 1;
    const br = XLSX.utils.encode_cell({ r: r0, c: 1 });
    const cr = XLSX.utils.encode_cell({ r: r0, c: 2 });
    const dr = XLSX.utils.encode_cell({ r: r0, c: 3 });
    if (meta.pronosticoVentaRow != null) {
      ws[br] = { f: `${sh}!$${colI}$${meta.pronosticoVentaRow}` };
    }
    if (meta.igfMargenRow != null) {
      ws[cr] = { f: `$${margenCol}$${meta.igfMargenRow}` };
    }
    if (meta.pronosticoDescRow != null) {
      ws[dr] = { f: `${sh}!$${colDesc}$${meta.pronosticoDescRow}` };
    }
    const igfR = meta.igfMargenRow;
    if (igfR != null) {
      const colF = colIndexToA1Letter(5);
      ws[XLSX.utils.encode_cell({ r: r0, c: 5 })] = { f: `$${colF}$${igfR}` };
      for (const c0 of IGF_RULE_OF_THREE_COL_INDICES) {
        ...
        ws[ref] = { f: `${L}$${igfR}*$${colB}$${igfR}/$${colB}${excelRow}` };
      }
    }
  }

  const rZona = row0Header + 1 + plantMetaList.length;
  const bZona = XLSX.utils.encode_cell({ r: rZona, c: 1 });
  const firstDataR = row0Header + 1;
  const lastPlantR = row0Header + plantMetaList.length;
  ws[bZona] = {
    f: `SUM(${XLSX.utils.encode_cell({ r: firstDataR, c: 1 })}:${XLSX.utils.encode_cell({ r: lastPlantR, c: 1 })})`,
  };
  ...
  if (!ws["!ref"]) {
    ...
    d.e.r = Math.max(d.e.r, rZona);
    d.e.c = Math.max(d.e.c, nCols - 1);
...
/** Util. oper. tal como en el IGF cargado (antes del recálculo con folios/presupuesto en el dashboard). */
function pickUtilOperKgImporteFromIgfRow(dataRow) {
  if (!dataRow) return { kg: null, importe: null };
  let kg =
    dataRow.util_oper_kg_igf != null && Number.isFinite(Number(dataRow.util_oper_kg_igf))
      ? Number(dataRow.util_oper_kg_igf)
      : null;
  let importe =
    dataRow.util_oper_importe_igf != null && Number.isFinite(Number(dataRow.util_oper_importe_igf))
      ? Number(dataRow.util_oper_importe_igf)
      : null;
  if (kg == null && dataRow.util_oper_kg != null && Number.isFinite(Number(dataRow.util_oper_kg))) {
    kg = Number(dataRow.util_oper_kg);
  }
  if (importe == null && dataRow.util_oper_importe != null && Number.isFinite(Number(dataRow.util_oper_importe))) {
    importe = Number(dataRow.util_oper_importe);
  }
  return { kg, importe };
}

/**
 * Sobre la tabla IGF superior: fuerza Util. oper. ($/kg e importe) solo con util_oper_*_igf del payload
 * (valor IGF subido antes del recálculo en API). No usa util_oper_kg recalculado para no pisar la BD.
 */
function mergeIgfFirstTableUtilFromIgfPayload(dataRows, igfRows) {
  if (!dataRows || !igfRows || !igfRows.length) return;
  const idxKg = IGF_COMPROMISO_RAW_COLS.indexOf("util_oper_kg");
  const idxImp = IGF_COMPROMISO_RAW_COLS.indexOf("util_oper_importe");
  if (idxKg < 0 || idxImp < 0) return;
  for (const line of dataRows) {
    const emp = line[0] != null ? String(line[0]).trim() : "";
    if (!emp || /^TOTALES?$/i.test(emp)) continue;
    const dataRow = matchIgfForecastDataRow(igfRows, emp);
    if (!dataRow) continue;
    if (dataRow.util_oper_kg_igf != null && Number.isFinite(Number(dataRow.util_oper_kg_igf))) {
      line[idxKg] = Number(dataRow.util_oper_kg_igf);
    }
    if (dataRow.util_oper_importe_igf != null && Number.isFinite(Number(dataRow.util_oper_importe_igf))) {
      line[idxImp] = Number(dataRow.util_oper_importe_igf);
    }
  }
}
 ...
 */
function applyIgfResumenHgPctFromDashboard(ws, row0Header, igfRows, nCols) {
  ...
  for (let i = 0; i < IGF_PROVINCIA_RESUMEN_PLANTAS.length; i++) {
    ...
    const r0 = row0Header + 1 + i;
    const ref = XLSX.utils.encode_cell({ r: r0, c: hgCol0 });
    ws[ref] = { v: pct, t: "n" };
  }
  const d = ws["!ref"] ? XLSX.utils.decode_range(ws["!ref"]) : { s: { r: 0, c: 0 }, e: { r: 0, c: 0 } };
  const lastR = row0Header + IGF_PROVINCIA_RESUMEN_PLANTAS.length;
  d.e.r = Math.max(d.e.r, lastR);
  d.e.c = Math.max(d.e.c, nCols - 1);
  ws["!ref"] = XLSX.utils.encode_range(d);
}

/**
 * Bloque resumen plantilla 18 col: H regla de tres vs tabla superior; K/L = utilidad IGF (payload); Q = K-M-N-O-P; R = Q*B*1000.
 * H29 = H18*G29/G18; si no hay igfRows: K = C+D-E-F+H-I-J; L = K*B*1000.
 * Fila Zona Provincia: solo L y R con SUM; sin K/Q ni fórmulas extra en esa fila.
 */
function applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCols, igfRows) {
  ...
  const useIgfKl = igfRows && igfRows.length > 0;

  const setDerivedForPlantRow = (r0, igfR, resumenLabel) => {
    const er = r0 + 1;
    if (igfR != null) {
      ws[XLSX.utils.encode_cell({ r: r0, c: 7 })] = { f: `=${H}$${igfR}*$${G}${er}/$${G}$${igfR}` };
    }
    if (useIgfKl) {
      const dataRow = matchIgfForecastDataRow(igfRows, resumenLabel);
      const { kg, importe } = pickUtilOperKgImporteFromIgfRow(dataRow);
      if (kg != null && Number.isFinite(kg)) {
        ws[XLSX.utils.encode_cell({ r: r0, c: 10 })] = { v: kg, t: "n" };
      } else {
        ws[XLSX.utils.encode_cell({ r: r0, c: 10 })] = {
          f: `=${C}${er}+${D}${er}-${E}${er}-${F}${er}+${H}${er}-${I}${er}-${J}${er}`,
        };
      }
      if (importe != null && Number.isFinite(importe)) {
        ws[XLSX.utils.encode_cell({ r: r0, c: 11 })] = { v: importe, t: "n" };
      } else {
        ws[XLSX.utils.encode_cell({ r: r0, c: 11 })] = { f: `=${K}${er}*${B}${er}*1000` };
      }
    } else {
      ws[XLSX.utils.encode_cell({ r: r0, c: 10 })] = {
        f: `=${C}${er}+${D}${er}-${E}${er}-${F}${er}+${H}${er}-${I}${er}-${J}${er}`,
    ...
    ws[XLSX.utils.encode_cell({ r: r0, c: 16 })] = {
      f: `=${K}${er}-${M}${er}-${N}${er}-${O}${er}-${P}${er}`,
    };
    ws[XLSX.utils.encode_cell({ r: r0, c: 17 })] = { f: `=${Q}${er}*${B}${er}*1000` };
  };

  for (let i = 0; i < plantMetaList.length; i++) {
    setDerivedForPlantRow(row0Header + 1 + i, plantMetaList[i].igfMargenRow, IGF_PROVINCIA_RESUMEN_PLANTAS[i]);
  }

  const rZona = row0Header + 1 + plantMetaList.length;
  const firstPlantExcel = row0Header + 2;
  const lastPlantExcel = row0Header + 1 + plantMetaList.length;
  ws[XLSX.utils.encode_cell({ r: rZona, c: 11 })] = {
    f: `SUM($${L}$${firstPlantExcel}:$${L}$${lastPlantExcel})`,
  };
  ws[XLSX.utils.encode_cell({ r: rZona, c: 17 })] = {
    f: `SUM($${R}$${firstPlantExcel}:$${R}$${lastPlantExcel})`,
  };
 ...
 */
function appendProvinciaResumenToIgfData(data, nCol, dataRows, secondBlockHeaders) {
...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  if (rawIgfRows.length > 0) {
    ...
    if (options.igfForecast && Array.isArray(options.igfForecast.rows) && options.igfForecast.rows.length > 0) {
      mergeIgfFirstTableUtilFromIgfPayload(dataRows, options.igfForecast.rows);
    }
    const data = buildIgfCompromisoTemplateAoa(year, month, dataRows);
    const nCol = IGF_COMPROMISO_HEADER_ROW7.length;
    const { row0Header, empMap } = appendProvinciaResumenToIgfData(data, nCol, dataRows, IGF_COMPROMISO_HEADER_ROW7);
    const ws = XLSX.utils.aoa_to_sheet(data);
    const plantMetaList = IGF_PROVINCIA_RESUMEN_PLANTAS.map((label) => ({
      pronosticoVentaRow: matchPronosticoProyRow(pronosticoMeta.plantToProyVentaExcelRow, label),
      pronosticoDescRow: matchPronosticoProyRow(pronosticoMeta.plantToProyDescExcelRow, label),
    ...
    const igfRowsForResumen =
      options.igfForecast && Array.isArray(options.igfForecast.rows) && options.igfForecast.rows.length > 0
        ? options.igfForecast.rows
        : null;
    applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCol, igfRowsForResumen);
    ...
    const igfRowsForResumen20 =
      options.igfForecast && Array.isArray(options.igfForecast.rows) && options.igfForecast.rows.length > 0
        ? options.igfForecast.rows
        : null;
    applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCol, igfRowsForResumen20);
    XLSX.utils.book_append_sheet(wb, ws, "IGF Forecast");
  } else {
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  XLSX.utils.book_append_sheet(wb, ws, SHEET_PRONOSTICO);
  return {
