...
const IGF_COMPROMISO_RAW_HEADERS = [
  "Empresa", "Venta (ton)", "Margen ($/kg)", "Com. y Desc. ($/kg)", "Gasto ($/kg)", "Impuesto ($/kg)", "HG (%)", "HG ($/kg)",
  "Bancos Planta", "Prov. Planta", "Util. Oper. ($/kg)", "Util. Oper. (Importe)",
  "Gtos/Apoyos Corp", "Bancos Corp.", "Otros Programas", "Inversiones", "Resultado ($/kg)", "Resultado (Importe)",
];
...
function buildIgfCompromisoRawAoa(dbRows) {
  const toNum = (v) => {
    if (v == null || v === "") return null;
    const n = typeof v === "number" ? v : Number(v);
    return Number.isFinite(n) ? n : null;
  };
  const rowsAoa = [IGF_COMPROMISO_RAW_HEADERS.slice()];
...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  if (rawIgfRows.length > 0) {
    const firstBlock = buildIgfCompromisoRawAoa(rawIgfRows);
    const IGF_FORECAST_REPEAT_START_ROW = 10;
    const padCount = Math.max(0, IGF_FORECAST_REPEAT_START_ROW - firstBlock.length - 1);
    const data = firstBlock.slice();
    for (let i = 0; i < padCount; i++) data.push([]);
    for (const line of firstBlock) data.push(line.slice());
    ...
    const buildIgfForecastAoa = () => {
      const rowsAoa = [IGF_FORECAST_HEADERS.slice()];
    ...
    const firstBlock = buildIgfForecastAoa();
    const IGF_FORECAST_REPEAT_START_ROW = 10;
    const padCount = Math.max(0, IGF_FORECAST_REPEAT_START_ROW - firstBlock.length - 1);
    const data = firstBlock.slice();
    for (let i = 0; i < padCount; i++) data.push([]);
