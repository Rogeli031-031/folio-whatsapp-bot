 ...
 */
async function appendHojaIgfMesHistorico(client, wb, year, month, sheetName) {
  ...
  applyIgfResumenHgPctFromDashboard(ws, row0Header, igfRowsForHg, nCol);
...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  const ventaCanal = await getVentaDiariaPorCanalGrid(
    client,
    year,
    month,
    fechaCorte,
  ...
  const descuentoCanal = await getDescuentoDiarioPorCanalGrid(
    client,
    year,
    month,
    fechaCorte,
  ...
  for (let m = 1; m < month; m++) {
    try {
      await appendHojaIgfMesHistorico(client, wb, year, m, `IGF ${MESES_NOMBRE_ES[m] || m}`);
    } catch (e) {
  ...
  if (acumRow) {
    plants.forEach((p, pi) => {
      ...
      if (mode === "venta") {
        ...
        setCanalNumber(ws, acumRow, cCasa, sumCanal("CASA"), excelTheme.FMT_NUMBER);
        setCanalNumber(ws, acumRow, cCom, sumCanal("COMISIONISTA"), excelTheme.FMT_NUMBER);
      } else {
  ...
  if (mode === "venta") {
    ...
    if (promRow && acumRow) {
      plants.forEach((p, pi) => {
        ...
        setCanalNumber(ws, promRow, cCasa, realDays.length && typeof casa === "number" ? casa / realDays.length : null, excelTheme.FMT_NUMBER);
        setCanalNumber(ws, promRow, cCom, realDays.length && typeof com === "number" ? com / realDays.length : null, excelTheme.FMT_NUMBER);
      });
    }
  }
...
function writeCanalPairCells(ws, excelRow, cCasa, pair, isReal, mode, fill) {
  ...
  if (!isReal) {
    casa = 0;
    com = 0;
    fmt = mode === "descuento" ? FMT_DESC_KG_3 : excelTheme.FMT_NUMBER;
  } else if (mode === "venta") {
    casa = pair ? Number(pair.CASA) || 0 : 0;
    com = pair ? Number(pair.COMISIONISTA) || 0 : 0;
    fmt = excelTheme.FMT_NUMBER;
  } else {
