...
async function generarDashboardArrForecast(client, year, month, plantCode = null) {
  ...
  hojaB(wb, descuentoGrid, year, month, lastDayStr, todayStr);
 ...
 * Fuente: arr.descuento_por_kilo_diario_provincia.
 */
...
function hojaB(wb, descuentoGrid, year, month, lastDayStr, todayStr) {
  ...
  const sumByPlant = plants.map((p) => byDate.reduce((s, d) => s + (Number(d.byPlant[p]) || 0), 0));
  const countByPlant = plants.map((p) => byDate.filter((d) => d.byPlant[p] != null && d.byPlant[p] !== "").length);
  const acumByPlant = plants.map((p, i) => (countByPlant[i] ? sumByPlant[i] / countByPlant[i] : ""));
  data.push([]);
