...
const { appendComprasWorksheet } = require("./compras-excel");
const igfDiarioPuebla = require("./igf-diario-puebla");
...
/** Alias exactos (sin substring). GT Puebla ↔ Puebla, GTM Querétaro ↔ Querétaro, etc. */
const FORECAST_EXPORT_PLANT_ALIASES = [
  { canon: "Puebla", keys: ["puebla", "gt puebla"] },
  { canon: "Tehuacan", keys: ["tehuacan", "tehuacán"] },
  { canon: "Acapulco", keys: ["acapulco"] },
  { canon: "Queretaro", keys: ["queretaro", "querétaro", "gtm queretaro", "gtm querétaro"] },
  { canon: "San Luis", keys: ["san luis", "gtm san luis"] },
  { canon: "Morelos", keys: ["morelos"] },
];

const FMT_VENTA_TON_3 = "#,##0.000";

function canonicalForecastPlantKey(raw) {
  const n = normEmpresaKey(raw);
  if (!n) return "";
  for (const group of FORECAST_EXPORT_PLANT_ALIASES) {
    if (group.keys.some((k) => normEmpresaKey(k) === n)) return group.canon;
  }
  return "";
}

function plantsEquivalent(a, b) {
  const left = String(a || "").trim();
  const right = String(b || "").trim();
  if (!left || !right) return false;
  if (normEmpresaKey(left) === normEmpresaKey(right)) return true;
  const ca = canonicalForecastPlantKey(left);
  const cb = canonicalForecastPlantKey(right);
  return Boolean(ca && cb && ca === cb);
}

function igfLabelForForecastPlant(raw) {
  const canon = canonicalForecastPlantKey(raw) || normEmpresaKey(raw);
  for (const [code, labels] of Object.entries(PRONOSTICO_PLANT_CODE_TO_IGF_RESUMEN_LABELS)) {
    if (plantsEquivalent(code, raw) || normEmpresaKey(code) === canon) {
      if (labels && labels[0]) return labels[0];
    }
  }
  return String(raw || "").trim();
}

function filterLabelsToPlant(labels, selected) {
  if (!selected) return (labels || []).slice();
  return (labels || []).filter((label) => plantsEquivalent(label, selected));
}

function scopedResumenLabels(selected) {
  if (!selected) return IGF_PROVINCIA_RESUMEN_PLANTAS.slice();
  const filtered = filterLabelsToPlant(IGF_PROVINCIA_RESUMEN_PLANTAS, selected);
  if (filtered.length) return filtered;
  const label = igfLabelForForecastPlant(selected);
  return label ? [label] : [];
}

function scopeProvinciaGrid(grid, selected) {
  if (!selected || !grid) return grid;
  const plants = (grid.plants || []).filter((p) => plantsEquivalent(p, selected));
  const byDate = (grid.byDate || []).map((d) => {
    const byPlant = {};
    const src = d.byPlant || {};
    for (const p of plants) {
      if (Object.prototype.hasOwnProperty.call(src, p)) byPlant[p] = src[p];
      else {
        const key = Object.keys(src).find((k) => plantsEquivalent(k, p));
        if (key) byPlant[p] = src[key];
      }
    }
    let tot = 0;
    for (const p of plants) {
      const v = byPlant[p];
      if (typeof v === "number" && Number.isFinite(v)) tot += v;
    }
    const next = { ...d, byPlant };
    if (d.tot != null) next.tot = tot;
    return next;
  });
  let forecastByPlant = grid.forecastByPlant;
  if (forecastByPlant instanceof Map) {
    forecastByPlant = new Map(
      [...forecastByPlant.entries()].filter(([k]) => plants.some((p) => plantsEquivalent(p, k)))
    );
  }
  return { ...grid, plants, byDate, forecastByPlant };
}

function empresaMatchesForecastPlant(empresa, selected) {
  return plantsEquivalent(empresa, selected);
}

async function resolveForecastExportPlant(client, raw) {
  const label = String(raw || "").trim();
  const canon = canonicalForecastPlantKey(label);
  if (!label || !canon || !client || typeof client.query !== "function") return null;
  const plantas = await client.query(
    `SELECT id, nombre, clave FROM public.plantas ORDER BY id`
  );
  const row = (plantas.rows || []).find((p) => {
    const nombre = String(p.nombre || "").trim();
    const clave = String(p.clave || "").trim();
    return plantsEquivalent(nombre, label) || (clave && plantsEquivalent(clave, label)) || plantsEquivalent(nombre, canon);
  });
  if (!row || !Number.isFinite(Number(row.id))) return null;
  let provinciaPlantCode = canon;
  try {
    const resolved = await resolveArrClientesMesPlantCode(client, String(row.nombre || canon));
    if (resolved) provinciaPlantCode = resolved;
  } catch {
    provinciaPlantCode = canon;
  }
  return {
    plantaId: Number(row.id),
    nombre: String(row.nombre || "").trim(),
    clave: String(row.clave || "").trim(),
    canon,
    provinciaPlantCode,
    igfLabel: igfLabelForForecastPlant(label),
  };
}
...
function normalizeArrEmpresaAccents(s) {
  if (s == null || s === "") return "";
  return String(s).normalize("NFD").replace(/\p{M}/gu, "").trim().toUpperCase();
}

/**
 * Misma resolución de planta que GET /api/dashboard/arr-clientes-mes:
 * match accent-insensitive contra arr.provincia_plants.plant_code; si hay varios, el más largo.
 */
async function resolveArrClientesMesPlantCode(client, empresa) {
  const emp = String(empresa || "").trim();
  if (!emp || !client || typeof client.query !== "function") return null;
  const provRes = await client.query("SELECT plant_code FROM arr.provincia_plants ORDER BY plant_code");
  const provinciaPlantCodes = (provRes.rows || []).map((r) => String(r.plant_code || "").trim()).filter(Boolean);
  const empNorm = normalizeArrEmpresaAccents(emp);
  const matches = provinciaPlantCodes.filter((p) => {
    const pNorm = normalizeArrEmpresaAccents(p);
 ...
 */
function applyIgfResumenHgPctFromDashboard(worksheet, row0Header, igfRows, nCols, resumenPlantas) {
  const labels = resumenPlantas && resumenPlantas.length ? resumenPlantas : IGF_PROVINCIA_RESUMEN_PLANTAS;
  const hgCol0 = nCols >= 20 ? 8 : 6;
  for (let i = 0; i < labels.length; i++) {
    const dataRow = matchIgfForecastDataRow(igfRows, labels[i]);
...
/**
 * Variante de `applyIgfProvinciaResumenFormulas` para meses ya cerrados (sin hoja `Pronostico`):
 * - B (Venta) y D (Com. y Desc.) se toman de la fila IGF de la misma hoja en lugar de `Pronostico`.
 * - C (Margen), F (Impuestos) y E,I,J,M,N,O,P se copian directamente de la fila IGF (sin regla de tres,
 *   porque venta resumen = venta IGF cuando se trata del cierre real).
 * Mantiene la misma forma del bloque (encabezado en `row0Header`, plantas debajo y "Zona Provincia" al final con SUM).
 */
function applyIgfProvinciaResumenStaticFromIgf(worksheet, nCols, row0Header, plantMetaList) {
  const colB = colIndexToA1Letter(1);
  const colC = colIndexToA1Letter(2);
  const colD = colIndexToA1Letter(3);
  const colF = colIndexToA1Letter(5);
  ...
  for (let i = 0; i < plantMetaList.length; i++) {
    ...
    const r0 = row0Header + 1 + i;
    const igfR = meta.igfMargenRow;
    if (igfR == null) continue;
    excelTheme.setCellFormula(worksheet, r0, 1, `$${colB}$${igfR}`);
    excelTheme.setCellFormula(worksheet, r0, 2, `$${colC}$${igfR}`);
    excelTheme.setCellFormula(worksheet, r0, 3, `$${colD}$${igfR}`);
    excelTheme.setCellFormula(worksheet, r0, 5, `$${colF}$${igfR}`);
    for (const c0 of IGF_RULE_OF_THREE_COL_INDICES) {
      if (c0 >= nCols) continue;
      const L = colIndexToA1Letter(c0);
      excelTheme.setCellFormula(worksheet, r0, c0, `$${L}$${igfR}`);
    }
  }
  ...
  const firstDataR = row0Header + 1;
  const lastPlantR = row0Header + plantMetaList.length;
  const sumRange = `${colB}${firstDataR + 1}:${colB}${lastPlantR + 1}`;
  excelTheme.setCellFormula(worksheet, rZona, 1, `SUM(${sumRange})`);
}
 ...
 */
function appendProvinciaResumenToIgfData(data, nCol, dataRows, secondBlockHeaders, dataStartRow0 = 8, resumenPlantas) {
  const labels = resumenPlantas && resumenPlantas.length ? resumenPlantas : IGF_PROVINCIA_RESUMEN_PLANTAS;
  ...
  for (const pl of labels) data.push(padRowToCols([pl], nCol));
...
/** Lunes (ISO) de la semana que contiene la fecha YYYY-MM-DD (para cuadrícula tipo Excel). */
function mondayOfIsoWeekContainingYmd(ymdStr) {
  const d = parseYmdToDate(ymdStr);
  if (!d) return null;
  const isoDow = (dt) => (dt.getDay() === 0 ? 7 : dt.getDay());
  const m = new Date(d.getFullYear(), d.getMonth(), d.getDate());
  m.setDate(m.getDate() - (isoDow(m) - 1));
  return m;
}

/**
 * Rellena fechas del lookback (p. ej. marzo) cuando `venta_toneladas_diarias_provincia` no trae filas
 * pero sí existen kg en `ventas_diarias_cliente` (misma lógica que el fallback de getVentaToneladasGrid).
 * Solo escribe si la planta aún no tiene valor para esa fecha (no pisa provincial).
 */
async function mergeVentaLookbackGapsFromVentasCliente(client, ventaMapByPlant, plants, fromYmd, toYmd) {
  if (!fromYmd || !toYmd || !ventaMapByPlant || !plants || !plants.length) return;
  const r = await client.query(
    `WITH prov_map AS (
       ...
       SELECT pm.prov_name AS plant_code, v.fecha, ROUND(SUM(v.kg) / 1000.0, 4)::numeric AS venta_ton
  ...
  for (const row of r.rows || []) {
    ...
    m.set(fecha, Math.round(v * 10000) / 10000);
  }
}

/**
 * Rellena fechas del lookback para descuento ($/kg) cuando `descuento_por_kilo_diario_provincia` no trae filas
 * pero sí existen descuentos + ventas en tablas diarias ARR.
 * Solo escribe si la planta aún no tiene valor para esa fecha (no pisa provincial).
 */
async function mergeDescLookbackGapsFromDiariosCliente(client, descMapByPlant, plants, fromYmd, toYmd) {
  if (!fromYmd || !toYmd || !descMapByPlant || !plants || !plants.length) return;
  const r = await client.query(
    `WITH prov_map AS (
         SELECT DISTINCT p.nombre AS prov_name, UPPER(TRIM(p.nombre)) AS key_nombre, UPPER(TRIM(COALESCE(p.clave, ''))) AS key_clave
         FROM public.plantas p
       ...
       SELECT kg.plant_code, kg.fecha,
              ROUND((descs.total_monto / NULLIF(kg.total_kg, 0))::numeric, 2) AS descuento_por_kg
       FROM kg
       JOIN descs ON descs.plant_code = kg.plant_code AND descs.fecha = kg.fecha
       ORDER BY kg.fecha`,
    [fromYmd, toYmd]
  );
  for (const row of r.rows || []) {
    ...
    if (!Number.isFinite(v)) continue;
    m.set(fecha, Math.round(v * 100) / 100);
  }
}
...
/**
 * Crea una hoja con el mismo formato visual de "IGF Forecast" (plantilla Compromiso 18 columnas)
 * pero para un mes ya cerrado del mismo año: usa los valores reales del IGF de cierre
 * (`igf.compromiso_lines` última versión del mes) y NO referencia la hoja `Pronostico`
 * (porque para meses cerrados no se genera).
 *
 * No modifica la hoja "IGF Forecast" original. Si no hay IGF cargado para ese mes, no crea hoja.
 *
 * @returns {Promise<boolean>} true si la hoja se creó, false si no había datos.
 */
async function appendHojaIgfMesHistorico(client, wb, year, month, sheetName, plantCode) {
  let rawIgfRows = await fetchIgfCompromisoRawRows(client, year, month);
  if (plantCode) rawIgfRows = (rawIgfRows || []).filter((r) => empresaMatchesForecastPlant(r && r.empresa, plantCode));
  if (!rawIgfRows || rawIgfRows.length === 0) return false;

  const dataRows = buildIgfCompromisoRawDataRows(rawIgfRows);
  // Convención de signo de "Com. y Desc." en hojas IGF históricas: igual que en IGF Forecast (negativo).
  // En igf.compromiso_lines del cierre suele guardarse positivo; aquí lo invertimos para consistencia visual
  // con la hoja IGF Forecast (que NO se modifica). Índice 3 = com_desc_kg en IGF_COMPROMISO_RAW_COLS.
  const COM_DESC_COL_IDX = IGF_COMPROMISO_RAW_COLS.indexOf("com_desc_kg");
  if (COM_DESC_COL_IDX >= 0) {
    for (const row of dataRows) {
      const v = row[COM_DESC_COL_IDX];
      if (typeof v === "number" && Number.isFinite(v) && v !== 0) {
        row[COM_DESC_COL_IDX] = -v;
      }
    }
  }
  const data = buildIgfCompromisoTemplateAoa(year, month, dataRows);
  const nCol = IGF_COMPROMISO_HEADER_ROW7.length;
  const resumenPlantas = scopedResumenLabels(plantCode);
  ...
  const safeName = String(sheetName || `IGF ${MESES_NOMBRE_ES[month] || month}`).slice(0, 31);
  const ws = wb.addWorksheet(safeName, {
    views: [{ state: "frozen", ySplit: IGF_COMPROMISO_MAIN_HEADER_EXCEL_ROW + 1, showGridLines: true }],
  });
  excelTheme.writeAoa(ws, data);

  const plantMetaList = (resumenPlantas.length ? resumenPlantas : IGF_PROVINCIA_RESUMEN_PLANTAS).map((label) => ({
    igfMargenRow: matchIgfMargenRow(empMap, label),
  }));

  applyIgfProvinciaResumenStaticFromIgf(ws, nCol, row0Header, plantMetaList);

  const igfRowsForHg = rawIgfRows.map((r) => ({
    empresa: r.empresa,
    hg_pct: r.hg_pct != null && Number.isFinite(Number(r.hg_pct)) ? Number(r.hg_pct) : null,
  }));
  applyIgfResumenHgPctFromDashboard(ws, row0Header, igfRowsForHg, nCol, resumenPlantas);

  applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCol);
  applyIgfMiniResumenFormulas(ws, row0Header, plantMetaList.length, nCol, { miniAtTop: true });
  ...
  excelTheme.applyIgfForecastSheetLayout(ws, {
    ...
    row0Header,
    resumenPlantCount: plantMetaList.length,
...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  const exportPlant = String(plantCode || "").trim();
  const includeIgfDiario = Boolean(exportPlant);
  if (includeIgfDiario) igfDiarioPuebla.reserveSheet(wb, exportPlant);
  if (exportPlant && options && options.igfForecast && Array.isArray(options.igfForecast.rows)) {
    options = {
      ...options,
      igfForecast: {
        ...options.igfForecast,
        rows: options.igfForecast.rows.filter((r) => empresaMatchesForecastPlant(r && r.empresa, exportPlant)),
      },
    };
  }
  if (exportPlant && options && options.proyeccionCatSubForecast && options.proyeccionCatSubForecast.forecastKgByPlant) {
    const src = options.proyeccionCatSubForecast.forecastKgByPlant;
    const nextMap = {};
    for (const [k, v] of Object.entries(src)) {
      if (plantsEquivalent(k, exportPlant)) nextMap[k] = v;
    }
    options = {
      ...options,
      proyeccionCatSubForecast: { ...options.proyeccionCatSubForecast, forecastKgByPlant: nextMap },
    };
  }
  const resumenPlantas = scopedResumenLabels(exportPlant);

  const plantsForArr = exportPlant ? [exportPlant] : await getPlantCodesFromArr(client, year, month);
  const plantsForProvinciaSheets = provinciaPlantas.length
    ? plantsForArr.filter((p) => provinciaPlantas.some((emp) => plantsEquivalent(emp, p)))
    : plantsForArr;

  const empresasComp = exportPlant
    ? provinciaPlantas.filter((e) => plantsEquivalent(e, exportPlant))
    : provinciaPlantas;
  const compEmpresas = empresasComp.length ? empresasComp : (exportPlant ? [exportPlant] : provinciaPlantas);
  const compTotalKg = await getCompromisoTotalProvincia(client, year, month, compEmpresas);
  let ventaTonGrid = await getVentaToneladasGrid(client, year, month, fechaCorte);
  let descuentoGrid = await getDescuentoPorKiloGrid(client, year, month, fechaCorte);
  let ventaCanal = await getVentaDiariaPorCanalGrid(
    client,
    year,
    month,
    fechaCorte,
    ventaTonGrid.plants,
    ventaTonGrid.cutoffDay
  );
  let descuentoCanal = await getDescuentoDiarioPorCanalGrid(
    client,
    year,
    month,
    fechaCorte,
    descuentoGrid.plants,
    descuentoGrid.cutoffDay
  );
  if (exportPlant) {
    ventaTonGrid = scopeProvinciaGrid(ventaTonGrid, exportPlant);
    descuentoGrid = scopeProvinciaGrid(descuentoGrid, exportPlant);
    ventaCanal = scopeProvinciaGrid(ventaCanal, exportPlant);
    descuentoCanal = scopeProvinciaGrid(descuentoCanal, exportPlant);
  }

  const pronosticoProjection = exportPlant && fechaCorte
    ? await buildPronosticoProjectionContext(client, year, month, fechaCorte)
    : null;
  ...
  hojaA(wb, ventaTonGrid, year, month, compTotalKg, lastDayStr, todayStr, {
    proyTonByPlant: exportPlant && pronosticoProjection ? null : proyTonByPlantFromIgf,
    ventaCanal,
    omitTotProvincia: Boolean(exportPlant),
    pronosticoProjection: exportPlant ? pronosticoProjection : null,
  });
  hojaB(wb, descuentoGrid, ventaTonGrid, year, month, lastDayStr, todayStr, descuentoCanal, {
    pronosticoProjection: exportPlant ? pronosticoProjection : null,
  });
  if (options && Array.isArray(options.precioDiario)) {
    appendPrecioWorksheet(wb, year, month, options.precioDiario);
  }
  if (options && options.comprasPayload) {
    await appendComprasWorksheet(wb, options.comprasPayload, {
      plantName: options.comprasPlantName || exportPlant || "",
      corteYmd: fechaCorte,
      ventaKgByYmd: forecastVentaKgByYmd(pronosticoProjection, exportPlant, year, month),
    });
  }
  if (includeIgfDiario) {
    const gastos = options && options.igfDiarioGastos;
    igfDiarioPuebla.fillIgfDiarioPuebla(wb, {
      year,
      month,
      sheetLabel: exportPlant,
      humanName: (options && options.comprasPlantName) || exportPlant,
      plantEquivalent: (label) => plantsEquivalent(label, exportPlant),
      corporativos: gastos && gastos.corporativos,
      operativos: gastos && gastos.operativos,
      cierresEmpresariales: options && options.cierresEmpresariales,
      corteYmd: fechaCorte,
    });
  }
  const pronosticoMeta = await appendHojaAcapulcoSemanaDow(client, wb, year, month, {
    fechaCorte,
    plantCode: exportPlant || null,
    projection: exportPlant ? pronosticoProjection : null,
  });

  let rawIgfRows = await fetchIgfCompromisoRawRows(client, year, month);
  if (exportPlant) rawIgfRows = (rawIgfRows || []).filter((r) => empresaMatchesForecastPlant(r && r.empresa, exportPlant));
  if (rawIgfRows.length > 0) {
    ...
    excelTheme.writeAoa(ws, data);
    const plantMetaList = resumenPlantas.map((label) => ({
    ...
    if (options.igfForecast && Array.isArray(options.igfForecast.rows) && options.igfForecast.rows.length > 0) {
      applyIgfResumenHgPctFromDashboard(ws, row0Header, options.igfForecast.rows, nCol, resumenPlantas);
    }
    applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCol);
    applyIgfMiniResumenFormulas(ws, row0Header, resumenPlantas.length, nCol, { miniAtTop: true });
    excelTheme.applyIgfForecastSheetLayout(ws, {
      nCol,
      headerRow6: IGF_COMPROMISO_HEADER_ROW6,
      mainHeaderExcelRow1Based: IGF_COMPROMISO_MAIN_HEADER_EXCEL_ROW,
      ...
      dataRowCount: dataRows.length,
      row0Header,
      resumenPlantCount: resumenPlantas.length,
      lastRow0: data.length - 1,
    });
    excelTheme.setColumnWidths(
      ws,
    ...
    const { row0Header, empMap } = appendProvinciaResumenToIgfData(data, nCol, forecastDataRows, IGF_FORECAST_HEADERS, 8, resumenPlantas);
    const ws = wb.addWorksheet("IGF Forecast", {
    ...
    applyIgfResumenBlockDerivedFormulas(ws, row0Header, plantMetaList, nCol);
    applyIgfMiniResumenFormulas(ws, row0Header, resumenPlantas.length, nCol);
    excelTheme.applyIgfForecastSheetLayout(ws, {
      nCol,
    ...
    const provinciaScoped = exportPlant
      ? provinciaPlantas.filter((e) => plantsEquivalent(e, exportPlant))
      : provinciaPlantas;
    const centroScoped = exportPlant ? (centro || []).filter((e) => plantsEquivalent(e, exportPlant)) : centro;
    await hojaC(client, wb, year, month, provinciaScoped, centroScoped, plantsForProvinciaSheets);
  }

  // Hojas IGF de meses anteriores del mismo año (cierre real). Misma plantilla visual que "IGF Forecast"
  // pero con datos estáticos del IGF de cierre (igf.compromiso_lines última versión del mes), sin fórmulas
  // a `Pronostico` (que solo existe para el mes en curso). No modifica la hoja "IGF Forecast" original.
  for (let m = 1; m < month; m++) {
    try {
      await appendHojaIgfMesHistorico(client, wb, year, m, `IGF ${MESES_NOMBRE_ES[m] || m}`, exportPlant || null);
    } catch (e) {
      console.error(`[dashboard-arr-forecast] hoja IGF ${MESES_NOMBRE_ES[m] || m} falló`, e);
    }
  }
  ...
  // Hojas históricas: misma información de "Clientes desc mes" pero para los meses
  // anteriores del mismo año (ya concluidos), usando Kg mes (real) en lugar de proyectado.
  {
    const MESES_ES = [
    ...
    for (let m = 1; m < month; m++) {
      try {
        await appendHojaClientesDescuentoMes(client, wb, year, m, plantCode, {
          historico: true,
          sheetName: `Clientes desc ${MESES_ES[m - 1]}`,
        });
      } catch (e) {
        console.error(`[dashboard-arr-forecast] hoja histórica Clientes desc ${MESES_ES[m - 1]} falló`, e);
      }
    }
  }

  if (!exportPlant || plantsEquivalent(exportPlant, "Puebla")) appendHojaPueblaConsolidada(wb);
...
/**
 * Cómputo puro (sin Excel) de la lista de clientes por planta con kg, descuento $/kg y estatus
 * vs. mes anterior. Reutilizado por `appendHojaClientesDescuentoMes` (Excel) y por el endpoint
 * `/api/dashboard/arr-clientes-mes` (JSON).
 *
 * @returns {Promise<{
 *   historico: boolean,
 *   ty: number,
 *   tm: number,
 *   fechaHastaNote: string,
 *   rows: Array<{
 *     planta: string,
 *     cliente: string,
 *     categoria: string,
 ...
 * @param {boolean} [options.historico]
 * @param {number} [options.targetKgOverride] — kg objetivo (p. ej. venta_ton IGF × 1000) para prorratear proyección alineada al mini IGF.
 */
async function computeClientesDescuentoMes(client, year, month, plantCode = null, options = {}) {
  ...
  /** Filtra por planta provincia usando prov_name (alinea códigos en ventas vs ap.plant_code). */
  const plantWhereV = plantFilter
    ? `AND (
        COALESCE(pm.prov_name, '') IN (
          SELECT pmf.prov_name FROM prov_map pmf
          WHERE UPPER(TRIM(pmf.ap_plant_code)) = UPPER(TRIM($3::text))
        )
        OR UPPER(TRIM(v.plant_code)) = UPPER(TRIM($3::text))
      )`
    : "";
  const plantWhereD = plantFilter
    ? `AND (
        COALESCE(pm.prov_name, '') IN (
          SELECT pmf.prov_name FROM prov_map pmf
          WHERE UPPER(TRIM(pmf.ap_plant_code)) = UPPER(TRIM($3::text))
    ...
    : "";
  const plantWhereC = plantFilter
    ? `AND (
        COALESCE(pm.prov_name, '') IN (
          SELECT pmf.prov_name FROM prov_map pmf
          WHERE UPPER(TRIM(pmf.ap_plant_code)) = UPPER(TRIM($3::text))
        )
        OR UPPER(TRIM(c.plant_code)) = UPPER(TRIM($3::text))
      )`
    : "";
  ...
  const ty = options.targetYear != null ? Number(options.targetYear) : year;
  const tm = options.targetMonth != null ? Number(options.targetMonth) : month;
  let fechaHastaNote = (options.fechaHasta || "").toString().trim().slice(0, 10);
  if (!fechaHastaNote) {
    const today = new Date();
  ...
  const forecastKgByPlant = historico
    ? new Map()
    : await fetchForecastKgByPlantMap(client, ty, tm, options.forecastKgByPlant);
  ...
  const q = await client.query(
    ...
    SELECT
      ...
      d.monto AS monto,
      (d.cliente IS NOT NULL) AS has_discount_row
    FROM ventas v
  ...
  const rows = (q.rows || [])
    .map((r) => {
      ...
      const hasDiscountRow = r.has_discount_row === true || r.has_discount_row === "t" || r.has_discount_row === 1;
      const monto = hasDiscountRow && r.monto != null ? Number(r.monto) : null;
      const descKg = hasDiscountRow && kg > 0 && Number.isFinite(Number(monto)) ? Math.abs(Number(monto)) / kg : null;
      ...
      // Sin fila en cliente_categoria_mes ≡ Casa (misma regla que COALESCE(c.canal,'Casa')).
      const categoria = (cat.categoria || "").toString().trim() || "Casa";
      return {
        planta,
        cliente,
        categoria,
        ...
        kg,
        monto: hasDiscountRow ? Number(monto) : 0,
        descKg,
        hasDiscountRow,
        prevKg,
        factor: 0,
  ...
  if (!historico) {
    const overrideKgRaw = options.targetKgOverride;
    const overrideKg =
      overrideKgRaw != null && Number.isFinite(Number(overrideKgRaw)) && Number(overrideKgRaw) > 0
        ? Number(overrideKgRaw)
        : null;
    for (const [plantaKey, list] of byPlant) {
      let targetKg = resolveTargetKgForPlantaLabel(plantaKey, forecastKgByPlant);
      if (overrideKg != null) targetKg = overrideKg;
  ...
  return { historico, ty, tm, fechaHastaNote, rows };
}
 ...
 * @param {boolean} [forecastOpts.historico] — true para meses ya concluidos (omite columna "Kg mes (proyectado)" y la lógica de factor/forecast).
 * @param {number} [forecastOpts.targetKgOverride] — kg objetivo por planta (ej. venta ton IGF × 1000) para alinear Σ kg proyectados con el mini IGF; si se omite se usa arr.forecast_mensual.
 * @param {string} [forecastOpts.sheetName] — nombre de la hoja a crear (por defecto "Clientes desc mes").
 */
async function appendHojaClientesDescuentoMes(client, wb, year, month, plantCode = null, forecastOpts = {}) {
  const sheetName = (forecastOpts.sheetName || "Clientes desc mes").toString().trim().slice(0, 31) || "Clientes desc mes";
  const plantFilter = (plantCode || "").toString().trim();
  const { historico, ty, tm, fechaHastaNote, rows } = await computeClientesDescuentoMes(
    client,
    year,
    month,
    plantCode,
  ...
  aoa.push([
    historico
      ? "Clientes por planta — descuento del mes ($/kg) — mes histórico (real)"
  ...
  if (historico) {
    aoa.push([
      `Kg mes (real) = SUM(kg) en arr.ventas_diarias_cliente del mes calendario ${year}-${String(month).padStart(2, "0")} (mes ya concluido, sin proyección).`,
    ]);
  } else {
    aoa.push([
  ...
  const lastCol0 = historico ? 7 : 8;
  excelTheme.applyTitleStyle(ws, 1, 0, lastCol0);
  excelTheme.applyTitleStyle(ws, 2, 0, lastCol0);
  excelTheme.applyTitleStyle(ws, 3, 0, lastCol0);
  excelTheme.applyTitleStyle(ws, 4, 0, lastCol0);
  excelTheme.applyHeaderRow(ws, 6, 0, lastCol0);
  const lastR = aoa.length - 1;
  excelTheme.applyBordersRange(ws, 5, 0, lastR, lastCol0);
  if (historico) {
    excelTheme.setColumnWidths(ws, [22, 36, 16, 18, 14, 18, 16, 22]);
  } else {
  ...
  for (let r0 = 6; r0 <= lastR; r0++) {
    if (historico) {
      ...
      const mxn = ws.getCell(r0 + 1, 6).value;
      const dk = ws.getCell(r0 + 1, 7).value;
      if (typeof kgReal === "number") ws.getCell(r0 + 1, 5).numFmt = excelTheme.FMT_NUMBER;
      if (typeof mxn === "number") ws.getCell(r0 + 1, 6).numFmt = excelTheme.FMT_CURRENCY_MXN;
      if (typeof dk === "number") ws.getCell(r0 + 1, 7).numFmt = excelTheme.FMT_NUMBER;
    } else {
      const kgReal = ws.getCell(r0 + 1, 5).value;
...
/**
 * PROM mes (por día de semana) — misma receta que el modal Pronóstico.
 * Orden ISO lunes→domingo. Día desmarcado no entra. Día ausente/no finito se omite.
 * @returns {(number|string)[]} 7 celdas; "" si no hay observaciones en ese weekday.
 */
function computePromMesByDow(valueMap, opts = {}) {
  const prom = ["", "", "", "", "", "", ""];
  if (!opts.enableLookback || !opts.lookbackStartYmd || !opts.lookbackEndYmd) return prom;
  const startDt = parseYmdToDate(opts.lookbackQueryFromYmd || opts.lookbackVisualStartYmd || opts.lookbackStartYmd);
  const endDt = parseYmdToDate(opts.lookbackEndYmd);
  if (!startDt || !endDt) return prom;
  const sel = opts.selectedByYmd && typeof opts.selectedByYmd.get === "function" ? opts.selectedByYmd : new Map();
  const valsByDow = [[], [], [], [], [], [], []];
  const promDayCount = Math.min(120, Math.max(1, Math.round((endDt.getTime() - startDt.getTime()) / 86400000) + 1));
  for (let i = 0; i < promDayCount; i++) {
    ...
    const raw = valueMap && valueMap.get ? valueMap.get(key) : undefined;
    if (raw == null || !Number.isFinite(Number(raw))) continue;
    const inStrict = key >= opts.lookbackStartYmd && key <= opts.lookbackEndYmd;
    const ok = sel.has(key) ? sel.get(key) !== false : inStrict;
    if (!ok) continue;
    valsByDow[isoDowFromDate(d) - 1].push(Number(raw));
  }
  for (let k = 0; k < 7; k++) {
    const m = meanNonEmpty(valsByDow[k]);
    prom[k] = m != null ? Math.round(m * 100) / 100 : "";
  }
  return prom;
}
...
function buildSemanaDowTableRows(year, month, byDateValue, isSum, corteDtForPending = null, selPlant = null) {
  ...
  // Primer lunes de fila: el más temprano entre la semana del día 1 del mes y la del inicio de lookback (como cuadrícula Excel).
  const monthWeekMon = startOfIsoWeekMon(firstDt);
  const lbWeekMon = lookbackStart && lookbackEnd ? startOfIsoWeekMon(lookbackStart) : null;
  const firstGridMonday =
    lbWeekMon != null ? (lbWeekMon.getTime() < monthWeekMon.getTime() ? lbWeekMon : monthWeekMon) : monthWeekMon;
  ...
  const cellFechaRows = []; // YYYY-MM-DD por celda DOW (misma fila que cells)
  ...
  for (let wkMon = firstGridMonday, guard = 0; guard < 80 && wkMon <= endWeekSun; guard++, wkMon = addDays(wkMon, 7)) {
    ...
    for (let i = 0; i < 7; i++) {
      const d = addDays(wkMon, i);
      const inMonth = d.getFullYear() === year && (d.getMonth() + 1) === month;
      const inLookbackStrict = lookbackStart && lookbackEnd ? d >= lookbackStart && d <= lookbackEnd : false;
      const inLookbackVisual = lbWeekMon && lookbackEnd ? d >= firstGridMonday && d <= lookbackEnd : false;
      if (!inMonth && !inLookbackVisual) continue;
      const col = isoDow(d) - 1; // 0..6 (L..D)
      if (!inMonth && inLookbackVisual) lbMask[col] = true;
      const fecha = fmtYmd(d);
      cellFechas[col] = fecha;
      ...
      if (corteNorm && inMonth && d0 >= corteNorm) {
        ...
        if (!inMonth && inLookbackVisual) cellBg[col] = "lookback";
        else if (inMonth) cellBg[col] = "hist";
      }
      if (inLookbackVisual && t != null && Number.isFinite(t)) {
        const defaultSel = inLookbackStrict;
        const selOk = selPlant && selPlant.has(fecha) ? selPlant.get(fecha) !== false : defaultSel;
        if (selOk) promH[col] = true;
      }
      if (t != null) {
 ...
 */
function promOptsFromCtx(ctx, selPlant) {
  return {
    enableLookback: ctx.enableLookback,
    lookbackStartYmd: ctx.lookbackStartYmd,
    lookbackEndYmd: ctx.lookbackEndYmd,
    lookbackQueryFromYmd: ctx.lookbackQueryFromYmd,
    lookbackVisualStartYmd: ctx.lookbackVisualStartYmd,
    selectedByYmd: selPlant || new Map(),
  };
}

function selectionForPlant(selAll, plant) {
  if (!selAll || typeof selAll.get !== "function") return new Map();
  const key = String(plant || "").trim();
  if (selAll.has(key)) return selAll.get(key) || new Map();
  for (const [k, v] of selAll) {
    if (plantsEquivalent(k, key)) return v || new Map();
  }
  return new Map();
}

function projectionForPlant(projection, plant) {
  if (!projection || !projection.byPlant || typeof projection.byPlant.get !== "function") return null;
  const key = String(plant || "").trim();
  if (projection.byPlant.has(key)) return projection.byPlant.get(key);
  for (const [k, v] of projection.byPlant) {
    if (plantsEquivalent(k, key)) return v;
  }
  return null;
}

function forecastVentaKgByYmd(projection, plant, year, month) {
  const pack = projectionForPlant(projection, plant);
  if (!pack) return null;
  const last = new Date(year, month, 0).getDate();
  const map = {};
  for (let d = 1; d <= last; d += 1) {
    const tons = weekdayPromAt(pack.promVentaTotal, new Date(year, month - 1, d));
...
function weekdayPromAt(promDow, date) {
  if (!promDow || !date) return null;
  const iso = date.getDay() === 0 ? 7 : date.getDay();
  const v = promDow[iso - 1];
  if (v === "" || v == null || !Number.isFinite(Number(v))) return null;
  return Number(v);
}

function categoryVentaMaps(byFecha, plant) {
  const casa = new Map();
  const com = new Map();
  for (const [fecha, rec] of byFecha || []) {
    const slot = rec && rec[plant];
    if (!slot) continue;
    if (Number(slot.CASA) > 0) casa.set(fecha, Number(slot.CASA) / 1000);
    if (Number(slot.COMISIONISTA) > 0) com.set(fecha, Number(slot.COMISIONISTA) / 1000);
  }
  return { casa, com };
}

function categoryDescMaps(byFecha, plant) {
  const casa = new Map();
  const com = new Map();
  for (const [fecha, rec] of byFecha || []) {
    const slot = rec && rec[plant];
    if (!slot) continue;
    const casaRate = descuentoCategoriaRate(slot.CASA && slot.CASA.kg, slot.CASA && slot.CASA.monto);
    const comRate = descuentoCategoriaRate(slot.COMISIONISTA && slot.COMISIONISTA.kg, slot.COMISIONISTA && slot.COMISIONISTA.monto);
    if (casaRate != null) casa.set(fecha, casaRate);
    if (comRate != null) com.set(fecha, comRate);
  }
  return { casa, com };
}

function assemblePronosticoProjection(ctx, selAll, canal) {
  const ventaByFecha = (canal && canal.ventaByFecha) || new Map();
  const descByFecha = (canal && canal.descByFecha) || new Map();
  const byPlant = new Map();
  const year = ctx.corteDt.getFullYear();
  const month = ctx.corteDt.getMonth() + 1;
  for (const p of ctx.plants || []) {
    const selPlant = selectionForPlant(selAll, p);
    const opts = promOptsFromCtx(ctx, selPlant);
    const ventaSheet = buildVentaPronosticoSheetLike(year, month, ctx, p, selPlant);
    const ventaCat = categoryVentaMaps(ventaByFecha, p);
    const descCat = categoryDescMaps(descByFecha, p);
    const descMap = ctx.descMapByPlant.get(p) || new Map();
    byPlant.set(String(p).trim(), {
      plant: p,
      selPlant,
      ventaSheet,
      promVentaTotal: ventaSheet.prom_mes_dow,
      promDescTotal: computePromMesByDow(descMap, opts),
      promVentaCasa: computePromMesByDow(ventaCat.casa, opts),
      promVentaComisionista: computePromMesByDow(ventaCat.com, opts),
      promDescCasa: computePromMesByDow(descCat.casa, opts),
      promDescComisionista: computePromMesByDow(descCat.com, opts),
      proyVentaTotal: ventaSheet.proy_total_ton,
    });
  }
  return { ctx, selAll: selAll || new Map(), byPlant, corteYmdStr: ctx.corteYmdStr };
}

async function loadCanalLookback(client, plants, fromYmd, toYmd) {
  const empty = { ventaByFecha: new Map(), descByFecha: new Map() };
  if (!client || !fromYmd || !toYmd || !plants || !plants.length) return empty;
  let ventaByFecha = new Map();
  let descByFecha = new Map();
  try {
    const ventaR = await client.query(
      `WITH prov_map AS (${SQL_PROV_MAP})
       SELECT pm.prov_name AS plant_code, v.fecha, v.canal AS canal, SUM(v.kg) AS kg
         FROM arr.ventas_diarias_cliente v
         JOIN prov_map pm ON UPPER(TRIM(v.plant_code)) = pm.key_nombre
            OR (pm.key_clave <> '' AND UPPER(TRIM(v.plant_code)) = pm.key_clave)
        WHERE v.fecha >= $1::date AND v.fecha <= $2::date
        GROUP BY pm.prov_name, v.fecha, v.canal`,
      [fromYmd, toYmd]
    );
    ventaByFecha = aggregateVentaDiariaPorCanal(ventaR.rows || [], plants).byFecha;
  } catch {
    ventaByFecha = new Map();
  }
  try {
    const kgR = await client.query(
      `WITH prov_map AS (${SQL_PROV_MAP})
       SELECT pm.prov_name AS plant_code, v.fecha, v.canal AS canal, SUM(v.kg) AS kg
         FROM arr.ventas_diarias_cliente v
         JOIN prov_map pm ON UPPER(TRIM(v.plant_code)) = pm.key_nombre
    ...
    const montoR = await client.query(
      `WITH prov_map AS (${SQL_PROV_MAP})
       SELECT pm.prov_name AS plant_code, d.fecha, c.canal AS canal, SUM(d.monto) AS monto
         FROM arr.descuentos_diarios_cliente d
         JOIN prov_map pm ON UPPER(TRIM(d.plant_code)) = pm.key_nombre
            OR (pm.key_clave <> '' AND UPPER(TRIM(d.plant_code)) = pm.key_clave)
         LEFT JOIN arr.cliente_categoria_mes c
           ON c.cliente_norm = d.cliente_norm
          AND c.year = EXTRACT(YEAR FROM d.fecha)::int
          AND c.month = EXTRACT(MONTH FROM d.fecha)::int
          AND (
                UPPER(TRIM(c.plant_code)) = UPPER(TRIM(d.plant_code))
             OR UPPER(TRIM(c.plant_code)) = UPPER(TRIM(pm.prov_name))
             OR (pm.key_clave <> '' AND UPPER(TRIM(c.plant_code)) = pm.key_clave)
              )
        WHERE d.fecha >= $1::date AND d.fecha <= $2::date
        GROUP BY pm.prov_name, d.fecha, c.canal`,
      [fromYmd, toYmd]
    );
    descByFecha = aggregateDescuentoDiarioPorCanal(kgR.rows || [], montoR.rows || [], plants).byFecha;
  } catch {
    descByFecha = new Map();
  }
...
async function buildPronosticoProjectionContext(client, year, month, fechaCorte) {
  const ctx = await buildPronosticoVentaDescMaps(client, year, month, fechaCorte);
  const selAll = await loadPronosticoDiasSeleccionMap(client, year, month, ctx.corteYmdStr);
  const canal = ctx.enableLookback
    ? await loadCanalLookback(client, ctx.plants, ctx.lookbackQueryFromYmd || ctx.lookbackStartYmd, ctx.lookbackEndYmd)
    : { ventaByFecha: new Map(), descByFecha: new Map() };
  return assemblePronosticoProjection(ctx, selAll, canal);
}
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  /** Misma fuente que el modal Pronóstico / mini IGF: mapas + lookback extendido + arr.pronostico_dias_seleccion. */
  const projection = opts && opts.projection ? opts.projection : null;
  const ctx = projection
    ? projection.ctx
    : await buildPronosticoVentaDescMaps(client, year, month, fechaCorte);
  const selAll = projection
    ? projection.selAll
    : await loadPronosticoDiasSeleccionMap(client, year, month, ctx.corteYmdStr);
  const plants = opts.plantCode
    ? (ctx.plants || []).filter((p) => plantsEquivalent(p, opts.plantCode))
    : (ctx.plants || []);
  ...
  const countRemainingDowInMonth = (dow1to7) => {
    let c = 0;
    const startDay = ctx.isCorteEnMes ? ctx.corteDt.getDate() : ctx.corteDt.getDate() + 1;
    for (let day = startDay; day <= ctx.lastDay; day++) {
  ...
  for (const p of plants) {
    const pack = projection ? projectionForPlant(projection, p) : null;
    const selPlant = pack ? pack.selPlant : (selAll.get(String(p).trim()) || new Map());
    const ventaSheet = pack ? pack.ventaSheet : buildVentaPronosticoSheetLike(year, month, ctx, p, selPlant);
    const ventaWeeks = ventaSheet.ventaWeeks;
    const promMesVenta = ventaSheet.prom_mes_dow;
    const totalMesVenta = ventaSheet.total_mes_dow;
    const porComprarVenta = ventaSheet.por_comprar_dow;
    const proyVenta = ventaSheet.proy_dow;

    const ventaMap = ctx.ventaMapByPlant.get(p) || new Map();
    const descMap = new Map(ctx.descMapByPlant.get(p) || new Map());
    if (ctx.enableLookback && ctx.lookbackStartYmd && ctx.lookbackEndYmd) {
      descMap.__meta = { lookbackStart: ctx.lookbackStartYmd, lookbackEnd: ctx.lookbackEndYmd };
    }
    ...
    // === Bloque VENTA (ton): mismos totales que modal Pronóstico / IGF (buildVentaPronosticoSheetLike) ===
    ...
    // === Bloque DESCUENTO ($/kg) + cálculos en dinero (PROM desc con misma regla de días que venta) ===
    const corteForPending = ctx.isCorteEnMes ? ctx.corteDt : null;
    const descWeeks = buildSemanaDowTableRows(year, month, descMap, false, corteForPending, selPlant);

    const totalMesDescMxn = [0, 0, 0, 0, 0, 0, 0];
    // TOTAL mes en dinero solo incluye días "cerrados" (excluye el día de corte si está "en curso"/azul).
    const lastClosedDay = ctx.isCorteEnMes ? Math.max(0, corteDt.getDate() - 1) : corteDt.getDate();
    ...
    const promMesDescRatio = pack
      ? pack.promDescTotal
      : computePromMesByDow(descMap, {
        enableLookback: ctx.enableLookback,
        lookbackStartYmd: ctx.lookbackStartYmd,
        lookbackEndYmd: ctx.lookbackEndYmd,
        lookbackQueryFromYmd: ctx.lookbackQueryFromYmd,
        lookbackVisualStartYmd: ctx.lookbackVisualStartYmd,
        selectedByYmd: selPlant,
      });
  ...
  // No llamar applyPronosticoSummaryFormulas: esa rutina reemplazaba PROM/POR COMPRAR/PROY por
  // "promedio de los 2 últimos valores" en cada columna y descartaba lookback + selección de días (dashboard).
  return {
...
const PROVINCIA_SEP_COL = 9;
const PROVINCIA_CAT_START_COL = 10;
const FMT_DESC_KG_3 = "#,##0.000";
const SQL_PROV_MAP = `
  SELECT DISTINCT p.nombre AS prov_name,
         UPPER(TRIM(p.nombre)) AS key_nombre,
         UPPER(TRIM(COALESCE(p.clave, ''))) AS key_clave
...
function arrFechaKey(value) {
  if (!value) return "";
  if (typeof value === "string") return value.slice(0, 10);
  if (value instanceof Date && typeof value.toISOString === "function") return value.toISOString().slice(0, 10);
  return String(value).slice(0, 10);
}

/** CASA / COMISIONISTA: case, acento y espacios. Sin alias fuzzy. Canal desconocido → null. */
function canonicalArrCanal(raw) {
  ...
  if (s === "COMISIONISTA") return "COMISIONISTA";
  return null;
}

function resolveProvinciaCutoffDay(year, month, fechaCorteYmd, lastDay) {
  let cutoffDay = lastDay + 1;
  const corte = String(fechaCorteYmd || "").trim().slice(0, 10);
  if (/^\d{4}-\d{2}-\d{2}$/.test(corte)) {
    const [cy, cm, cd] = corte.split("-").map((x) => parseInt(x, 10));
...
function emptyCanalKg() {
  return { CASA: 0, COMISIONISTA: 0 };
}

function emptyCanalMonto() {
  return {
    CASA: { kg: 0, monto: 0 },
    COMISIONISTA: { kg: 0, monto: 0 },
  };
}

/**
 * Kg observados por planta/fecha/canal. Los kg con canal null u otro no entran a CASA.
 * @returns {{ byFecha: Map<string, Record<string, {CASA:number, COMISIONISTA:number}>>, unclassifiedKg: number }}
 */
function aggregateVentaDiariaPorCanal(rows, plants) {
  const byFecha = new Map();
  let unclassifiedKg = 0;
  for (const row of rows || []) {
    const fecha = arrFechaKey(row.fecha);
    const plant = resolveCanonicalPlantCode(row.plant_code, plants);
    if (!fecha || !plant) continue;
    const canal = canonicalArrCanal(row.canal);
    ...
    const rec = byFecha.get(fecha);
    if (!rec[plant]) rec[plant] = emptyCanalKg();
    rec[plant][canal] += kg;
  }
  return { byFecha, unclassifiedKg };
}

/**
 * Numerador y denominador de la MISMA categoría.
 * kg: arr.ventas_diarias_cliente.canal. monto: arr.cliente_categoria_mes.canal (sin default Casa).
 */
function aggregateDescuentoDiarioPorCanal(kgRows, montoRows, plants) {
  const byFecha = new Map();
  let unclassifiedKg = 0;
  let unclassifiedMonto = 0;
  function slot(fecha, plant) {
    if (!byFecha.has(fecha)) byFecha.set(fecha, {});
    const rec = byFecha.get(fecha);
    if (!rec[plant]) rec[plant] = emptyCanalMonto();
    return rec[plant];
  }
  for (const row of kgRows || []) {
    ...
    slot(fecha, plant)[canal].kg += kg;
  }
  for (const row of montoRows || []) {
    const fecha = arrFechaKey(row.fecha);
    const plant = resolveCanonicalPlantCode(row.plant_code, plants);
    if (!fecha || !plant) continue;
    const canal = canonicalArrCanal(row.canal);
    ...
    if (!canal) {
      unclassifiedMonto += monto;
      continue;
    }
    slot(fecha, plant)[canal].monto += monto;
  }
  return { byFecha, unclassifiedKg, unclassifiedMonto };
}

/** null = sin evidencia (kg <= 0). 0 = descuento real cero con kg > 0. Presentación en magnitud positiva. */
function descuentoCategoriaRate(kg, monto) {
  const k = Number(kg);
  if (!Number.isFinite(k) || k <= 0) return null;
  const m = Number(monto);
  const mag = Number.isFinite(m) ? Math.abs(m) : 0;
  return mag / k;
}

/** ACUM = ABS(SUM(monto)) / SUM(kg) del mismo canal. No es promedio de tasas diarias. */
function acumDescuentoPonderado(pairs) {
  let kg = 0;
  let monto = 0;
  for (const p of pairs || []) {
    kg += Number(p && p.kg) || 0;
    monto += Number(p && p.monto) || 0;
  }
  return descuentoCategoriaRate(kg, monto);
}

function findWorksheetLabelRow(ws, label) {
...
function writeProvinciaCanalColumns(ws, opts) {
  const plants = (opts && opts.plants) || [];
  const byDate = (opts && opts.byDate) || [];
  const cutoffDay = opts && opts.cutoffDay != null ? opts.cutoffDay : 999;
  const mode = opts && opts.mode === "descuento" ? "descuento" : "venta";
  const projection = opts && opts.pronosticoProjection;
  const corteYmd = projection && projection.corteYmdStr ? String(projection.corteYmdStr) : "";
  const year = Number(opts && opts.year);
  const month = Number(opts && opts.month);
  if (!plants.length || !byDate.length) return;
  const isClosedDay = (d) => (corteYmd ? d.fecha < corteYmd : d.day < cutoffDay);

  const startCol = PROVINCIA_CAT_START_COL;
  const endCol = startCol + plants.length * 2 - 1;
  ws.getColumn(PROVINCIA_SEP_COL).width = 4;
  ws.getCell(1, PROVINCIA_SEP_COL).value = null;

  plants.forEach((p, i) => {
    const cCasa = startCol + i * 2;
    const cCom = cCasa + 1;
    ws.getCell(1, cCasa).value = `${p}\nCASA`;
    ws.getCell(1, cCom).value = `${p}\nCOMISIONISTA`;
    ws.getColumn(cCasa).width = 14;
    ws.getColumn(cCom).width = 16;
  });
  excelTheme.applyHeaderRow(ws, 1, startCol - 1, endCol - 1);
  const row1 = ws.getRow(1);
  if (!row1.height || row1.height < 30) row1.height = 32;

  const realDays = byDate.filter(isClosedDay);
  byDate.forEach((d, i) => {
    const excelRow = i + 2;
    const isReal = isClosedDay(d);
    const fill = isReal ? excelTheme.FILL_BAND_REAL : excelTheme.FILL_BAND_PROY;
    const dt = !isReal && corteYmd ? new Date(year, month - 1, d.day) : null;
    plants.forEach((p, pi) => {
      const pair = (d.byPlant && d.byPlant[p]) || null;
      const cCasa = startCol + pi * 2;
      const pack = !isReal && projection ? projectionForPlant(projection, p) : null;
      const forecast = pack
        ? {
          casa: weekdayPromAt(mode === "venta" ? pack.promVentaCasa : pack.promDescCasa, dt),
          com: weekdayPromAt(mode === "venta" ? pack.promVentaComisionista : pack.promDescComisionista, dt),
        }
        : null;
      writeCanalPairCells(ws, excelRow, cCasa, pair, isReal, mode, fill, forecast);
    });
  });

  const acumRow = findWorksheetLabelRow(ws, "ACUM");
  if (acumRow) {
    plants.forEach((p, pi) => {
      const cCasa = startCol + pi * 2;
      const cCom = cCasa + 1;
      if (mode === "venta") {
        const sumCanal = (canal) => realDays.reduce((s, d) => s + (Number(d.byPlant && d.byPlant[p] && d.byPlant[p][canal]) || 0), 0);
        setCanalNumber(ws, acumRow, cCasa, sumCanal("CASA"), FMT_VENTA_TON_3);
        setCanalNumber(ws, acumRow, cCom, sumCanal("COMISIONISTA"), FMT_VENTA_TON_3);
      } else {
        const pairs = (canal) => realDays.map((d) => (d.byPlant && d.byPlant[p] && d.byPlant[p][canal]) || { kg: 0, monto: 0 });
        setCanalNumber(ws, acumRow, cCasa, acumDescuentoPonderado(pairs("CASA")), FMT_DESC_KG_3);
        setCanalNumber(ws, acumRow, cCom, acumDescuentoPonderado(pairs("COMISIONISTA")), FMT_DESC_KG_3);
      }
    });
  }

  if (mode === "venta") {
    const promRow = findWorksheetLabelRow(ws, "PROM");
    if (promRow && acumRow) {
      plants.forEach((p, pi) => {
        const cCasa = startCol + pi * 2;
        const cCom = cCasa + 1;
        const casa = ws.getCell(acumRow, cCasa).value;
        const com = ws.getCell(acumRow, cCom).value;
        setCanalNumber(ws, promRow, cCasa, realDays.length && typeof casa === "number" ? casa / realDays.length : null, FMT_VENTA_TON_3);
        setCanalNumber(ws, promRow, cCom, realDays.length && typeof com === "number" ? com / realDays.length : null, FMT_VENTA_TON_3);
      });
    }
  }

  const lastLabel = mode === "venta" ? findWorksheetLabelRow(ws, "Dif Comp") : acumRow;
  const lastR0 = (lastLabel || byDate.length + 1) - 1;
  excelTheme.applyBordersRange(ws, 0, startCol - 1, lastR0, endCol - 1);
}

function writeCanalPairCells(ws, excelRow, cCasa, pair, isReal, mode, fill, forecast) {
  const cCom = cCasa + 1;
  let casa;
  let com;
  let fmt;
  if (!isReal) {
    if (forecast) {
      casa = forecast.casa == null ? null : forecast.casa;
      com = forecast.com == null ? null : forecast.com;
    } else {
      casa = 0;
      com = 0;
    }
    fmt = mode === "descuento" ? FMT_DESC_KG_3 : FMT_VENTA_TON_3;
  } else if (mode === "venta") {
    casa = pair ? Number(pair.CASA) || 0 : 0;
    com = pair ? Number(pair.COMISIONISTA) || 0 : 0;
    fmt = FMT_VENTA_TON_3;
  } else {
    const casaPart = pair && pair.CASA ? pair.CASA : { kg: 0, monto: 0 };
    const comPart = pair && pair.COMISIONISTA ? pair.COMISIONISTA : { kg: 0, monto: 0 };
    casa = descuentoCategoriaRate(casaPart.kg, casaPart.monto);
    com = descuentoCategoriaRate(comPart.kg, comPart.monto);
    fmt = FMT_DESC_KG_3;
  }
  setCanalNumber(ws, excelRow, cCasa, casa, fmt, fill);
  setCanalNumber(ws, excelRow, cCom, com, fmt, fill);
}

function setCanalNumber(ws, excelRow, col, value, fmt, fill) {
  const cell = ws.getCell(excelRow, col);
  if (value == null || value === "") cell.value = null;
  else cell.value = value;
  if (typeof cell.value === "number" && fmt) cell.numFmt = fmt;
  if (fill) cell.fill = fill;
}

async function getVentaDiariaPorCanalGrid(client, year, month, fechaCorteYmd, plants, cutoffDay) {
  const lastDay = new Date(year, month, 0).getDate();
  const list = Array.isArray(plants) ? plants : [];
  const cutoff = Number.isFinite(cutoffDay) ? cutoffDay : resolveProvinciaCutoffDay(year, month, fechaCorteYmd, lastDay);
  if (!list.length) return { plants: [], byDate: [], cutoffDay: cutoff, unclassifiedKg: 0 };
  const firstDay = `${year}-${String(month).padStart(2, "0")}-01`;
  const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
  const r = await client.query(
    `WITH prov_map AS (${SQL_PROV_MAP})
  ...
  const { byFecha, unclassifiedKg } = aggregateVentaDiariaPorCanal(r.rows || [], list);
  const byDate = [];
  for (let d = 1; d <= lastDay; d++) {
    const fecha = `${year}-${String(month).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
    const rec = byFecha.get(fecha) || {};
    const byPlant = {};
    for (const p of list) {
      const raw = rec[p] || emptyCanalKg();
      byPlant[p] = { CASA: Number(raw.CASA) / 1000, COMISIONISTA: Number(raw.COMISIONISTA) / 1000 };
    }
    byDate.push({ day: d, fecha, byPlant });
  }
  return { plants: list, byDate, cutoffDay: cutoff, unclassifiedKg };
}

async function getDescuentoDiarioPorCanalGrid(client, year, month, fechaCorteYmd, plants, cutoffDay) {
  const lastDay = new Date(year, month, 0).getDate();
  const list = Array.isArray(plants) ? plants : [];
  const cutoff = Number.isFinite(cutoffDay) ? cutoffDay : resolveProvinciaCutoffDay(year, month, fechaCorteYmd, lastDay);
  if (!list.length) return { plants: [], byDate: [], cutoffDay: cutoff, unclassifiedKg: 0, unclassifiedMonto: 0 };
  const firstDay = `${year}-${String(month).padStart(2, "0")}-01`;
  const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
  const kgR = await client.query(
    `WITH prov_map AS (${SQL_PROV_MAP})
  ...
  const montoR = await client.query(
    `WITH prov_map AS (${SQL_PROV_MAP})
     SELECT pm.prov_name AS plant_code, d.fecha, c.canal AS canal, SUM(d.monto) AS monto
      ...
      WHERE d.fecha >= $1::date AND d.fecha <= $2::date
      GROUP BY pm.prov_name, d.fecha, c.canal`,
    [firstDay, lastDayStr, year, month]
  );
  const { byFecha, unclassifiedKg, unclassifiedMonto } = aggregateDescuentoDiarioPorCanal(kgR.rows || [], montoR.rows || [], list);
  const byDate = [];
  for (let d = 1; d <= lastDay; d++) {
    const fecha = `${year}-${String(month).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
    const rec = byFecha.get(fecha) || {};
    const byPlant = {};
    for (const p of list) byPlant[p] = rec[p] || emptyCanalMonto();
    byDate.push({ day: d, fecha, byPlant });
  }
  return { plants: list, byDate, cutoffDay: cutoff, unclassifiedKg, unclassifiedMonto };
}

function precioYmd(value) {
...
const PRECIO_PLANT_PAIRS = [
  ["Queretaro", "Querétaro"],
  ["Tehuacan", "Tehuacán"],
];

function precioPlantAlias(exact) {
  for (const [a, b] of PRECIO_PLANT_PAIRS) {
    if (exact === a) return b;
    if (exact === b) return a;
  }
  return null;
...
function mapPrecioRows(rows) {
  return (rows || []).map((row) => ({
    fecha: precioYmd(row.fecha),
    precio: precioNumero(row.precio),
  }));
}

function mergePrecioAliasRows(rows, exact, alias) {
  const byFecha = new Map();
  for (const row of rows || []) {
    const fecha = precioYmd(row.fecha);
    if (!fecha) continue;
    const code = String(row.plant_code || "").trim();
    if (code !== exact && code !== alias) continue;
    const slot = byFecha.get(fecha) || {};
    slot[code === exact ? "exact" : "alias"] = precioNumero(row.precio);
    byFecha.set(fecha, slot);
  }
  const out = [];
  for (const [fecha, slot] of byFecha) {
    let precio = null;
    if (slot.exact !== undefined && precioValido(slot.exact)) precio = slot.exact;
    else if (slot.alias !== undefined && precioValido(slot.alias)) precio = slot.alias;
    else if (slot.exact !== undefined) precio = slot.exact;
    else precio = slot.alias;
    out.push({ fecha, precio });
  }
  return out;
}

async function loadPrecioDiario(client, plantCode, year, month) {
  const y = Number(year);
  const m = Number(month);
  const start = `${y}-${String(m).padStart(2, "0")}-01`;
  const next = m === 12 ? `${y + 1}-01-01` : `${y}-${String(m + 1).padStart(2, "0")}-01`;
  const exact = String(plantCode || "").trim();
  const alias = precioPlantAlias(exact);
  if (!alias) {
    ...
    return mapPrecioRows(result.rows);
  }
  const result = await client.query(
    `SELECT fecha, precio, plant_code
       FROM arr.precio_diario
      WHERE plant_code IN ($1, $4)
        AND fecha >= $2::date
        AND fecha < $3::date
      ORDER BY fecha ASC`,
    [exact, start, next, alias]
  );
  return mergePrecioAliasRows(result.rows, exact, alias);
}

function appendPrecioWorksheet(wb, year, month, rows) {
  const y = Number(year);
  const m = Number(month);
  const lastDay = new Date(y, m, 0).getDate();
  const byFecha = new Map();
  for (const row of rows || []) {
    const fecha = precioYmd(row && row.fecha);
    if (!fecha) continue;
    byFecha.set(fecha, row.precio == null || !Number.isFinite(Number(row.precio)) ? null : Number(row.precio));
  }
  const ws = wb.addWorksheet("PRECIO");
  const border = { style: "thin", color: { argb: "FF000000" } };
  const box = { top: border, left: border, bottom: border, right: border };
  const headers = ["Fecha", "PRECIO"];
  ...
  let lastValidPrecio = null;
  for (let day = 1; day <= lastDay; day += 1) {
    const fecha = `${y}-${String(m).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
    const dateCell = ws.getCell(day + 1, 1);
    dateCell.value = new Date(Date.UTC(y, m - 1, day));
    dateCell.numFmt = "dd-mmm";
    dateCell.alignment = { horizontal: "center" };
    dateCell.border = box;
    const priceCell = ws.getCell(day + 1, 2);
    const stored = byFecha.has(fecha) ? byFecha.get(fecha) : null;
    const own = stored != null && Number.isFinite(Number(stored)) && Number(stored) > 0 ? Number(stored) : null;
    if (own != null) lastValidPrecio = own;
    const precio = own != null ? own : lastValidPrecio;
    priceCell.value = precio == null ? null : precio;
    if (precio != null) priceCell.numFmt = "0.00000000";
    priceCell.alignment = { horizontal: "right" };
    priceCell.border = box;
  }
...
function renderProvinciaDiariaSheets(partial) {
  const wb = new ExcelJS.Workbook();
  const year = partial.year;
  const month = partial.month;
  hojaA(wb, partial.ventaTonGrid, year, month, partial.compTotalKg != null ? partial.compTotalKg : 0, "", "", {
    proyTonByPlant: partial.proyTonByPlant || null,
    ventaCanal: partial.ventaCanal || null,
    omitTotProvincia: Boolean(partial && partial.omitTotProvincia),
    pronosticoProjection: partial.pronosticoProjection || null,
  });
  hojaB(wb, partial.descuentoGrid, partial.ventaTonGrid, year, month, "", "", partial.descuentoCanal || null, {
    pronosticoProjection: partial.pronosticoProjection || null,
  });
  if (partial && Array.isArray(partial.precioDiario)) {
    appendPrecioWorksheet(wb, year, month, partial.precioDiario);
  }
  if (partial && partial.comprasPayload) {
    const comprasOpts = { plantName: partial.comprasPlantName || "" };
    if (partial.fechaCorte) comprasOpts.corteYmd = String(partial.fechaCorte).trim().slice(0, 10);
    appendComprasWorksheet(wb, partial.comprasPayload, comprasOpts);
  }
  return wb;
}
...
function hojaA(wb, ventaTonGrid, year, month, compTotalKg, lastDayStr, todayStr, opts = {}) {
  ...
  const omitTotProvincia = Boolean(opts && opts.omitTotProvincia);
  const headers = ["DÍA", ...plants, omitTotProvincia ? null : "Tot Provincia"];
  const data = [headers];
  ...
  for (const d of byDate) {
    const row = [d.day];
    const projected = Boolean(corteYmd) && d.fecha >= corteYmd;
    const dt = projected ? new Date(year, month - 1, d.day) : null;
    let tot = 0;
    let totSeen = false;
    for (const p of plants) {
      let v;
      if (projected) {
        const pack = projectionForPlant(projection, p);
        const forecast = pack ? weekdayPromAt(pack.promVentaTotal, dt) : null;
        v = forecast == null ? "" : forecast;
      } else {
    ...
    row.push(omitTotProvincia ? null : (totSeen ? tot : ""));
    data.push(row);
  }

  const realDates = byDate.filter(isClosedDay);
  ...
  const proys = plants.map((p) => {
    const pack = projection ? projectionForPlant(projection, p) : null;
    if (pack && Number.isFinite(Number(pack.proyVentaTotal))) return Number(pack.proyVentaTotal);
    return (proyTonByPlant && proyTonByPlant.has(p)) ? proyTonByPlant.get(p) : (forecastByPlant.get(p) ?? "");
  });
  const totProy = plants.reduce((s, p, i) => s + (Number(proys[i]) || 0), 0) || "";

  data.push([]);
  data.push(["ACUM", ...acums, omitTotProvincia ? null : totAcum]);
  data.push(["PROM", ...proms, omitTotProvincia ? null : totProm]);
  data.push(["PROY", ...proys, omitTotProvincia ? null : totProy]);
  data.push(["Comp", ...plants.map(() => ""), omitTotProvincia ? null : (compTotalKg != null ? compTotalKg / 1000 : "")]);
  data.push(["Dif Comp", ...plants.map(() => ""), omitTotProvincia ? null : (compTotalKg != null ? totAcum - compTotalKg / 1000 : "")]);
  ...
  for (let i = 0; i < byDate.length; i++) {
    ...
    const isReal = corteYmd ? byDate[i].fecha < corteYmd : day < cutoffDay;
    const fill = isReal ? excelTheme.FILL_BAND_REAL : excelTheme.FILL_BAND_PROY;
    for (let j = 0; j < headers.length; j++) {
      const cell = ws.getCell(i + 2, j + 1);
      cell.fill = fill;
      if (j >= 1 && typeof cell.value === "number") cell.numFmt = FMT_VENTA_TON_3;
    }
  }
  for (let r = 1; r <= data.length; r++) {
    const label = String(ws.getCell(r, 1).value == null ? "" : ws.getCell(r, 1).value).trim();
    if (!["ACUM", "PROM", "PROY", "Comp", "Dif Comp"].includes(label)) continue;
    for (let c = 2; c <= headers.length; c++) {
      const cell = ws.getCell(r, c);
      if (typeof cell.value === "number") cell.numFmt = FMT_VENTA_TON_3;
    }
  }
  ...
  if (opts && opts.ventaCanal && Array.isArray(opts.ventaCanal.byDate)) {
    writeProvinciaCanalColumns(ws, {
      mode: "venta",
      plants,
      byDate: opts.ventaCanal.byDate,
      cutoffDay,
      year,
      month,
...
function hojaB(wb, descuentoGrid, ventaTonGrid, year, month, lastDayStr, todayStr, descuentoCanal, opts = {}) {
  ...
  for (const d of byDate) {
    ...
    for (const p of plants) {
      if (projected) {
        const pack = projectionForPlant(projection, p);
        const forecast = pack ? weekdayPromAt(pack.promDescTotal, dt) : null;
        row.push(forecast == null ? "" : forecast);
      } else {
        const v = d.byPlant[p];
        row.push(v != null ? v : "");
      }
  ...
  const acumByPlant = plants.map((p) => {
    let num = 0;
    let den = 0;
    for (const d of byDate.filter(isClosedDay)) {
  ...
  if (descuentoCanal && Array.isArray(descuentoCanal.byDate)) {
    writeProvinciaCanalColumns(ws, {
      mode: "descuento",
      plants,
      byDate: descuentoCanal.byDate,
      cutoffDay,
      year,
      month,
      pronosticoProjection: projection,
...
async function buildPronosticoVentaDescMaps(client, year, month, fechaCorte) {
  ...
  let lookbackVisualStartYmd = null;
  let lookbackQueryFromYmd = null;
  if (enableLookback) {
    ...
    const monLb = mondayOfIsoWeekContainingYmd(lookbackStartYmd);
    lookbackVisualStartYmd = monLb ? fmtYmd(monLb) : lookbackStartYmd;
    const monthMondayDt = mondayOfIsoWeekContainingYmd(`${year}-${pad2(month)}-01`);
    const lbMondayDt = monLb;
    const gridDataFromDt =
      lbMondayDt && monthMondayDt
        ? lbMondayDt.getTime() < monthMondayDt.getTime()
          ? lbMondayDt
          : monthMondayDt
        : monthMondayDt || lbMondayDt;
    lookbackQueryFromYmd = gridDataFromDt ? fmtYmd(gridDataFromDt) : lookbackVisualStartYmd;
    ...
    try {
      const qVenta = await client.query(
        `SELECT plant_code, fecha, venta_ton
          ...
          WHERE fecha >= $1::date AND fecha <= $2::date
          ORDER BY fecha`,
        [lookbackQueryFromYmd, lookbackEndYmd]
      );
    ...
    try {
      await mergeVentaLookbackGapsFromVentasCliente(client, ventaMapByPlant, plants, lookbackQueryFromYmd, lookbackEndYmd);
    } catch {
      /* best-effort */
    }
    try {
    ...
    try {
      await mergeDescLookbackGapsFromDiariosCliente(client, descMapByPlant, plants, lookbackQueryFromYmd, lookbackEndYmd);
    } catch {
      /* best-effort */
    }
  }
  ...
  return {
    ...
    lookbackStartYmd,
    lookbackEndYmd,
    lookbackVisualStartYmd,
    lookbackQueryFromYmd,
    corteYmdStr,
    fmtYmd,
    isoDow,
    sum,
...
function buildVentaPronosticoSheetLike(year, month, ctx, plantKey, selPlant) {
  ...
  // TOTAL mes solo incluye días "cerrados". Si el corte está en el mes, el día de corte está "en curso" (azul) y NO se suma.
  const lastClosedDay = ctx.isCorteEnMes ? Math.max(0, corteDt.getDate() - 1) : corteDt.getDate();
  for (let day = 1; day <= lastClosedDay; day++) {
    const d = new Date(year, month - 1, day);
    const key = fmtYmd(d);
  ...
  const promMesVenta = computePromMesByDow(vm, {
    enableLookback: ctx.enableLookback,
    lookbackStartYmd: ctx.lookbackStartYmd,
    lookbackEndYmd: ctx.lookbackEndYmd,
    lookbackQueryFromYmd: ctx.lookbackQueryFromYmd,
  ...
  for (let i = 1; i < ventaWeeks.rows.length; i++) {
    ...
    const cf = (ventaWeeks.cellFechaRows && ventaWeeks.cellFechaRows[i]) || ["", "", "", "", "", "", ""];
    weeks.push({
      label: row[0],
  ...
  return {
    ...
    /** Misma tabla que export Excel / estilos (filas semana + máscaras). */
    ventaWeeks,
  };
}
...
async function getPronosticoPlantDetail(client, year, month, plantCode, fechaCorte) {
  ...
  const days = [];
  if (ctx.enableLookback && ctx.lookbackStartYmd && ctx.lookbackEndYmd) {
    const startDt = parseYmdToDate(ctx.lookbackQueryFromYmd || ctx.lookbackVisualStartYmd || ctx.lookbackStartYmd);
    ...
    for (let d = new Date(startDt); d <= endDt; d.setDate(d.getDate() + 1)) {
      ...
      const inStrict = key >= ctx.lookbackStartYmd && key <= ctx.lookbackEndYmd;
      const selected = selPlant.has(key) ? selPlant.get(key) !== false : inStrict;
  ...
  return {
    ...
    lookback_visual_start: ctx.lookbackQueryFromYmd || ctx.lookbackVisualStartYmd || ctx.lookbackStartYmd,
    days,
    venta_sheet,
...
async function computePronosticoProyByPlant(client, year, month, opts = {}) {
  ...
  for (const p of plants) {
    ...
    const totalMesVenta = [0, 0, 0, 0, 0, 0, 0];
    // TOTAL mes solo incluye días "cerrados". Si el corte está en el mes, el día de corte está "en curso" (azul) y NO se suma.
    const lastClosedDay = isCorteEnMes ? Math.max(0, corteDt.getDate() - 1) : corteDt.getDate();
    for (let day = 1; day <= lastClosedDay; day++) {
      const d = new Date(year, month - 1, day);
      const key = fmtYmd(d);
    ...
    const promMesVenta = computePromMesByDow(ventaMap, promOpts);
    ...
    const promMesDescRatio = computePromMesByDow(descMap, promOpts);
...
/**
 * Venta real del mes calendario (ton) por planta provincia.
 * Fuente: SUM(kg) en arr.ventas_diarias_cliente / 1000 (sin proyección).
 * @returns {Promise<Map<string, number>>} plant_code → venta_ton
 */
async function getVentaRealTonProvinciaByPlant(client, year, month) {
  const firstDay = `${year}-${String(month).padStart(2, "0")}-01`;
  const lastDay = new Date(year, month, 0).getDate();
  const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
  const r = await client.query(
    `WITH prov_map AS (
       ...
       SELECT pm.prov_name AS plant_code, ROUND(SUM(v.kg) / 1000.0, 2)::numeric AS venta_ton
         FROM arr.ventas_diarias_cliente v
         JOIN prov_map pm ON UPPER(TRIM(v.plant_code)) = pm.key_nombre
            OR (pm.key_clave <> '' AND UPPER(TRIM(v.plant_code)) = pm.key_clave)
        WHERE v.fecha >= $1::date AND v.fecha <= $2::date
  ...
  for (const row of r.rows || []) {
    const code = String(row.plant_code || "").trim();
    const ton = Number(row.venta_ton);
    if (!code || !Number.isFinite(ton)) continue;
    out.set(code, ton);
  }
  return out;
}
...
module.exports = {
  ...
  resolveArrClientesMesPlantCode,
  computeClientesDescuentoMes,
  getVentaRealTonProvinciaByPlant,
  canonicalArrCanal,
  aggregateVentaDiariaPorCanal,
  aggregateDescuentoDiarioPorCanal,
  descuentoCategoriaRate,
  acumDescuentoPonderado,
  getVentaDiariaPorCanalGrid,
  getDescuentoDiarioPorCanalGrid,
  renderProvinciaDiariaSheets,
  appendHojaAcapulcoSemanaDow,
  assemblePronosticoProjection,
  buildPronosticoProjectionContext,
  weekdayPromAt,
  projectionForPlant,
  loadPrecioDiario,
  appendPrecioWorksheet,
  canonicalForecastPlantKey,
  plantsEquivalent,
  scopeProvinciaGrid,
  resolveForecastExportPlant,
  igfLabelForForecastPlant,
  empresaMatchesForecastPlant,
  FMT_VENTA_TON_3,
};
