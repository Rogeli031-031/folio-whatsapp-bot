...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  hojaA(wb, ventaTonGrid, year, month, compTotalKg, lastDayStr, todayStr, {
    proyTonByPlant: proyTonByPlantFromIgf,
    ventaCanal,
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  for (const p of plants) {
    ...
    const promMesDescRatio = computePromMesByDow(descMap, {
...
function writeProvinciaCanalColumns(ws, opts) {
  ...
  const realDays = byDate.filter((d) => d.day < cutoffDay);
  byDate.forEach((d, i) => {
    const excelRow = i + 2;
    const isReal = d.day < cutoffDay;
    ...
    plants.forEach((p, pi) => {
      ...
      writeCanalPairCells(ws, excelRow, cCasa, pair, isReal, mode, fill);
    });
  });
...
function writeCanalPairCells(ws, excelRow, cCasa, pair, isReal, mode, fill) {
  const cCom = cCasa + 1;
  let casa;
  let com;
  let fmt;
...
function hojaA(wb, ventaTonGrid, year, month, compTotalKg, lastDayStr, todayStr, opts = {}) {
  ...
  for (const d of byDate) {
    ...
    row.push(omitTotProvincia ? null : tot);
    data.push(row);
  }
...
function hojaB(wb, descuentoGrid, ventaTonGrid, year, month, lastDayStr, todayStr, descuentoCanal) {
