...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  if (rawIgfRows.length > 0) {
    ...
    excelTheme.applyTitleStyle(ws, 1, 0, nCol - 1);
    excelTheme.applyHeaderRow(ws, 6, 0, nCol - 1);
    excelTheme.applyHeaderRow(ws, 7, 0, nCol - 1);
    excelTheme.applyHeaderRow(ws, row0Header + 1, 0, nCol - 1);
    ...
    excelTheme.applyBordersRange(ws, 5, 0, data.length - 1, nCol - 1);
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  // Estilos: encabezado de planta más grande/bold + lookback (amarillo suave) — ahora sí persisten en Excel.
  for (const b of styleBlocks) {
    for (const col of [0, COLK]) {
      const cell = ws.getCell(b.header1Row + 1, col + 1);
    ...
    for (let i = 0; i < b.height; i++) {
      ...
      if (Array.isArray(vm)) {
        for (let k = 0; k < 7; k++) {
          if (!vm[k]) continue;
          ws.getCell(b.tableStartRow + i + 1, 1 + k + 1).fill = { ...excelTheme.FILL_LOOKBACK };
        }
      }
      ...
      if (Array.isArray(dm)) {
        for (let k = 0; k < 7; k++) {
          if (!dm[k]) continue;
          ws.getCell(b.tableStartRow + i + 1, b.colOffsetRight + 1 + k + 1).fill = { ...excelTheme.FILL_LOOKBACK };
        }
      }
    }
  }
