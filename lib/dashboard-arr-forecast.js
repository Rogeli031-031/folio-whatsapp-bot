 ...
 */
function applyIgfMiniResumenFormulas(worksheet, row0Header, plantCount, nCols) {
  // Solo tiene sentido en la plantilla "Compromiso" (18 cols: A..R) porque usa M:P.
  if (nCols !== IGF_COMPROMISO_HEADER_ROW7.length) return;
  const headerExcelRow = row0Header + 1; // 1-based
  const firstPlantExcelRow = headerExcelRow + 1;
  const zonaExcelRow = firstPlantExcelRow + plantCount;
  const miniHeaderExcelRow = zonaExcelRow + 1; // A36 típico
  const miniFirstDataExcelRow = miniHeaderExcelRow + 1; // primera planta
  ...
  const miniZonaExcelRow = miniFirstDataExcelRow + plantCount;
...
async function appendHojaClientesDescuentoMes(client, wb, year, month, plantCode = null) {
  ...
  // Kg proyectado por cliente (regla 14 días × DOW)
  const ventaProyeccionMes = require("./venta-proyeccion-mes");
  const fechaHastaAuto = (() => {
    const today = new Date();
  ...
  const ventanaInicio = ventaProyeccionMes.addDaysStr(fechaHastaAuto, -13);
  const diasPorDow = ventaProyeccionMes.countDowInMonth(year, month);
  const projMapByKey = new Map(); // planta|cliente -> { kgProy }
  if (ventanaInicio) {
    const vq = await client.query(
      ...
      GROUP BY COALESCE(pm.prov_name, TRIM(v.plant_code)), v.cliente_norm, v.fecha::date
      `,
      plantFilter ? [ventanaInicio, fechaHastaAuto, plantFilter] : [ventanaInicio, fechaHastaAuto]
    );
    const sumByDowByKey = new Map(); // key -> [7]
    const ensure = (k) => {
      if (!sumByDowByKey.has(k)) sumByDowByKey.set(k, [0, 0, 0, 0, 0, 0, 0]);
      return sumByDowByKey.get(k);
    };
    for (const row of vq.rows || []) {
      const p = (row.planta || "").toString().trim();
      const c = (row.cliente || "").toString().trim();
      if (!p || !c) continue;
      const dt = row.fecha instanceof Date ? row.fecha : new Date(String(row.fecha).slice(0, 10));
      if (!(dt instanceof Date) || Number.isNaN(dt.getTime())) continue;
      const dow = dt.getDay();
      const key = `${p}|${c}`;
      ensure(key)[dow] += Number(row.kg || 0);
    }
    for (const [key, sumByDow] of sumByDowByKey) {
      const avgByDow = sumByDow.map((s) => s / 2);
      let kg = 0;
      for (let d = 0; d < 7; d++) kg += (avgByDow[d] || 0) * (diasPorDow[d] || 0);
      projMapByKey.set(key, { kgProy: Math.round(kg * 100) / 100 });
    }
  }
  ...
  const rows = (q.rows || []).map((r) => {
    ...
    const proj = projMapByKey.get(key) || { kgProy: null };
    return {
      planta,
      cliente,
  ...
  aoa.push([`Kg proyectado = regla 14 días × DOW (ventana: ${ventanaInicio || "(n/a)"} a ${fechaHastaAuto || "(n/a)"})`]);
  aoa.push([]);
  aoa.push(["Planta", "Cliente", "Categoría", "Subcategoría", "Kg mes (real)", "Kg mes (proyectado)", "Descuento MXN mes", "Descuento $/kg mes"]);
  ...
  for (const r of rows) {
    aoa.push([
      ...
      r.kgProy != null ? Math.round(Number(r.kgProy || 0) * 100) / 100 : "",
  ...
  excelTheme.applyTitleStyle(ws, 1, 0, 7);
  excelTheme.applyHeaderRow(ws, 5, 0, 7);
  const lastR = aoa.length - 1;
  excelTheme.applyBordersRange(ws, 4, 0, lastR, 7);
  excelTheme.setColumnWidths(ws, [22, 36, 16, 18, 14, 18, 18, 16]);
  for (let r0 = 5; r0 <= lastR; r0++) {
 ...
 */
function buildSemanaDowTableRows(year, month, byDateValue, isSum, corteDtForPending = null) {
  ...
  return { rows, height: rows.length, lookbackMaskRows, pendingMonthMaskRows };
}
