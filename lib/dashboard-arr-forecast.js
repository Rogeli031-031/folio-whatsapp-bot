  ...
  for (let i = 0; i < plantMetaList.length; i++) {
    ...
    if (igfR != null) {
      ...
      for (const c0 of IGF_RULE_OF_THREE_COL_INDICES) {
        ...
        excelTheme.setCellFormula(worksheet, r0, c0, `${L}$${igfR}*$${colB}$${igfR}/$${colB}${excelRow}`);
      }
    }
  }
...
function applyIgfResumenBlockDerivedFormulas(worksheet, row0Header, plantMetaList, nCols) {
  ...
  const setDerivedForPlantRow = (r0, igfR) => {
    const er = r0 + 1;
    if (igfR != null) {
      excelTheme.setCellFormula(worksheet, r0, 7, `=${H}$${igfR}*$${G}${er}/$${G}$${igfR}`);
    }
...
function applyIgfMiniResumenFormulas(worksheet, row0Header, plantCount, nCols) {
  ...
  for (let i = 0; i < plantCount; i++) {
    ...
    // A: Empresa (mismo texto)
    {
      const cell = worksheet.getCell(dstExcelRow, 1);
      cell.value = { formula: `A${srcExcelRow}`.replace(/^=/, "") };
      cell.fill = fillLabel;
    ...
    // H = SUMA(M:P)*B*1000 (operativos) tomando la fila fuente
    excelTheme.setCellFormula(worksheet, dstExcelRow - 1, 7, `=SUMA(M${srcExcelRow}:P${srcExcelRow})*B${dstExcelRow}*1000`);
    // I = SUMA(M:P)*B*1000 (corporativos) tomando la fila fuente
    excelTheme.setCellFormula(worksheet, dstExcelRow - 1, 8, `=SUMA(M${srcExcelRow}:P${srcExcelRow})*B${dstExcelRow}*1000`);
    // J = H+I
  ...
  // Bordes negros para todo el bloque (encabezado + plantas), A..L
  try {
    excelTheme.applyBordersRangeBlack(worksheet, miniHeaderExcelRow - 1, 0, miniFirstDataExcelRow - 1 + plantCount - 1, 11);
  } catch {
    /* ignore */
  }
}
...
async function appendHojaClientesDescuentoMes(client, wb, year, month, plantCode = null) {
  ...
  excelTheme.applyTitleStyle(ws, 1, 0, 4);
  excelTheme.applyHeaderRow(ws, 4, 0, 4);
  const lastR = aoa.length - 1;
  excelTheme.applyBordersRange(ws, 3, 0, lastR, 4);
  excelTheme.setColumnWidths(ws, [22, 36, 14, 18, 16]);
  for (let r0 = 4; r0 <= lastR; r0++) {
    const kg = ws.getCell(r0 + 1, 3).value;
    const mxn = ws.getCell(r0 + 1, 4).value;
    const dk = ws.getCell(r0 + 1, 5).value;
    if (typeof kg === "number") ws.getCell(r0 + 1, 3).numFmt = excelTheme.FMT_NUMBER;
    if (typeof mxn === "number") ws.getCell(r0 + 1, 4).numFmt = excelTheme.FMT_CURRENCY_MXN;
    if (typeof dk === "number") ws.getCell(r0 + 1, 5).numFmt = excelTheme.FMT_NUMBER;
  }
}
