...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month, opts = {}) {
  ...
  for (const p of plants) {
    ...
    styleBlocks.push({
      ...
      tableStartRow,
      height,
      ventaMask: ventaTable.lookbackMaskRows || [],
      descMask: descTable.lookbackMaskRows || [],
      colOffsetRight: COLK,
    });
  }
  ...
  const rgbGreyFill = "F2F2F2";
  const rgbGreyFont = "666666";
    ...
    for (let i = 0; i < b.height; i++) {
      ...
      if (Array.isArray(vm)) {
        for (let k = 0; k < 7; k++) {
          ...
          if (!ws[ref]) continue;
          ws[ref].s = { ...(ws[ref].s || {}), fill: { fgColor: { rgb: rgbGreyFill } }, font: { color: { rgb: rgbGreyFont } } };
        }
      }
