...
function buildSemanaDowTableRows(year, month, byDateValue, isSum) {
  ...
  const rows = [];
  rows.push(["Semana", ...DOW_HEADERS_EXCEL, "Total semana"]);
  ...
  for (let w = 0; w < numWeeks; w++) {
    ...
    rows.push([`Semana ${w + 1} (${startDay}–${endDay})`, ...cells, totalSemana]);
  }
...
async function appendHojaAcapulcoSemanaDow(client, wb, year, month) {
  ...
  for (const p of plants) {
    ...
    const header2 = [];
    header2[0] = "Filas: semanas del mes (días 1–7 = semana 1, 8–14 = semana 2, …). Columnas: Lun a Dom. Cada celda es el valor del día del mes que cae en esa columna.";
