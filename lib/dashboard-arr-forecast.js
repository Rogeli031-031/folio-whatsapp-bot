 ...
 * Fuente: arr.venta_toneladas_diarias_provincia.
 */
async function getVentaToneladasGrid(client, year, month) {
  ...
  if (plants.length === 0) return { plants: [], byDate: [], forecastByPlant: new Map() };
  ...
  const placeholders = plants.map((_, i) => `$${i + 1}`).join(",");
  const forecastR = await client.query(
    `SELECT plant_code, COALESCE(SUM(kg_forecast), 0) AS kg_forecast
     FROM arr.forecast_mensual WHERE plant_code IN (${placeholders}) AND year = $${plants.length + 1} AND month = $${plants.length + 2}
     GROUP BY plant_code`,
    [...plants, year, month]
  );
  const forecastByPlant = new Map((forecastR.rows || []).map((row) => [row.plant_code, Number(row.kg_forecast || 0) / 1000]));

  // Rellenar días desde hoy con proyección (mes actual): real = inicio mes hasta ayer, proyección = hoy hasta fin de mes (corte por fecha del servidor).
  const today = new Date();
  ...
  return { plants, byDate, forecastByPlant };
}

/**
 * Grid descuento por kilo: plantas en columnas, días en filas.
 * Fuente: arr.descuento_por_kilo_diario_provincia; días desde hoy se rellenan con desc_kg_forecast del mes.
 */
async function getDescuentoPorKiloGrid(client, year, month) {
  ...
  const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
  const plants = await getProvinciaPlantsOrdered(client);
  if (plants.length === 0) return { plants: [], byDate: [] };
  ...
  // Rellenar días desde hoy con proyección (desc_kg_forecast por planta); corte por fecha del servidor.
  const placeholders = plants.map((_, i) => `$${i + 1}`).join(",");
  const forecastR = await client.query(
    `SELECT plant_code, COALESCE(SUM(desc_forecast), 0) AS desc_forecast, COALESCE(SUM(kg_forecast), 0) AS kg_forecast
     FROM arr.forecast_mensual WHERE plant_code IN (${placeholders}) AND year = $${plants.length + 1} AND month = $${plants.length + 2}
     GROUP BY plant_code`,
    [...plants, year, month]
  );
...
function hojaA(wb, ventaTonGrid, year, month, compTotalKg, lastDayStr, todayStr) {
  const { plants, byDate, forecastByPlant } = ventaTonGrid;
...
function hojaB(wb, descuentoGrid, ventaTonGrid, year, month, lastDayStr, todayStr) {
  const { plants, byDate } = descuentoGrid;
  ...
  // ACUM ponderado por venta diaria: SUM(desc_kg * venta_ton) / SUM(venta_ton)
  // (equivalente a usar kg, porque el factor 1000 cancela).
