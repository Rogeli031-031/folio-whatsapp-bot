...
async function generarDashboardArrForecast(client, year, month, plantCode = null, options = {}) {
  ...
  if (rawIgfRows.length > 0) {
    ...
    excelTheme.writeAoa(ws, data);
    /** Plantilla 20 col.: columna G «Folios carro» = (Margen − Folios Aprob + Com/Desc) × Venta(ton) × 1000 → importe MXN. */
    const IGF_FORECAST_MAIN_DATA_START_ROW0 = 8;
    const colFoliosCarro0 = 6;
    for (let i = 0; i < forecastDataRows.length; i++) {
      const r0 = IGF_FORECAST_MAIN_DATA_START_ROW0 + i;
      const excelRow = r0 + 1;
      excelTheme.setCellFormula(
        ws,
        r0,
        colFoliosCarro0,
        `IFERROR((C${excelRow}-F${excelRow}+D${excelRow})*B${excelRow}*1000,"")`
      );
    }
    const plantMetaList = IGF_PROVINCIA_RESUMEN_PLANTAS.map((label) => ({
      ...pronosticoProyRowsForIgfLabel(
...
async function getPronosticoPlantDetail(client, year, month, plantCode, fechaCorte) {
  ...
  const proyMap = await computePronosticoProyByPlant(client, year, month, { fechaCorte });
