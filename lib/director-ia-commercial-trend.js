...
function requestedMoverTipo(raw) {
  const q = normalizeQuestion(raw);
  if (isLargestSalesLossQuestion(raw)) return null;
...
function isLargestSalesLossQuestion(raw) {
  const q = normalizeQuestion(raw);
  if (!q) return false;
  ...
  if (/\b(igf|folio|folios|taller|descuento|margen)\b/.test(q)) return false;
  if (/\bdisminuy/.test(q)) return false;
  if (/\bdejaron\s+de\s+comprar\b/.test(q) || /\bdejo\s+de\s+comprar\b/.test(q)) return false;
  if (/\baumentaron\b/.test(q) || (/\baument/.test(q) && /\bclientes?\b/.test(q))) return false;
  ...
  if (!listCue) return false;

  const lossToken = /\b(perdida|perdidas|perdio|caida|caidas)\b/.test(q);
  const rankToken = /\b(mayor|mas|peor)\b/.test(q);
  if (!lossToken || !rankToken) return false;

  const namedSale = /\bventas?\b/.test(q) || /\bcompras?\b/.test(q);
  if (namedSale) return true;
  return hasClient && /\b(perdida|perdidas)\b/.test(q);
}
...
function isCommercialMoversQuestion(raw) {
  ...
  if (isLargestSalesLossQuestion(raw)) return true;
  return false;
}
...
function selectLargestSalesLoss(movers) {
  const candidates = [];
  for (const m of movers || []) {
    const delta = Number(m && m.delta_kg);
    if (!Number.isFinite(delta) || delta >= 0) continue;
    const tipo = String((m && m.tipo) || "");
    if (tipo === "aumento" || tipo === "nuevo" || tipo === "sin_cambio") continue;
    if (tipo !== "disminucion" && tipo !== "perdido") continue;
    candidates.push(m);
  }
  if (!candidates.length) {
    return {
      ...
      worst_delta: null,
      tie: false,
      rank_rule: "raw_delta_ascending",
    };
  }
  let worst = Number(candidates[0].delta_kg);
  for (const m of candidates) {
    const delta = Number(m.delta_kg);
    if (delta < worst) worst = delta;
  }
  const winners = candidates
    .filter((m) => Number(m.delta_kg) === worst)
    .slice()
...
function calendarMonthPlain(spec) {
  return MESES_ES[(Number(spec && spec.month) || 1) - 1] || "";
}

function calendarMonthTitle(spec) {
  const name = calendarMonthPlain(spec);
  return name ? name.charAt(0).toUpperCase() + name.slice(1) : "";
}

function formatLargestSalesLossClientBlock(m, monthA, monthB) {
  const lines = [
    String(m.cliente || "").trim(),
    `${calendarMonthTitle(monthA)}: ${formatKg(m.kg_a)} kg`,
    `${calendarMonthTitle(monthB)}: ${formatKg(m.kg_b)} kg`,
    `Pérdida: ${formatKg(Number(m.delta_kg) < 0 ? -Number(m.delta_kg) : 0)} kg`,
    "",
  ];
  if (m.tipo === "perdido") {
    lines.push(`Además, dejó de comprar en ${calendarMonthPlain(monthB)}.`);
  } else if (m.tipo === "disminucion") {
    lines.push("Clasificación: disminuyó.");
  }
  return lines.join("\n");
}

function buildLargestSalesLossAnswer(assembled, selection) {
  const a = assembled.month_a;
  const b = assembled.month_b;
  const header = `Mayor pérdida de venta entre ${calendarMonthPlain(a)} ${a.year} y ${calendarMonthPlain(b)} ${b.year}:`;
  if (!selection || !selection.winners || !selection.winners.length) {
    return `${header}\n\n(sin clientes con pérdida)`;
  }
  if (selection.tie) {
    const lines = [
      `Empate en la mayor pérdida de venta entre ${calendarMonthPlain(a)} ${a.year} y ${calendarMonthPlain(b)} ${b.year}:`,
      "",
    ];
    for (const w of selection.winners) {
      lines.push(formatLargestSalesLossClientBlock(w, a, b));
      lines.push("");
    }
    return lines.join("\n").trim();
  }
  return [header, "", formatLargestSalesLossClientBlock(selection.winners[0], a, b)].join("\n").trim();
}
...
async function loadCalendarCompare(db, plant, slots, opts) {
  ...
  const lossQuestion = isLargestSalesLossQuestion(opts.question);
  const focused = requested ? movers.filter((m) => m.tipo === requested) : movers;
  const loss_ranking = lossQuestion ? selectLargestSalesLoss(movers) : null;
  const assembled = {
    ok: true,
    semantic_class: SEMANTIC_CLASS,
    period_kind: "calendar_compare",
    ...
    partial: false,
    assembly_status: "ok",
    first_mover: lossQuestion
      ? (loss_ranking && !loss_ranking.tie && loss_ranking.winners[0]) || null
      : focused[0] || null,
  };
  assembled.deterministic_answer = lossQuestion
    ? buildLargestSalesLossAnswer(assembled, loss_ranking)
    : buildCalendarCompareAnswer(assembled, opts.question);
