...
function buildClientRankingAnswer(payload) {
  ...
  if (payload.data_semantics === "LAST_SAFE_CUT" && ranked.length) {
    ...
    return [
      `NO_ROWS_OBSERVED para ${asked} en arr.ventas_diarias_cliente por cliente.`,
      `CLIENT_FORECAST_UNAVAILABLE: el ARR visible puede mostrar proyección de planta, no un ranking contractual por cliente.`,
      `LAST_SAFE_CUT ${spec.period} — ranking observado (no es cierre de ${asked}):`,
      "",
      ...ranked.map((row, i) => `${i + 1}. ${row.cliente} — ${formatTon(row.venta_ton)}`),
    ].join("\n");
  }
  if (!ranked.length) {
    const open = isOpenCalendarMonth(spec.period, payload.now);
    return open
      ? `NO_ROWS_OBSERVED: no hay venta observada por cliente cargada para ${spec.period} en arr.ventas_diarias_cliente. FORECAST_AVAILABLE puede existir a nivel planta en el ARR, pero CLIENT_FORECAST_UNAVAILABLE: no distribuyo el forecast total como ranking por cliente. Puedo mostrarte el último ranking observado si hay un corte seguro.`
      : `No encontré clientes con venta observada para ${spec.customer_segment} en ${spec.plant_label || "la planta"} (${spec.period || "periodo no determinado"}).`;
  }
  const open = isOpenCalendarMonth(spec.period, payload.now);
  const observedTag = open ? "OBSERVED_PARTIAL" : "OBSERVED_CLOSED";
  const cutNote = open
    ? `compra observada en ${spec.period} al corte (mes abierto; no es cierre)`
    : `observado de ${spec.period}`;
    ...
    spec.metric === "DISCOUNT"
      ...
      : spec.customer_segment === "ALL"
        ...
        : spec.customer_segment === "CASA"
          ? "Venta Casa"
          : "Venta Comisionista"
  } — ${spec.plant_label || "planta"} — ${cutNote} — ${observedTag}`;
  const factor = Number(payload.uniform_projection_factor);
  const uniformNote =
    open && Number.isFinite(factor) && factor > 0
      ? "El orden del ranking observado no cambia bajo la proyección uniforme de planta (kg × factor). PROJECTED_ESTIMATE aparte; no es forecast por cliente."
      : null;
  const lines = ranked.map((row, i) => {
    if (spec.metric === "DISCOUNT") {
      return `${i + 1}. ${row.cliente} — descuento observado ${Number(row.monto).toLocaleString("es-MX")}`;
    }
    const projected =
      uniformNote && Number.isFinite(Number(row.venta_ton))
        ? ` | proyección estimada ${formatTon(Math.round(row.venta_ton * factor * 10) / 10)}`
        : "";
    return `${i + 1}. ${row.cliente} — ${formatTon(row.venta_ton)}${projected}`;
  });
  return [title, uniformNote, "", ...lines].filter((line) => line !== null).join("\n");
}
...
async function loadClientRankingForChat(pool, plantaId, req, opts = {}) {
  ...
  if (!ranked.length && isOpenCalendarMonth(spec.period, now) && spec.metric !== "DISCOUNT") {
    const lastCut = previousYearMonth(spec.period);
    let fallbackRows = Array.isArray(opts.lastSafeCutSalesRows) ? opts.lastSafeCutSalesRows : null;
    if (!fallbackRows && typeof (opts.queryMonthlySales || queryMonthlySales) === "function" && !Array.isArray(opts.salesRows)) {
      const bounds = periodBounds(lastCut);
      const resolveCodes = opts.resolvePlantCodes || resolvePlantCodes;
      let codes = opts.plantCodes;
      if (!codes) {
      ...
      if (bounds && codes && codes.length) {
        const qSales = opts.queryMonthlySales || queryMonthlySales;
        const result = await qSales(opts.db || pool, codes, bounds.startStr, bounds.endStr, canalFilter);
        fallbackRows = (result && result.rows) || result || [];
      }
    }
    if (fallbackRows && fallbackRows.length) {
      const fallbackRanked = rankClients(fallbackRows, spec);
      if (fallbackRanked.length) {
        ranked = fallbackRanked;
        spec = { ...spec, period: lastCut };
