...
function hasClientSalesCue(n) {
  return Boolean(
    /\bclientes?\b/.test(n) ||
      ...
      /\bcomercial\b/.test(n) ||
      (/\bventa\b/.test(n) && (/\branking\b/.test(n) || /\btop\b/.test(n) || /\bmayor\b/.test(n))) ||
...
function buildClientRankingAnswer(payload) {
  ...
  if (!ranked.length) {
    ...
    return open
      ? `No hay venta observada en arr.ventas_diarias_cliente para ${spec.period} en ${spec.plant_label || "la planta"}. El ARR de UI de mes abierto puede mostrar proyección; no la presento como ranking observado.`
  ...
  const observedTag = isOpenCalendarMonth(spec.period, payload.now)
    ? "observado parcial (no forecast)"
    : "observado";
    ...
    spec.metric === "DISCOUNT"
      ...
      : spec.customer_segment === "ALL"
        ...
        : spec.customer_segment === "CASA"
          ? "Venta Casa"
          : "Venta Comisionista"
  } — ${spec.plant_label || "planta"} — ${spec.period} — ${observedTag}`;
