...
function buildClientRankingAnswer(payload) {
  ...
  if (payload.data_semantics === "LAST_SAFE_CUT" && ranked.length) {
    ...
    const lines = ranked.map((row, i) =>
      spec.metric === "DISCOUNT"
        ? `${i + 1}. ${row.cliente} — $${Number(row.monto).toLocaleString("es-MX")}`
  ...
  if (!ranked.length) {
    if (aggregateDiscount) {
      return spec.period_kind === "RANGE"
        ? `No tengo descuentos observados por cliente cargados ${formatPeriodHuman(spec)}.`
        : `No tengo descuentos observados por cliente cargados para ${formatMonthLabel(spec.period)}.`;
    }
  ...
  if (aggregateDiscount && spec.period_kind === "RANGE") {
    title =
      ranked.length === 1
        ? `${formatPeriodHuman(spec).replace(/^de /, "De ")}, el cliente con ${discountDir} descuento total observado (MXN, no $/kg) es:`
        : `Clientes con ${discountDir} descuento total observado (MXN, no $/kg) ${formatPeriodHuman(spec)}:`;
  } else if (aggregateDiscount) {
    title = `En ${formatMonthLabel(spec.period)}, los clientes con ${discountDir} descuento total observado (MXN, no $/kg)${open ? " — mes aún abierto, no es cierre" : ""}:`;
  } else {
...
function encodeRankingPrior(spec, ranked) {
  ...
  return {
    ...
    discount_metric: spec.discount_metric || (spec.metric === "DISCOUNT" ? "DISCOUNT_TOTAL_MXN" : null),
...
function priorRankingFromState(state) {
  ...
  return {
    ...
    discount_metric: ent.discount_metric || (ent.metric === "DISCOUNT" ? "DISCOUNT_TOTAL_MXN" : null),
    ranked_names: names,
  };
}
