...
function isClientRankingQuestion(question) {
  ...
  if (require("./director-ia-predictive-commercial").isPredictiveCommercialQuestion(question)) return false;
...
function isPreviousMonthShift(n) {
  return Boolean(
    (/\bmes\s+anterior\b/.test(n) ||
      /\bmes\s+previo\b/.test(n) ||
      /\bmes\s+pasado\b/.test(n) ||
      /\bun\s+mes\s+antes\b/.test(n) ||
      /\bmes\s+antes\b/.test(n)) &&
      !/\bcontra\b/.test(n)
  );
}
...
function isClientRankingFollowUp(question) {
  ...
  return Boolean(
    /\bcuanto\s+compro\s+el\s+(primero|segundo|tercero)\b/.test(n) ||
      ...
      /\bcompar/.test(n) && !/\bcompara\s+mentalmente\b/.test(n) ||
      ...
      /\btop\b/.test(n) ||
      /\branking\b/.test(n) ||
      /\bcompara\b/.test(n) && !/\bcompara\s+mentalmente\b/.test(n) ||
...
function extractLimit(n, prior) {
  const digit =
    n.match(/\btop\s+(\d{1,2})\b/) ||
    n.match(/\blos\s+(\d{1,2})\s+clientes\b/) ||
    n.match(/\b(\d{1,2})\s+clientes\b/) ||
    n.match(/\blos\s+(\d{1,2})\b/);
  if (digit) {
...
function extractDirection(n, prior) {
  if (
    /\bbottom\b/.test(n) ||
    /\bpeores\b/.test(n) ||
    /\bmenos\s+descuento\b/.test(n) ||
    (/\bmenor(?:es)?\b/.test(n) && /\bdescuento/.test(n) && !/\bmayor(?:es)?\b/.test(n)) ||
    (/\bmenos\b/.test(n) && !/\bdescuento/.test(n))
  ) {
    return "BOTTOM";
  }
...
function extractRankingPeriod(n, now, prior, selectedPeriod) {
  ...
  if (isPreviousMonthShift(n)) {
    const base =
      (prior && prior.period && /^\d{4}-\d{2}$/.test(prior.period) && prior.period) ||
      (selectedPeriod && /^\d{4}-\d{2}$/.test(selectedPeriod) && selectedPeriod) ||
      `${resolveNow(now).year}-${pad2(resolveNow(now).month)}`;
    const prev = previousYearMonth(base);
    if (prev) {
      return { ok: true, period_kind: "SINGLE", period: prev, period_start: null, period_end: null };
    }
  }
...
function extractClientRankingSpec(question, prior, opts = {}) {
  ...
  return {
    ...
    wants_prior_month: isPreviousMonthShift(n),
    discount_metric:
...
function rankClients(rows, spec) {
  const map = new Map();
  const segment = spec && spec.customer_segment;
  const canalFilter = canalFilterForSegment(segment);
  for (const row of rows || []) {
    if (canalFilter !== "ambos" && !rowMatchesCanal((row && (row.canal || row.categoria)) || "Casa", canalFilter)) {
      continue;
    }
    const name = String((row && (row.cliente_norm || row.cliente)) || "").trim();
    if (!name) continue;
    ...
    const prev = map.get(name) || { cliente: name, kg: 0, canal: row.canal || row.categoria || null };
    prev.kg += kg;
    if (!prev.canal) prev.canal = row.canal || row.categoria || null;
    map.set(name, prev);
  }
...
function buildClientRankingAnswer(payload) {
  ...
  if (aggregateDiscount && spec.period_kind === "RANGE") {
    ...
    if (spec.customer_segment === "COMISIONISTA" || spec.customer_segment === "CASA") {
      const channelLabel = spec.customer_segment === "COMISIONISTA" ? "Comisionista" : "Casa";
      title = `Top ${ranked.length} ${channelLabel} — ${formatMonthLabel(spec.period)}${open ? " (mes aún abierto; no es el cierre)" : ""}`;
    } else {
