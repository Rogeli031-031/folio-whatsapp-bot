...
const { getPlantaIdsEquivalentes, loadHistorialBatch } = require("./dicf-acciones");
...
const DEFAULT_DICF_DETAILS_LIMIT = 40;
...
function formatHistorialTimestamp(value) {
  if (value instanceof Date) return value.toISOString();
  return String(value || "");
}

/**
 * Contexto DICF para Director IA: acciones abiertas y cerradas + historial + resultado de cierre.
 * @param {import("pg").PoolClient} client
 * @param {number} planta_id
 * @param {{ limit?: number }} [opts]
 */
async function summarizeDicfContext(client, planta_id, opts = {}) {
  const limit =
    opts.limit != null && Number.isFinite(opts.limit) && opts.limit > 0
      ? Math.floor(opts.limit)
      : DEFAULT_DICF_DETAILS_LIMIT;
  const equiv = getPlantaIdsEquivalentes(planta_id);
  if (!equiv.length) return [];

  const r = await client.query(
    `SELECT a.id, a.public_code, a.planta_label, a.cliente_nombre, a.descripcion, a.estado,
     ...
     ORDER BY a.cerrado_at DESC NULLS LAST, a.created_at DESC`,
    [equiv]
  );
  ...
  const histMap = await loadHistorialBatch(
    client,
    rows.map((row) => row.id)
  );
  ...
  return rows.slice(0, limit).map((row) => {
    ...
    const cerrada = !!row.cerrado_at || estado === "hecho";
    return {
      dicf_id: Number(row.id),
      public_code: String(row.public_code || ""),
      cliente_nombre: String(row.cliente_nombre || "").trim(),
      descripcion: String(row.descripcion || "").trim(),
      planta_label: String(row.planta_label || "").trim() || null,
      estado,
      cerrada,
      fecha_compromiso: pgCalendarDateToYmd(row.fecha_compromiso) || null,
      resultado_cierre: String(row.resultado_cierre || "").trim() || null,
      responsable: String(row.responsable || "").trim() || null,
      cerrado_at: row.cerrado_at ? formatHistorialTimestamp(row.cerrado_at) : null,
      historial: histMap.get(Number(row.id)) || [],
    };
  });
}
