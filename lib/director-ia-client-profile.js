...
async function loadClientProfileForChat(pool, plantaId, req, opts = {}) {
  ...
  try {
    ...
    if (hintTakesPrecedence) {
      ...
      const resolved = resolveExactRankedByHints(ranked, hintCandidatesEarly);
      ...
      const ranked = rankClientsByKg(salesRows, new Set(months.map((m) => m.yyyymm)));
      const resolved = resolveExactRankedByHints(ranked, hintCandidatesEarly.length ? hintCandidatesEarly : [opts.entity_hint]);
      if (resolved.status === "ambiguous") {
        clarification = {
          status: "ambiguous",
          hint: resolved.hint,
