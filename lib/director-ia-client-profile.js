...
const NAME_INTERNAL_CONNECTORS = new Set(["de", "del", "el", "la", "los", "las"]);

const EMBED_SENTENCE_STOP = new Set([
  "desde",
  "hasta",
  "fecha",
  "planta",
...
function isEmbeddedNameHead(tok, n) {
  if (!tok || !/^[A-ZÁÉÍÓÚÑ]/.test(tok)) return false;
  if (EMBED_SENTENCE_STOP.has(n)) return false;
  if (NAME_INTERNAL_CONNECTORS.has(n)) return false;
  return true;
}

function collectEmbeddedNameTokens(rest) {
  const tokens = [];
  ...
  while (i < parts.length) {
    ...
    const n = embedTokenNorm(tok);
    if (!tok || EMBED_SENTENCE_STOP.has(n)) break;
    if (NAME_INTERNAL_CONNECTORS.has(n)) {
      if (!tokens.length) break;
      let j = i;
      const connectors = [];
      while (j < parts.length) {
        const ct = parts[j].replace(/[?¿!.]+$/g, "");
        const cn = embedTokenNorm(ct);
        if (!NAME_INTERNAL_CONNECTORS.has(cn) || EMBED_SENTENCE_STOP.has(cn)) break;
        connectors.push(ct);
        j += 1;
      }
      if (j >= parts.length) break;
      const nextTok = parts[j].replace(/[?¿!.]+$/g, "");
      const nextN = embedTokenNorm(nextTok);
      if (!isEmbeddedNameHead(nextTok, nextN) && !(tokens.length > 0 && nextN === "y" && /^[A-ZÁÉÍÓÚÑ]/.test(nextTok))) {
        break;
      }
      tokens.push(...connectors);
      i = j;
      continue;
    }
    if (!isEmbeddedNameHead(tok, n) && !(tokens.length === 0 && n === "y" && /^[A-ZÁÉÍÓÚÑ]/.test(tok))) {
      break;
    }
    tokens.push(tok);
    i += 1;
  }
  while (tokens.length && NAME_INTERNAL_CONNECTORS.has(embedTokenNorm(tokens[tokens.length - 1]))) {
    tokens.pop();
  }
  return tokens;
}

function extractEmbeddedClientHintCandidates(question) {
  const raw = String(question || "").trim();
  if (!raw) return null;
  const spans = [];
  const seen = new Set();
  const re = /\b(?:cliente|de)\s+/gi;
  let m;
  while ((m = re.exec(raw))) {
    const tokens = collectEmbeddedNameTokens(raw.slice(m.index + m[0].length));
    if (!tokens.length) continue;
    const span = tokens.join(" ");
    const key = exactNorm(span);
  ...
  return {
    spans,
    longest,
    name_token_count: longest.split(/\s+/).length,
    requires_canonical_evidence: true,
  };
}

function isExactKnownPlantNombre(span) {
  const want = exactNorm(span);
  if (!want) return false;
  const names = [];
  if (Array.isArray(PLANTS_PROVINCIA)) names.push(...PLANTS_PROVINCIA);
  if (ALIAS_PLANTA_NOMBRE && typeof ALIAS_PLANTA_NOMBRE === "object") {
    names.push(...Object.keys(ALIAS_PLANTA_NOMBRE), ...Object.values(ALIAS_PLANTA_NOMBRE));
  }
  return names.some((n) => exactNorm(n) === want);
}

function hasEmbeddedClientSpan(raw) {
  const parsed = extractEmbeddedClientHintCandidates(raw);
  return Boolean(parsed && parsed.longest);
}

function hasDefensibleEmbeddedClientSpan(raw) {
  const parsed = extractEmbeddedClientHintCandidates(raw);
  if (!parsed || !parsed.longest) return false;
  if (isExactKnownPlantNombre(parsed.longest)) return false;
  return true;
}

function namesPurchaseOrKgMetric(q) {
  return /\bcompr/.test(q) || /\bkg\b/.test(q);
}
...
function isClientProfileQuestion(raw, opts = {}) {
  ...
  if (namesPurchaseOrKgMetric(q) && hasDefensibleEmbeddedClientSpan(raw)) {
    return true;
  }
...
function uniqueClientHintList(hints) {
  const out = [];
  const seen = new Set();
...
function dropStrictPrefixHints(hints) {
  const list = uniqueClientHintList(hints);
  return list.filter((hint) => {
    const key = exactNorm(hint);
    return !list.some((other) => {
      if (other === hint) return false;
      const o = exactNorm(other);
      return o.startsWith(`${key} `) || o.endsWith(` ${key}`);
    });
  });
}

function resolveExactRankedByHints(ranked, hints) {
  const ordered = uniqueClientHintList(hints).sort((a, b) => b.length - a.length);
  for (const hint of ordered) {
    const want = exactNorm(hint);
    const hits = (ranked || []).filter((c) => exactNorm(c.cliente_norm) === want);
    if (hits.length > 1) return { status: "ambiguous", hint, hits };
    if (hits.length === 1) return { status: "unique", hint, hit: hits[0] };
  }
  return { status: "none" };
}

function identityFromRankedHit(plant, hit) {
  return {
...
async function loadClientProfileForChat(pool, plantaId, req, opts = {}) {
  ...
  const hintCandidatesEarly = uniqueClientHintList([].concat(opts.entity_hint_candidates || [], currentHintEarly));
  const requiresCanonicalY = Boolean(opts.leading_y_requires_canonical);
  const requiresCanonicalEmbedded = Boolean(opts.embedded_client_requires_canonical);
  const requiresCanonical = requiresCanonicalY || requiresCanonicalEmbedded;
  const hintTakesPrecedence = hintCandidatesEarly.some((hint) =>
    explicitClientHintTakesPrecedence(hint, inheritedNormEarly)
  );
  ...
  try {
    ...
    const hintsForExact = requiresCanonical
      ? requiresCanonicalEmbedded
        ? dropStrictPrefixHints(hintCandidatesEarly)
        : hintCandidatesEarly
      : uniqueClientHintList([currentHintEarly || opts.entity_hint]);

    if (hintTakesPrecedence) {
      const ranked = rankClientsByKg(salesRows, new Set(months.map((m) => m.yyyymm)));
      const resolved = resolveExactRankedByHints(ranked, hintsForExact);
      if (resolved.status === "ambiguous") {
        clarification = {
          status: "ambiguous",
          hint: resolved.hint,
          candidates: resolved.hits.map((h) => h.cliente_norm),
        };
      } else if (resolved.status === "unique") {
        identity = identityFromRankedHit(plant, resolved.hit);
      } else if (requiresCanonical) {
        clarification = {
          status: "not_found",
          hint: (requiresCanonicalEmbedded ? dropStrictPrefixHints(hintCandidatesEarly)[0] : hintCandidatesEarly[0])
            || currentHintEarly,
        };
      } else {
        identity = {
    ...
    } else if (opts.entity_hint || hintCandidatesEarly.length) {
      const ranked = rankClientsByKg(salesRows, new Set(months.map((m) => m.yyyymm)));
      const resolved = resolveExactRankedByHints(
        ranked,
        hintsForExact.length ? hintsForExact : uniqueClientHintList([opts.entity_hint])
      );
      if (resolved.status === "ambiguous") {
        clarification = {
          status: "ambiguous",
        ...
        clarification = {
          status: "not_found",
          hint:
            (requiresCanonicalEmbedded ? dropStrictPrefixHints(hintCandidatesEarly)[0] : hintCandidatesEarly[0]) ||
            opts.entity_hint,
        };
