...
const EMBED_STOP = new Set([...NAME_STOP, "desde", "hasta", "fecha", "planta", "cada", "por", "and"]);

function collectEmbeddedNameTokens(rest) {
  const tokens = [];
  for (const part of String(rest || "")
    .trim()
    .split(/\s+/)
    .filter(Boolean)) {
    const tok = part.replace(/[?¿!.]+$/g, "");
    if (!tok || !/^[A-ZÁÉÍÓÚÑ]/.test(tok)) break;
    const n = tok
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "")
      .toLowerCase();
    if (EMBED_STOP.has(n) && !(tokens.length === 0 && n === "y")) break;
    tokens.push(tok);
  }
  return tokens;
}
...
function isClientProfileQuestion(raw, opts = {}) {
  ...
  if (namesPurchaseOrKgMetric(q) && hasEmbeddedClientSpan(raw)) {
    return true;
  }
...
function dropStrictPrefixHints(hints) {
  ...
  return list.filter((hint) => {
    ...
    return !list.some((other) => other !== hint && exactNorm(other).startsWith(`${key} `));
  });
}
