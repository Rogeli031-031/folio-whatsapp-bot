...
async function loadClientProfileForChat(pool, plantaId, req, opts = {}) {
  ...
  try {
    ...
    const hintsForExact = requiresCanonicalY
      ? hintCandidatesEarly
    ...
    if (hintTakesPrecedence) {
      ...
      } else if (requiresCanonicalY) {
        clarification = {
          status: "not_found",
          hint: hintCandidatesEarly[0] || currentHintEarly,
        };
      } else {
        identity = {
      ...
      if (resolved.status === "ambiguous") {
        ...
        clarification = {
          status: "not_found",
          hint: hintCandidatesEarly[0] || opts.entity_hint,
        };
