...
const CLIENT_PROFILE_SYSTEM_ADDENDUM = [
  ...
  "Prohibido verbalizar «no se han registrado acciones» de forma global. DICF vacío = ninguna para estas keys. Action Register no se consultó.",
...
function formatClientProfileContext(assembled) {
  ...
  if (!(assembled.dicf_actions || []).length) {
    lines.push(
      "  (NO_ENCONTRADA_EN_ESTA_RUTA para estas cliente_key. Action Register no consultado. Prohibido afirmar que no existen acciones.)"
    );
  }
