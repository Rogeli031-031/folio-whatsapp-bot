 ...
 * Runtime alinea meses. Sin persistencia. Sin HTTP. Sin join por nombre.
...
const CLIENT_PROFILE_SYSTEM_ADDENDUM = [
  ...
  "missing != 0. Ausencia de comments/DICF no es prueba de que no existan fuera de estas keys.",
...
function parseExplicitMonths(raw, now) {
  ...
  for (const [name, num] of Object.entries(MONTH_NAME_TO_NUM)) {
    if (new RegExp(`\\b${name}\\b`).test(q)) found.push(num);
  }
  ...
  return uniq
    ...
    .map((month) => {
      ...
      return {
        ...
        completeness:
          year === today.year && month === today.month ? "PARTIAL" : "COMPLETE",
      };
    });
}
...
function resolveClientProfileSlots(question, inherited = {}, now) {
  const explicit = parseExplicitMonths(question, now);
  const inheritedMonths = sanitizePeriodMonths(inherited.active_period_months);
  const months = explicit && explicit.length ? explicit : inheritedMonths.length ? inheritedMonths : defaultThreeMonths(now);
...
async function queryMonthlyDiscount(client, codesUpper, startStr, endStr, canalFilter) {
  ...
  return client.query(
    `SELECT to_char(d.fecha::date, 'YYYY-MM') AS month,
            ...
            TRIM(COALESCE(cat.canal, d.canal, 'Casa')) AS canal,
            TRIM(COALESCE(cat.subcanal, d.subcanal, '')) AS subcanal,
...
function assembleClientProfilePack(input) {
  ...
  else if (!(input.comments || []).length) limitations.push("comments_none_for_cliente_key");
  if (!input.actionsQueried) limitations.push("dicf_actions_not_queried");
  else if (!(input.actions || []).length) limitations.push("dicf_actions_none_for_cliente_key");
  ...
  return {
    ...
    provenance: {
      ...
      comments: "arr.cliente_comentarios by cliente_key",
...
async function loadClientProfileForChat(pool, plantaId, req, opts = {}) {
  ...
  try {
    ...
    const first = slots.months[0];
    const last = slots.months[slots.months.length - 1];
    const windowStart = monthStartEnd(first.year, first.month).start;
    const windowEnd = monthStartEnd(last.year, last.month).end;
    ...
    const inheritedKeys = []
      ...
      .map((k) => String(k).trim())
      .filter(Boolean);
    const inheritedNorm = opts.cliente_norm ? String(opts.cliente_norm).trim() : "";
    const inheritedCanal = opts.identity_canal || canalLabelFromFilter(slots.channel);
    const inheritedSub = opts.identity_subcanal != null ? String(opts.identity_subcanal) : "";
    ...
    if (clarification) {
      return {
        ...
        clarification,
        plant,
        period: { months: months.map((m) => m.yyyymm) },
        semantic_class: SEMANTIC_CLASS,
      };
    }
    ...
    const comments = await qComments(db, plant.planta_id, identity.cliente_keys);
...
function formatClientProfileContext(assembled) {
  ...
  const lines = [
    ...
    "join=cliente_key (sin join por nombre)",
  ...
  lines.push("");
  lines.push("=== COMMENTS (cliente_key) ===");
  if (!(assembled.comments || []).length) lines.push("  (ninguno para estas keys; missing != inexistencia global)");
  for (const c of (assembled.comments || []).slice(0, 8)) {
    lines.push(`  ${c.created_at || ""} | ${c.author_name || "—"} | ${String(c.body || "").slice(0, 240)}`);
  }
  lines.push("");
  lines.push("=== DICF ACCIONES (cliente_key) ===");
  if (!(assembled.dicf_actions || []).length) lines.push("  (ninguna para estas keys)");
