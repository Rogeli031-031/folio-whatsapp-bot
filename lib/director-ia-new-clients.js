...
const IGF_MARGIN_SOURCE = "igf.compromiso_lines";
const CLOSED_MONTH_SOURCES = Object.freeze([
  "arr.ventas_diarias_cliente",
  "arr.descuentos_diarios_cliente",
  IGF_MARGIN_SOURCE,
]);

/** Reproduce el truco Number(null)===0 del código anterior. No usar para clasificar. */
function legacyCoerceMargin(raw) {
  return Number.isFinite(Number(raw)) ? Number(raw) : 1;
}

function isDefendableHistoricalMargin(raw) {
  if (raw == null) return false;
  const n = typeof raw === "number" ? raw : Number(raw);
  return Number.isFinite(n) && n > 0;
}

function missingMarginPayload(base, factsConsulted) {
  return {
    ...base,
    ok: false,
    code: DIRECTOR_IA_VERACITY.DATA_NOT_FOUND,
    status: 404,
    error: "No hay margen histórico defendible para completar la clasificación de clientes nuevos.",
    period_kind: "closed_month",
    presented_as_closed_actual: false,
    facts_consulted: Boolean(factsConsulted),
    margin_source_available: false,
    sources: factsConsulted
      ? ["arr.ventas_diarias_cliente", "arr.descuentos_diarios_cliente"]
      : [],
    clients: [],
    source_count: 0,
    transport_count: 0,
...
function computeIngreso(kg, margen, descKg) {
  if (!isDefendableHistoricalMargin(margen)) return null;
  const k = Number(kg) || 0;
...
function classifyClosedNewClient(row, margenA, margenB) {
  ...
  const canClassify = ingresoA != null && ingresoB != null;
  return {
    ...row,
    kgA,
    kgB,
    ...
    ingresoA,
    ingresoB,
    es_nuevo: canClassify && ingresoA <= 0 && ingresoB > 0,
  };
}
...
async function loadHistoricalNewClientsForChat(pool, plantaId, req, opts = {}) {
  ...
  const run = async (client) => {
    ...
    if (!isDefendableHistoricalMargin(margenARaw) || !isDefendableHistoricalMargin(margenBRaw)) {
      return missingMarginPayload({ ...base, period_kind: "closed_month" }, true);
    }
    const margenA = Number(margenARaw);
    const margenB = Number(margenBRaw);
    ...
    return {
      ...
      margin_source: IGF_MARGIN_SOURCE,
      sources: [...CLOSED_MONTH_SOURCES],
      margen_a: margenA,
      margen_b: margenB,
      formula:
        "ingreso_A <= 0 && ingreso_B > 0; ingreso = max(0, kg_real * (margen_periodo - |desc_kg|)); desc missing → 0 solo para clasificar; margen null/<=0 → fail-closed",
      ...classified,
    };
  };
...
function buildHistoricalNewClientsChatResult(payload, opts = {}) {
  ...
  return {
    ...
    context_meta: {
      ...
      forecast_used: false,
      margin_used_as_discount: false,
      margin_source_available: Boolean(payload && payload.margin_source_available),
      ...
      conversation_state: {
        ...
        periodoA: payload && payload.periodoA,
        periodoB: payload && payload.periodoB,
        active_subtopic: "NEW_CLIENTS",
        active_entities: [
          {
            kind: "historical_new_clients",
            family: "NEW_CLIENTS",
            movement: "NUEVOS",
            period: payload && payload.periodoB,
            periodoB: payload && payload.periodoB,
            result_set: ((payload && payload.clients) || []).map((c) => c.cliente || c.nombre).filter(Boolean).slice(0, 50),
            ranked_names: ((payload && payload.clients) || []).map((c) => c.cliente || c.nombre).filter(Boolean).slice(0, 20),
          },
        ],
      },
    },
