...
async function computeDicf(client, plantCode, plantaNombre, getMargenKg) {
  ...
  const configRow = await client.query(
    `SELECT window_days FROM arr.dicf_config WHERE UPPER(TRIM(plant_code)) = UPPER(TRIM($1)) AND year = $2 AND month = $3`,
    [canonicalPlantCode, year, month]
  );
  const window_days = (configRow.rows?.[0]?.window_days != null && configRow.rows[0].window_days > 0)
    ? Number(configRow.rows[0].window_days)
    : WINDOW_DAYS;
  ...
  for (const cliente of allClientes) {
    ...
    const es_dejaron = ingreso_anterior > 0 && ingreso_forecast <= 0;
...
async function computeDicfExcel(client, plantCode, plantaNombre, getMargenKg) {
  ...
  for (const r of ventasRows) {
    ...
    if (!kgByClient.has(c)) kgByClient.set(c, new Array(30).fill(0));
  ...
  for (const r of descRows) {
    ...
    const idx = idxByDate.get(f);
    if (idx == null) continue;
    if (!descByClient.has(c)) descByClient.set(c, new Array(30).fill(0));
  ...
  const clientesOut = clientesBase.map((c) => {
    const key = c.cliente;
    const kgArr = kgByClient.get(key) || new Array(30).fill(0);
    const descArr = descByClient.get(key) || new Array(30).fill(0);
