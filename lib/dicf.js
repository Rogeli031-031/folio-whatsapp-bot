...
function projectKgToMonthEnd(series, startDateStr, endDateStr, monthEndStr) {
  ...
  // El cap original equivale a un horizonte "tipo 7 días" (porque usa last14/2).
  // Ajuste: en lugar de usar el mínimo (cap conservador), usamos el promedio entre
  // la tasa basada en last7 y la tasa basada en (last14/2), para minimizar sesgo a la baja.
  const capBase = last7 > 0 && last14 > 0 ? (last7 + last14 / 2) / 2 : (last7 || last14);

  const daysRemaining = (() => {
    const remStart2 = new Date(end);
    remStart2.setDate(remStart2.getDate() + 1);
    if (remStart2 > monthEnd) return 0;
    return Math.floor((monthEnd - remStart2) / 86400000) + 1; // inclusive
  })();

  const capScaled = capBase > 0 && daysRemaining > 0 ? capBase * (daysRemaining / 7) : capBase;
  if (capScaled > 0 && proj > capScaled) proj = capScaled;
...
async function computeDicf(client, plantCode, plantaNombre, getMargenKg) {
  // Para todas las plantas usamos siempre el código de ARR (plantCode)
  // como clave principal; el nombre solo se usa para mostrar en pantalla.
  const plantaParam = (plantCode || plantaNombre || "").trim();
  let maxFecha = null;
  let canonicalPlantCode = plantaParam;
  let useProvMap = true;
  ...
  const onlyCapIfYesterdayInMonth = yesterdayStr >= firstDayMes;
  const effectiveLastDateStr = (lastDateStr > yesterdayStr && onlyCapIfYesterdayInMonth) ? yesterdayStr : lastDateStr;
  ...
  for (const cliente of allClientes) {
    ...
    /** Hasta 8 semanas (56 días) de serie diaria; la UI elige cuántas semanas mostrar. */
    const last4Weeks = [];
    const numLast = Math.min(56, kgSeries.length);
    ...
    rows.push({
      ...
      desc_kg_hist,
      kg_mes_anterior,
      desc_monto_anterior: desc_monto_ant,
      desc_kg_anterior,
      kg_mes_real,
      kg_mes_forecast,
      ingreso_anterior,
  ...
  /** Una fila por cliente: mismas cifras que Ingreso A / Ingreso B en listas y delta (evita desalinear la tabla mensual del Excel). */
  const dicfRowsByCliente = Object.fromEntries(
    rows.map((r) => [
      r.cliente,
      {
        kg_mes_anterior: r.kg_mes_anterior,
        desc_monto_anterior: r.desc_monto_anterior,
        desc_kg_anterior: r.desc_kg_anterior,
        ingreso_anterior: r.ingreso_anterior,
        kg_mes_forecast: r.kg_mes_forecast,
        ingreso_forecast: r.ingreso_forecast,
        desc_kg_hist: r.desc_kg_hist,
      },
    ])
  );
  ...
  return {
    ...
    window_days,
    periodoMes,
    plantCodeUsed: canonicalPlantCode,
...
/**
 * Venta (kg) y descuento (MXN) por cliente en un mes calendario completo.
 * Misma idea que la hoja "Enero"/"Febrero" del Dashboard ARR (sumas reales en BD).
 */
async function fetchKgMontoPorClienteMesCalendario(client, plantaParam, year, month) {
  const firstDay = `${year}-${String(month).padStart(2, "0")}-01`;
  const lastD = new Date(year, month, 0).getDate();
  const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastD).padStart(2, "0")}`;

  let ventasR = await client.query(
    `${PROV_MAP_CTE}
     SELECT v.cliente_norm, SUM(v.kg) AS kg
       FROM arr.ventas_diarias_cliente v
       JOIN prov_map pm ON UPPER(TRIM(v.plant_code)) = pm.key_nombre
      ...
      WHERE (pm.prov_name = $1 OR UPPER(TRIM(pm.prov_name)) = UPPER(TRIM($1)))
        AND v.fecha >= $2::date AND v.fecha <= $3::date
      GROUP BY v.cliente_norm`,
    [plantaParam, firstDay, lastDayStr]
  );
  if (!(ventasR.rows || []).length) {
    ventasR = await client.query(
      `SELECT cliente_norm, SUM(kg) AS kg
         FROM arr.ventas_diarias_cliente
  ...
  if (!(descR.rows || []).length) {
    descR = await client.query(
      `SELECT cliente_norm, SUM(monto) AS monto
         FROM arr.descuentos_diarios_cliente
        WHERE UPPER(TRIM(plant_code)) = UPPER(TRIM($1))
          AND fecha >= $2::date AND fecha <= $3::date
  ...
  const catMap = new Map((catR.rows || []).map((r) => [r.cliente_norm, { canal: r.canal || "Casa", subcanal: r.subcanal || "" }]));

  const out = new Map();
  for (const r of ventasR.rows || []) {
    const k = r.cliente_norm;
    if (!k) continue;
    const cat = catMap.get(k) || { canal: "Casa", subcanal: "" };
    out.set(k, {
      kg: Number(r.kg) || 0,
      monto: 0,
      canal: cat.canal,
  ...
  for (const r of descR.rows || []) {
    const k = r.cliente_norm;
    if (!k) continue;
    const monto = Number(r.monto) || 0;
    const cat = catMap.get(k) || { canal: "Casa", subcanal: "" };
    if (!out.has(k)) {
      out.set(k, { kg: 0, monto, canal: cat.canal, subcanal: cat.subcanal });
    } else {
      const rec = out.get(k);
      rec.monto = monto;
    }
...
async function computeDicfExcel(client, plantCode, plantaNombre, getMargenKg) {
  ...
  // Usar el mismo plant_code con el que computeDicf obtuvo la lista de clientes (p. ej. "Puebla"), no el nombre del dropdown ("GT Puebla").
  const plantaParam = (base.plantCodeUsed != null && String(base.plantCodeUsed).trim() !== "")
    ? String(base.plantCodeUsed).trim()
    : (plantCode || plantaNombre || "").trim();
  ...
  const dicfRowsByCliente = base.dicfRowsByCliente && typeof base.dicfRowsByCliente === "object" ? base.dicfRowsByCliente : {};
  const periodoMesStr = base.periodoMes || "";
  const matchPeriodo = periodoMesStr.match(/^(\d{4})-(\d{2})$/);
  const forecastYear = matchPeriodo ? parseInt(matchPeriodo[1], 10) : lastDate.getFullYear();
  const forecastMonth = matchPeriodo ? parseInt(matchPeriodo[2], 10) : lastDate.getMonth() + 1;
  let mesPrevYear = forecastYear;
  let mesPrevMonth = forecastMonth - 1;
  if (mesPrevMonth < 1) {
    mesPrevMonth += 12;
    mesPrevYear -= 1;
  }
  const mesesList = [];
  for (let y = 2026; y <= forecastYear; y++) {
    const startM = 1;
    const endM = y === forecastYear ? forecastMonth : 12;
    for (let m = startM; m <= endM; m++) {
      const label = (y === forecastYear && m === forecastMonth)
        ? `${MESES_NOMBRES[m - 1]} forecast`
        : MESES_NOMBRES[m - 1];
      mesesList.push({ year: y, month: m, label });
    }
  }

  /** Meses que no son forecast ni el mes "previo" DICF: aquí la ventana móvil no cubre el mes completo → usar BD (mes calendario) e incluir todos los clientes. */
  const mesesCalendarioKeys = new Set();
  for (const { year: y, month: mo } of mesesList) {
    const isFc = (y === forecastYear && mo === forecastMonth);
    if (isFc) continue;
    if (y === mesPrevYear && mo === mesPrevMonth) continue;
    mesesCalendarioKeys.add(`${y}-${mo}`);
  }

  const fullMonthCache = new Map();
  const clientesExtraKeys = new Set();
  for (const mk of mesesCalendarioKeys) {
    const [yy, mm] = mk.split("-").map(Number);
    const fm = await fetchKgMontoPorClienteMesCalendario(client, plantaParam, yy, mm);
    fullMonthCache.set(mk, fm);
    for (const cl of fm.keys()) clientesExtraKeys.add(cl);
  }

  const baseClienteKeys = new Set((clientesBase || []).map((c) => c.cliente));
  const clientesMerged = [...(clientesBase || [])];
  for (const ex of clientesExtraKeys) {
    if (baseClienteKeys.has(ex)) continue;
    let canal = "Casa";
    let subcanal = "";
    for (const mk of mesesCalendarioKeys) {
      const fm = fullMonthCache.get(mk);
      const rec = fm && fm.get(ex);
      if (rec && (rec.kg > 0 || Math.abs(rec.monto) > 0)) {
        canal = rec.canal || canal;
        subcanal = rec.subcanal != null ? rec.subcanal : subcanal;
        break;
      }
    }
    clientesMerged.push({
  ...
  const ventaByMesCliente = new Map();
  const descByMesCliente = new Map();
  const margenByMes = [];

  for (let i = 0; i < mesesList.length; i++) {
    const { year, month, label } = mesesList[i];
    const lastDay = new Date(year, month, 0).getDate();
    const firstDayStr = `${year}-${String(month).padStart(2, "0")}-01`;
    const lastDayStr = `${year}-${String(month).padStart(2, "0")}-${String(lastDay).padStart(2, "0")}`;
    const isForecast = (year === forecastYear && month === forecastMonth);
    let mg = base.excelData?.margen;
    if (!isForecast) {
      mg = await getMargenKg(client, plantaNombre || plantCode, year, month);
      if (mg == null || !Number.isFinite(mg)) mg = 0;
    } else {
      mg = base.excelData?.margen != null && Number.isFinite(base.excelData.margen) ? base.excelData.margen : 0;
    }
    margenByMes.push(mg);

    if (year === forecastYear && month === forecastMonth) {
      for (const c of clientesMerged) {
        const key = c.cliente;
        const dr = dicfRowsByCliente[key];
        const kgForecast = dr != null && dr.kg_mes_forecast != null ? Number(dr.kg_mes_forecast) : Number(c.kg_mes_forecast) || 0;
        const descKg = dr != null && dr.desc_kg_hist != null ? Number(dr.desc_kg_hist) : Number(c.desc_kg_hist) || 0;
        const descPesos = kgForecast * Math.abs(descKg);
        if (!ventaByMesCliente.has(key)) ventaByMesCliente.set(key, []);
        if (!descByMesCliente.has(key)) descByMesCliente.set(key, []);
        const vArr = ventaByMesCliente.get(key);
        const dArr = descByMesCliente.get(key);
        while (vArr.length < i) { vArr.push(null); dArr.push(null); }
        vArr.push(kgForecast / 1000);
        dArr.push(descPesos);
      }
      continue;
    }

    /** Mes calendario anterior al forecast (p. ej. febrero si el periodo es marzo): mismas cifras que DICF / Ingreso A. */
    if (!isForecast && year === mesPrevYear && month === mesPrevMonth) {
      const needFallback = clientesMerged.some((cc) => !dicfRowsByCliente[cc.cliente]);
      const fmFallback = needFallback
        ? await fetchKgMontoPorClienteMesCalendario(client, plantaParam, year, month)
        : new Map();
      for (const c of clientesMerged) {
        const key = c.cliente;
        const dr = dicfRowsByCliente[key];
        ...
        while (vArr.length < i) { vArr.push(null); dArr.push(null); }
        if (dr) {
          vArr.push((Number(dr.kg_mes_anterior) || 0) / 1000);
          dArr.push(Number(dr.desc_monto_anterior) || 0);
        } else {
          const rec = fmFallback.get(key);
          if (rec) {
            vArr.push(rec.kg / 1000);
            dArr.push(Math.abs(rec.monto));
          } else {
            vArr.push(null);
            dArr.push(null);
          }
    ...
    /** Resto de meses históricos: mes calendario completo en BD (suma kg y descuentos = dashboard ARR), no solo días en la ventana móvil. */
    const fm = fullMonthCache.get(`${year}-${month}`) || new Map();
    for (const c of clientesMerged) {
      const key = c.cliente;
      const rec = fm.get(key);
      if (!ventaByMesCliente.has(key)) ventaByMesCliente.set(key, []);
      if (!descByMesCliente.has(key)) descByMesCliente.set(key, []);
      const vArr = ventaByMesCliente.get(key);
      const dArr = descByMesCliente.get(key);
  ...
  for (const [key, vArr] of ventaByMesCliente) {
    const dArr = descByMesCliente.get(key) || [];
    while (vArr.length < mesesList.length) vArr.push(null);
    while (dArr.length < mesesList.length) dArr.push(null);
  }

  const clientesOut = clientesMerged.map((c) => {
    const key = c.cliente;
    ...
    const ventaPorMes = ventaByMesCliente.get(key) || [];
    const descuentoPorMes = descByMesCliente.get(key) || [];
    return {
      ...c,
      kgLast30,
      descKgLast30,
