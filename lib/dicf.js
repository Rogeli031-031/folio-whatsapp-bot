  ...
  for (let i = 0; i < mesesList.length; i++) {
    ...
    if (year === forecastYear && month === forecastMonth) {
      for (const c of clientesBase) {
        const key = c.cliente;
        const kgForecast = Number(c.kg_mes_forecast) || 0;
        const descKg = Number(c.desc_kg_hist) || 0;
    ...
    // Sumar por mes desde los mismos datos diarios (kgByClient/descByClient) para que Venta Enero/Febrero coincida con la diaria.
    for (const c of clientesBase) {
      const key = c.cliente;
