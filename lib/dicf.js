...
async function computeDicf(client, plantCode, plantaNombre, getMargenKg) {
  const rMax = await client.query(
    `SELECT MAX(fecha) AS max_fecha FROM arr.ventas_diarias_cliente WHERE plant_code = $1`,
    [plantCode]
  );
  const maxFecha = rMax.rows?.[0]?.max_fecha;
  if (!maxFecha) {
    return {
  ...
  const ventas = await client.query(
    ...
    [plantCode, startStr, lastDateStr]
  );
  ...
  const catMes = await client.query(
  ...
  for (const cliente of allClientes) {
    ...
    const umbral_pct_pos = 0.15;
    const umbral_pct_neg = 0.15;
    const min_kg_dejaron = 50;
  ...
  try {
    await client.query(
      `DELETE FROM arr.dicf_cliente_mes WHERE plant_code = $1 AND year = $2 AND month = $3`,
      [plantCode, year, month]
    );
    const insertText = `
