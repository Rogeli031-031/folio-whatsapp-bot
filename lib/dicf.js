...
async function computeDicfExcel(client, plantCode, plantaNombre, getMargenKg) {
  ...
  for (let i = 0; i < mesesList.length; i++) {
      ...
      const vQ = await client.query(
        `${PROV_MAP_CTE}
         SELECT v.cliente_norm, SUM(v.kg) AS kg
          ...
          WHERE (pm.prov_name = $1 OR UPPER(TRIM(pm.prov_name)) = UPPER(TRIM($1))) AND v.fecha >= $2::date AND v.fecha <= $3::date
          GROUP BY v.cliente_norm`,
        [plantaParam, firstDayStr, lastDayStr]
      );
      vRows = vQ.rows || [];
      const dQ = await client.query(
        `${PROV_MAP_CTE}
         SELECT d.cliente_norm, SUM(d.monto) AS monto
          ...
          WHERE (pm.prov_name = $1 OR UPPER(TRIM(pm.prov_name)) = UPPER(TRIM($1))) AND d.fecha >= $2::date AND d.fecha <= $3::date
          GROUP BY d.cliente_norm`,
        [plantaParam, firstDayStr, lastDayStr]
      );
      dRows = dQ.rows || [];
    } catch (_) {}
    if (!vRows.length && !dRows.length) {
      try {
        const vD = await client.query(
          `SELECT cliente_norm, SUM(kg) AS kg FROM arr.ventas_diarias_cliente
           WHERE UPPER(TRIM(plant_code)) = UPPER(TRIM($1)) AND fecha >= $2::date AND fecha <= $3::date GROUP BY cliente_norm`,
          [plantCode || plantaParam, firstDayStr, lastDayStr]
        );
        vRows = vD.rows || [];
    ...
    const vMap = new Map((vRows || []).map((r) => [r.cliente_norm, Number(r.kg) || 0]));
    const dMap = new Map((dRows || []).map((r) => [r.cliente_norm, Math.abs(Number(r.monto)) || 0]));
    for (const c of clientesBase) {
      const key = c.cliente;
      const kg = vMap.get(key) || 0;
      const descPesos = dMap.get(key) || 0;
      if (!ventaByMesCliente.has(key)) ventaByMesCliente.set(key, []);
      if (!descByMesCliente.has(key)) descByMesCliente.set(key, []);
      const vArr = ventaByMesCliente.get(key);
      const dArr = descByMesCliente.get(key);
