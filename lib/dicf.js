...
async function computeDicfExcel(client, plantCode, plantaNombre, getMargenKg) {
  ...
  for (let i = 0; i < mesesList.length; i++) {
    ...
    /** Mes calendario anterior = mismas ton y desc que Ingreso A (no solo días dentro de la ventana móvil). */
    if (!isForecast && year === mesPrevYear && month === mesPrevMonth) {
      for (const c of clientesBase) {
        ...
        if (dr) {
          ...
          const kgArr = kgByClient.get(key) || [];
          const descArr = descByClient.get(key) || [];
          let sumKg = 0;
          let sumDesc = 0;
          for (let j = 0; j < dates.length; j++) {
            const dateStr = dates[j];
            if (dateStr >= firstDayStr && dateStr <= lastDayStr) {
              sumKg += Number(kgArr[j]) || 0;
              sumDesc += Math.abs(Number(descArr[j]) || 0);
            }
          }
          vArr.push(sumKg / 1000);
          dArr.push(sumDesc);
        }
      }
      continue;
    ...
    // Sumar por mes desde los mismos datos diarios (kgByClient/descByClient). Meses no cubiertos arriba (p. ej. enero parcial).
    for (const c of clientesBase) {
      const key = c.cliente;
      const kgArr = kgByClient.get(key) || [];
      const descArr = descByClient.get(key) || [];
