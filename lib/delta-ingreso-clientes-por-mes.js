...
const { resolveEffectiveIgfTarget } = require("./igf-effective-proy-target");
...
async function computeDeltaIngresoClientesPorMes(client, plantCode, yearA, monthA, yearB, monthB, opts = {}) {
  ...
  let ventaTonB = metricsB.ventaTon != null ? Number(metricsB.ventaTon) : null;
  let targetSource = "compromiso";
  let uploadDayB = null;
  if (!histB) {
    const effective = await resolveEffectiveIgfTarget(client, plantCode, yearB, monthB, opts);
    if (effective && Number(effective.targetKg) > 0) {
      targetKgB = Number(effective.targetKg);
      if (effective.ventaTon != null && Number.isFinite(Number(effective.ventaTon))) {
        ventaTonB = Number(effective.ventaTon);
      }
      targetSource = effective.target_source || "proy";
      uploadDayB = effective.upload_day || opts.upload_day || null;
    }
  }
