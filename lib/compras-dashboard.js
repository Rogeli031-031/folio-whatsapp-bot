...
function freightRollup(providers, ymds, dayMap, tarifas) {
  ...
  for (const prov of providers || []) {
    ...
    const cell = freightCell(kg, tarifas.has(Number(prov.id)) ? tarifas.get(Number(prov.id)) : null);
    perProv[prov.id] = cell;
    consKg += cell.kg;
    consImp += Number(cell.importe) || 0;
  }
  consImp = Math.round(consImp * 100) / 100;
  return {
    providers: perProv,
    consolidado: { kg: consKg, importe: consImp, tarifa: freightWeightedTarifa(consKg, consImp) },
  };
}
