...
function validatePurchaseInput(body) {
  ...
  const fecha = String((body && body.fecha) || "").slice(0, 10);
...
async function ensureDefaultProviders(client, plantaId) {
  ...
  if (existing.length) return { seeded: false, providers: existing };
  const created = [];
  for (let i = 0; i < DEFAULT_PROVIDER_NAMES.length; i += 1) {
    const out = await createProvider(client, plantaId, { nombre: DEFAULT_PROVIDER_NAMES[i], orden: i });
    if (out.ok) created.push(out.provider);
  }
  return { seeded: created.length > 0, providers: created };
}
...
async function loadMonth(client, plantaId, year, month) {
  ...
  let providers = await listProviders(client, plantaId, { includeInactive: true });
  if (!providers.length) {
    const seeded = await ensureDefaultProviders(client, plantaId);
  ...
  const visibleProviders = visibleProvidersForPeriod(providers, purchases);
  const grid = aggregatePurchases(purchases, visibleProviders, year, month);
  return {
