...
const { conceptoSimilarity } = require("./folio-duplicados");

const DUP_CONCEPTO_UMBRAL = 0.72;
/** Cuando el importe ya es muy parecido, aceptar un poco menos de similitud de texto. */
const DUP_CONCEPTO_UMBRAL_IMPORTE_CERCANO = 0.65;
...
/**
 * sheet_to_json omite columnas a la izquierda del !ref (p. ej. hoja que empieza en B).
 * Rellenamos para que índice 0 = columna A de Excel y las letras del mapeo coincidan.
 */
function sheetToAoAAligned(ws) {
  if (!ws || !ws["!ref"]) return [];
  ...
  const padLeft = range.s.c > 0 ? range.s.c : 0;
  const padTop = range.s.r > 0 ? range.s.r : 0;
  let rows = data;
  if (padLeft > 0) {
    const empty = Array(padLeft).fill("");
    rows = rows.map((row) => empty.concat(Array.isArray(row) ? row : []));
  }
  if (padTop > 0) {
    const width = rows.reduce((m, r) => Math.max(m, (r && r.length) || 0), 0);
    const blanks = Array.from({ length: padTop }, () => Array(width).fill(""));
    rows = blanks.concat(rows);
  }
  return rows;
}
...
function inspectClasificacionWorkbook(fileBuffer) {
  ...
  for (const sheetName of wb.SheetNames || []) {
    ...
    if (!ws) continue;
    const data = sheetToAoAAligned(ws);
 ...
 * - Match exacto (cat+planta+concepto+importe+unidad)
 * - Soft-match (análisis duplicados): mismo cat/planta, concepto similar ≥0.72 aunque el importe difiera
 * - Filas Excel sin importe → rechazos CDJZ
 */
function compareExcelVsDashboard(excelRows, excelRechazos, dashboardRows) {
  ...
  for (const ex of excelRows || []) {
    ...
    if (idx >= 0) {
      ...
      unmatchedExcel.push({ ...ex, match_key: key });
    }
  }

  // Soft-match: concepto similar (mismo criterio que análisis de duplicados), importe puede diferir
  for (const ex of unmatchedExcel) {
    let bestIdx = -1;
    let bestScore = 0;
    for (let i = 0; i < dash.length; i++) {
      const d = dash[i];
      if (d._used) continue;
      if (ex.categoria && d.categoria && ex.categoria !== d.categoria) continue;
      if (ex.planta_clave && d.planta_clave && ex.planta_clave !== d.planta_clave) continue;
      const score = conceptoSimilarity(ex.concepto || "", d.concepto || "");
      let umbral = DUP_CONCEPTO_UMBRAL;
      if (ex.importe != null && d.importe != null) {
        const diff = Math.abs(Number(ex.importe) - Number(d.importe));
        if (diff <= 5) umbral = DUP_CONCEPTO_UMBRAL_IMPORTE_CERCANO;
        else if (diff > 0.02) umbral = Math.min(umbral, 0.68);
      }
      if (score < umbral) continue;
      let adj = score;
      if (ex.unidad && d.unidad && normalizeText(ex.unidad) === normalizeText(d.unidad)) adj += 0.02;
      if (ex.importe != null && d.importe != null) {
        const diff = Math.abs(Number(ex.importe) - Number(d.importe));
        if (diff < 0.02) adj += 0.05;
        else if (diff <= 5) adj += 0.02;
        else if (diff <= 100) adj += 0.005;
      }
      if (adj > bestScore) {
        bestScore = adj;
        bestIdx = i;
    ...
    if (bestIdx >= 0) {
      const d = dash[bestIdx];
      d._used = true;
      const score = Math.round(conceptoSimilarity(ex.concepto || "", d.concepto || "") * 1000) / 1000;
      posiblesDuplicados.push({
        score,
        categoria: ex.categoria || d.categoria,
        planta_clave: ex.planta_clave || d.planta_clave,
        planta_id: ex.planta_id || d.planta_id,
        unidad: ex.unidad || d.unidad,
        concepto_excel: ex.concepto,
        concepto_dashboard: d.concepto,
        importe_excel: ex.importe,
        importe_dashboard: d.importe,
        folio_id: d.folio_id,
        numero_folio: d.numero_folio,
        estatus: d.estatus,
        sheet: ex.sheet,
        row_excel: ex.row_excel,
        subcategoria: ex.subcategoria || d.subcategoria,
        beneficiario: ex.beneficiario || d.beneficiario,
        banco: ex.banco || null,
        cuenta_bancaria: ex.cuenta_bancaria || null,
        excel: ex,
        dashboard: {
          folio_id: d.folio_id,
          numero_folio: d.numero_folio,
        ...
        motivo:
          ex.importe != null && d.importe != null && Math.abs(Number(ex.importe) - Number(d.importe)) >= 0.01
            ? "Concepto similar; el importe difiere entre Excel y dashboard"
            : "Concepto similar (análisis de duplicados)",
      });
    } else {
      missingInDashboard.push(ex);
    }
  }
  ...
  for (const rej of excelRechazos || []) {
    ...
    if (candidates.length === 1) {
      ...
      // soft match: same concepto only within categoria, then similarity
      let soft = dash.find(
        (d) =>
          !d._used &&
      ...
      if (!soft) {
        ...
        for (const d of dash) {
          if (d._used) continue;
          if (rej.categoria && d.categoria && rej.categoria !== d.categoria) continue;
          if (rej.planta_clave && d.planta_clave && rej.planta_clave !== d.planta_clave) continue;
          const score = conceptoSimilarity(rej.concepto || "", d.concepto || "");
          if (score >= DUP_CONCEPTO_UMBRAL && score > bestScore) {
            bestScore = score;
            best = d;
          }
        }
